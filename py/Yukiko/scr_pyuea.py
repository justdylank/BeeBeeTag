@State
def PYU_PersonaCreate():

    def upon_IMMEDIATE():
        Unknown23022(1)
        Unknown13043(-75000)
        teleportRelativeY(0)
        Unknown23013(1)
    sprite('null', 32767)
    Unknown3038(1)
    Unknown24('5059555f506572736f6e6157616974000000000000000000000000000000000064000000')

@State
def PYU_PersonaWait():

    def upon_IMMEDIATE():
        SLOT_11 = 0
        SLOT_59 = (-1)
        callSubroutine('PYU_ReceptionSignal')
    sprite('null', 32767)
    Unknown3038(1)
    Unknown3001(0)
    Unknown3004(0)
    loopRest()

@Subroutine
def PYU_ReceptionSignal():

    def upon_43():
        if (SLOT_48 == 101):
            Unknown23185('PYU_Persona5B', 50)
        if (SLOT_48 == 102):
            Unknown23185('PYU_Persona5C', 50)
        if (SLOT_48 == 103):
            Unknown23185('PYU_Persona6C', 50)
        if (SLOT_48 == 201):
            Unknown23185('PYU_Persona2B', 50)
        if (SLOT_48 == 203):
            Unknown23185('PYU_Persona2C', 50)
        if (SLOT_48 == 301):
            Unknown23185('PYU_PersonaJB', 50)
        if (SLOT_48 == 302):
            Unknown23185('PYU_PersonaJC', 50)
        if (SLOT_48 == 4010):
            Unknown23185('PYU_PersonaReversalAction', 100)
        if (SLOT_48 == 503):
            Unknown23185('PYU_PersonaCrushAttack', 100)
        if (SLOT_48 == 501):
            Unknown23185('PYU_Crash_Rush1st', 100)
        if (SLOT_48 == 502):
            Unknown23185('PYU_Crash_Rush', 100)
        if (SLOT_48 == 4020):
            Unknown23185('PYU_PersonaAgiA', 100)
        if (SLOT_48 == 4030):
            Unknown23185('PYU_PersonaAgiB', 100)
        if (SLOT_48 == 4040):
            Unknown23185('PYU_PersonaAgiC', 100)
        if (SLOT_48 == 4050):
            Unknown23185('PYU_PersonaMaharagiA', 100)
        if (SLOT_48 == 4060):
            Unknown23185('PYU_PersonaMaharagiB', 100)
        if (SLOT_48 == 4071):
            Unknown23185('PYU_PersonaMaharagiC', 100)
        if (SLOT_48 == 5010):
            Unknown23185('PYU_PersonaFireAttackA', 100)
        if (SLOT_48 == 5032):
            Unknown23185('PYU_PersonaAirAgiA', 100)
        if (SLOT_48 == 5033):
            Unknown23185('PYU_PersonaAirAgiB', 100)
        if (SLOT_48 == 5034):
            Unknown23185('PYU_PersonaAirAgiC', 100)
        if (SLOT_48 == 5035):
            Unknown23185('PYU_PersonaAirFirePitA', 100)
        if (SLOT_48 == 5036):
            Unknown23185('PYU_PersonaAirFirePitB', 100)
        if (SLOT_48 == 5037):
            Unknown23185('PYU_PersonaAirFirePitC', 100)
        if (SLOT_48 == 5020):
            Unknown23185('PersonaFireAttackB', 100)
        if (SLOT_48 == 5021):
            Unknown23185('FlameDanceA', 100)
        if (SLOT_48 == 5022):
            Unknown23185('FlameDanceB', 100)
        if (SLOT_48 == 5023):
            Unknown23185('FlameDanceEX', 100)
        if (SLOT_48 == 5030):
            Unknown23185('PYU_PersonaFireBird', 100)
        if (SLOT_48 == 50031):
            Unknown23185('PYU_PersonaFireBirdA', 100)
        if (SLOT_48 == 50032):
            Unknown23185('PYU_PersonaFireBirdB', 100)
        if (SLOT_48 == 50033):
            Unknown23185('PYU_PersonaFireBirdC', 100)
        if (SLOT_48 == 4080):
            Unknown23185('PYU_PersonaFireBooster', 100)
        if (SLOT_48 == 108):
            Unknown23185('PYU_Persona5C_TAG', 100)
        if (SLOT_48 == 4074):
            Unknown23185('PersonaFireAttack_TAG', 100)
        if (SLOT_48 == 4072):
            Unknown23185('PYU_PersonaMaharagi_TAG', 100)
        if (SLOT_48 == 4011):
            Unknown23185('PYU_PersonaReversalAction_TAG', 100)
        if (SLOT_48 == 10010):
            Unknown23185('PYU_PersonaAgidine', 200)
        if (SLOT_48 == 10020):
            Unknown23185('PYU_PersonaAgidine_OD', 200)
        if (SLOT_48 == 10030):
            Unknown23185('PYU_PersonaMaharagidine', 200)
        if (SLOT_48 == 10053):
            Unknown23185('PYU_PersonaMaharagidine_OD', 200)
        if (SLOT_48 == 10060):
            Unknown23185('PYU_PersonaAgidineDUO', 200)
        if (SLOT_48 == 10061):
            Unknown23185('PYU_PersonaAgidineDUO_OD', 200)
        if (SLOT_48 == 50):
            Unknown23185('PersonaEntry2', 200)
        if (SLOT_48 == 55):
            Unknown23185('PersonaEntry_TAG', 200)
        if (SLOT_48 == 51):
            Unknown23185('PersonaMatchWin', 200)
        if (SLOT_48 == 13):
            Unknown23185('PersonaDeleteAndIdling', 200)
        if (SLOT_48 == 10070):
            Unknown23185('PersonaIchigeki', 200)

@Subroutine
def PersonaReset():
    Unknown4061(1)
    Unknown23022(0)
    Unknown2053(0)
    Unknown23015(11)
    Unknown2019(-1)
    Unknown1084(1)
    Unknown4009(0)
    Unknown4008(0)
    Unknown4007(0)
    EnableCollision(0)
    Unknown30041('')
    Unknown3031()

@Subroutine
def PYU_EffectInit():
    Unknown2019(5)
    Unknown23015(11)

@Subroutine
def PYU_AttackInit():
    Unknown23028(1, 1)
    Unknown30037('')
    Unknown30035(1)
    callSubroutine('PersonaReset')
    Unknown23015(11)
    Unknown2019(10)
    if (SLOT_11 == 0):
        SLOT_58 = 1
    if (SLOT_11 == 10):
        SLOT_58 = 1
    SLOT_11 = 100
    callSubroutine('PYU_ReceptionSignal')
    Unknown11034(1)
    ProjectileDurabilityLvl(0)
    Unknown23023()
    EnableCollision(1)
    Unknown30040(1)
    Unknown30040(2)

@Subroutine
def PYU_SPAttackInit():
    Unknown23028(2, 1)
    Unknown30037('')
    Unknown30035(1)
    callSubroutine('PersonaReset')
    Unknown23015(11)
    Unknown2019(10)
    if (SLOT_11 == 0):
        SLOT_58 = 1
    if (SLOT_11 == 10):
        SLOT_58 = 1
    SLOT_11 = 110
    callSubroutine('PYU_ReceptionSignal')
    Unknown11034(1)
    ProjectileDurabilityLvl(0)
    Unknown23023()
    EnableCollision(1)
    Unknown30040(1)
    Unknown30040(2)

@Subroutine
def PYU_AstralAttackInit():
    Unknown23028(6, 1)
    Unknown30037('')
    Unknown30035(1)
    callSubroutine('PersonaReset')
    Unknown23015(11)
    Unknown2019(10)
    if (SLOT_11 == 0):
        SLOT_58 = 1
    if (SLOT_11 == 10):
        SLOT_58 = 1
    SLOT_11 = 110
    callSubroutine('PYU_ReceptionSignal')
    Unknown11034(1)
    ProjectileDurabilityLvl(0)
    Unknown23023()
    EnableCollision(1)
    Unknown30040(1)
    Unknown30040(2)

@Subroutine
def PYU_DDAttackInit():
    Unknown23028(3, 1)
    Unknown30037('')
    callSubroutine('PersonaReset')
    Unknown23015(11)
    Unknown2019(10)
    if (SLOT_11 == 0):
        SLOT_58 = 1
    if (SLOT_11 == 10):
        SLOT_58 = 1
    Unknown11034(1)
    ProjectileDurabilityLvl(0)
    SLOT_11 = 120
    callSubroutine('PYU_ReceptionSignal')
    Unknown23023()

@Subroutine
def PYU_CheckWarp():
    if SLOT_58:
        Unknown3001(0)
        Unknown3004(85)
        loopRelated(17, 4)

        def upon_17():
            Unknown3001(255)
            Unknown3004(0)

@Subroutine
def PYU_ForceWarp():
    SLOT_58 = 1
    Unknown3001(0)
    Unknown3004(85)
    loopRelated(17, 4)

    def upon_17():
        Unknown3001(255)
        Unknown3004(0)

@State
def PersonaDeleteAndIdling():

    def upon_IMMEDIATE():
        callSubroutine('PersonaReset')
    sprite('keep', 32767)
    if SLOT_143:
        GFX_0('PersonaDeleteEffect', 100)
        SFX_0('persona_disappear')
    SLOT_11 = 10
    SLOT_59 = (-1)
    enterState('PYU_PersonaWait')

@State
def PersonaSummonEffect():
    sprite('null', 30)
    Unknown23015(13)
    Unknown4054(11)
    Unknown4045('persona_summon', 100)

@State
def PersonaSummonEffect_PLAYER():
    sprite('null', 30)
    Unknown1007(288000)
    teleportRelativeX(112000)
    GFX_1('persona_summon_ply', 100)

@State
def PersonaDeleteEffect():

    def upon_IMMEDIATE():
        callSubroutine('PersonaReset')
        callSubroutine('PYU_ReceptionSignal')
    sprite('null', 30)
    Unknown2019(1)
    Unknown23053('190000000b000000')
    Unknown4061(1)
    Unknown3032()
    Unknown1059(-100)
    Unknown1067(150)
    Unknown3004(-20)
    Unknown23022(1)
    Unknown4054(11)
    Unknown4045('persona_delete', 100)
    Unknown2054(1)
    EnableCollision(0)
    Unknown1084(1)

@Subroutine
def BoosterDamageUp():
    Unknown48(25, 2, 60, 3, 2, 31)
    if (SLOT_60 == 1):
        Unknown10000(115)
    if (SLOT_60 == 2):
        Unknown10000(130)
    if (SLOT_60 == 3):
        Unknown10000(145)
    if (SLOT_60 == 4):
        Unknown10000(150)
    if (SLOT_60 == 5):
        Unknown10000(165)
        ChipDamagePct(6)
    if (SLOT_60 == 6):
        Unknown10000(185)
        ChipDamagePct(8)
    if (SLOT_60 == 7):
        Unknown10000(205)
        ChipDamagePct(10)
    if (SLOT_60 == 8):
        Unknown10000(225)
        ChipDamagePct(12)
    if (SLOT_60 == 9):
        Unknown10000(250)
        ChipDamagePct(15)

@Subroutine
def Init_CantCancelPersonaAction():
    SLOT_10 = 1

    def upon_STATE_END():
        SLOT_10 = 0

@State
def PYU_Persona5B():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 0, -200000, 2000000, 0, 0)
        callSubroutine('PYU_AttackInit')
        EnableCollision(1)
        AttackLevel_(3)
        Damage(250)
        AttackP1(90)
        Hitstop(1)
        AirPushbackX(11000)
        AirPushbackY(-25000)
        Unknown9310(1)
        Unknown30056(100000, 50, 0)
        Unknown9016(1)
        Unknown11057(500)
        Unknown11092(1)
        AttackAttributes(0, 1, 0, 0, 0)

        def upon_11():
            SLOT_51 = 1
        callSubroutine('PYU_CheckWarp')
        Unknown2006()
        Unknown23078(1)
        Unknown23066(0)
        Unknown23059(1)
    sprite('ko205_00', 2)
    teleportRelativeY(0)
    physicsXImpulse(5000)
    physicsYImpulse(18000)
    setGravity(1800)
    Unknown2053(1)
    sprite('ko205_01', 2)
    sprite('ko205_00', 2)
    sprite('ko205_03', 3)
    SFX_3('slash_rapier_middle')
    sprite('ko205_04', 1)
    GFX_1('koef_firewing', 0)
    RefreshMultihit()
    sprite('ko205_04', 1)
    Unknown23022(0)
    RefreshMultihit()
    sprite('ko205_04', 1)
    RefreshMultihit()
    sprite('ko205_05', 1)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    RefreshMultihit()
    sprite('ko205_05', 1)
    RefreshMultihit()
    sprite('ko205_05', 1)
    RefreshMultihit()
    sprite('ko205_06', 1)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    RefreshMultihit()
    sprite('ko205_06', 1)
    RefreshMultihit()
    sprite('ko205_06', 1)
    Unknown23066(1)
    RefreshMultihit()
    if SLOT_51:
        Unknown23029(2, 105, 0)
        clearUponHandler(11)
    sprite('ko205_07', 2)
    GFX_1('koef_firewing', 0)
    sprite('ko205_08', 2)
    GFX_1('koef_firewing', 0)
    sprite('ko205_09', 3)
    GFX_1('koef_firewing', 0)
    Unknown8000(104, 1, 1)
    sprite('ko205_10', 4)
    Unknown1084(1)
    GFX_1('koef_firewing', 0)
    sprite('ko205_11', 4)
    sprite('ko205_12', 4)
    sprite('ko205_11', 4)
    sprite('ko205_12', 4)
    sprite('ko205_11', 4)
    sprite('ko205_12', 4)
    sprite('ko205_11', 4)
    sprite('ko205_12', 4)
    sprite('ko205_11', 4)
    sprite('ko205_12', 4)
    label(580)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_Persona5C():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 0, 0, 0, 2000000, -300000, 500000)
        callSubroutine('PYU_AttackInit')
        AttackLevel_(3)
        Damage(225)
        AttackP1(90)
        Hitstop(1)
        AirUntechableTime(17)
        Unknown9016(1)
        AirPushbackX(11000)
        AirPushbackY(15000)
        Unknown11057(500)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown11092(1)
        callSubroutine('PYU_CheckWarp')
        Unknown2006()
        EnableCollision(1)
        Unknown2034(1)
        Unknown2053(1)

        def upon_11():
            SLOT_51 = 1
        Unknown23078(1)

        def upon_3():
            if (SLOT_19 <= 380000):
                sendToLabel(1)
        Unknown23059(1)

        def upon_STATE_END():
            Unknown2034(0)
            Unknown2053(0)
    sprite('ko206_00', 4)
    physicsXImpulse(15000)
    SFX_3('airdash_m')
    sprite('ko206_01', 4)
    physicsXImpulse(25000)
    sprite('ko206_00', 3)
    Unknown1019(100)
    label(1)
    sprite('ko206_01', 5)
    clearUponHandler(3)
    Unknown1019(60)
    sprite('ko206_00', 2)
    sprite('ko206_02', 2)
    sprite('ko206_03', 1)
    sprite('ko206_03', 1)
    GFX_1('koef_firewing', 0)
    RefreshMultihit()
    Unknown1019(20)
    SFX_3('slash_sword_middle')
    sprite('ko206_03', 1)
    RefreshMultihit()
    sprite('ko206_03', 1)
    RefreshMultihit()
    sprite('ko206_04', 1)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    RefreshMultihit()
    Unknown1019(40)
    sprite('ko206_04', 1)
    RefreshMultihit()
    sprite('ko206_04', 1)
    RefreshMultihit()
    if SLOT_51:
        Unknown23029(2, 106, 0)
        clearUponHandler(11)
    sprite('ko206_05', 1)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    RefreshMultihit()
    Unknown1019(40)
    sprite('ko206_05', 1)
    RefreshMultihit()
    sprite('ko206_05', 1)
    RefreshMultihit()
    if SLOT_51:
        Unknown23029(2, 106, 0)
        clearUponHandler(11)
    sprite('ko206_06', 3)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    Unknown1019(20)
    Unknown8000(104, 1, 0)
    Unknown23029(2, 109, 0)
    sprite('ko206_07', 3)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    Unknown1019(0)
    sprite('ko206_08', 4)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    sprite('ko206_09', 4)
    sprite('ko206_10', 2)
    sprite('ko206_10', 1)
    Unknown23029(2, 107, 0)
    sprite('ko206_08', 4)
    sprite('ko206_09', 4)
    sprite('ko206_10', 4)
    sprite('ko206_08', 4)
    sprite('ko206_09', 4)
    sprite('ko206_10', 4)
    sprite('ko206_08', 4)
    sprite('ko206_09', 4)
    sprite('ko206_10', 4)
    label(580)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_Persona2B():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 0, -200000, 2000000, 0, 100000)
        callSubroutine('PYU_AttackInit')
        AttackLevel_(3)
        Damage(225)
        AttackP1(90)
        AirPushbackY(10000)
        AirUntechableTime(30)
        Hitstop(1)
        Unknown9016(1)
        HitLow(2)
        Unknown11057(500)
        AttackAttributes(0, 0, 1, 0, 0)
        Unknown11092(1)

        def upon_11():
            SLOT_51 = 1
        callSubroutine('PYU_CheckWarp')
        Unknown2006()
        EnableCollision(1)
        Unknown23078(1)
        Unknown23066(0)
        Unknown23059(1)
    sprite('ko232_00', 2)
    teleportRelativeY(0)
    Unknown2053(1)
    sprite('ko232_01', 3)
    physicsXImpulse(7000)
    sprite('ko232_02', 2)
    SFX_3('slash_rapier_middle')
    sprite('ko232_03', 1)
    GFX_1('koef_firewing', 0)
    RefreshMultihit()
    sprite('ko232_03', 1)
    RefreshMultihit()
    sprite('ko232_03', 1)
    RefreshMultihit()
    sprite('ko232_03', 1)
    RefreshMultihit()
    sprite('ko232_04', 1)
    GFX_1('koef_firewing', 0)
    RefreshMultihit()
    sprite('ko232_04', 1)
    RefreshMultihit()
    Unknown1019(80)
    sprite('ko232_04', 1)
    RefreshMultihit()
    sprite('ko232_04', 1)
    Unknown23066(1)
    RefreshMultihit()
    if SLOT_51:
        Unknown23029(2, 202, 0)
        clearUponHandler(11)
    sprite('ko232_05', 4)
    GFX_1('koef_firewing', 0)
    sprite('ko232_06', 3)
    GFX_1('koef_firewing', 0)
    Unknown1019(80)
    sprite('ko232_06', 1)
    Unknown23029(2, 210, 0)
    sprite('ko232_07', 4)
    sprite('ko232_08', 6)
    Unknown1019(50)
    sprite('ko232_08', 6)
    Unknown1084(1)
    sprite('ko232_08', 6)
    sprite('ko232_08', 10)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaJB():

    def upon_IMMEDIATE():
        Unknown23184(3, 100, -160000, 0, 0, 0, 0, 0)
        callSubroutine('PYU_AttackInit')
        AttackLevel_(4)
        Damage(120)
        Hitstop(1)
        AirUntechableTime(24)
        AirPushbackY(30000)
        Unknown9016(1)
        Unknown11057(500)
        AttackAttributes(1, 0, 0, 0, 0)
        Unknown11092(1)
        callSubroutine('PYU_ForceWarp')
        Unknown2006()
        Unknown2053(1)
        Unknown23078(1)

        def upon_11():
            SLOT_51 = 1
        Unknown23059(1)
    sprite('ko254_00', 2)
    physicsXImpulse(5000)
    sprite('ko254_01', 2)
    sprite('ko254_02', 1)
    SFX_3('slash_rapier_middle')
    RefreshMultihit()
    sprite('ko254_02', 1)
    RefreshMultihit()
    sprite('ko254_02', 1)
    RefreshMultihit()
    Unknown1019(120)
    sprite('ko254_03', 1)
    GFX_1('koef_firewing', 0)
    RefreshMultihit()
    sprite('ko254_03', 1)
    RefreshMultihit()
    Unknown23022(0)
    sprite('ko254_03', 1)
    RefreshMultihit()
    Unknown1019(120)
    sprite('ko254_04', 1)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    RefreshMultihit()
    sprite('ko254_04', 1)
    RefreshMultihit()
    sprite('ko254_04', 1)
    RefreshMultihit()
    Unknown1019(120)
    if SLOT_51:
        Unknown23029(2, 5031, 0)
        clearUponHandler(11)
    sprite('ko254_05', 4)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    sprite('ko254_06', 4)
    GFX_1('koef_firewing', 0)
    sprite('ko254_07', 4)
    Unknown1019(50)
    sprite('ko254_08', 4)
    Unknown1019(50)
    sprite('ko254_07', 4)
    sprite('ko254_08', 4)
    Unknown1084(1)
    sprite('ko254_07', 4)
    sprite('ko254_08', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaJC():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 0, 0, -200000, 2000000, -200000, 20000000)
        callSubroutine('PYU_AttackInit')
        AttackLevel_(3)
        Damage(500)
        AttackP1(90)
        Unknown11092(1)
        AirUntechableTime(21)
        hitstun(21)
        Hitstop(1)
        Unknown23078(1)
        Unknown9016(1)
        GroundedHitstunAnimation(3)
        PushbackX(0)
        AirPushbackX(2000)
        AirPushbackY(-25000)
        YImpluseBeforeWallbounce(1000)
        AttackAttributes(1, 0, 0, 0, 0)
        callSubroutine('PYU_CheckWarp')
        EnableCollision(0)
        Unknown2053(1)

        def upon_11():
            SLOT_51 = 1
        Unknown23059(1)
        Unknown2049(1)
    sprite('ko233_00', 1)
    sprite('ko233_01', 1)
    sprite('ko233_00', 1)
    Unknown3029(1)
    physicsXImpulse(20000)
    physicsYImpulse(70000)
    Unknown2053(1)
    SFX_0('highjump_m')
    sprite('ko233_01', 1)
    Unknown1028(10000)
    sprite('ko233_00', 1)
    YAccel(50)
    sprite('ko233_01', 1)
    sprite('ko233_00', 1)
    Unknown1019(80)
    YAccel(50)
    Unknown3029(1)
    sprite('ko233_01', 1)
    sprite('ko233_02', 1)
    Unknown1084(1)
    Unknown3029(1)
    sprite('ko233_03', 4)
    clearUponHandler(3)
    sprite('ko233_04', 3)
    sprite('ko233_05', 1)
    physicsYImpulse(-28000)
    RefreshMultihit()
    SFX_3('slash_sword_slow')
    sprite('ko233_07', 1)
    sprite('ko233_09', 1)
    sendToLabelUpon(2, 1)
    label(0)
    sprite('ko233_05', 1)
    GFX_1('koef_233firering', 0)
    GFX_1('koef_233firering', 1)
    GFX_1('koef_233firering', 2)
    RefreshMultihit()
    if SLOT_51:
        Unknown23029(2, 5031, 0)
        clearUponHandler(11)
    sprite('ko233_06', 1)
    sprite('ko233_07', 1)
    sprite('ko233_08', 1)
    sprite('ko233_09', 1)
    loopRest()
    gotoLabel(0)
    label(1)
    sprite('ko233_05', 2)
    DisableAttackRestOfMove()
    Unknown8000(104, 1, 1)
    sprite('ko233_06', 2)
    sprite('ko233_07', 2)
    sprite('ko233_08', 2)
    sprite('ko233_09', 2)
    sprite('ko233_05', 3)
    Unknown8000(104, 1, 0)
    sprite('ko233_06', 2)
    sprite('ko233_07', 2)
    sprite('ko233_08', 2)
    sprite('ko233_09', 2)
    sprite('ko233_05', 3)
    Unknown8000(104, 1, 0)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaAgiA():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 0, 0, 2000000, -2000000, 0)
        callSubroutine('PYU_SPAttackInit')
        AttackLevel_(3)
        Damage(700)
        AttackP1(90)
        AttackP2(100)
        AirHitstunAnimation(9)
        GroundedHitstunAnimation(9)
        AirPushbackX(20000)
        AirPushbackY(10000)
        PushbackX(30400)
        AirUntechableTime(19)
        Unknown9016(1)
        AttackAttributes(0, 1, 0, 0, 0)
        callSubroutine('PYU_CheckWarp')
        Unknown2006()
        Unknown2053(1)
        Unknown23078(1)
        Unknown23059(1)
    sprite('ko401_00', 4)
    physicsXImpulse(12000)
    sprite('ko401_01', 2)
    SFX_3('slash_rapier_fast')
    physicsXImpulse(12000)
    physicsYImpulse(5000)
    sprite('ko401_02', 1)
    RefreshMultihit()
    sprite('ko401_02', 2)
    sprite('ko401_03', 2)
    GFX_0('AgishotA', 100)
    Unknown1019(50)
    YAccel(50)
    sprite('ko401_04', 2)
    sprite('ko401_05', 2)
    sprite('ko401_06', 2)
    physicsXImpulse(-15000)
    physicsYImpulse(-2000)
    sprite('ko401_07', 4)
    sprite('ko401_08', 4)
    physicsYImpulse(1000)
    sprite('ko401_09', 4)
    SFX_3('cloth_h')
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_10', 4)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_11', 8)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_12', 8)
    sprite('ko401_13', 8)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaAirAgiA():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 0, -2000000, 2000000, -2000000, 0)
        callSubroutine('PYU_SPAttackInit')
        AttackLevel_(3)
        Damage(700)
        AttackP1(90)
        AttackP2(100)
        GroundedHitstunAnimation(9)
        AirHitstunAnimation(9)
        AirPushbackX(20000)
        AirPushbackY(20000)
        PushbackX(30400)
        AirUntechableTime(24)
        Unknown9016(1)
        AttackAttributes(1, 0, 0, 0, 0)
        callSubroutine('PYU_CheckWarp')
        Unknown2006()
        Unknown2053(1)
        Unknown23078(1)
        Unknown23059(1)
    sprite('ko401_00', 4)
    physicsXImpulse(12000)
    sprite('ko401_01', 2)
    SFX_3('slash_rapier_fast')
    physicsXImpulse(12000)
    physicsYImpulse(5000)
    sprite('ko401_02', 1)
    RefreshMultihit()
    sprite('ko401_02', 2)
    sprite('ko401_03', 2)
    GFX_0('AgishotA', 100)
    Unknown1019(50)
    YAccel(50)
    sprite('ko401_04', 2)
    sprite('ko401_05', 2)
    sprite('ko401_06', 2)
    physicsXImpulse(-15000)
    physicsYImpulse(-2000)
    sprite('ko401_07', 4)
    sprite('ko401_08', 4)
    physicsYImpulse(1000)
    sprite('ko401_09', 4)
    SFX_3('cloth_h')
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_10', 4)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_11', 8)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_12', 8)
    sprite('ko401_13', 8)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaAgiB():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 0, 0, 2000000, -2000000, 0)
        callSubroutine('PYU_SPAttackInit')
        AttackLevel_(3)
        Damage(700)
        AttackP1(90)
        AttackP2(100)
        AirHitstunAnimation(9)
        GroundedHitstunAnimation(9)
        AirPushbackX(37000)
        AirPushbackY(20000)
        PushbackX(30400)
        AirUntechableTime(30)
        Unknown9016(1)
        AttackAttributes(0, 1, 0, 0, 0)
        callSubroutine('PYU_CheckWarp')
        Unknown2006()
        Unknown2053(1)
        Unknown23078(1)
        Unknown23059(1)
    sprite('ko401_00', 4)
    physicsXImpulse(12000)
    sprite('ko401_01', 2)
    SFX_3('slash_rapier_fast')
    physicsXImpulse(12000)
    physicsYImpulse(5000)
    sprite('ko401_02', 1)
    RefreshMultihit()
    sprite('ko401_02', 2)
    Unknown23022(0)
    sprite('ko401_03', 2)
    GFX_0('AgishotB', 100)
    Unknown1019(50)
    YAccel(50)
    sprite('ko401_04', 2)
    sprite('ko401_05', 2)
    sprite('ko401_06', 2)
    physicsXImpulse(-15000)
    physicsYImpulse(-2000)
    sprite('ko401_07', 4)
    sprite('ko401_08', 4)
    physicsYImpulse(1000)
    sprite('ko401_09', 4)
    SFX_3('cloth_h')
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_10', 4)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_11', 8)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_12', 8)
    sprite('ko401_13', 8)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaAirAgiB():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 0, -2000000, 2000000, -2000000, 0)
        callSubroutine('PYU_SPAttackInit')
        AttackLevel_(3)
        Damage(700)
        AttackP1(90)
        AttackP2(100)
        AirHitstunAnimation(9)
        GroundedHitstunAnimation(9)
        AirUntechableTime(24)
        AirPushbackX(35000)
        AirPushbackY(20000)
        PushbackX(30400)
        Unknown9016(1)
        AttackAttributes(1, 0, 0, 0, 0)
        callSubroutine('PYU_CheckWarp')
        Unknown2006()
        Unknown2053(1)
        Unknown23078(1)
        Unknown23059(1)
    sprite('ko401_00', 4)
    physicsXImpulse(12000)
    sprite('ko401_01', 2)
    SFX_3('slash_rapier_fast')
    physicsXImpulse(12000)
    physicsYImpulse(5000)
    sprite('ko401_02', 1)
    RefreshMultihit()
    sprite('ko401_02', 2)
    Unknown23022(0)
    sprite('ko401_03', 2)
    GFX_0('AgishotB', 100)
    Unknown1019(50)
    YAccel(50)
    sprite('ko401_04', 2)
    sprite('ko401_05', 2)
    sprite('ko401_06', 2)
    physicsXImpulse(-15000)
    physicsYImpulse(-2000)
    sprite('ko401_07', 4)
    sprite('ko401_08', 4)
    physicsYImpulse(1000)
    sprite('ko401_09', 4)
    SFX_3('cloth_h')
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_10', 4)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_11', 8)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_12', 8)
    sprite('ko401_13', 8)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaAgiC():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 0, 0, 2000000, -2000000, 0)
        callSubroutine('PYU_SPAttackInit')
        AttackLevel_(3)
        Damage(700)
        AttackP1(90)
        AttackP2(100)
        Unknown30065(0)
        MinimumDamagePct(10)
        GroundedHitstunAnimation(9)
        AirHitstunAnimation(9)
        AirPushbackX(2000)
        AirPushbackY(28000)
        PushbackX(30400)
        AirUntechableTime(24)
        Unknown9016(1)
        AttackAttributes(0, 1, 0, 0, 0)
        callSubroutine('PYU_CheckWarp')
        Unknown2006()
        Unknown2053(1)
        Unknown23078(1)
        Unknown23059(1)
    sprite('ko401_00', 6)
    physicsXImpulse(6000)
    sprite('ko401_01', 2)
    SFX_3('slash_rapier_fast')
    physicsXImpulse(12000)
    physicsYImpulse(5000)
    sprite('ko401_02', 1)
    RefreshMultihit()
    Unknown4007(0)
    sprite('ko401_02', 2)
    Unknown23022(0)
    sprite('ko401_03', 2)
    GFX_0('AgishotAB', 100)
    Unknown1019(50)
    YAccel(50)
    sprite('ko401_04', 2)
    sprite('ko401_05', 2)
    sprite('ko401_06', 2)
    physicsXImpulse(-15000)
    physicsYImpulse(-2000)
    sprite('ko401_07', 4)
    sprite('ko401_08', 4)
    physicsYImpulse(1000)
    sprite('ko401_09', 4)
    SFX_3('cloth_h')
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_10', 4)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_11', 8)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_12', 8)
    sprite('ko401_13', 8)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaAirAgiC():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 0, 0, 2000000, -2000000, 0)
        callSubroutine('PYU_SPAttackInit')
        AttackLevel_(3)
        Damage(700)
        AttackP1(90)
        AttackP2(100)
        Unknown30065(0)
        MinimumDamagePct(10)
        GroundedHitstunAnimation(9)
        AirHitstunAnimation(9)
        AirPushbackX(2000)
        AirPushbackY(22000)
        PushbackX(30400)
        AirUntechableTime(24)
        Unknown9016(1)
        AttackAttributes(1, 0, 0, 0, 0)
        callSubroutine('PYU_CheckWarp')
        Unknown2006()
        Unknown2053(1)
        Unknown23078(1)
        Unknown23059(1)
    sprite('ko401_00', 4)
    physicsXImpulse(6000)
    sprite('ko401_01', 2)
    SFX_3('slash_rapier_fast')
    physicsXImpulse(12000)
    physicsYImpulse(5000)
    sprite('ko401_02', 1)
    RefreshMultihit()
    Unknown4007(0)
    sprite('ko401_02', 2)
    Unknown23022(0)
    sprite('ko401_03', 2)
    GFX_0('AgishotAB', 100)
    Unknown1019(50)
    YAccel(50)
    sprite('ko401_04', 2)
    sprite('ko401_05', 2)
    sprite('ko401_06', 2)
    physicsXImpulse(-15000)
    physicsYImpulse(-2000)
    sprite('ko401_07', 4)
    sprite('ko401_08', 4)
    physicsYImpulse(1000)
    sprite('ko401_09', 4)
    SFX_3('cloth_h')
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_10', 4)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_11', 8)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_12', 8)
    sprite('ko401_13', 8)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaAirFirePitA():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 0, -2000000, 2000000, -2000000, 0)
        callSubroutine('PYU_SPAttackInit')
        AttackLevel_(3)
        Damage(500)
        AttackP1(90)
        AttackP2(100)
        GroundedHitstunAnimation(9)
        AirHitstunAnimation(9)
        AirPushbackX(15000)
        AirPushbackY(20000)
        PushbackX(30400)
        AirUntechableTime(24)
        Unknown9016(1)
        AttackAttributes(1, 0, 0, 0, 0)
        callSubroutine('PYU_CheckWarp')
        Unknown2006()
        Unknown2053(1)
        Unknown23078(1)
        Unknown23059(1)
    sprite('ko401_00', 4)
    physicsXImpulse(12000)
    sprite('ko401_01', 2)
    SFX_3('slash_rapier_fast')
    physicsXImpulse(12000)
    physicsYImpulse(5000)
    sprite('ko401_02', 1)
    RefreshMultihit()
    sprite('ko401_02', 2)
    sprite('ko401_03', 2)
    GFX_0('FirePitA', 100)
    Unknown1019(50)
    YAccel(50)
    sprite('ko401_04', 2)
    sprite('ko401_05', 2)
    sprite('ko401_06', 2)
    physicsXImpulse(-15000)
    physicsYImpulse(-2000)
    sprite('ko401_07', 4)
    sprite('ko401_08', 4)
    physicsYImpulse(1000)
    sprite('ko401_09', 4)
    SFX_3('cloth_h')
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_10', 4)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_11', 8)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_12', 8)
    sprite('ko401_13', 8)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaAirFirePitB():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 0, -2000000, 2000000, -2000000, 0)
        callSubroutine('PYU_SPAttackInit')
        AttackLevel_(3)
        Damage(500)
        AttackP1(90)
        AttackP2(100)
        AirHitstunAnimation(9)
        GroundedHitstunAnimation(9)
        AirUntechableTime(24)
        AirPushbackX(30000)
        AirPushbackY(20000)
        PushbackX(30400)
        Unknown9016(1)
        AttackAttributes(1, 0, 0, 0, 0)
        callSubroutine('PYU_CheckWarp')
        Unknown2006()
        Unknown2053(1)
        Unknown23078(1)
        Unknown23059(1)
    sprite('ko401_00', 4)
    physicsXImpulse(12000)
    sprite('ko401_01', 2)
    SFX_3('slash_rapier_fast')
    physicsXImpulse(12000)
    physicsYImpulse(5000)
    sprite('ko401_02', 1)
    RefreshMultihit()
    sprite('ko401_02', 2)
    Unknown23022(0)
    sprite('ko401_03', 2)
    GFX_0('FirePitB', 100)
    Unknown1019(50)
    YAccel(50)
    sprite('ko401_04', 2)
    sprite('ko401_05', 2)
    sprite('ko401_06', 2)
    physicsXImpulse(-15000)
    physicsYImpulse(-2000)
    sprite('ko401_07', 4)
    sprite('ko401_08', 4)
    physicsYImpulse(1000)
    sprite('ko401_09', 4)
    SFX_3('cloth_h')
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_10', 4)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_11', 8)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_12', 8)
    sprite('ko401_13', 8)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaAirFirePitC():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 0, 0, 2000000, -2000000, 0)
        callSubroutine('PYU_SPAttackInit')
        AttackLevel_(3)
        Damage(500)
        AttackP1(90)
        AttackP2(100)
        Unknown30065(0)
        MinimumDamagePct(10)
        GroundedHitstunAnimation(9)
        AirHitstunAnimation(9)
        AirPushbackX(1000)
        AirPushbackY(22000)
        PushbackX(30400)
        AirUntechableTime(24)
        Unknown9016(1)
        AttackAttributes(1, 0, 0, 0, 0)
        callSubroutine('PYU_CheckWarp')
        Unknown2006()
        Unknown2053(1)
        Unknown23078(1)
        Unknown23059(1)
    sprite('ko401_00', 4)
    physicsXImpulse(6000)
    sprite('ko401_01', 2)
    SFX_3('slash_rapier_fast')
    physicsXImpulse(12000)
    physicsYImpulse(5000)
    sprite('ko401_02', 1)
    RefreshMultihit()
    Unknown4007(0)
    sprite('ko401_02', 2)
    Unknown23022(0)
    sprite('ko401_03', 2)
    GFX_0('FirePitC', 100)
    Unknown1019(50)
    YAccel(50)
    sprite('ko401_04', 2)
    sprite('ko401_05', 2)
    sprite('ko401_06', 2)
    physicsXImpulse(-15000)
    physicsYImpulse(-2000)
    sprite('ko401_07', 4)
    sprite('ko401_08', 4)
    physicsYImpulse(1000)
    sprite('ko401_09', 4)
    SFX_3('cloth_h')
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_10', 4)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_11', 8)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_12', 8)
    sprite('ko401_13', 8)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaMaharagiA():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -50000, 0, 0, 0, 0, 0)
        callSubroutine('PYU_ForceWarp')
        callSubroutine('Init_CantCancelPersonaAction')
        Unknown4007(2)
        Unknown4008(2)
        callSubroutine('PYU_SPAttackInit')
        Unknown2006()
        Unknown23059(1)
    sprite('ko402_00', 5)
    physicsXImpulse(2000)
    sprite('ko402_01', 2)
    sprite('ko402_02', 1)
    SFX_3('slash_sword_slow')
    GFX_1('koef_firewing', 0)
    GFX_0('SlidingA', 1)
    Unknown23029(1, 4050, 0)
    sprite('ko402_02', 2)
    Unknown23022(0)
    sprite('ko402_03', 2)
    GFX_1('koef_firewing', 0)
    sprite('ko402_04', 2)
    GFX_1('koef_firewing', 0)
    sprite('ko402_05', 2)
    GFX_1('koef_firewing', 0)
    sprite('ko402_06', 2)
    GFX_1('koef_firewing', 0)
    physicsXImpulse(0)
    sprite('ko402_07', 6)
    sprite('ko402_08', 6)
    sprite('ko402_09', 6)
    SLOT_10 = 0
    sprite('ko402_07', 6)
    sprite('ko402_08', 6)
    sprite('ko402_09', 6)
    sprite('ko402_07', 6)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaMaharagiB():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -50000, 0, 0, 0, 0, 0)
        callSubroutine('PYU_ForceWarp')
        callSubroutine('Init_CantCancelPersonaAction')
        Unknown4007(2)
        Unknown4008(2)
        callSubroutine('PYU_SPAttackInit')
        Unknown2006()
        Unknown23059(1)
    sprite('ko402_00', 5)
    physicsXImpulse(2000)
    sprite('ko402_01', 2)
    sprite('ko402_02', 1)
    SFX_3('slash_sword_slow')
    GFX_1('koef_firewing', 0)
    GFX_0('SlidingB', 1)
    Unknown23029(1, 4060, 0)
    sprite('ko402_02', 2)
    Unknown23022(0)
    sprite('ko402_03', 2)
    GFX_1('koef_firewing', 0)
    sprite('ko402_04', 2)
    GFX_1('koef_firewing', 0)
    sprite('ko402_05', 2)
    GFX_1('koef_firewing', 0)
    sprite('ko402_06', 2)
    GFX_1('koef_firewing', 0)
    physicsXImpulse(0)
    sprite('ko402_07', 6)
    sprite('ko402_08', 6)
    sprite('ko402_09', 6)
    SLOT_10 = 0
    sprite('ko402_07', 6)
    sprite('ko402_08', 6)
    sprite('ko402_09', 6)
    sprite('ko402_07', 6)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaMaharagiC():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -50000, 0, 0, 0, 0, 0)
        callSubroutine('PYU_ForceWarp')
        callSubroutine('Init_CantCancelPersonaAction')
        Unknown4007(2)
        Unknown4008(2)
        callSubroutine('PYU_SPAttackInit')
        Unknown2006()
        Unknown23059(1)
    sprite('ko402_00', 5)
    physicsXImpulse(2000)
    sprite('ko402_01', 2)
    sprite('ko402_02', 1)
    SFX_3('slash_sword_slow')
    GFX_1('koef_firewing', 0)
    GFX_0('SlidingC', 1)
    Unknown23029(1, 4071, 0)
    sprite('ko402_02', 2)
    Unknown23022(0)
    sprite('ko402_03', 2)
    GFX_1('koef_firewing', 0)
    sprite('ko402_04', 2)
    GFX_1('koef_firewing', 0)
    sprite('ko402_05', 2)
    GFX_1('koef_firewing', 0)
    sprite('ko402_06', 2)
    GFX_1('koef_firewing', 0)
    physicsXImpulse(0)
    sprite('ko402_07', 6)
    sprite('ko402_08', 6)
    sprite('ko402_09', 6)
    SLOT_10 = 0
    sprite('ko402_07', 6)
    sprite('ko402_08', 6)
    sprite('ko402_09', 6)
    sprite('ko402_07', 6)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaFireBooster():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 0, 0, 0, 0, 0, 0)
        callSubroutine('PYU_SPAttackInit')
        callSubroutine('PYU_ForceWarp')
        Unknown4007(3)
        Unknown2019(100)
        Unknown2006()
        Unknown3032()
        Unknown3001(50)
        Unknown3004(20)
        Unknown23022(1)

        def upon_STATE_END():
            SLOT_10 = 0
            Unknown3031()
        Unknown23059(1)
    sprite('ko403_00', 4)
    sprite('ko403_01', 4)
    sprite('ko403_02', 4)
    sprite('ko403_03', 6)
    sprite('ko403_04', 6)
    sprite('ko403_05', 6)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaFireAttackA():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 0, 0, 0, 0, 0, 0)
        callSubroutine('PYU_SPAttackInit')
        Unknown17003()
        AttackLevel_(3)
        Damage(300)
        AirPushbackY(22000)
        Hitstop(5)
        Unknown9016(1)
        GroundedHitstunAnimation(9)
        AirHitstunAnimation(9)
        Unknown11092(1)
        AttackAttributes(0, 1, 0, 0, 0)
        callSubroutine('PYU_ForceWarp')
        callSubroutine('Init_CantCancelPersonaAction')
        Unknown23078(0)
        Unknown2006()
        Unknown2053(1)
        callSubroutine('BoosterDamageUp')
        Unknown23059(1)
    sprite('ko404_00', 3)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_01', 2)
    physicsXImpulse(14000)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_02', 2)
    SFX_3('slash_rapier_fast')
    Unknown1019(110)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_03', 1)
    Unknown1019(110)
    RefreshMultihit()
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_03', 1)
    Unknown23022(0)
    sprite('ko404_04', 2)
    Unknown1019(110)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_05', 2)
    Unknown1019(110)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_06', 2)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_07', 2)
    SFX_3('bomb_m')
    SFX_3('blaze_normal')
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_08', 3)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    GFX_1('koef_404_bom', 2)
    GFX_1('koef_404_fire', 3)
    GFX_1('koef_404_fire', 4)
    GFX_1('koef_404_fire', 5)
    GFX_1('koef_404_fire', 6)
    Unknown1019(0)
    RefreshMultihit()
    Unknown11034(0)
    ProjectileDurabilityLvl(1)
    AttackLevel_(3)
    Damage(500)
    AirUntechableTime(32)
    Hitstop(1)
    GroundedHitstunAnimation(9)
    AirHitstunAnimation(9)
    AirPushbackX(14000)
    AirPushbackY(21000)
    YImpluseBeforeWallbounce(1800)
    Unknown9016(0)
    Unknown9017(1)
    Unknown9021(1)
    callSubroutine('BoosterDamageUp')
    AttackAttributes(0, 0, 0, 0, 1)
    sprite('ko404_09', 2)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_08', 4)
    DisableAttackRestOfMove()
    SLOT_10 = 0
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PersonaFireAttackB():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 50000, 0, 0, 2000000, 0, 0)
        callSubroutine('PYU_AttackInit')
        AttackLevel_(3)
        Damage(900)
        AttackP1(90)
        AirPushbackY(22000)
        Hitstop(1)
        Unknown11001(6, 6, 6)
        Unknown9016(1)
        AirHitstunAnimation(9)
        Unknown11092(1)
        Unknown11044(1)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown23078(1)
        callSubroutine('PYU_ForceWarp')
        Unknown2053(1)
        if Unknown23145('PYU_Persona2B'):
            Unknown2006()
        callSubroutine('BoosterDamageUp')
        Unknown23059(1)
    sprite('ko404_00', 2)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    physicsXImpulse(20000)
    sprite('ko404_01', 2)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_02', 1)
    SFX_3('slash_rapier_fast')
    Unknown1019(120)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_03', 1)
    Unknown1019(120)
    RefreshMultihit()
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_03', 1)
    Unknown23022(0)
    sprite('ko404_04', 2)
    Unknown1019(120)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_05', 2)
    Unknown1019(120)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_06', 3)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_07', 3)
    SFX_3('bomb_m')
    SFX_3('blaze_normal')
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_08', 3)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    GFX_1('koef_404_bom', 2)
    GFX_1('koef_404_fire', 3)
    GFX_1('koef_404_fire', 4)
    GFX_1('koef_404_fire', 5)
    GFX_1('koef_404_fire', 6)
    Unknown1019(0)
    RefreshMultihit()
    Unknown11034(0)
    ProjectileDurabilityLvl(1)
    AttackLevel_(3)
    Damage(1600)
    Hitstop(1)
    Unknown23078(1)
    AirUntechableTime(47)
    GroundedHitstunAnimation(9)
    AirHitstunAnimation(9)
    AirPushbackX(23000)
    AirPushbackY(20000)
    YImpluseBeforeWallbounce(1800)
    Unknown9016(0)
    Unknown9017(1)
    Unknown9021(1)
    callSubroutine('BoosterDamageUp')
    AttackAttributes(0, 0, 0, 1, 0)
    Unknown30088(1)
    sprite('ko404_09', 2)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_08', 4)
    DisableAttackRestOfMove()
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def FlameDanceA():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 50000, 0, 0, 2000000, 0, 0)
        callSubroutine('PYU_AttackInit')
        AttackLevel_(3)
        Damage(500)
        AttackP1(90)
        AirPushbackY(22000)
        Hitstop(1)
        Unknown11001(6, 6, 6)
        Unknown9016(1)
        AirHitstunAnimation(9)
        Unknown11092(1)
        Unknown11044(1)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown23078(1)
        callSubroutine('PYU_ForceWarp')
        Unknown2053(1)
        if Unknown23145('PYU_Persona2B'):
            Unknown2006()
        callSubroutine('BoosterDamageUp')
    sprite('ko404_00', 2)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    physicsXImpulse(20000)
    sprite('ko404_01', 2)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_02', 1)
    SFX_3('slash_rapier_fast')
    Unknown1019(120)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_03', 1)
    Unknown1019(120)
    RefreshMultihit()
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_03', 1)
    Unknown23022(0)
    sprite('ko404_04', 2)
    Unknown1019(120)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_05', 2)
    Unknown1019(120)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_06', 3)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_07', 3)
    SFX_3('bomb_m')
    SFX_3('blaze_normal')
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_08', 3)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    GFX_1('koef_404_bom', 2)
    GFX_1('koef_404_fire', 3)
    GFX_1('koef_404_fire', 4)
    GFX_1('koef_404_fire', 5)
    GFX_1('koef_404_fire', 6)
    Unknown1019(0)
    RefreshMultihit()
    Unknown11034(0)
    ProjectileDurabilityLvl(1)
    AttackLevel_(3)
    Damage(1600)
    Hitstop(1)
    Unknown23078(1)
    AirUntechableTime(47)
    GroundedHitstunAnimation(9)
    AirHitstunAnimation(9)
    AirPushbackX(23000)
    AirPushbackY(20000)
    YImpluseBeforeWallbounce(1800)
    Unknown9016(0)
    Unknown9017(1)
    Unknown9021(1)
    callSubroutine('BoosterDamageUp')
    AttackAttributes(0, 0, 0, 1, 0)
    sprite('ko404_09', 2)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_08', 4)
    DisableAttackRestOfMove()
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def FlameDanceB():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 50000, 0, 0, 2000000, 0, 0)
        callSubroutine('PYU_AttackInit')
        AttackLevel_(4)
        Damage(900)
        AttackP1(90)
        AirPushbackY(22000)
        Hitstop(1)
        Unknown11001(6, 6, 6)
        Unknown9016(1)
        AirHitstunAnimation(9)
        Unknown11092(1)
        Unknown11044(1)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown23078(1)
        callSubroutine('PYU_ForceWarp')
        Unknown2053(1)
        if Unknown23145('PYU_Persona2B'):
            Unknown2006()
        callSubroutine('BoosterDamageUp')
    sprite('ko404_00', 4)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    physicsXImpulse(20000)
    sprite('ko404_01', 4)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_02', 2)
    SFX_3('slash_rapier_fast')
    Unknown1019(120)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_03', 2)
    Unknown1019(120)
    RefreshMultihit()
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_03', 1)
    Unknown23022(0)
    sprite('ko404_04', 2)
    Unknown1019(120)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_05', 2)
    Unknown1019(120)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_06', 3)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_07', 3)
    SFX_3('bomb_m')
    SFX_3('blaze_normal')
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_08', 3)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    GFX_1('koef_404_bom', 2)
    GFX_1('koef_404_fire', 3)
    GFX_1('koef_404_fire', 4)
    GFX_1('koef_404_fire', 5)
    GFX_1('koef_404_fire', 6)
    Unknown1019(0)
    RefreshMultihit()
    Unknown11034(0)
    ProjectileDurabilityLvl(1)
    AttackLevel_(4)
    Damage(1600)
    Hitstop(1)
    Unknown23078(1)
    AirUntechableTime(47)
    GroundedHitstunAnimation(9)
    AirHitstunAnimation(9)
    AirPushbackX(23000)
    AirPushbackY(20000)
    YImpluseBeforeWallbounce(1800)
    Unknown9016(0)
    Unknown9017(1)
    Unknown9021(1)
    callSubroutine('BoosterDamageUp')
    AttackAttributes(0, 0, 0, 1, 0)
    sprite('ko404_09', 2)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_08', 4)
    DisableAttackRestOfMove()
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def FlameDanceEX():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 0, 0, 0, 2000000, 0, 0)
        callSubroutine('PYU_AttackInit')
        AttackLevel_(5)
        Damage(1200)
        AttackP1(90)
        AirPushbackY(22000)
        Hitstop(1)
        Unknown11001(6, 6, 6)
        Unknown9016(1)
        AirHitstunAnimation(9)
        Unknown11092(1)
        Unknown11044(1)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown23078(1)
        callSubroutine('PYU_ForceWarp')
        Unknown2053(1)
        if Unknown23145('PYU_Persona2B'):
            Unknown2006()
        callSubroutine('BoosterDamageUp')
    sprite('ko404_00', 2)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    physicsXImpulse(20000)
    sprite('ko404_01', 2)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_02', 1)
    SFX_3('slash_rapier_fast')
    Unknown1019(120)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_03', 1)
    Unknown1019(120)
    RefreshMultihit()
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_03', 1)
    Unknown23022(0)
    sprite('ko404_04', 2)
    Unknown1019(120)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_05', 2)
    Unknown1019(120)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_06', 3)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_07', 3)
    SFX_3('bomb_m')
    SFX_3('blaze_normal')
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_08', 3)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    GFX_1('koef_404_bom', 2)
    GFX_1('koef_404_fire', 3)
    GFX_1('koef_404_fire', 4)
    GFX_1('koef_404_fire', 5)
    GFX_1('koef_404_fire', 6)
    Unknown1019(0)
    RefreshMultihit()
    Unknown11034(0)
    ProjectileDurabilityLvl(1)
    AttackLevel_(5)
    Damage(1600)
    Hitstop(1)
    Unknown23078(1)
    AirUntechableTime(47)
    GroundedHitstunAnimation(9)
    AirHitstunAnimation(9)
    AirPushbackX(50000)
    AirPushbackY(20000)
    YImpluseBeforeWallbounce(1300)
    Unknown9178(1)
    Unknown9016(0)
    Unknown9017(1)
    Unknown9021(1)
    callSubroutine('BoosterDamageUp')
    AttackAttributes(0, 0, 0, 1, 0)
    sprite('ko404_09', 2)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_08', 4)
    DisableAttackRestOfMove()
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaFireBird():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 120000, 0, 0, 0, 0)
        callSubroutine('PYU_SPAttackInit')
        AttackLevel_(4)
        Damage(1000)
        AttackP1(70)
        AirUntechableTime(80)
        AirPushbackY(40000)
        AirPushbackX(10000)
        GroundedHitstunAnimation(1)
        Hitstop(1)
        Unknown11034(0)
        ProjectileDurabilityLvl(1)
        WallbounceReboundTime(3)
        Unknown9017(1)
        Unknown23022(1)
        Unknown11092(1)
        AttackAttributes(0, 0, 0, 1, 0)
        Unknown11042(1)
        Unknown11061(1)
        Unknown23089(3, 1, 1, 1, 1, 0, 0, 0)
        sendToLabelUpon(54, 580)
        callSubroutine('PYU_ForceWarp')
        EnableCollision(0)
        callSubroutine('BoosterDamageUp')
        Unknown23059(1)
    sprite('ko405_00', 3)
    sprite('ko405_01', 3)
    sprite('ko405_02', 3)
    sprite('ko405_03', 2)
    sprite('ko405_04', 2)
    sprite('ko405_05', 2)
    sprite('ko405_06', 3)
    SFX_3('airbackdash_l')
    physicsXImpulse(10000)
    sprite('ko405_07', 2)
    Unknown1019(200)
    sprite('ko405_08', 2)
    Unknown1019(200)
    label(0)
    sprite('ko405_09', 3)
    SLOT_51 = (SLOT_51 + 1)
    if (SLOT_51 >= 8):
        gotoLabel(1)
    GFX_0('firebard', 100)
    SFX_3('airdash_m')
    SFX_3('blaze_normal')
    RefreshMultihit()
    Unknown1019(110)
    sprite('ko405_10', 3)
    RefreshMultihit()
    Unknown1019(110)
    gotoLabel(0)
    label(580)
    sprite('ko405_09', 3)
    SFX_3('blaze_long')
    Unknown1019(110)
    sprite('ko405_10', 3)
    Unknown1019(110)
    sprite('ko405_09', 3)
    Unknown1019(110)
    sprite('ko405_10', 3)
    Unknown1019(110)
    sprite('ko405_09', 3)
    Unknown1019(110)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    label(1)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaFireBirdA():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 120000, 0, 0, 0, 0)
        callSubroutine('PYU_SPAttackInit')
        AttackLevel_(4)
        Damage(1000)
        AttackP1(70)
        AirUntechableTime(80)
        AirPushbackY(40000)
        AirPushbackX(10000)
        GroundedHitstunAnimation(1)
        Hitstop(1)
        Unknown11034(0)
        ProjectileDurabilityLvl(1)
        WallbounceReboundTime(3)
        Unknown9017(1)
        Unknown23022(1)
        Unknown11092(1)
        AttackAttributes(0, 0, 0, 1, 0)
        Unknown11042(1)
        Unknown11061(1)
        Unknown23089(3, 1, 1, 1, 1, 0, 0, 0)
        sendToLabelUpon(54, 580)
        callSubroutine('PYU_ForceWarp')
        EnableCollision(0)
        callSubroutine('BoosterDamageUp')
    sprite('ko405_00', 2)
    sprite('ko405_01', 2)
    sprite('ko405_02', 2)
    sprite('ko405_03', 2)
    sprite('ko405_04', 2)
    sprite('ko405_05', 2)
    sprite('ko405_06', 2)
    SFX_3('airbackdash_l')
    physicsXImpulse(10000)
    sprite('ko405_07', 2)
    Unknown1019(200)
    sprite('ko405_08', 2)
    Unknown1019(200)
    label(0)
    sprite('ko405_09', 3)
    SLOT_51 = (SLOT_51 + 1)
    if (SLOT_51 >= 8):
        gotoLabel(1)
    GFX_0('firebard', 100)
    SFX_3('airdash_m')
    SFX_3('blaze_normal')
    RefreshMultihit()
    Unknown1019(110)
    sprite('ko405_10', 3)
    RefreshMultihit()
    Unknown1019(110)
    gotoLabel(0)
    label(580)
    sprite('ko405_09', 3)
    SFX_3('blaze_long')
    Unknown1019(110)
    sprite('ko405_10', 3)
    Unknown1019(110)
    sprite('ko405_09', 3)
    Unknown1019(110)
    sprite('ko405_10', 3)
    Unknown1019(110)
    sprite('ko405_09', 3)
    Unknown1019(110)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    label(1)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaFireBirdB():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 120000, 0, 0, 0, 0)
        callSubroutine('PYU_SPAttackInit')
        AttackLevel_(4)
        Damage(1250)
        AttackP1(70)
        AirUntechableTime(80)
        AirPushbackY(40000)
        AirPushbackX(10000)
        GroundedHitstunAnimation(1)
        Hitstop(1)
        Unknown11034(0)
        ProjectileDurabilityLvl(1)
        WallbounceReboundTime(3)
        Unknown9017(1)
        Unknown23022(1)
        Unknown11092(1)
        AttackAttributes(0, 0, 0, 1, 0)
        Unknown11042(1)
        Unknown11061(1)
        Unknown23089(3, 1, 1, 1, 1, 0, 0, 0)
        sendToLabelUpon(54, 580)
        callSubroutine('PYU_ForceWarp')
        EnableCollision(0)
        callSubroutine('BoosterDamageUp')
    sprite('ko405_00', 3)
    sprite('ko405_01', 3)
    sprite('ko405_02', 3)
    sprite('ko405_03', 2)
    sprite('ko405_04', 2)
    sprite('ko405_05', 2)
    sprite('ko405_06', 3)
    SFX_3('airbackdash_l')
    physicsXImpulse(10000)
    sprite('ko405_07', 2)
    Unknown1019(200)
    sprite('ko405_08', 2)
    Unknown1019(200)
    label(0)
    sprite('ko405_09', 3)
    SLOT_51 = (SLOT_51 + 1)
    if (SLOT_51 >= 8):
        gotoLabel(1)
    GFX_0('firebard', 100)
    SFX_3('airdash_m')
    SFX_3('blaze_normal')
    RefreshMultihit()
    Unknown1019(110)
    sprite('ko405_10', 3)
    RefreshMultihit()
    Unknown1019(130)
    gotoLabel(0)
    label(580)
    sprite('ko405_09', 3)
    SFX_3('blaze_long')
    Unknown1019(130)
    sprite('ko405_10', 3)
    Unknown1019(130)
    sprite('ko405_09', 3)
    Unknown1019(130)
    sprite('ko405_10', 3)
    Unknown1019(130)
    sprite('ko405_09', 3)
    Unknown1019(130)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    label(1)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaFireBirdC():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 120000, 0, 0, 0, 0)
        callSubroutine('PYU_SPAttackInit')
        AttackLevel_(5)
        Damage(1500)
        AttackP1(80)
        AirUntechableTime(80)
        AirPushbackY(40000)
        AirPushbackX(10000)
        GroundedHitstunAnimation(1)
        Hitstop(1)
        Unknown11034(0)
        ProjectileDurabilityLvl(1)
        WallbounceReboundTime(3)
        Unknown9017(1)
        Unknown23022(1)
        Unknown11092(1)
        AttackAttributes(0, 0, 0, 1, 0)
        Unknown11042(1)
        Unknown11061(1)
        Unknown23089(3, 1, 1, 1, 1, 0, 0, 0)
        Unknown22019(1, 1, 1, 1, 1)
        sendToLabelUpon(54, 580)
        callSubroutine('PYU_ForceWarp')
        EnableCollision(0)
        callSubroutine('BoosterDamageUp')
    sprite('ko405_00', 2)
    sprite('ko405_01', 2)
    sprite('ko405_02', 2)
    sprite('ko405_03', 1)
    sprite('ko405_04', 2)
    sprite('ko405_05', 2)
    sprite('ko405_06', 2)
    SFX_3('airbackdash_l')
    physicsXImpulse(10000)
    sprite('ko405_07', 2)
    Unknown1019(200)
    sprite('ko405_08', 2)
    Unknown1019(200)
    label(0)
    sprite('ko405_09', 3)
    SLOT_51 = (SLOT_51 + 1)
    if (SLOT_51 >= 8):
        gotoLabel(1)
    GFX_0('firebard', 100)
    SFX_3('airdash_m')
    SFX_3('blaze_normal')
    RefreshMultihit()
    Unknown1019(110)
    sprite('ko405_10', 3)
    RefreshMultihit()
    Unknown1019(110)
    gotoLabel(0)
    label(580)
    sprite('ko405_09', 3)
    SFX_3('blaze_long')
    Unknown1019(150)
    sprite('ko405_10', 3)
    Unknown1019(150)
    sprite('ko405_09', 3)
    Unknown1019(150)
    sprite('ko405_10', 3)
    Unknown1019(150)
    sprite('ko405_09', 3)
    Unknown1019(150)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    label(1)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@Subroutine
def Cheak_IputHold():
    SLOT_52 = 0
    if CheckInput(0x1):
        SLOT_52 = 1
    if CheckInput(0xa):
        SLOT_52 = 1
    if CheckInput(0x13):
        SLOT_52 = 1
    if CheckInput(0x1c):
        SLOT_52 = 1

@Subroutine
def Agidine_Move():

    def upon_3():
        if SLOT_55:
            Unknown48(25, 2, 53, 2, 2, 38)
            if (SLOT_53 == SLOT_38):
                physicsYImpulse(0)
                physicsXImpulse(0)
                if CheckInput(0x37):
                    physicsXImpulse(-20000)
                    physicsYImpulse(-20000)
                if CheckInput(0x44):
                    physicsYImpulse(-20000)
                if CheckInput(0x51):
                    physicsXImpulse(20000)
                    physicsYImpulse(-20000)
                if CheckInput(0x5e):
                    physicsXImpulse(-20000)
                if CheckInput(0x6b):
                    pass
                if CheckInput(0x78):
                    physicsXImpulse(20000)
                if CheckInput(0x85):
                    physicsXImpulse(-20000)
                    physicsYImpulse(20000)
                if CheckInput(0x92):
                    physicsYImpulse(20000)
                if CheckInput(0x9f):
                    physicsXImpulse(20000)
                    physicsYImpulse(20000)
            else:
                if CheckInput(0x51):
                    physicsXImpulse(-20000)
                    physicsYImpulse(-20000)
                if CheckInput(0x44):
                    physicsYImpulse(-20000)
                if CheckInput(0x37):
                    physicsXImpulse(20000)
                    physicsYImpulse(-20000)
                if CheckInput(0x78):
                    physicsXImpulse(-20000)
                if CheckInput(0x6b):
                    pass
                if CheckInput(0x5e):
                    physicsXImpulse(20000)
                if CheckInput(0x9f):
                    physicsXImpulse(-20000)
                    physicsYImpulse(20000)
                if CheckInput(0x92):
                    physicsYImpulse(20000)
                if CheckInput(0x85):
                    physicsXImpulse(20000)
                    physicsYImpulse(20000)

@State
def PYU_PersonaAgidine():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 50000, 0, 0, 2000000, -1000000, 1000000)
        callSubroutine('PYU_DDAttackInit')
        AttackLevel_(4)
        Damage(540)
        Hitstop(1)
        PushbackX(2000)
        AirPushbackX(10000)
        AirPushbackY(20000)
        AttackP2(98)
        AirHitstunAnimation(10)
        GroundedHitstunAnimation(10)
        AirUntechableTime(30)
        Unknown9021(1)
        Unknown9017(1)
        Unknown11056(0)
        Unknown30055(0, 80, 0)
        Unknown30056(60000, 80, 60)
        Unknown11057(0)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown23022(1)
        Unknown23182(2)
        Unknown2054(1)
        callSubroutine('BoosterDamageUp')
        callSubroutine('PYU_CheckWarp')
        callSubroutine('Init_CantCancelPersonaAction')
        Unknown2006()
        EnableCollision(0)
        Unknown2053(1)
        Unknown2034(0)
        teleportRelativeY(0)
        Unknown23066(1)
        setInvincible(1)
        DisableAttackRestOfMove()
        Unknown23059(1)
    sprite('ko430_00', 2)
    sprite('ko430_01', 2)
    physicsXImpulse(14000)
    sprite('ko430_02', 2)
    sprite('ko430_03', 2)
    sprite('ko430_04', 2)
    sprite('ko430_05', 2)
    SFX_3('spin_m')
    label(0)
    sprite('ko430_00', 2)
    if (SLOT_51 >= 4):
        gotoLabel(1)
    SLOT_51 = (SLOT_51 + 1)
    Unknown1019(50)
    sprite('ko430_01', 2)
    sprite('ko430_02', 2)
    RefreshMultihit()
    if op(4, 2, 51, 0, 2):
        GFX_0('agidine_firering', 100)
        GFX_0('firering_sub', 100)
    SFX_3('blaze_normal')
    sprite('ko430_03', 2)
    sprite('ko430_04', 2)
    sprite('ko430_05', 2)
    RefreshMultihit()
    SFX_3('spin_m')
    loopRest()
    gotoLabel(0)
    label(1)
    sprite('ko430_00', 2)
    Unknown1084(1)
    Unknown2034(0)
    sprite('ko430_01', 2)
    sprite('ko430_02', 2)
    sprite('ko430_03', 2)
    RefreshMultihit()
    AirPushbackX(0)
    sprite('ko430_06', 2)
    Unknown1084(1)
    clearUponHandler(3)
    sprite('ko430_07', 3)
    GFX_1('koef_firewing', 0)
    sprite('ko430_08', 3)
    GFX_1('koef_firewing', 0)
    sprite('ko430_09', 3)
    GFX_1('koef_firewing', 0)
    Unknown2006()
    if (SLOT_51 >= 4):
        GFX_0('agidine_fireuzuM', 0)
    sprite('ko430_10', 4)
    GFX_1('koef_firewing', 0)
    Unknown21012('FireYugamiB2', 32)
    setInvincible(0)
    sprite('ko430_11', 4)
    sprite('ko430_12', 4)
    sprite('ko430_10', 4)
    sprite('ko430_11', 4)
    sprite('ko430_12', 4)
    sprite('ko430_10', 5)
    sprite('ko430_11', 5)
    sprite('ko430_12', 5)
    sprite('ko430_10', 5)
    sprite('ko430_11', 5)
    sprite('ko430_12', 5)
    SLOT_10 = 0
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaAgidine_OD():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 50000, 0, 0, 2000000, -1000000, 1000000)
        callSubroutine('PYU_DDAttackInit')
        Unknown23056()
        AttackLevel_(4)
        Damage(620)
        AttackP2(98)
        Hitstop(1)
        PushbackX(2000)
        AirPushbackX(10000)
        AirPushbackY(20000)
        AirHitstunAnimation(10)
        GroundedHitstunAnimation(10)
        AirUntechableTime(30)
        Unknown9021(1)
        Unknown9017(1)
        Unknown11056(0)
        Unknown30055(0, 80, 0)
        Unknown30056(60000, 80, 60)
        Unknown11057(0)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown23022(1)
        Unknown23182(2)
        Unknown2054(1)
        callSubroutine('BoosterDamageUp')
        callSubroutine('PYU_CheckWarp')
        callSubroutine('Init_CantCancelPersonaAction')
        Unknown2006()
        EnableCollision(0)
        Unknown2053(1)
        Unknown2034(0)
        teleportRelativeY(0)
        Unknown23066(1)
        setInvincible(1)
        DisableAttackRestOfMove()

        def upon_STATE_END():
            SLOT_10 = 0
        Unknown23059(1)
    sprite('ko430_00', 2)
    sprite('ko430_01', 2)
    physicsXImpulse(14000)
    sprite('ko430_02', 2)
    sprite('ko430_03', 2)
    sprite('ko430_04', 2)
    sprite('ko430_05', 2)
    SFX_3('spin_m')
    label(0)
    sprite('ko430_00', 2)
    if (SLOT_51 >= 4):
        gotoLabel(1)
    SLOT_51 = (SLOT_51 + 1)
    Unknown1019(50)
    sprite('ko430_01', 2)
    sprite('ko430_02', 2)
    RefreshMultihit()
    if op(4, 2, 51, 0, 2):
        GFX_0('agidine_firering', 100)
        GFX_0('firering_sub', 100)
    SFX_3('blaze_normal')
    sprite('ko430_03', 2)
    sprite('ko430_04', 2)
    sprite('ko430_05', 2)
    RefreshMultihit()
    SFX_3('spin_m')
    loopRest()
    gotoLabel(0)
    label(1)
    sprite('ko430_00', 2)
    Unknown1084(1)
    Unknown2034(0)
    sprite('ko430_01', 2)
    sprite('ko430_02', 2)
    sprite('ko430_03', 2)
    RefreshMultihit()
    AirPushbackX(0)
    sprite('ko430_06', 2)
    Unknown1084(1)
    clearUponHandler(3)
    sprite('ko430_07', 3)
    GFX_1('koef_firewing', 0)
    sprite('ko430_08', 3)
    GFX_1('koef_firewing', 0)
    sprite('ko430_09', 3)
    GFX_1('koef_firewing', 0)
    Unknown2006()
    if (SLOT_51 >= 4):
        GFX_0('agidine_fireuzuL', 0)
    sprite('ko430_10', 4)
    GFX_1('koef_firewing', 0)
    Unknown21012('FireYugamiB2', 32)
    setInvincible(0)
    sprite('ko430_11', 4)
    sprite('ko430_12', 4)
    sprite('ko430_10', 4)
    sprite('ko430_11', 4)
    sprite('ko430_12', 4)
    sprite('ko430_10', 5)
    sprite('ko430_11', 5)
    sprite('ko430_12', 5)
    sprite('ko430_10', 5)
    sprite('ko430_11', 5)
    sprite('ko430_12', 5)
    SLOT_10 = 0
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaAgidineDUO():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 50000, 0, 0, 2000000, -1000000, 1000000)
        callSubroutine('PYU_DDAttackInit')
        AttackLevel_(4)
        Damage(100)
        Hitstop(1)
        PushbackX(2000)
        AirPushbackX(10000)
        AirPushbackY(20000)
        AirHitstunAnimation(10)
        GroundedHitstunAnimation(10)
        AirUntechableTime(30)
        Unknown9021(1)
        Unknown9017(1)
        Unknown11056(0)
        Unknown30055(0, 80, 0)
        Unknown30056(60000, 80, 60)
        Unknown11057(0)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown23022(1)
        Unknown23182(2)
        Unknown11092(1)
        AttackP1(100)
        MinimumDamagePct(100)
        AttackP2(100)
        Unknown2054(1)
        callSubroutine('PYU_CheckWarp')
        callSubroutine('Init_CantCancelPersonaAction')
        Unknown2006()
        EnableCollision(0)
        Unknown2053(1)
        Unknown2034(0)
        teleportRelativeY(0)
        Unknown23066(1)
        setInvincible(1)
        DisableAttackRestOfMove()
        Unknown23059(1)
    sprite('ko430_06', 2)
    Unknown1084(1)
    clearUponHandler(3)
    sprite('ko430_07', 3)
    GFX_1('koef_firewing', 0)
    sprite('ko430_08', 3)
    GFX_1('koef_firewing', 0)
    sprite('ko430_09', 3)
    GFX_1('koef_firewing', 0)
    Unknown2006()
    GFX_0('agidine_fireuzuL_TAG', 0)
    sprite('ko430_10', 4)
    GFX_1('koef_firewing', 0)
    Unknown21012('FireYugamiB2', 32)
    setInvincible(0)
    sprite('ko430_11', 4)
    sprite('ko430_12', 4)
    sprite('ko430_10', 4)
    sprite('ko430_11', 4)
    sprite('ko430_12', 4)
    sprite('ko430_10', 5)
    sprite('ko430_11', 5)
    sprite('ko430_12', 5)
    sprite('ko430_10', 5)
    sprite('ko430_11', 5)
    sprite('ko430_12', 5)
    SLOT_10 = 0
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaAgidineDUO_OD():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 50000, 0, 0, 2000000, -1000000, 1000000)
        callSubroutine('PYU_DDAttackInit')
        Unknown23056()
        AttackLevel_(4)
        Damage(100)
        Hitstop(1)
        PushbackX(2000)
        AirPushbackX(10000)
        AirPushbackY(20000)
        AirHitstunAnimation(10)
        GroundedHitstunAnimation(10)
        AirUntechableTime(30)
        Unknown9021(1)
        Unknown9017(1)
        Unknown11056(0)
        Unknown30055(0, 80, 0)
        Unknown30056(60000, 80, 60)
        Unknown11057(0)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown23022(1)
        Unknown23182(2)
        Unknown11092(1)
        MinimumDamagePct(100)
        AttackP1(100)
        AttackP2(100)
        Unknown2054(1)
        callSubroutine('PYU_CheckWarp')
        callSubroutine('Init_CantCancelPersonaAction')
        Unknown2006()
        EnableCollision(0)
        Unknown2053(1)
        Unknown2034(0)
        teleportRelativeY(0)
        Unknown23066(1)
        setInvincible(1)
        DisableAttackRestOfMove()
        Unknown23059(1)
    sprite('ko430_06', 2)
    Unknown1084(1)
    clearUponHandler(3)
    sprite('ko430_07', 3)
    GFX_1('koef_firewing', 0)
    sprite('ko430_08', 3)
    GFX_1('koef_firewing', 0)
    sprite('ko430_09', 3)
    GFX_1('koef_firewing', 0)
    Unknown2006()
    GFX_0('agidine_fireuzuL_OD_TAG', 0)
    sprite('ko430_10', 4)
    GFX_1('koef_firewing', 0)
    Unknown21012('FireYugamiB2', 32)
    setInvincible(0)
    sprite('ko430_11', 4)
    sprite('ko430_12', 4)
    sprite('ko430_10', 4)
    sprite('ko430_11', 4)
    sprite('ko430_12', 4)
    sprite('ko430_10', 5)
    sprite('ko430_11', 5)
    sprite('ko430_12', 5)
    sprite('ko430_10', 5)
    sprite('ko430_11', 5)
    sprite('ko430_12', 5)
    SLOT_10 = 0
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaMaharagidine():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 0, 0, 0, 0, 0, 0)
        callSubroutine('PYU_DDAttackInit')
        callSubroutine('PYU_ForceWarp')
        callSubroutine('Init_CantCancelPersonaAction')
        Unknown4007(3)
        Unknown2019(100)
        Unknown1084(1)
        Unknown3032()
        Unknown3001(50)
        Unknown3004(10)
        Unknown2054(1)
        Unknown23022(1)
        Unknown2034(0)

        def upon_STATE_END():
            SLOT_10 = 0
            Unknown3031()
        Unknown23059(1)
    sprite('ko431_00', 6)
    physicsYImpulse(-5000)
    sprite('ko431_01', 6)
    sprite('ko431_02', 6)
    sprite('ko431_03', 4)
    sprite('ko431_04', 4)
    sprite('ko431_05', 4)
    sprite('ko431_06', 4)
    sprite('ko431_07', 4)
    sprite('ko431_08', 4)
    sprite('ko431_09', 4)
    sprite('ko431_10', 4)
    physicsYImpulse(8000)
    if (not CheckInput(0xa)):
        if (not CheckInput(0x13)):
            GFX_0('MaharagidineC_fire', 104)
            Unknown23029(2, 10055, 0)
        else:
            GFX_0('MaharagidineD_fire', 104)
    else:
        GFX_0('MaharagidineD_fire', 104)
    sprite('ko431_11', 4)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    YAccel(80)
    sprite('ko431_12', 6)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    YAccel(80)
    sprite('ko431_13', 6)
    YAccel(80)
    sprite('ko431_14', 6)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    YAccel(80)
    sprite('ko431_12', 6)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    YAccel(80)
    sprite('ko431_13', 6)
    YAccel(80)
    sprite('ko431_14', 6)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    YAccel(0)
    sprite('ko431_12', 8)
    SLOT_10 = 0
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaMaharagidine_OD():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 0, 0, 0, 0, 0, 0)
        callSubroutine('PYU_DDAttackInit')
        callSubroutine('PYU_ForceWarp')
        callSubroutine('Init_CantCancelPersonaAction')
        Unknown4007(3)
        Unknown2019(100)
        Unknown1084(1)
        Unknown3032()
        Unknown3001(50)
        Unknown3004(10)
        Unknown2054(1)
        Unknown23022(1)

        def upon_STATE_END():
            SLOT_10 = 0
            Unknown3031()
        Unknown23059(1)
    sprite('ko431_00', 6)
    physicsYImpulse(-5000)
    sprite('ko431_01', 6)
    sprite('ko431_02', 6)
    sprite('ko431_03', 4)
    sprite('ko431_04', 4)
    sprite('ko431_05', 4)
    sprite('ko431_06', 4)
    sprite('ko431_07', 4)
    sprite('ko431_08', 4)
    sprite('ko431_09', 4)
    sprite('ko431_10', 4)
    physicsYImpulse(8000)
    if CheckInput(0xa):
        if CheckInput(0x13):
            GFX_0('MaharagidineOD_D_fire', 104)
        else:
            GFX_0('MaharagidineOD_C_fire', 104)
            Unknown23029(2, 10055, 0)
    else:
        GFX_0('MaharagidineOD_C_fire', 104)
        Unknown23029(2, 10055, 0)
    sprite('ko431_11', 4)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    YAccel(80)
    sprite('ko431_12', 6)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    YAccel(80)
    sprite('ko431_13', 6)
    YAccel(80)
    sprite('ko431_14', 6)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    YAccel(80)
    sprite('ko431_12', 6)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    YAccel(80)
    sprite('ko431_13', 6)
    YAccel(80)
    sprite('ko431_14', 6)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    YAccel(0)
    sprite('ko431_12', 8)
    SLOT_10 = 0
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PersonaIchigeki():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -200000, 0, 0, 0, 0, 0)
        callSubroutine('PYU_AstralAttackInit')
        AttackDefaults_Astral()
        AttackLevel_(5)
        Damage(873)
        AttackP1(90)
        Hitstop(1)
        Unknown9016(1)
        Unknown11064(1)
        AirHitstunAnimation(9)
        GroundedHitstunAnimation(9)
        Unknown11056(0)
        AirPushbackX(84000)
        AirPushbackY(28000)
        AirUntechableTime(10000)
        Unknown9310(60)
        Unknown9178(1)
        Unknown9346(0)
        Unknown4007(3)
        callSubroutine('PYU_CheckWarp')
        Unknown23022(1)

        def upon_12():
            SLOT_51 = 1
            Unknown21007(3, 32)
            EnableCollision(0)
    sprite('ko450_00', 2)
    Unknown3029(1)
    Unknown3069(2)
    Unknown3070(2)
    AfterimageCount(5)
    AfterimageMask_1(0, 200, 0, 0)
    AfterimageMask_2(0, 0, 0, 0)
    physicsXImpulse(20000)
    Unknown2053(1)
    Unknown4007(0)
    sprite('ko450_01', 2)
    SFX_3('airdash_m')
    physicsXImpulse(30000)
    sprite('ko450_00', 2)
    physicsXImpulse(60000)
    sprite('ko450_01', 3)
    sprite('ko206_02', 2)
    Unknown1019(80)
    sprite('ko206_03', 2)
    Unknown23119(-2351616, 20, 2)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    Unknown1019(80)
    sprite('ko206_04', 2)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    Unknown1019(80)
    SFX_3('slash_sword_middle')
    sprite('ko206_05', 2)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    Unknown1019(80)
    sprite('ko206_06', 2)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    Unknown1019(50)
    sprite('ko206_07', 3)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    Unknown1084(1)
    sprite('ko206_08', 2)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    sprite('ko206_09', 2)
    sprite('ko206_10', 2)
    Unknown19(0, 2, 51)
    label(1)
    sprite('ko450_02', 2)

    def upon_3():
        Unknown48(25, 2, 52, 22, 2, 12)
        if (SLOT_52 > 0):
            clearUponHandler(3)
            Unknown36(22)
            Unknown1019(1)
            YAccel(1)
            Unknown1039(1)
            Unknown35()
    sprite('ko450_02', 3)
    sprite('ko450_03', 3)
    Unknown23119(-7602176, 40, 4)
    GFX_0('Konohana_charge', 0)
    Unknown38(5, 1)
    sprite('ko450_04', 3)
    sprite('ko450_05', 3)
    sprite('ko450_06', 3)
    sprite('ko450_07', 3)
    sprite('ko450_07', 6)
    Unknown23118(-19456)
    sprite('ko450_08', 80)
    GFX_0('Konohana_egg', 0)
    Unknown38(6, 1)
    sprite('ko450_08', 10)
    SFX_3('yu003')
    Unknown21007(5, 32)
    Unknown21007(6, 32)
    GFX_0('Konohana_burst', 0)
    GFX_0('BurstYugami', 0)
    ScreenShake(50000, 50000)
    sprite('null', 6)
    Unknown36(22)
    Unknown1019(0)
    YAccel(0)
    Unknown1039(-1)
    Unknown35()
    sprite('null', 32767)
    sendToLabel(99)
    label(0)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')
    label(99)
    sprite('keep', 32767)
    enterState('PYU_PersonaWait')

@State
def PYU_Crash_Rush1st():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 0, 0, 2000000, 0, 0)
        callSubroutine('PYU_AttackInit')
        AttackDefaults_StandingNormal()
        DisableAttackRestOfMove()
        Unknown9016(1)
        Unknown23022(1)
        Unknown4009(3)

        def upon_LANDING():
            Unknown1084(1)
            Unknown8000(104, 1, 1)
        callSubroutine('PYU_CheckWarp')
        Unknown2006()
        Unknown23078(1)
    sprite('ko205_00', 3)
    teleportRelativeY(10000)
    physicsXImpulse(5000)
    physicsYImpulse(18000)
    setGravity(1800)
    Unknown2053(1)
    sprite('ko205_01', 3)
    sprite('ko205_00', 3)
    sprite('ko205_03', 4)
    SFX_3('slash_rapier_middle')
    sprite('ko205_04', 4)
    GFX_1('koef_firewing', 0)
    sprite('ko205_04', 1)
    sprite('ko205_04', 1)
    sprite('ko205_06', 2)
    Unknown23054('ko205_05', 5)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    sprite('ko205_06', 1)
    sprite('ko205_06', 1)
    sprite('ko205_07', 4)
    GFX_1('koef_firewing', 0)
    sprite('ko205_08', 4)
    GFX_1('koef_firewing', 0)
    sprite('ko205_09', 4)
    GFX_1('koef_firewing', 0)
    sprite('ko205_10', 4)
    GFX_1('koef_firewing', 0)
    sprite('ko205_11', 4)
    sprite('ko205_12', 4)
    sprite('ko205_11', 4)
    sprite('ko205_12', 4)
    sprite('ko205_11', 4)
    sprite('ko205_12', 4)
    sprite('ko205_11', 4)
    sprite('ko205_12', 4)
    sprite('ko205_11', 4)
    sprite('ko205_12', 4)
    sprite('ko205_11', 4)
    sprite('ko205_12', 4)
    sprite('ko205_11', 4)
    sprite('ko205_12', 4)
    sprite('ko205_11', 4)
    sprite('ko205_12', 4)
    sprite('ko205_11', 4)
    sprite('ko205_12', 4)
    label(580)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaCrushAttack():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 0, -2000000, 2000000, -2000000, 2000000)
        callSubroutine('PYU_AttackInit')
        AttackDefaults_StandingNormal()
        AttackAttributes(1, 0, 0, 0, 0)
        Unknown4007(3)
        DisableAttackRestOfMove()
        Unknown9016(1)
        Unknown23022(1)
        Unknown4009(3)
        Unknown2006()
        Unknown23078(1)
    sprite('ko206_03', 1)
    GFX_1('koef_firewing', 0)
    Unknown1017()
    Unknown1022()
    Unknown1037()
    Unknown1084(1)
    SFX_3('slash_sword_middle')
    sprite('ko206_03', 1)
    Unknown1018()
    Unknown1023()
    Unknown1038()
    sprite('ko206_03', 1)
    sprite('ko206_04', 1)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    sprite('ko206_04', 1)
    sprite('ko206_04', 1)
    sprite('ko206_05', 1)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    sprite('ko206_05', 1)
    sprite('ko206_05', 1)
    sprite('ko206_06', 4)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    Unknown8000(104, 1, 0)
    sprite('ko206_07', 3)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    sprite('ko206_08', 3)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    sprite('ko206_09', 3)
    sprite('ko206_10', 3)
    label(580)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_Crash_Rush():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 0, 0, 0, 0, 0, 0)
        callSubroutine('PYU_SPAttackInit')
        Unknown17003()
        Unknown9016(1)
        DisableAttackRestOfMove()
        Unknown23022(1)
        callSubroutine('PYU_ForceWarp')
        callSubroutine('Init_CantCancelPersonaAction')
        EnableCollision(0)
    sprite('ko405_00', 2)
    sprite('ko405_01', 2)
    sprite('ko405_02', 2)
    sprite('ko405_03', 2)
    sprite('ko405_04', 2)
    sprite('ko405_05', 1)
    sprite('ko405_06', 1)
    SFX_3('airbackdash_l')
    physicsXImpulse(10000)
    sprite('ko405_07', 2)
    Unknown1019(200)
    sprite('ko405_08', 2)
    Unknown1019(200)
    sprite('ko405_09', 3)
    GFX_0('firebard', 100)
    SFX_3('airdash_m')
    SFX_3('blaze_normal')
    Unknown1019(110)
    sprite('ko405_10', 3)
    Unknown1019(110)
    sprite('ko405_09', 3)
    SFX_3('airdash_m')
    SFX_3('blaze_long')
    Unknown1019(110)
    sprite('ko405_10', 3)
    Unknown1019(110)
    sprite('ko405_09', 3)
    SFX_3('airdash_m')
    SFX_3('blaze_long')
    Unknown1019(110)
    sprite('ko405_10', 3)
    Unknown1019(110)
    sprite('ko405_09', 3)
    SFX_3('airdash_m')
    SFX_3('blaze_long')
    Unknown1019(110)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    sprite('ko405_09', 3)
    sprite('ko405_10', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_Persona5C_TAG():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 0, 0, 0, 2000000, 0, 0)
        callSubroutine('PYU_AttackInit')
        AttackDefaults_StandingNormal()
        AttackLevel_(3)
        Damage(300)
        Hitstop(1)
        AirUntechableTime(60)
        Unknown9016(1)
        GroundedHitstunAnimation(10)
        AirPushbackX(2500)
        AirPushbackY(35000)
        Unknown11057(500)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown11092(1)
        Unknown11042(1)
        callSubroutine('PYU_CheckWarp')
        Unknown2006()
        EnableCollision(1)

        def upon_11():
            SLOT_51 = 1
        Unknown23078(1)

        def upon_3():
            if (SLOT_19 <= 150000):
                sendToLabel(1)
        Unknown23059(1)
    sprite('ko206_00', 3)
    physicsXImpulse(20000)
    SFX_0('airdash_m')
    sprite('ko206_01', 3)
    physicsXImpulse(30000)
    sprite('ko206_00', 3)
    Unknown1019(100)
    label(1)
    sprite('ko206_01', 2)
    clearUponHandler(3)
    sprite('ko206_01', 2)
    sprite('ko206_00', 2)
    sprite('ko206_02', 2)
    sprite('ko206_03', 1)
    sprite('ko206_03', 1)
    GFX_1('koef_firewing', 0)
    RefreshMultihit()
    Unknown1019(20)
    SFX_0('slash_sword_middle')
    sprite('ko206_03', 1)
    RefreshMultihit()
    sprite('ko206_03', 1)
    RefreshMultihit()
    sprite('ko206_04', 1)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    RefreshMultihit()
    Unknown1019(40)
    sprite('ko206_04', 1)
    RefreshMultihit()
    sprite('ko206_04', 1)
    RefreshMultihit()
    sprite('ko206_05', 1)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    RefreshMultihit()
    Unknown1019(40)
    sprite('ko206_05', 1)
    RefreshMultihit()
    sprite('ko206_05', 1)
    RefreshMultihit()
    if SLOT_51:
        Unknown23029(2, 106, 0)
        clearUponHandler(11)
    sprite('ko206_06', 3)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    Unknown1019(20)
    Unknown8000(104, 1, 0)
    sprite('ko206_07', 3)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    GFX_1('koef_firewing', 2)
    Unknown1019(0)
    sprite('ko206_08', 4)
    GFX_1('koef_firewing', 0)
    GFX_1('koef_firewing', 1)
    sprite('ko206_09', 4)
    sprite('ko206_10', 2)
    sprite('ko206_10', 1)
    label(580)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaMaharagi_TAG():

    def upon_IMMEDIATE():
        Unknown23184(3, 100, 0, 0, 0, 0, 0, 0)
        callSubroutine('PYU_DDAttackInit')
        Unknown2010()
        callSubroutine('PYU_ForceWarp')
        callSubroutine('Init_CantCancelPersonaAction')
        Unknown4007(2)
        Unknown4008(2)
        setInvincible(1)
        Unknown2019(500)
        Unknown23059(1)
    sprite('ko402_00', 7)
    physicsXImpulse(2000)
    sprite('ko402_01', 3)
    sprite('ko402_02', 1)
    SFX_0('slash_sword_slow')
    GFX_1('koef_firewing', 0)
    GFX_0('Sliding_TAG', 1)
    sprite('ko402_02', 2)
    Unknown23022(0)
    sprite('ko402_03', 2)
    GFX_1('koef_firewing', 0)
    sprite('ko402_04', 2)
    GFX_1('koef_firewing', 0)
    sprite('ko402_05', 2)
    GFX_1('koef_firewing', 0)
    sprite('ko402_06', 2)
    GFX_1('koef_firewing', 0)
    physicsXImpulse(0)
    sprite('ko402_07', 6)
    sprite('ko402_08', 6)
    sprite('ko402_09', 6)
    SLOT_10 = 0
    sprite('ko402_07', 6)
    sprite('ko402_08', 6)
    sprite('ko402_09', 6)
    sprite('ko402_07', 6)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaAgi_TAG():

    def upon_IMMEDIATE():
        Unknown23184(3, 100, -100000, 0, 0, 2000000, 0, 0)
        callSubroutine('PYU_SPAttackInit')
        AttackDefaults_StandingSpecial()
        AttackLevel_(3)
        Damage(500)
        GroundedHitstunAnimation(9)
        AirHitstunAnimation(9)
        AirUntechableTime(37)
        AirPushbackX(2000)
        AirPushbackY(28000)
        Unknown9016(1)
        AttackAttributes(0, 1, 0, 0, 0)
        callSubroutine('PYU_CheckWarp')
        callSubroutine('Init_CantCancelPersonaAction')
        Unknown2006()
        Unknown2053(1)
        Unknown23059(1)
    sprite('ko401_00', 4)
    physicsXImpulse(12000)
    sprite('ko401_01', 2)
    SFX_0('slash_rapier_fast')
    physicsXImpulse(12000)
    physicsYImpulse(5000)
    sprite('ko401_02', 1)
    RefreshMultihit()
    sprite('ko401_02', 2)
    sprite('ko401_03', 2)
    GFX_0('Agishot_TAG', 100)
    Unknown1019(50)
    YAccel(50)
    sprite('ko401_04', 2)
    sprite('ko401_05', 2)
    sprite('ko401_06', 2)
    physicsXImpulse(-15000)
    physicsYImpulse(-2000)
    sprite('ko401_07', 4)
    sprite('ko401_08', 4)
    physicsYImpulse(1000)
    sprite('ko401_09', 4)
    SFX_0('cloth_h')
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_10', 4)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_11', 8)
    Unknown1019(80)
    YAccel(150)
    sprite('ko401_12', 8)
    sprite('ko401_13', 8)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaReversalAction():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -120000, 120000, -120000, -120000, 120000, 120000)
        callSubroutine('PYU_SPAttackInit')
        callSubroutine('Init_CantCancelPersonaAction')
        EnableCollision(0)
        Unknown2006()
        callSubroutine('PYU_ForceWarp')
        Unknown4007(3)
        Unknown3032()
        Unknown3001(50)
        Unknown3004(20)
        Unknown23022(1)
        sendToLabelUpon(56, 1)

        def upon_43():
            if (SLOT_48 == 11):
                sendToLabel(1)
                Unknown3031()
        Unknown23059(1)
    sprite('ko611_00', 6)
    sprite('ko611_01', 6)
    sprite('ko611_02', 6)
    sprite('ko611_03', 6)
    label(0)
    sprite('ko611_04', 6)
    sprite('ko611_05', 6)
    sprite('ko611_06', 6)
    loopRest()
    gotoLabel(0)
    label(1)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PYU_PersonaReversalAction_TAG():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -120000, 120000, 0, 0, 0, 0)
        callSubroutine('PYU_SPAttackInit')
        callSubroutine('PYU_ForceWarp')
        callSubroutine('Init_CantCancelPersonaAction')
        Unknown2006()
        Unknown4007(3)
        Unknown3032()
        Unknown3001(50)
        Unknown3004(20)
        Unknown23022(1)
        sendToLabelUpon(32, 1)
        sendToLabelUpon(56, 1)

        def upon_43():
            if (SLOT_48 == 11):
                sendToLabel(1)
                Unknown3031()
        Unknown23059(1)
    sprite('ko611_00', 6)
    sprite('ko611_01', 6)
    sprite('ko611_02', 6)
    sprite('ko611_03', 6)
    label(0)
    sprite('ko611_04', 6)
    sprite('ko611_05', 6)
    sprite('ko611_06', 6)
    loopRest()
    gotoLabel(0)
    label(1)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PersonaFireAttack_TAG():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 50000, 0, 0, 2000000, 0, 0)
        callSubroutine('PYU_SPAttackInit')
        AttackLevel_(3)
        Damage(900)
        AttackP1(70)
        AttackP2(85)
        AirPushbackX(30000)
        AirPushbackY(8000)
        AirUntechableTime(60)
        Hitstop(1)
        Unknown9016(1)
        AirHitstunAnimation(9)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown23078(1)
        Unknown11042(1)
        callSubroutine('PYU_ForceWarp')
        Unknown2053(1)
        callSubroutine('BoosterDamageUp')
        Unknown23059(1)
    sprite('ko404_00', 4)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_01', 3)
    physicsXImpulse(20000)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_02', 2)
    SFX_3('slash_rapier_fast')
    Unknown1019(120)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_03', 1)
    Unknown1019(120)
    RefreshMultihit()
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_03', 1)
    Unknown23022(0)
    sprite('ko404_04', 2)
    Unknown1019(120)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_05', 2)
    Unknown1019(120)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_06', 3)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_07', 3)
    SFX_3('bomb_m')
    SFX_3('blaze_normal')
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_08', 3)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    GFX_1('koef_404_bom', 2)
    GFX_1('koef_404_fire', 3)
    GFX_1('koef_404_fire', 4)
    GFX_1('koef_404_fire', 5)
    GFX_1('koef_404_fire', 6)
    Unknown1019(0)
    RefreshMultihit()
    Unknown11034(0)
    ProjectileDurabilityLvl(1)
    AttackLevel_(3)
    Damage(1300)
    Hitstop(6)
    Unknown23078(0)
    AirUntechableTime(60)
    GroundedHitstunAnimation(9)
    AirHitstunAnimation(9)
    AirPushbackX(8000)
    AirPushbackY(30000)
    YImpluseBeforeWallbounce(1800)
    Unknown9016(0)
    Unknown9017(1)
    Unknown9021(1)
    callSubroutine('BoosterDamageUp')
    AttackAttributes(0, 0, 0, 1, 0)
    sprite('ko404_09', 2)
    GFX_1('koef_404', 0)
    GFX_1('koef_404', 1)
    sprite('ko404_08', 4)
    DisableAttackRestOfMove()
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('ko404_08', 4)
    sprite('ko404_09', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def SBagidine_fireuzuS():

    def upon_IMMEDIATE():
        Unknown4010(2)
    sprite('null', 18)
    GFX_0('agidine_fireuzuS', 100)
    Unknown23029(1, 10023, 0)
    sprite('null', 1)
    GFX_0('agidine_fireuzuS', 100)
    Unknown23029(1, 10023, 0)
    sprite('null', 39)
    Unknown4010(0)

@State
def SBagidine_fireuzuM():

    def upon_IMMEDIATE():
        Unknown4010(2)
    sprite('null', 18)
    GFX_0('agidine_fireuzuM', 100)
    Unknown23029(1, 10023, 0)
    sprite('null', 1)
    GFX_0('agidine_fireuzuM', 100)
    Unknown23029(1, 10023, 0)
    sprite('null', 39)
    Unknown4010(0)

@State
def SBagidine_fireuzuL():

    def upon_IMMEDIATE():
        Unknown4010(2)
        callSubroutine('BoosterDamageUp')
        Unknown23056()
    sprite('null', 18)
    GFX_0('agidine_fireuzuL', 100)
    Unknown23029(1, 10023, 0)
    sprite('null', 1)
    GFX_0('agidine_fireuzuL', 100)
    Unknown23029(1, 10023, 0)
    sprite('null', 39)
    Unknown4010(0)

@State
def SBagidine_fireuzuL_TAG():

    def upon_IMMEDIATE():
        Unknown4010(2)
        callSubroutine('BoosterDamageUp')
        Unknown23056()
    sprite('null', 18)
    GFX_0('agidine_fireuzuL_OD_TAG', 100)
    Unknown23029(1, 10023, 0)
    sprite('null', 1)
    GFX_0('agidine_fireuzuL_OD_TAG', 100)
    Unknown23029(1, 10023, 0)
    sprite('null', 39)
    Unknown4010(0)

@State
def reaction_sakura():

    def upon_IMMEDIATE():
        GFX_2('yuef_sakura')
        Unknown4010(2)
        Unknown4007(2)
    sprite('null', 50)

@Subroutine
def Init_Sensu():
    Unknown2009()
    AttackLevel_(2)
    Damage(1500)
    PushbackX(12000)
    blockstun(11)
    ProjectileDurabilityLvl(1)
    Hitstop(0)
    Unknown11001(10, 10, 10)
    Unknown53(2)
    Unknown53(1)
    Unknown9016(1)
    Unknown4061(0)
    Unknown3032()
    Unknown1096(900)
    Unknown3029(1)
    AfterimageCount(3)
    Unknown3070(1)
    AfterimageColor_1(120, 100, 100, 100)
    AfterimageColor_2(60, 100, 100, 100)
    AfterimageMask_1(0, 255, 0, 255)
    AfterimageMask_2(0, 255, 0, 255)
    EnableAfterimagePulse(1)
    Unknown3069(2)
    Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
    sendToLabelUpon(54, 2)
    GFX_0('sensunage_sakura', 100)

@Subroutine
def Init_Sensu_StandExSignal():
    physicsXImpulse(26000)

    def upon_43():
        if (SLOT_48 == 1011):
            physicsXImpulse(35000)
        if (SLOT_48 == 1012):
            physicsXImpulse(20000)

@Subroutine
def Init_Sensu_Gravity():
    Unknown2037(0)

    def upon_3():
        Unknown2038(1)
        if (SLOT_2 >= 10):
            setGravity(1400)
            Unknown1015(-100)
        if (SLOT_23 <= 100000):
            Unknown2003(1)
        if (SLOT_12 <= 300):
            physicsXImpulse(300)
            clearUponHandler(3)

@State
def Sensu_Stand_A():

    def upon_IMMEDIATE():
        callSubroutine('Init_Sensu')
        callSubroutine('Init_Sensu_StandExSignal')
    sprite('vr_yu204_sensu01ex', 3)
    SFX_3('slash_knife_slow')
    sprite('vr_yu204_sensu02', 2)
    Unknown11032(250000, -250000, -1, -1)
    sprite('vr_yu204_sensu03', 2)
    sprite('vr_yu204_sensu04', 2)
    sprite('vr_yu204_sensu05', 2)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_3('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    sprite('null', 1)
    GFX_1('yuef_sakura', 100)
    Unknown1084(1)
    GFX_0('Sensu_hitA', 100)
    ExitState()
    label(3)
    sprite('null', 1)
    GFX_1('yuef_sakura', 100)
    Unknown1084(1)
    GFX_0('Sensu_LandingA', 100)

@State
def Sensu_Stand_A_2nd():

    def upon_IMMEDIATE():
        callSubroutine('Init_Sensu')
        callSubroutine('Init_Sensu_StandExSignal')

        def upon_ON_HIT_OR_BLOCK():
            pass
    sprite('vr_yu204_sensu01ex', 3)
    SFX_3('slash_knife_slow')
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_3('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    sprite('null', 1)
    GFX_1('yuef_sakura', 100)
    Unknown1084(1)
    GFX_0('Sensu_hitA', 100)
    ExitState()
    label(3)
    sprite('null', 1)
    GFX_1('yuef_sakura', 100)
    Unknown1084(1)
    GFX_0('Sensu_LandingA', 100)

@State
def Sensu_2A():

    def upon_IMMEDIATE():
        callSubroutine('Init_Sensu')
        Unknown1072(-45000)
        Unknown1110(35000, 0)

        def upon_43():
            if (SLOT_48 == 1021):
                Unknown1072(-20000)
                Unknown1110(35000, 0)
            if (SLOT_48 == 1022):
                Unknown1072(-70000)
                Unknown1110(32000, 0)
    sprite('vr_yu204_sensu01ex', 3)
    SFX_3('slash_knife_slow')
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    loopRest()
    gotoLabel(10)
    label(2)
    sprite('null', 1)
    GFX_1('yuef_sakura', 100)
    Unknown1084(1)
    GFX_0('Sensu_hitA', 100)

@State
def Sensu_Air():

    def upon_IMMEDIATE():
        callSubroutine('Init_Sensu')
        HitOverhead(2)
        Unknown1072(750000)
        Unknown1110(32000, 0)

        def upon_43():
            if (SLOT_48 == 1031):
                Unknown1072(740000)
                Unknown1110(35000, 0)
            if (SLOT_48 == 1032):
                Unknown1072(770000)
                Unknown1110(38000, 0)

        def upon_LANDING():
            sendToLabel(3)
    sprite('vr_yu204_sensu01', 3)
    SFX_3('slash_knife_slow')
    GFX_0('sensunage_sakura', 100)
    RefreshMultihit()
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_3('slash_knife_slow')
    RefreshMultihit()
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_3('slash_knife_slow')
    RefreshMultihit()
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_3('slash_knife_slow')
    RefreshMultihit()
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    label(10)
    sprite('vr_yu204_sensu01', 3)
    RefreshMultihit()
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    loopRest()
    gotoLabel(10)
    label(0)
    sprite('vr_yu204_sensu01', 8)
    GFX_0('Sensu_jimen', 100)
    Unknown1084(1)
    Unknown3029(0)
    Unknown2001()
    Unknown23118(-1)
    Unknown1099(30)
    Unknown3004(-32)
    loopRest()
    ExitState()
    label(2)
    sprite('null', 1)
    GFX_1('yuef_sakura', 100)
    Unknown1084(1)
    GFX_0('Sensu_hitA', 100)
    ExitState()
    label(3)
    sprite('null', 1)
    GFX_1('yuef_sakura', 100)
    Unknown1084(1)
    GFX_0('Sensu_LandingA', 100)

@State
def Sensu_hitA():

    def upon_IMMEDIATE():
        Unknown53(2)
        Unknown2035(1)
        Unknown4061(0)
        Unknown3032()
        physicsYImpulse(7000)
        physicsXImpulse(-3500)
        setGravity(800)
        teleportRelativeX(100000)
    sprite('vr_yu204_03', 3)
    sprite('vr_yu204_04', 3)
    sprite('vr_yu204_05', 3)
    sprite('vr_yu204_06', 3)
    sprite('vr_yu204_03', 3)
    sprite('vr_yu204_04', 3)
    sprite('vr_yu204_05', 3)
    sprite('vr_yu204_06', 8)
    Unknown3004(-31)

@State
def Sensu_LandingA():

    def upon_IMMEDIATE():
        Unknown53(2)
        Unknown2035(1)
        Unknown4061(0)
        Unknown3032()
        physicsYImpulse(7000)
        physicsXImpulse(3500)
        setGravity(800)
    sprite('vr_yu204_03', 3)
    sprite('vr_yu204_04', 3)
    sprite('vr_yu204_05', 3)
    sprite('vr_yu204_06', 3)
    sprite('vr_yu204_03', 3)
    sprite('vr_yu204_04', 3)
    sprite('vr_yu204_05', 3)
    sprite('vr_yu204_06', 8)
    Unknown3004(-31)

@State
def sensunage_sakura():

    def upon_IMMEDIATE():
        GFX_2('yuef_nagesakura')
    sprite('null', 50)

@State
def sensu_sakura():

    def upon_IMMEDIATE():
        GFX_2('yuef_sakura')
    sprite('null', 50)

@State
def summon_sakura():

    def upon_IMMEDIATE():
        GFX_2('yuef_summon')
        Unknown4010(2)
    sprite('null', 50)

@State
def firebard():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
        Unknown3033()
        Unknown4061(3)
        Unknown1056(2000)
        Unknown1064(2000)
        Unknown1007(380000)
        teleportRelativeX(-400000)
    label(0)
    sprite('vr_ko405_00', 2)
    GFX_1('koef_405_fire01', 106)
    gotoLabel(0)

@State
def firebard_yugami():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown3032()
        Unknown3057(1)
        Unknown3058(15000)
        Unknown1096(1500)
        Unknown3004(-16)
        Unknown1100(100)
    sprite('vr_yu_yugami', 8)
    sprite('vr_yu_yugami', 10)
    Unknown3004(-26)

@State
def rinkakuaura():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4007(2)
        Unknown4010(2)
        Unknown4015()
        Unknown4009(2)
        Unknown3001(0)
        Unknown3026(9474192)

        def upon_43():
            if (SLOT_48 == 52):
                sendToLabel(0)
            if (SLOT_48 == 56):
                Unknown2019(4000)
    sprite('null', 3)
    Unknown3004(25)
    Unknown4003('yuef_400aura00.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura01.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura02.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura03.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura04.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura05.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura06.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura03.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura04.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura05.DIG', '')
    label(999)
    sprite('null', 3)
    Unknown4003('yuef_400aura03.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura04.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura05.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura06.DIG', '')
    gotoLabel(999)
    label(0)
    sprite('null', 3)
    Unknown3004(-35)
    Unknown4003('yuef_400aura03.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura04.DIG', '')
    sprite('null', 2)
    Unknown4003('yuef_400aura05.DIG', '')

@State
def rinkakuaura_Assist():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4007(2)
        Unknown4010(2)
        Unknown4015()
        Unknown4009(2)
        Unknown3001(0)
        Unknown3026(9474192)

        def upon_43():
            if (SLOT_48 == 52):
                sendToLabel(0)
            if (SLOT_48 == 56):
                Unknown23015(2)
    sprite('null', 3)
    Unknown3004(25)
    Unknown4003('yuef_400aura00.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura01.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura02.DIG', '')
    sprite('null', 4)
    Unknown4003('yuef_400aura03.DIG', '')
    sprite('null', 4)
    Unknown4003('yuef_400aura04.DIG', '')
    sprite('null', 4)
    Unknown4003('yuef_400aura05.DIG', '')
    sprite('null', 4)
    Unknown4003('yuef_400aura06.DIG', '')
    label(999)
    sprite('null', 3)
    Unknown4003('yuef_400aura03.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura04.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura05.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura06.DIG', '')
    gotoLabel(999)
    label(0)
    sprite('null', 3)
    Unknown3004(-35)
    Unknown4003('yuef_400aura03.DIG', '')
    sprite('null', 3)
    Unknown4003('yuef_400aura04.DIG', '')
    sprite('null', 2)
    Unknown4003('yuef_400aura05.DIG', '')

@State
def rinkakuaura2():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
        GFX_2('yuef_reversalaura')

        def upon_43():
            if (SLOT_48 == 57):
                Unknown23015(2)
    sprite('null', 180)

@State
def diacharge():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('yuef_400charge.DIG', '')
        GFX_2('yuef_dia')
        Unknown4007(3)
        Unknown4010(2)
        Unknown4015()
        Unknown4009(2)
        Unknown3001(0)

        def upon_43():
            if (SLOT_48 == 53):
                sendToLabel(0)
            if (SLOT_48 == 58):
                Unknown23015(2)
    sprite('null', 32767)
    Unknown3004(25)
    label(0)
    sprite('null', 15)
    Unknown3004(-17)

@State
def diacharge_floor():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4007(2)
        Unknown4010(2)

        def upon_43():
            if (SLOT_48 == 54):
                sendToLabel(0)
            if (SLOT_48 == 59):
                Unknown23015(2)
    label(5)
    sprite('null', 2)
    GFX_1('yuef_diafloor', 100)
    loopRest()
    gotoLabel(5)
    label(0)
    sprite('null', 15)
    Unknown3004(-17)

@Subroutine
def InitAgi():
    Unknown2010()
    AttackLevel_(3)
    Damage(900)
    Unknown9017(1)
    Unknown23089(1, 0, 0, 1, 1, 0, 1, 1)
    sendToLabelUpon(54, 580)
    Unknown1056(2000)
    Unknown2053(1)
    callSubroutine('BoosterDamageUp')

    def upon_3():
        SLOT_51 = (SLOT_51 + 1)

@State
def AgishotA():

    def upon_IMMEDIATE():
        callSubroutine('InitAgi')
        AirUntechableTime(29)
        Hitstop(7)
        AirPushbackX(6000)
        AirPushbackY(9000)
        GroundedHitstunAnimation(1)
        Unknown1007(170000)
        teleportRelativeX(450000)
        sendToLabelUpon(23, 0)

        def upon_43():
            if (SLOT_48 == 4043):
                sendToLabel(0)
    sprite('null', 5)
    GFX_0('agi_firering', 100)
    label(5)
    sprite('null', 35)
    GFX_0('agi_firering_ex', 100)
    loopRest()
    gotoLabel(5)
    label(0)
    sprite('keep', 5)
    Unknown36(1)
    Unknown13(25)
    Unknown35()
    GFX_0('explosion', 100)
    clearUponHandler(23)
    clearUponHandler(3)
    sprite('vr_ko401_col', 15)
    RefreshMultihit()
    label(580)
    clearUponHandler(54)
    clearUponHandler(43)
    sprite('null', 5)

@State
def AgishotB():

    def upon_IMMEDIATE():
        callSubroutine('InitAgi')
        AirUntechableTime(29)
        Hitstop(7)
        AirPushbackX(6000)
        AirPushbackY(9000)
        GroundedHitstunAnimation(1)
        Unknown1007(170000)
        teleportRelativeX(900000)
        sendToLabelUpon(24, 0)

        def upon_43():
            if (SLOT_48 == 4044):
                sendToLabel(0)
    sprite('null', 10)
    GFX_0('agi_firering', 100)
    label(5)
    sprite('null', 35)
    GFX_0('agi_firering_ex', 100)
    loopRest()
    gotoLabel(5)
    label(0)
    sprite('keep', 5)
    Unknown36(1)
    Unknown13(25)
    Unknown35()
    sendToLabelUpon(24, 0)
    GFX_0('explosion_2nd', 100)
    clearUponHandler(24)
    clearUponHandler(3)
    sprite('vr_ko401_col', 15)
    RefreshMultihit()
    label(580)
    clearUponHandler(54)
    clearUponHandler(43)
    sprite('null', 5)

@State
def AgishotAB():

    def upon_IMMEDIATE():
        Unknown2010()
        callSubroutine('BoosterDamageUp')
        AttackLevel_(3)
        Damage(2000)
        Unknown30065(0)
        MinimumDamagePct(10)
        Unknown9017(1)
        AirUntechableTime(29)
        AirPushbackY(17000)
        AirPushbackX(0)
        Unknown23089(1, 0, 0, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 580)
        Unknown1056(2000)
        Unknown2053(1)
        callSubroutine('BoosterDamageUp')
        Hitstop(7)
        GroundedHitstunAnimation(1)
        SLOT_52 = SLOT_23
        Unknown1086(22)
        SLOT_23 = SLOT_52
        Unknown1007(200000)

        def upon_3():
            SLOT_51 = (SLOT_51 + 1)

        def upon_43():
            if (SLOT_48 == 4045):
                sendToLabel(0)
    sprite('null', 3)
    GFX_0('agi_firering', 100)
    sprite('null', 32)
    sendToLabelUpon(25, 0)
    label(5)
    sprite('null', 35)
    GFX_0('agi_firering_ex', 100)
    loopRest()
    gotoLabel(5)
    label(0)
    sprite('keep', 5)
    Unknown36(1)
    Unknown13(25)
    Unknown35()
    GFX_0('explosionC', 100)
    clearUponHandler(25)
    clearUponHandler(3)
    sprite('vr_ko401_col', 15)
    RefreshMultihit()
    label(580)
    clearUponHandler(54)
    clearUponHandler(43)
    sprite('null', 5)

@State
def AgishotAB2nd():

    def upon_IMMEDIATE():
        callSubroutine('InitAgi')
        Hitstop(7)
        AirUntechableTime(36)
        GroundedHitstunAnimation(1)
        Unknown1007(320000)
        SLOT_52 = SLOT_23
        Unknown1086(22)
        SLOT_23 = SLOT_52

        def upon_3():
            SLOT_51 = (SLOT_51 + 1)
        callSubroutine('BoosterDamageUp')
        sendToLabelUpon(25, 0)
    sprite('null', 35)
    GFX_0('agi_firering', 100)
    label(5)
    sprite('null', 35)
    GFX_0('agi_firering_ex', 100)
    loopRest()
    gotoLabel(5)
    label(0)
    sprite('keep', 5)
    Unknown36(1)
    Unknown13(25)
    Unknown35()
    GFX_0('explosion_2nd', 100)
    clearUponHandler(25)
    clearUponHandler(3)
    sprite('vr_ko401_col', 15)
    RefreshMultihit()
    label(580)
    clearUponHandler(54)
    clearUponHandler(43)
    sprite('null', 5)

@State
def agi_firering():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('yuef_401firering.DIG', '')
        GFX_2('koef236_firering')
        Unknown4010(2)
        Unknown4007(2)
        Unknown4015()
    sprite('null', 15)
    sprite('null', 20)
    Unknown3004(-10)

@State
def agi_firepit():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('yuef_401firering.DIG', '')
        GFX_2('koef236_firering')
        Unknown4010(2)
        Unknown4007(2)
        Unknown4015()
        Unknown3001(0)
    sprite('null', 15)
    sprite('null', 20)
    Unknown3004(-10)

@State
def agi_firering_ex():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('yuef_401firering.DIG', '')
        GFX_2('koef236_firering')
        Unknown4010(2)
        Unknown4015()
    sprite('null', 35)
    Unknown3001(36)
    Unknown3004(0)
    loopRest()

@State
def explosion():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('yuef_401baku.DIG', '')
        Unknown4007(2)
        Unknown4015()
    sprite('null', 35)
    SFX_3('bomb_m')
    SFX_3('blaze_normal')
    GFX_1('koef_401baku', 100)
    GFX_0('FireYugamiA', 100)
    Unknown1099(10)
    sprite('null', 25)
    Unknown3004(-15)

@State
def explosionC():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('yuef_401baku.DIG', '')
        Unknown4007(2)
        Unknown4015()
        Unknown1096(1500)
    sprite('null', 35)
    SFX_3('bomb_m')
    SFX_3('blaze_normal')
    GFX_1('koef_401baku', 100)
    GFX_0('FireYugamiA', 100)
    Unknown1099(10)
    sprite('null', 25)
    Unknown3004(-15)

@State
def agi_firering_2nd():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('yuef_401firering.DIG', '')
        GFX_2('koef236_firering')
        Unknown4010(2)
        Unknown4015()
    sprite('null', 15)
    sprite('null', 20)
    Unknown3004(-10)

@State
def agi_firering_2nd_ex():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('yuef_401firering.DIG', '')
        GFX_2('koef236_firering')
        Unknown4010(2)
        Unknown4015()
    sprite('null', 35)
    Unknown3001(36)
    Unknown3004(0)

@State
def explosion_2nd():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('yuef_401baku.DIG', '')
        Unknown4015()
    sprite('null', 35)
    SFX_3('bomb_m')
    SFX_3('blaze_normal')
    GFX_1('koef_401baku', 100)
    GFX_0('FireYugamiA', 100)
    Unknown1099(10)
    sprite('null', 25)
    Unknown3004(-15)

@State
def FireYugamiA():
    sprite('keep', 5)
    GFX_0('fireyugamisub', 100)
    sprite('keep', 5)
    GFX_0('fireyugamisub', 100)
    sprite('keep', 5)
    GFX_0('fireyugamisub', 100)
    sprite('keep', 5)
    GFX_0('fireyugamisub', 100)
    sprite('keep', 5)
    GFX_0('fireyugamisub', 100)
    sprite('keep', 5)
    GFX_0('fireyugamisub', 100)

@State
def fireyugamisub():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown3032()
        Unknown3057(1)
        Unknown3058(14000)
        Unknown1096(900)
        Unknown3004(-13)
        Unknown1062(300, -300)
        Unknown1070(300, -300)
        Unknown1060(100)
        Unknown1068(100)
    sprite('vr_yu_yugami', 8)
    sprite('vr_yu_yugami', 10)
    Unknown3004(-26)

@Subroutine
def Init_Maragi_Sliding():
    Unknown2010()
    AttackLevel_(4)
    ProjectileDurabilityLvl(1)
    Damage(1000)
    AttackP2(100)
    Hitstop(0)
    PushbackX(10000)
    Unknown9310(10)
    Unknown9017(1)
    AirHitstunAnimation(11)
    GroundedHitstunAnimation(11)
    AirPushbackX(0)
    AirPushbackY(12000)
    HitLow(2)
    AttackAttributes(0, 0, 0, 1, 0)
    callSubroutine('BoosterDamageUp')
    Unknown23089(1, 0, 0, 1, 1, 0, 1, 1)
    sendToLabelUpon(54, 580)
    sendToLabelUpon(55, 0)
    sendToLabelUpon(11, 0)
    teleportRelativeY(0)
    Unknown2053(1)
    GFX_2('koef_slidefire04')

@State
def SlidingA():

    def upon_IMMEDIATE():
        callSubroutine('Init_Maragi_Sliding')
        physicsXImpulse(25000)

        def upon_43():
            if (SLOT_48 == 4050):

                def upon_24():
                    if (SLOT_18 >= 5):
                        clearUponHandler(24)
                        sendToLabel(0)
                    else:
                        clearUponHandler(24)
            SLOT_5 = (SLOT_5 + 1)

        def upon_3():
            if CheckInput(0xb):
                if (SLOT_18 >= 5):
                    clearUponHandler(3)
                    sendToLabel(0)
    sprite('vr_ko402_col', 5)
    teleportRelativeX(50000)
    SFX_3('airdash_l')
    RefreshMultihit()
    sprite('vr_ko402_col', 10)
    SFX_3('blaze_normal')
    sprite('vr_ko402_col', 10)
    SFX_3('blaze_normal')
    sprite('vr_ko402_col', 10)
    SFX_3('blaze_normal')
    sprite('vr_ko402_col', 10)
    SFX_3('blaze_normal')
    sprite('vr_ko402_col', 10)
    SFX_3('blaze_normal')
    sprite('vr_ko402_col', 10)
    SFX_3('blaze_normal')
    loopRest()
    label(0)
    sprite('keep', 3)
    clearUponHandler(3)
    clearUponHandler(11)
    clearUponHandler(55)
    clearUponHandler(54)
    clearUponHandler(24)
    clearUponHandler(25)
    Unknown3004(-25)
    DisableAttackRestOfMove()
    GFX_0('FireYugamiC', 100)
    GFX_0('Maharagi_fire', 100)
    ExitState()
    label(580)
    sprite('keep', 20)
    clearUponHandler(3)
    clearUponHandler(11)
    clearUponHandler(55)
    clearUponHandler(54)
    clearUponHandler(23)
    physicsXImpulse(10000)
    Unknown3004(-20)
    DisableAttackRestOfMove()
    SLOT_5 = (SLOT_5 + (-1))

@State
def SlidingB():

    def upon_IMMEDIATE():
        callSubroutine('Init_Maragi_Sliding')
        physicsXImpulse(60000)
        Unknown2053(1)

        def upon_43():
            if (SLOT_48 == 4060):

                def upon_23():
                    if (SLOT_18 >= 5):
                        clearUponHandler(23)
                        sendToLabel(0)
                    else:
                        clearUponHandler(23)
            SLOT_5 = (SLOT_5 + 1)

        def upon_3():
            if CheckInput(0x2):
                if (SLOT_18 >= 5):
                    clearUponHandler(3)
                    sendToLabel(0)
    sprite('vr_ko402_col', 2)
    teleportRelativeX(50000)
    SFX_3('airdash_l')
    RefreshMultihit()
    sprite('vr_ko402_col', 5)
    SFX_3('blaze_normal')
    sprite('vr_ko402_col', 5)
    SFX_3('blaze_normal')
    sprite('vr_ko402_col', 5)
    SFX_3('blaze_normal')
    sprite('vr_ko402_col', 5)
    SFX_3('blaze_normal')
    loopRest()
    label(0)
    sprite('keep', 1)
    clearUponHandler(3)
    clearUponHandler(11)
    clearUponHandler(55)
    clearUponHandler(54)
    clearUponHandler(23)
    clearUponHandler(25)
    Unknown3004(-50)
    DisableAttackRestOfMove()
    GFX_0('FireYugamiC', 100)
    GFX_0('Maharagi_fireB', 100)
    Unknown23029(1, 4061, 0)
    if SLOT_51:
        Unknown21007(1, 32)
    ExitState()
    label(580)
    sprite('keep', 5)
    clearUponHandler(3)
    clearUponHandler(11)
    clearUponHandler(55)
    clearUponHandler(54)
    clearUponHandler(24)
    physicsXImpulse(10000)
    Unknown3004(-20)
    DisableAttackRestOfMove()
    SLOT_5 = (SLOT_5 + (-1))

@State
def SlidingC():

    def upon_IMMEDIATE():
        callSubroutine('Init_Maragi_Sliding')
        Unknown30065(0)
        MinimumDamagePct(10)
        physicsXImpulse(60000)
        AirPushbackX(3000)
        Unknown2053(1)

        def upon_43():
            if (SLOT_48 == 4071):

                def upon_25():
                    if (SLOT_18 >= 5):
                        clearUponHandler(25)
                        sendToLabel(0)
                    else:
                        clearUponHandler(25)

        def upon_3():
            if CheckInput(0x14):
                if (SLOT_18 >= 5):
                    clearUponHandler(3)
                    sendToLabel(0)
        SLOT_5 = (SLOT_5 + 1)
    sprite('vr_ko402_col', 5)
    teleportRelativeX(50000)
    SFX_3('airdash_l')
    RefreshMultihit()
    sprite('vr_ko402_col', 5)
    SFX_3('blaze_normal')
    sprite('vr_ko402_col', 5)
    SFX_3('blaze_normal')
    sprite('vr_ko402_col', 5)
    SFX_3('blaze_normal')
    label(0)
    sprite('null', 10)
    Unknown1019(0)
    clearUponHandler(3)
    clearUponHandler(11)
    clearUponHandler(55)
    clearUponHandler(54)
    clearUponHandler(25)
    Unknown3004(-50)
    DisableAttackRestOfMove()
    GFX_0('FireYugamiC', 100)
    GFX_0('Maharagi_fireC', 100)
    sprite('null', 10)
    GFX_0('FireYugamiC', 100)
    GFX_0('Maharagi_fireC', 100)
    Unknown36(1)
    teleportRelativeX(160000)
    Unknown35()
    sprite('null', 10)
    GFX_0('FireYugamiC', 100)
    GFX_0('Maharagi_fireC', 100)
    Unknown23029(1, 4061, 0)
    Unknown36(1)
    teleportRelativeX(320000)
    Unknown35()
    if (SLOT_5 == 1):
        SLOT_5 = (SLOT_5 + (-1))
    ExitState()
    label(580)
    sprite('keep', 5)
    clearUponHandler(3)
    clearUponHandler(11)
    clearUponHandler(55)
    clearUponHandler(54)
    clearUponHandler(25)
    physicsXImpulse(10000)
    Unknown3004(-20)
    DisableAttackRestOfMove()
    SLOT_5 = (SLOT_5 + (-1))

@State
def FirePitA():

    def upon_IMMEDIATE():
        teleportRelativeX(300000)
        Unknown2053(1)
        Unknown1024(0)
        
        def upon_43():
            if (SLOT_48 == 4060):

                def upon_23():
                    if (SLOT_18 >= 5):
                        clearUponHandler(23)
                        sendToLabel(0)
                    else:
                        clearUponHandler(23)
            SLOT_5 = (SLOT_5 + 1)
        
        def upon_LANDING():
                sendToLabel(3)
    sprite('null', 1)
    GFX_0('agi_firepit', 100)
    sprite('null', 12)
    physicsYImpulse(-1000000)
    physicsXImpulse(571)
    Unknown23022(1)
    Unknown3001(255)
    loopRest()
    Unknown1019(2200)
    YAccel(2200)
    label(1)
    sprite('null', 1)
    loopRest()
    gotoLabel(1)
    label(3)
    sprite('null', 2)
    SLOT_54 = 1
    Unknown3032()
    teleportRelativeY(0)
    Unknown1084(1)
    sprite('null', 10)
    label(5)
    
    def upon_3():
            if CheckInput(0x05):
                clearUponHandler(3)
                sendToLabel(0)
    sprite('null', 35)
    GFX_2('yuef_403boost')
    loopRest()
    gotoLabel(5)
    label(0)
    sprite('keep', 5)
    Unknown36(1)
    Unknown13(25)
    Unknown35()
    GFX_0('FireYugamiC', 100)
    GFX_0('Maharagi_fire', 100)
    clearUponHandler(23)
    clearUponHandler(3)
    sprite('vr_ko401_col', 15)
    RefreshMultihit()
    label(580)
    clearUponHandler(54)
    clearUponHandler(43)
    sprite('null', 5)

@State
def FirePitB():

    def upon_IMMEDIATE():
        teleportRelativeX(600000)
        Unknown2053(1)
        Unknown1024(0)
        
        def upon_43():
            if (SLOT_48 == 4060):

                def upon_23():
                    if (SLOT_18 >= 5):
                        clearUponHandler(23)
                        sendToLabel(0)
                    else:
                        clearUponHandler(23)
            SLOT_5 = (SLOT_5 + 1)
        
        def upon_LANDING():
                sendToLabel(3)
    sprite('null', 1)
    GFX_0('agi_firepit', 100)
    sprite('null', 12)
    physicsYImpulse(-1000000)
    physicsXImpulse(571)
    Unknown23022(1)
    Unknown3001(255)
    loopRest()
    Unknown1019(2200)
    YAccel(2200)
    label(1)
    sprite('null', 1)
    loopRest()
    gotoLabel(1)
    label(3)
    sprite('null', 2)
    SLOT_54 = 1
    Unknown3032()
    teleportRelativeY(0)
    Unknown1084(1)
    sprite('null', 10)
    label(5)

    def upon_3():
            if CheckInput(0x0E):
                clearUponHandler(3)
                sendToLabel(0)
    sprite('null', 35)
    GFX_2('yuef_403boost')
    loopRest()
    gotoLabel(5)
    label(0)
    sprite('keep', 3)
    clearUponHandler(3)
    clearUponHandler(11)
    clearUponHandler(55)
    clearUponHandler(54)
    clearUponHandler(24)
    clearUponHandler(25)
    Unknown3004(-25)
    DisableAttackRestOfMove()
    GFX_0('FireYugamiC', 100)
    GFX_0('Maharagi_fireB', 100)
    ExitState()
    label(580)
    sprite('keep', 20)
    clearUponHandler(3)
    clearUponHandler(11)
    clearUponHandler(55)
    clearUponHandler(54)
    clearUponHandler(23)
    physicsXImpulse(10000)
    Unknown3004(-20)
    DisableAttackRestOfMove()
    SLOT_5 = (SLOT_5 + (-1))

@State
def FirePitC():

    def upon_IMMEDIATE():
        teleportRelativeX(150000)
        Unknown2053(1)
        Unknown1024(0)
        
        def upon_43():
            if (SLOT_48 == 4060):

                def upon_23():
                    if (SLOT_18 >= 5):
                        clearUponHandler(23)
                        sendToLabel(0)
                    else:
                        clearUponHandler(23)
            SLOT_5 = (SLOT_5 + 1)
        
        def upon_LANDING():
                sendToLabel(3)
    sprite('null', 1)
    GFX_0('agi_firepit', 100)
    sprite('null', 12)
    physicsYImpulse(-1000000)
    physicsXImpulse(571)
    Unknown23022(1)
    Unknown3001(255)
    loopRest()
    Unknown1019(2200)
    YAccel(2200)
    label(1)
    sprite('null', 1)
    loopRest()
    gotoLabel(1)
    label(3)
    sprite('null', 2)
    SLOT_54 = 1
    Unknown3032()
    teleportRelativeY(0)
    Unknown1084(1)
    sprite('null', 10)
    label(5)

    def upon_3():
            if CheckInput(0x17):
                clearUponHandler(3)
                sendToLabel(0)
    sprite('null', 35)
    GFX_2('yuef_403boost')
    loopRest()
    gotoLabel(5)
    label(0)
    sprite('null', 20)
    Unknown1019(0)
    clearUponHandler(3)
    clearUponHandler(11)
    clearUponHandler(55)
    clearUponHandler(54)
    clearUponHandler(25)
    Unknown3004(-50)
    DisableAttackRestOfMove()
    GFX_0('FireYugamiC', 100)
    GFX_0('Maharagi_firePitC', 100)
    sprite('null', 20)
    GFX_0('FireYugamiC', 100)
    GFX_0('Maharagi_firePitC', 100)
    Unknown36(1)
    teleportRelativeX(160000)
    Unknown35()
    sprite('null', 20)
    GFX_0('FireYugamiC', 100)
    GFX_0('Maharagi_firePitClast', 100)
    Unknown23029(1, 4061, 0)
    Unknown36(1)
    teleportRelativeX(320000)
    Unknown35()
    if (SLOT_5 == 1):
        SLOT_5 = (SLOT_5 + (-1))
    ExitState()
    label(580)
    sprite('keep', 5)
    clearUponHandler(3)
    clearUponHandler(11)
    clearUponHandler(55)
    clearUponHandler(54)
    clearUponHandler(25)
    physicsXImpulse(10000)
    Unknown3004(-20)
    DisableAttackRestOfMove()
    SLOT_5 = (SLOT_5 + (-1))

@State
def Sliding_TAG():

    def upon_IMMEDIATE():
        DisableAttackRestOfMove()
        physicsXImpulse(40000)
        Unknown2053(1)
    sprite('null', 5)
    sprite('null', 10)
    Unknown1019(0)
    GFX_0('FireYugamiC', 100)
    GFX_0('Maharagi_fire_TAG', 100)
    sprite('null', 10)
    GFX_0('FireYugamiC', 100)
    GFX_0('Maharagi_fire_TAG', 100)
    Unknown36(1)
    teleportRelativeX(160000)
    Unknown35()
    sprite('null', 10)
    GFX_0('FireYugamiC', 100)
    GFX_0('Maharagi_fire_TAG', 100)
    Unknown23029(1, 4073, 0)
    Unknown36(1)
    teleportRelativeX(320000)
    Unknown35()

@State
def Maharagi_fire():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(4)
        Damage(1500)
        Hitstop(0)
        AirUntechableTime(60)
        Unknown9017(1)
        ProjectileDurabilityLvl(1)
        AirHitstunAnimation(13)
        GroundedHitstunAnimation(13)
        AirPushbackX(5000)
        AirPushbackY(30000)
        YImpluseBeforeWallbounce(1800)
        callSubroutine('BoosterDamageUp')
        Unknown3032()
        Unknown4003('yuef_431hibashiraC.DIG', '')
        Unknown23067('koef_431baku')
        Unknown23015(1)
        Unknown4015()
        Unknown3001(0)
        Unknown3004(45)
        Unknown1064(100)
        Unknown2053(1)
        Unknown1088(10)
    sprite('vr_yu431_dmg04', 9)
    Unknown1067(100)
    StartMultihit()
    sprite('vr_yu431_dmg04', 30)
    SFX_3('blaze_long')
    Unknown1067(0)
    RefreshMultihit()
    sprite('vr_yu431_dmg04', 10)
    clearUponHandler(43)
    DisableAttackRestOfMove()
    Unknown3033()
    Unknown1067(30)
    Unknown23118(-65536)
    Unknown3004(-25)
    if (SLOT_5 == 1):
        SLOT_5 = (SLOT_5 + (-1))

@State
def Maharagi_fireB():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(4)
        Damage(1500)
        Hitstop(0)
        AirUntechableTime(60)
        Unknown9017(1)
        ProjectileDurabilityLvl(1)
        AirHitstunAnimation(13)
        GroundedHitstunAnimation(13)
        AirPushbackX(10000)
        AirPushbackY(30000)
        YImpluseBeforeWallbounce(1800)
        callSubroutine('BoosterDamageUp')
        Unknown3032()
        Unknown4003('yuef_431hibashiraC.DIG', '')
        Unknown23067('koef_431baku')
        Unknown23015(1)
        Unknown4015()
        Unknown3001(0)
        Unknown3004(45)
        Unknown1064(100)
        Unknown2053(1)
        Unknown1088(10)
    sprite('vr_yu431_dmg04', 9)
    Unknown1067(100)
    StartMultihit()
    sprite('vr_yu431_dmg04', 45)
    SFX_3('blaze_long')
    Unknown1067(0)
    RefreshMultihit()
    sprite('vr_yu431_dmg04', 10)
    clearUponHandler(43)
    DisableAttackRestOfMove()
    Unknown3033()
    Unknown1067(30)
    Unknown23118(-65536)
    Unknown3004(-25)
    if (SLOT_5 == 1):
        SLOT_5 = (SLOT_5 + (-1))

@State
def Maharagi_fireC():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(4)
        Damage(1000)
        AttackP2(85)
        Unknown30065(0)
        MinimumDamagePct(10)
        Hitstop(0)
        AirUntechableTime(60)
        Unknown9017(1)
        ProjectileDurabilityLvl(1)
        AirHitstunAnimation(13)
        GroundedHitstunAnimation(13)
        AirPushbackX(12000)
        AirPushbackY(17000)
        YImpluseBeforeWallbounce(1800)
        callSubroutine('BoosterDamageUp')
        Unknown11092(1)
        Unknown23182(2)
        Unknown3032()
        Unknown4003('yuef_431hibashiraC.DIG', '')
        Unknown23067('koef_431baku')
        Unknown23015(1)
        Unknown4015()
        Unknown3001(0)
        Unknown3004(45)
        Unknown1064(100)
        Unknown2053(1)
        Unknown1088(10)

        def upon_43():
            if (SLOT_48 == 4061):
                AirPushbackX(3000)
    sprite('vr_yu431_dmg04', 9)
    Unknown1067(100)
    StartMultihit()
    sprite('vr_yu431_dmg04', 5)
    SFX_3('blaze_long')
    Unknown1067(0)
    RefreshMultihit()
    sprite('vr_yu431_dmg04', 10)
    clearUponHandler(43)
    DisableAttackRestOfMove()
    Unknown3033()
    Unknown1067(30)
    Unknown23118(-65536)
    Unknown3004(-25)

@State
def Maharagi_firePitC():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(4)
        Damage(1000)
        AttackP2(85)
        Unknown30065(0)
        MinimumDamagePct(10)
        Hitstop(0)
        AirUntechableTime(60)
        Unknown9017(1)
        ProjectileDurabilityLvl(1)
        AirHitstunAnimation(13)
        GroundedHitstunAnimation(13)
        AirPushbackX(10000)
        AirPushbackY(17000)
        YImpluseBeforeWallbounce(1800)
        callSubroutine('BoosterDamageUp')
        Unknown11092(1)
        Unknown23182(2)
        Unknown3032()
        Unknown4003('yuef_431hibashiraC.DIG', '')
        Unknown23067('koef_431baku')
        Unknown23015(1)
        Unknown4015()
        Unknown3001(0)
        Unknown3004(45)
        Unknown1064(100)
        Unknown2053(1)
        Unknown1088(10)

        def upon_43():
            if (SLOT_48 == 4061):
                AirPushbackX(3000)
    sprite('vr_yu431_dmg04', 9)
    Unknown1067(100)
    StartMultihit()
    sprite('vr_yu431_dmg04', 5)
    SFX_3('blaze_long')
    Unknown1067(0)
    RefreshMultihit()
    sprite('vr_yu431_dmg04', 10)
    clearUponHandler(43)
    DisableAttackRestOfMove()
    Unknown3033()
    Unknown1067(30)
    Unknown23118(-65536)
    Unknown3004(-25)

@State
def Maharagi_firePitClast():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(4)
        Damage(1000)
        AttackP2(85)
        Unknown30065(0)
        MinimumDamagePct(10)
        Hitstop(0)
        AirUntechableTime(60)
        Unknown9017(1)
        ProjectileDurabilityLvl(1)
        AirHitstunAnimation(13)
        GroundedHitstunAnimation(13)
        AirPushbackX(12000)
        AirPushbackY(20000)
        YImpluseBeforeWallbounce(1800)
        callSubroutine('BoosterDamageUp')
        Unknown11092(1)
        Unknown23182(2)
        Unknown3032()
        Unknown4003('yuef_431hibashiraC.DIG', '')
        Unknown23067('koef_431baku')
        Unknown23015(1)
        Unknown4015()
        Unknown3001(0)
        Unknown3004(45)
        Unknown1064(100)
        Unknown2053(1)
        Unknown1088(10)

        def upon_43():
            if (SLOT_48 == 4061):
                AirPushbackX(3000)
    sprite('vr_yu431_dmg04', 9)
    Unknown1067(100)
    StartMultihit()
    sprite('vr_yu431_dmg04', 5)
    SFX_3('blaze_long')
    Unknown1067(0)
    RefreshMultihit()
    sprite('vr_yu431_dmg04', 10)
    clearUponHandler(43)
    DisableAttackRestOfMove()
    Unknown3033()
    Unknown1067(30)
    Unknown23118(-65536)
    Unknown3004(-25)

@State
def Maharagi_fire_TAG():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(3)
        Damage(1000)
        Hitstop(0)
        AirUntechableTime(45)
        Unknown9017(1)
        ProjectileDurabilityLvl(1)
        AirHitstunAnimation(17)
        GroundedHitstunAnimation(17)
        AirPushbackX(12000)
        AirPushbackY(25000)
        YImpluseBeforeWallbounce(1800)
        callSubroutine('BoosterDamageUp')

        def upon_43():
            if (SLOT_48 == 4073):
                AirPushbackX(0)
        Unknown3032()
        Unknown4003('yuef_431hibashiraC.DIG', '')
        Unknown23067('koef_431baku')
        Unknown23015(1)
        Unknown4015()
        Unknown3001(0)
        Unknown3004(45)
        Unknown1064(100)
        Unknown2053(1)
        Unknown1088(10)
    sprite('vr_yu431_dmg04', 9)
    Unknown1067(100)
    StartMultihit()
    sprite('vr_yu431_dmg04', 5)
    SFX_3('blaze_long')
    Unknown1067(0)
    RefreshMultihit()
    sprite('vr_yu431_dmg04', 10)
    clearUponHandler(43)
    DisableAttackRestOfMove()
    Unknown3033()
    Unknown1067(30)
    Unknown23118(-65536)
    Unknown3004(-25)

@State
def FireYugamiC():

    def upon_IMMEDIATE():
        Unknown4007(2)
    sprite('keep', 10)
    GFX_0('fireyugamisubC', 100)
    sprite('keep', 10)
    GFX_0('fireyugamisubC', 100)
    sprite('keep', 10)
    GFX_0('fireyugamisubC', 100)
    sprite('keep', 5)
    Unknown3004(-60)

@State
def fireyugamisubC():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown3057(1)
        Unknown3058(19000)
        Unknown1096(1800)
        Unknown1062(-200, -300)
        Unknown1070(500, -500)
        Unknown1060(20)
        Unknown1068(380)
    sprite('vr_yu_yugami', 15)
    sprite('vr_yu_yugami', 13)
    Unknown3004(-20)

@State
def fire_booster():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('yuef_403aura.DIG', '')
        Unknown4010(2)
        Unknown1096(1200)
        Unknown4015()
        Unknown4007(3)
    sprite('null', 15)
    GFX_2('yuef_403boost')
    sprite('null', 10)
    Unknown3004(-26)

@State
def agidine_firering():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('yuef_430firering.DIG', '')
        Unknown4007(2)
        Unknown4015()
        Unknown1007(300000)
        Unknown3001(0)
    sprite('null', 10)
    Unknown3004(25)
    Unknown1067(70)
    GFX_0('firering_sub', 100)
    sprite('null', 32)
    GFX_0('firering_sub2', 100)
    sprite('null', 10)
    Unknown3004(-26)

@State
def firering_sub():

    def upon_IMMEDIATE():
        GFX_2('koef_430firering')
    sprite('null', 32)
    GFX_0('FireYugamiB2', 100)
    Unknown1067(20)
    sprite('null', 30)
    Unknown3004(-9)

@State
def firering_sub2():

    def upon_IMMEDIATE():
        GFX_2('koef_430firering')
    sprite('null', 32)
    GFX_0('FireYugamiB2', 100)
    Unknown1067(20)
    sprite('null', 30)
    Unknown3004(-9)

@Subroutine
def Init_AgiDine_AtkData():
    Unknown2011()
    AttackLevel_(5)
    AttackP1(70)
    AttackP2(70)
    AirUntechableTime(60)
    GroundedHitstunAnimation(9)
    AirHitstunAnimation(9)
    AirPushbackX(75000)
    AirPushbackY(35000)
    Hitstop(7)
    Unknown11001(0, 7, 0)
    Unknown9017(1)
    Unknown9346(0)
    Unknown9178(1)
    WallbounceReboundTime(10)
    Unknown11057(0)
    Unknown23182(2)

@State
def agidine_fireuzuS():

    def upon_IMMEDIATE():
        callSubroutine('Init_AgiDine_AtkData')
        Damage(1400)
        Hitstop(7)
        Unknown3032()
        Unknown4003('yuef_430fireuzu.DIG', 'yuef_430fireuzu_000.mmot')
        Unknown23067('koef_430firecore')
        Unknown23015(1)
        Unknown4015()
        Unknown3001(0)
        Unknown1096(900)
        physicsXImpulse(70000)

        def upon_43():
            if (SLOT_48 == 10023):
                Hitstop(0)
                AirPushbackY(25000)
                MinimumDamagePct(20)
    sprite('vr_ko430_col', 2)
    GFX_0('FireYugamiB1s', 100)
    GFX_1('koef_430firecoresub', 100)
    SFX_3('bomb_s')
    Unknown3004(25)
    RefreshMultihit()
    sprite('vr_ko430_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 2)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 2)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 2)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 10)
    GFX_1('koef_430firecoresub', 100)
    sprite('null', 10)
    Unknown3004(-26)

@State
def agidine_fireuzuM():

    def upon_IMMEDIATE():
        Unknown23056()
        callSubroutine('Init_AgiDine_AtkData')
        Damage(2200)
        Hitstop(7)
        callSubroutine('BoosterDamageUp')
        Unknown3032()
        Unknown4003('yuef_430fireuzu.DIG', 'yuef_430fireuzu_000.mmot')
        Unknown23067('koef_430firecore')
        Unknown23015(1)
        Unknown4015()
        Unknown3001(0)
        Unknown1096(1200)
        physicsXImpulse(65000)

        def upon_43():
            if (SLOT_48 == 10023):
                Hitstop(0)
                AirPushbackY(25000)
                MinimumDamagePct(20)
    sprite('vr_ko430_col', 2)
    GFX_0('FireYugamiB1m', 100)
    GFX_0('430firecoresub_m', 100)
    SFX_3('bomb_m')
    Unknown3004(25)
    RefreshMultihit()
    sprite('vr_ko430_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 2)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 2)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 2)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 10)
    GFX_1('koef_430firecoresub', 100)
    sprite('null', 10)
    Unknown3004(-26)

@State
def agidine_fireuzuL():

    def upon_IMMEDIATE():
        Unknown23056()
        callSubroutine('Init_AgiDine_AtkData')
        Damage(3200)
        Hitstop(16)
        callSubroutine('BoosterDamageUp')
        Unknown3032()
        Unknown4003('yuef_430fireuzu2.DIG', '')
        Unknown23067('koef_430firecore')
        Unknown23015(1)
        Unknown4015()
        Unknown3001(0)
        Unknown1096(1500)
        physicsXImpulse(60000)

        def upon_43():
            if (SLOT_48 == 10023):
                Hitstop(0)
                AirPushbackY(25000)
                MinimumDamagePct(20)
    sprite('vr_ko430_col', 2)
    GFX_0('FireYugamiB1l', 100)
    GFX_0('430firecoresub_l', 100)
    SFX_3('bomb_l')
    Unknown3004(25)
    RefreshMultihit()
    sprite('vr_ko430_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 2)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 2)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 2)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_col', 10)
    GFX_1('koef_430firecoresub', 100)
    sprite('null', 10)
    Unknown3004(-26)

@State
def agidine_fireuzuL_TAG():

    def upon_IMMEDIATE():
        Unknown23056()
        callSubroutine('Init_AgiDine_AtkData')
        Damage(2000)
        Hitstop(16)
        AttackP1(100)
        AttackP2(100)
        MinimumDamagePct(100)
        Unknown3032()
        Unknown4003('yuef_430fireuzu.DIG', 'yuef_430fireuzu_000.mmot')
        Unknown23067('koef_430firecore')
        Unknown23015(1)
        Unknown4015()
        Unknown3001(50)
        Unknown1096(1200)
        physicsXImpulse(65000)
    sprite('vr_ko430_DUO_col', 2)
    GFX_0('FireYugamiB1l', 100)
    GFX_0('430firecoresub_l', 100)
    SFX_3('bomb_l')
    Unknown3004(25)
    RefreshMultihit()
    sprite('vr_ko430_DUO_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 2)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 2)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 2)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 10)
    GFX_1('koef_430firecoresub', 100)
    sprite('null', 10)
    Unknown3004(-26)

@State
def agidine_fireuzuL_OD_TAG():

    def upon_IMMEDIATE():
        Unknown23056()
        callSubroutine('Init_AgiDine_AtkData')
        Damage(2500)
        Hitstop(16)
        AttackP2(100)
        MinimumDamagePct(100)
        AttackP1(100)
        Unknown3032()
        Unknown4003('yuef_430fireuzu2.DIG', '')
        Unknown23067('koef_430firecore')
        Unknown23015(1)
        Unknown4015()
        Unknown3001(50)
        Unknown1096(1500)
        physicsXImpulse(60000)
    sprite('vr_ko430_DUO_col', 2)
    GFX_0('FireYugamiB1l', 100)
    GFX_0('430firecoresub_l', 100)
    SFX_3('bomb_l')
    Unknown3004(25)
    RefreshMultihit()
    sprite('vr_ko430_DUO_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 1)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 2)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 2)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 2)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 3)
    GFX_1('koef_430firecoresub', 100)
    sprite('vr_ko430_DUO_col', 10)
    GFX_1('koef_430firecoresub', 100)
    sprite('null', 10)
    Unknown3004(-26)

@State
def __430firecoresub_m():

    def upon_IMMEDIATE():
        Unknown23067('koef_430firecoresub')
        Unknown4007(2)
        Unknown2054(1)
        sendToLabelUpon(32, 0)
        Unknown1096(1200)
    sprite('null', 25)
    sprite('null', 8)
    Unknown3004(-35)

@State
def __430firecoresub_l():

    def upon_IMMEDIATE():
        Unknown23067('koef_430firecoresub')
        Unknown4007(2)
        Unknown2054(1)
        sendToLabelUpon(32, 0)
        Unknown1096(1400)
    sprite('null', 25)
    sprite('null', 8)
    Unknown3004(-35)

@State
def FireYugamiB1s():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown4010(2)
    label(0)
    sprite('keep', 10)
    GFX_0('fireyugamisubB1s', 100)
    loopRest()
    gotoLabel(0)

@State
def FireYugamiB1m():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown4010(2)
    label(0)
    sprite('keep', 10)
    GFX_0('fireyugamisubB1m', 100)
    loopRest()
    gotoLabel(0)

@State
def FireYugamiB1l():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown4010(2)
    label(0)
    sprite('keep', 10)
    GFX_0('fireyugamisubB1l', 100)
    loopRest()
    gotoLabel(0)

@State
def fireyugamisubB1s():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown3057(1)
        Unknown3058(30000)
        Unknown1096(1200)
        Unknown1062(400, -400)
        Unknown1070(400, -400)
        Unknown1060(200)
        Unknown1068(200)
    sprite('vr_yu_yugami', 6)
    sprite('vr_yu_yugami', 6)
    Unknown3004(-50)

@State
def fireyugamisubB1m():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown3057(1)
        Unknown3058(30000)
        Unknown1096(1400)
        Unknown1062(400, -400)
        Unknown1070(400, -400)
        Unknown1060(300)
        Unknown1068(300)
    sprite('vr_yu_yugami', 6)
    sprite('vr_yu_yugami', 6)
    Unknown3004(-50)

@State
def fireyugamisubB1l():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown3057(1)
        Unknown3058(30000)
        Unknown1096(1700)
        Unknown1062(400, -400)
        Unknown1070(400, -400)
        Unknown1060(400)
        Unknown1068(400)
    sprite('vr_yu_yugami', 6)
    sprite('vr_yu_yugami', 6)
    Unknown3004(-50)

@State
def FireYugamiB2():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown4010(2)
        sendToLabelUpon(32, 5)
    label(0)
    sprite('keep', 8)
    GFX_0('fireyugamisubB2', 100)
    loopRest()
    gotoLabel(0)
    label(5)
    sprite('keep', 5)
    Unknown3004(-60)

@State
def fireyugamisubB2():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown3057(1)
        Unknown3058(19000)
        Unknown1096(2200)
        Unknown3004(-13)
        Unknown1062(400, -400)
        Unknown1070(400, -400)
        Unknown1060(80)
        Unknown1068(80)
    sprite('vr_yu_yugami', 9)
    Unknown3004(-30)

@Subroutine
def Maharagidine_AtkData():
    Unknown2011()
    Unknown23056()
    AttackLevel_(5)
    Damage(800)
    MinimumDamagePct(15)
    AttackP2(60)
    AirUntechableTime(40)
    Hitstop(0)
    GroundedHitstunAnimation(1)
    PushbackX(-39900)
    AirPushbackY(11000)
    Unknown9021(1)
    Unknown9017(1)
    Unknown23182(2)
    Unknown11092(1)
    Unknown11110(88)
    Unknown3032()
    Unknown4003('yuef_431hibashiraC.DIG', '')
    Unknown23067('koef_431baku')
    Unknown23015(1)
    Unknown4015()
    Unknown3001(0)
    Unknown3004(45)
    Unknown1064(100)
    Unknown1056(1200)
    Unknown1088(10)
    callSubroutine('BoosterDamageUp')

@State
def MaharagidineC_fire():

    def upon_IMMEDIATE():
        Unknown2011()
        Unknown2003(1)
    sprite('null', 10)
    Unknown23024(1)
    GFX_0('MaharagidineC_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-150000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-150000)
    Unknown35()
    GFX_0('MaharagidineC_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(150000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(150000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineC_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-300000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-300000)
    Unknown35()
    GFX_0('MaharagidineC_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(300000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(300000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineC_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-450000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-450000)
    Unknown35()
    GFX_0('MaharagidineC_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(450000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(450000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineC_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-600000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-600000)
    Unknown35()
    GFX_0('MaharagidineC_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(600000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(600000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineC_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-750000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-750000)
    Unknown35()
    GFX_0('MaharagidineC_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(750000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(750000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineC_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-900000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-900000)
    Unknown35()
    GFX_0('MaharagidineC_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(900000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(900000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineC_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1050000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1050000)
    Unknown35()
    GFX_0('MaharagidineC_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1050000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1050000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineC_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1200000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1200000)
    Unknown35()
    GFX_0('MaharagidineC_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1200000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1200000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineC_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1350000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1350000)
    Unknown35()
    GFX_0('MaharagidineC_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1350000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1350000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineC_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1500000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1500000)
    Unknown35()
    GFX_0('MaharagidineC_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1500000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1500000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineC_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1650000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1650000)
    Unknown35()
    GFX_0('MaharagidineC_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1650000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1650000)
    Unknown35()
    sprite('null', 30)

@State
def MaharagidineC_Frontfire_col():

    def upon_IMMEDIATE():
        callSubroutine('Maharagidine_AtkData')
        AirPushbackX(18000)
        Unknown30055(100000, 50, 50)
        PushbackX(39900)
        DisableAttackRestOfMove()
    sprite('vr_yu431_dmg04', 9)
    Unknown1067(100)
    sprite('vr_yu431_dmg04', 5)
    Unknown1067(0)
    RefreshMultihit()
    SFX_3('blaze_long')
    sprite('vr_yu431_dmg04', 10)
    DisableAttackRestOfMove()
    Unknown3033()
    Unknown1067(30)
    Unknown3004(-25)

@State
def MaharagidineC_Backfire_col():

    def upon_IMMEDIATE():
        callSubroutine('Maharagidine_AtkData')
        AirPushbackX(-18000)
        Unknown30055(-100000, 50, 50)
        DisableAttackRestOfMove()
    sprite('vr_yu431_dmg04', 9)
    Unknown1067(100)
    sprite('vr_yu431_dmg04', 5)
    Unknown1067(0)
    RefreshMultihit()
    SFX_3('blaze_long')
    sprite('vr_yu431_dmg04', 10)
    DisableAttackRestOfMove()
    Unknown3033()
    Unknown1067(30)
    Unknown3004(-25)

@State
def MaharagidineD_fire():

    def upon_IMMEDIATE():
        Unknown2011()
        Unknown2003(1)
    sprite('null', 10)
    GFX_0('MaharagidineD_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1650000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1650000)
    Unknown35()
    GFX_0('MaharagidineD_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1650000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1650000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineD_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1500000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1500000)
    Unknown35()
    GFX_0('MaharagidineD_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1500000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1500000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineD_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1350000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1350000)
    Unknown35()
    GFX_0('MaharagidineD_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1350000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1350000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineD_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1200000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1200000)
    Unknown35()
    GFX_0('MaharagidineD_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1200000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1200000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineD_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1050000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1050000)
    Unknown35()
    GFX_0('MaharagidineD_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1050000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1050000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineD_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-900000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-900000)
    Unknown35()
    GFX_0('MaharagidineD_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(900000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(900000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineD_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-750000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-750000)
    Unknown35()
    GFX_0('MaharagidineD_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(750000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(750000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineD_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-600000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-600000)
    Unknown35()
    GFX_0('MaharagidineD_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(600000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(600000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineD_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-450000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-450000)
    Unknown35()
    GFX_0('MaharagidineD_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(450000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(450000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineD_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-300000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-300000)
    Unknown35()
    GFX_0('MaharagidineD_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(300000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(300000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineD_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-150000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-150000)
    Unknown35()
    GFX_0('MaharagidineD_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(150000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(150000)
    Unknown35()
    sprite('null', 30)
    clearUponHandler(3)

@State
def MaharagidineD_Frontfire_col():

    def upon_IMMEDIATE():
        callSubroutine('Maharagidine_AtkData')
        Unknown30055(-100000, 50, 50)
        AirPushbackX(-18000)
        DisableAttackRestOfMove()
    sprite('vr_yu431_dmg04', 9)
    Unknown1067(100)
    sprite('vr_yu431_dmg04', 5)
    Unknown1067(0)
    RefreshMultihit()
    SFX_3('blaze_long')
    sprite('vr_yu431_dmg04', 10)
    DisableAttackRestOfMove()
    Unknown3033()
    Unknown1067(30)
    Unknown3004(-25)

@State
def MaharagidineD_Backfire_col():

    def upon_IMMEDIATE():
        callSubroutine('Maharagidine_AtkData')
        Unknown30055(100000, 50, 50)
        AirPushbackX(18000)
        DisableAttackRestOfMove()
    sprite('vr_yu431_dmg04', 9)
    Unknown1067(100)
    sprite('vr_yu431_dmg04', 5)
    Unknown1067(0)
    RefreshMultihit()
    SFX_3('blaze_long')
    sprite('vr_yu431_dmg04', 10)
    DisableAttackRestOfMove()
    Unknown3033()
    Unknown1067(30)
    Unknown3004(-25)

@State
def MaharagidineOD_C_fire():

    def upon_IMMEDIATE():
        Unknown2011()
        Unknown2003(1)
    sprite('null', 10)
    Unknown23024(1)
    GFX_0('MaharagidineOD_C_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-150000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-150000)
    Unknown35()
    GFX_0('MaharagidineOD_C_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(150000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(150000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_C_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-300000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-300000)
    Unknown35()
    GFX_0('MaharagidineOD_C_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(300000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(300000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_C_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-450000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-450000)
    Unknown35()
    GFX_0('MaharagidineOD_C_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(450000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(450000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_C_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-600000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-600000)
    Unknown35()
    GFX_0('MaharagidineOD_C_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(600000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(600000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_C_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-750000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-750000)
    Unknown35()
    GFX_0('MaharagidineOD_C_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(750000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(750000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_C_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-900000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-900000)
    Unknown35()
    GFX_0('MaharagidineOD_C_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(900000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(900000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_C_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1050000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1050000)
    Unknown35()
    GFX_0('MaharagidineOD_C_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1050000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1050000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_C_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1200000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1200000)
    Unknown35()
    GFX_0('MaharagidineOD_C_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1200000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1200000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_C_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1350000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1350000)
    Unknown35()
    GFX_0('MaharagidineOD_C_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1350000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1350000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_C_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1500000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1500000)
    Unknown35()
    GFX_0('MaharagidineOD_C_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1500000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1500000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_C_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1650000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1650000)
    Unknown35()
    GFX_0('MaharagidineOD_C_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1650000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1650000)
    Unknown35()
    sprite('null', 30)

@State
def MaharagidineOD_C_Frontfire_col():

    def upon_IMMEDIATE():
        callSubroutine('Maharagidine_AtkData')
        Damage(1000)
        callSubroutine('BoosterDamageUp')
        AirPushbackX(18000)
        Unknown30055(100000, 50, 50)
        PushbackX(39900)
        DisableAttackRestOfMove()
    sprite('vr_yu431_dmg04', 9)
    Unknown1067(100)
    sprite('vr_yu431_dmg04', 5)
    Unknown1067(0)
    RefreshMultihit()
    SFX_3('blaze_long')
    sprite('vr_yu431_dmg04', 10)
    DisableAttackRestOfMove()
    Unknown3033()
    Unknown1067(30)
    Unknown3004(-25)

@State
def MaharagidineOD_C_Backfire_col():

    def upon_IMMEDIATE():
        callSubroutine('Maharagidine_AtkData')
        Damage(1000)
        callSubroutine('BoosterDamageUp')
        AirPushbackX(-18000)
        Unknown30055(-100000, 50, 50)
        DisableAttackRestOfMove()
    sprite('vr_yu431_dmg04', 9)
    Unknown1067(100)
    sprite('vr_yu431_dmg04', 5)
    Unknown1067(0)
    RefreshMultihit()
    SFX_3('blaze_long')
    sprite('vr_yu431_dmg04', 10)
    DisableAttackRestOfMove()
    Unknown3033()
    Unknown1067(30)
    Unknown3004(-25)

@State
def MaharagidineOD_D_fire():

    def upon_IMMEDIATE():
        Unknown2011()
        Unknown2003(1)
    sprite('null', 10)
    GFX_0('MaharagidineOD_D_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1650000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1650000)
    Unknown35()
    GFX_0('MaharagidineOD_D_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1650000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1650000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_D_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1500000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1500000)
    Unknown35()
    GFX_0('MaharagidineOD_D_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1500000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1500000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_D_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1350000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1350000)
    Unknown35()
    GFX_0('MaharagidineOD_D_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1350000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1350000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_D_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1200000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1200000)
    Unknown35()
    GFX_0('MaharagidineOD_D_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1200000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1200000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_D_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-1050000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-1050000)
    Unknown35()
    GFX_0('MaharagidineOD_D_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(1050000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(1050000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_D_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-900000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-900000)
    Unknown35()
    GFX_0('MaharagidineOD_D_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(900000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(900000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_D_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-750000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-750000)
    Unknown35()
    GFX_0('MaharagidineOD_D_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(750000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(750000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_D_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-600000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-600000)
    Unknown35()
    GFX_0('MaharagidineOD_D_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(600000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(600000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_D_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-450000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-450000)
    Unknown35()
    GFX_0('MaharagidineOD_D_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(450000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(450000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_D_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-300000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-300000)
    Unknown35()
    GFX_0('MaharagidineOD_D_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(300000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(300000)
    Unknown35()
    sprite('null', 10)
    GFX_0('MaharagidineOD_D_Backfire_col', 100)
    Unknown36(1)
    teleportRelativeX(-150000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(-150000)
    Unknown35()
    GFX_0('MaharagidineOD_D_Frontfire_col', 100)
    Unknown36(1)
    teleportRelativeX(150000)
    Unknown35()
    if SLOT_51:
        Unknown23029(1, 10052, 0)
    GFX_0('FireYugamiD', 100)
    Unknown36(1)
    teleportRelativeX(150000)
    Unknown35()
    sprite('null', 30)
    clearUponHandler(3)

@State
def MaharagidineOD_D_Frontfire_col():

    def upon_IMMEDIATE():
        callSubroutine('Maharagidine_AtkData')
        Damage(1000)
        callSubroutine('BoosterDamageUp')
        Unknown30055(-100000, 50, 50)
        AirPushbackX(-18000)
        DisableAttackRestOfMove()
    sprite('vr_yu431_dmg04', 9)
    Unknown1067(100)
    sprite('vr_yu431_dmg04', 5)
    Unknown1067(0)
    RefreshMultihit()
    SFX_3('blaze_long')
    sprite('vr_yu431_dmg04', 10)
    DisableAttackRestOfMove()
    Unknown3033()
    Unknown1067(30)
    Unknown3004(-25)

@State
def MaharagidineOD_D_Backfire_col():

    def upon_IMMEDIATE():
        callSubroutine('Maharagidine_AtkData')
        Damage(1000)
        Unknown30055(100000, 50, 50)
        AirPushbackX(18000)
        DisableAttackRestOfMove()
    sprite('vr_yu431_dmg04', 9)
    Unknown1067(100)
    sprite('vr_yu431_dmg04', 5)
    Unknown1067(0)
    RefreshMultihit()
    SFX_3('blaze_long')
    sprite('vr_yu431_dmg04', 10)
    DisableAttackRestOfMove()
    Unknown3033()
    Unknown1067(30)
    Unknown3004(-25)

@State
def FireYugamiD():

    def upon_IMMEDIATE():
        Unknown4007(2)
    sprite('keep', 7)
    GFX_0('fireyugamisubD', 100)
    sprite('keep', 7)
    GFX_0('fireyugamisubD', 100)
    sprite('keep', 7)
    GFX_0('fireyugamisubD', 100)
    sprite('keep', 5)
    Unknown3004(-60)

@State
def fireyugamisubD():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown3057(1)
        Unknown3058(19000)
        Unknown1096(1300)
        Unknown1062(-200, -300)
        Unknown1070(500, -500)
        Unknown1060(20)
        Unknown1068(380)
    sprite('vr_yu_yugami', 12)
    sprite('vr_yu_yugami', 9)
    Unknown3004(-30)

@State
def Konohana_charge():

    def upon_IMMEDIATE():
        Unknown23067('koef_450charge')
        Unknown4007(2)
        Unknown2054(1)
        sendToLabelUpon(32, 0)
    sprite('null', 32767)
    label(0)
    sprite('null', 8)
    Unknown3004(-35)

@State
def Konohana_egg():

    def upon_IMMEDIATE():
        Unknown23067('koef_450egg')
        Unknown4007(2)
        Unknown2054(1)
        sendToLabelUpon(32, 0)
    label(1)
    sprite('null', 8)
    SFX_3('magiccircle_b')
    loopRest()
    gotoLabel(1)
    label(0)
    sprite('null', 8)
    SFX_3('damage_magic_mh')
    Unknown3004(-35)

@State
def Konohana_burst():

    def upon_IMMEDIATE():
        Unknown23067('koef_450sakuraburst')
        Unknown4007(2)
        Unknown2054(1)
    sprite('dmy_atk01', 80)
    Unknown2012()
    Damage(0)
    Hitstop(0)
    AirUntechableTime(10000)
    AirHitstunAnimation(9)
    GroundedHitstunAnimation(9)
    AirPushbackX(0)
    AirPushbackY(4000)
    YImpluseBeforeWallbounce(100)
    Unknown11050(5, '')
    Unknown11023(1)
    RefreshMultihit()
    loopRest()

@State
def BurstYugami():
    sprite('vr_yu450_yugami', 1)
    Unknown4007(2)
    Unknown4010(2)
    Unknown3032()
    Unknown3057(1)
    Unknown3058(14000)
    label(0)
    sprite('keep', 5)
    Unknown1074(1000)
    Unknown1096(3000)
    Unknown1099(0)
    Unknown1062(400, -400)
    Unknown1070(400, -400)
    Unknown1063(80, -80)
    Unknown1071(80, -80)
    GFX_0('yugamisub', 100)
    loopRest()
    gotoLabel(0)

@State
def yugamisub():
    sprite('vr_yu450_yugami', 80)
    Unknown4007(2)
    Unknown4010(2)
    Unknown3032()
    Unknown3057(1)
    Unknown3058(14000)
    Unknown3004(-10)
    Unknown1025(1000, -1000)
    Unknown1026(4000, -4000)

@State
def IchigekiPicture():

    def upon_IMMEDIATE():
        Unknown6001(1)
        Unknown2007()
        Unknown23015(2)
        Unknown3032()
        Unknown3001(0)
        Unknown1000(-500000)
        Unknown1007(128000)
    sprite('ichigeki', 12)
    GFX_0('Cutinbg', 100)
    sprite('ichigeki', 12)
    physicsXImpulse(40000)
    Unknown3004(15)
    sprite('ichigeki', 110)
    physicsXImpulse(400)
    sprite('ichigeki', 20)
    Unknown23119(-1, 20, 1)
    physicsXImpulse(80000)
    Unknown3004(-9)

@State
def Cutinbg():

    def upon_IMMEDIATE():
        Unknown4003('yuef_450cutinbg.DIG', '')
        Unknown23015(1)
        Unknown3032()
        Unknown2007()
        Unknown3001(0)
    sprite('null', 15)
    Unknown3004(11)
    sprite('null', 125)
    Unknown3004(0)
    sprite('null', 30)
    Unknown3004(-9)

@State
def BG_sakura():

    def upon_IMMEDIATE():
        sendToLabelUpon(32, 0)
    sprite('null', 10)
    teleportRelativeX(300000)
    GFX_0('Sakurafubuki_storm', 100)
    Unknown36(1)
    teleportRelativeX(-800000)
    Unknown35()
    GFX_0('BGscr', 100)
    Unknown3004(20)
    sprite('null', 32767)
    label(0)
    sprite('null', 26)
    Unknown3004(-10)

@State
def Sakurafubuki_slow():

    def upon_IMMEDIATE():
        GFX_2('yuef_450sakurafubuki0')
    sprite('null', 32767)
    label(0)
    sprite('null', 10)
    Unknown3004(-26)

@State
def Sakurafubuki_storm():

    def upon_IMMEDIATE():
        GFX_2('yuef_450sakurafubuki')
        Unknown3001(0)
    sprite('null', 250)
    Unknown3004(40)
    sprite('null', 5)
    GFX_0('Sakurafubuki_KO', 100)
    Unknown3004(-60)

@State
def Sakurafubuki_KO():

    def upon_IMMEDIATE():
        GFX_2('yuef_450cutinsakura02')
        Unknown3001(0)
    sprite('null', 100)
    Unknown3004(40)
    sprite('null', 5)
    Unknown3004(-60)

@State
def SakuraSE():

    def upon_IMMEDIATE():
        sendToLabelUpon(32, 0)
    label(1)
    sprite('null', 35)
    SFX_3('yu003')
    sprite('null', 20)
    SFX_3('yu003')
    loopRest()
    gotoLabel(1)
    label(0)
    sprite('null', 1)

@State
def BGscr():

    def upon_IMMEDIATE():
        Unknown4003('yuef_450BGsakura.DIG', '')
        Unknown3032()
        Unknown3001(0)
    sprite('null', 250)
    Unknown3004(5)
    sprite('null', 5)
    Unknown3004(-60)

@State
def Sakuraend():

    def upon_IMMEDIATE():
        Unknown4003('yuef_450BGhikari.DIG', '')
        Unknown23067('yuef_450sakuraend')
    sprite('null', 32767)

@State
def IchigekiBlack():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4061(2)
        Unknown3001(0)
        Unknown1096(5000)
        Unknown23015(2)
        Unknown4007(2)
        sendToLabelUpon(32, 0)
    sprite('vr_ko450_black', 32767)
    Unknown3004(7)
    label(0)
    sprite('vr_ko450_black', 5)
    Unknown3004(-60)

@State
def IchigekiWhite():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4061(2)
        Unknown3001(0)
        Unknown23015(4)
        Unknown1096(5000)
        Unknown4007(2)
    sprite('vr_ko450_white', 8)
    Unknown3004(60)
    sprite('vr_ko450_white', 6)
    Unknown3004(-50)

@State
def __450flash():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4061(2)
        Unknown3001(0)
        Unknown23015(4)
        Unknown1096(5000)
        Unknown4007(2)
    sprite('vr_ko450_white', 30)
    Unknown3004(15)
    sprite('vr_ko450_white', 30)
    Unknown3004(-9)

@State
def Ichigeki_HitEff():

    def upon_IMMEDIATE():
        Unknown1086(22)
    SLOT_51 = 25
    label(1)
    sprite('dmy_atk01', 1)
    Unknown2012()
    Damage(247)
    Hitstop(0)
    AirHitstunAnimation(9)
    GroundedHitstunAnimation(9)
    AirPushbackX(0)
    AirPushbackY(100)
    YImpluseBeforeWallbounce(0)
    Unknown11050(5, '')
    Unknown11023(1)
    Unknown23011(1)
    RefreshMultihit()
    Unknown23151(22, 105)
    Unknown4045('yuef_450hit', 100)
    SLOT_51 = (SLOT_51 + (-1))
    loopRest()
    Unknown19(10, 2, 51)
    gotoLabel(1)
    label(10)
    SLOT_51 = 25
    label(11)
    sprite('dmy_atk01', 1)
    Unknown2012()
    Damage(372)
    Hitstop(0)
    AirHitstunAnimation(9)
    GroundedHitstunAnimation(9)
    AirPushbackX(0)
    AirPushbackY(100)
    YImpluseBeforeWallbounce(0)
    Unknown11050(5, '')
    Unknown11023(1)
    Unknown23011(1)
    RefreshMultihit()
    Unknown23151(22, 105)
    Unknown4045('yuef_450hit', 100)
    SLOT_51 = (SLOT_51 + (-1))
    loopRest()
    Unknown19(20, 2, 51)
    gotoLabel(11)
    label(20)
    SLOT_51 = 140
    label(21)
    sprite('dmy_atk01', 1)
    Unknown2012()
    Damage(297)
    Hitstop(0)
    AirHitstunAnimation(9)
    GroundedHitstunAnimation(9)
    AirPushbackX(0)
    AirPushbackY(100)
    YImpluseBeforeWallbounce(0)
    Unknown11050(5, '')
    Unknown11023(1)
    Unknown23011(1)
    RefreshMultihit()
    Unknown23151(22, 105)
    Unknown4045('yuef_450hit', 100)
    SLOT_51 = (SLOT_51 + (-1))
    loopRest()
    Unknown19(0, 2, 51)
    gotoLabel(21)
    label(0)
    sprite('dmy_atk00', 1)
    RefreshMultihit()
    Unknown2012()
    AttackLevel_(5)
    Hitstop(0)
    Damage(37564)
    MinimumDamagePct(100)
    AirHitstunAnimation(9)
    GroundedHitstunAnimation(9)
    AirPushbackX(10000)
    AirPushbackY(70000)
    YImpluseBeforeWallbounce(-1)
    Unknown11064(4)
    Unknown11050(5, '')
    Unknown11023(1)
    Unknown4045('yuef_450hit', 100)
    sprite('null', 1)
    Unknown23011(15)
    Unknown36(22)
    Unknown3038(1)
    Unknown35()

@State
def IchigekiCamera():

    def upon_IMMEDIATE():
        sendToLabelUpon(32, 0)
        sendToLabelUpon(33, 1)
        sendToLabelUpon(34, 2)
        sendToLabelUpon(35, 3)
        sendToLabelUpon(36, 4)
        sendToLabelUpon(37, 5)
        Unknown20000(1)
        Unknown20001(1)
    sprite('null', 32767)
    label(0)
    sprite('null', 1)
    loopRest()
    label(1)
    sprite('null', 32767)
    Unknown20000(1)
    Unknown20002(1)
    Unknown1086(3)
    Unknown20003(1)
    Unknown1000(0)
    label(2)
    sprite('null', 32767)
    Unknown1086(22)
    teleportRelativeX(-200000)
    Unknown20000(1)
    Unknown20002(1)
    Unknown20003(1)
    label(3)
    sprite('null', 32767)
    Unknown20002(1)
    Unknown1086(3)
    Unknown20003(1)
    physicsXImpulse(5000)

@State
def __612flash():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4061(2)
        Unknown3001(0)
        Unknown23015(4)
        Unknown1096(1000)
        Unknown4007(2)
    sprite('vr_ko450_white', 15)
    Unknown3004(15)
    sprite('vr_ko450_white', 10)
    Unknown3004(0)
    Unknown3001(235)
    sprite('vr_ko450_white', 25)
    Unknown3004(-10)

@State
def PersonaEntry2():

    def upon_IMMEDIATE():
        Unknown23022(1)
        Unknown4061(1)
        Unknown23023()
        Unknown23184(3, 100, 40000, 20000, 0, 0, 0, 0)
        callSubroutine('PYU_CheckWarp')
        Unknown2019(1000)
        Unknown4007(3)
    sprite('ko601_00', 60)
    sprite('ko601_01', 6)
    Unknown23015(0)
    Unknown2019(-500)
    sprite('ko601_02', 6)
    physicsXImpulse(-3000)
    sprite('ko601_03', 8)
    sprite('ko601_04', 10)
    Unknown21007(3, 32)
    sprite('ko601_05', 8)
    sprite('ko601_06', 8)
    physicsXImpulse(-2000)
    sprite('ko601_07', 8)
    Unknown2019(4000)
    sprite('ko601_08', 6)
    sprite('ko601_09', 6)
    physicsXImpulse(0)
    sprite('ko601_10', 6)
    sprite('ko601_09', 6)
    sprite('ko601_08', 6)
    sprite('ko601_09', 6)
    sprite('ko601_10', 6)
    sprite('ko601_09', 6)
    sprite('ko601_08', 6)
    sprite('ko601_09', 6)
    sprite('ko601_10', 6)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PersonaEntry_TAG():

    def upon_IMMEDIATE():
        Unknown23022(1)
        Unknown4061(1)
        Unknown2019(1000)
        Unknown23023()
        Unknown23184(3, 100, 40000, 20000, 0, 0, 0, 0)
        callSubroutine('PYU_CheckWarp')
        Unknown4007(3)
    label(0)
    sprite('ko601_00', 1)
    sendToLabelUpon(32, 1)
    gotoLabel(0)
    label(1)
    sprite('ko601_00', 60)
    sprite('ko601_01', 6)
    Unknown23015(0)
    Unknown2019(-500)
    sprite('ko601_02', 6)
    physicsXImpulse(-3000)
    sprite('ko601_03', 8)
    sprite('ko601_04', 10)
    Unknown21007(3, 32)
    sprite('ko601_05', 8)
    sprite('ko601_06', 8)
    physicsXImpulse(-2000)
    sprite('ko601_07', 8)
    Unknown2019(4000)
    sprite('ko601_08', 6)
    sprite('ko601_09', 6)
    physicsXImpulse(0)
    sprite('ko601_10', 6)
    sprite('ko601_09', 6)
    sprite('ko601_08', 6)
    sprite('ko601_09', 6)
    sprite('ko601_10', 6)
    sprite('ko601_09', 6)
    sprite('ko601_08', 6)
    sprite('ko601_09', 6)
    sprite('ko601_10', 6)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PersonaMatchWin():

    def upon_IMMEDIATE():
        Unknown23022(1)
        Unknown4061(1)
        Unknown2019(1000)
        Unknown23023()
        Unknown23184(3, 100, -120000, 120000, 0, 0, 0, 0)
        callSubroutine('PYU_CheckWarp')
        Unknown2053(1)
    sprite('ko611_00', 6)
    sprite('ko611_01', 6)
    sprite('ko611_02', 6)
    sprite('ko611_03', 6)
    label(0)
    sprite('ko611_04', 8)
    sprite('ko611_05', 8)
    sprite('ko611_06', 8)
    gotoLabel(0)

@State
def P4U_Cutin_Parent():

    def upon_IMMEDIATE():
        Unknown2054(1)
    sprite('null', 1)
    GFX_0('P4U_Cutin_Face', 100)

@State
def P4U_Cutin_Face():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown23015(5)
        Unknown2007()
        Unknown4009(2)
        Unknown2054(1)
        Unknown13044(0)
        Unknown6001(1)
        Unknown1000(-650000)
        teleportRelativeY(-292000)
        Unknown1096(1350)
        Unknown2037(0)
        Unknown3001(0)

        def upon_3():
            if (SLOT_2 == 2):
                Unknown1007(15000)
                Unknown3001(150)
            if (SLOT_2 == 3):
                Unknown1007(-30000)
                Unknown3001(180)
            if (SLOT_2 == 4):
                Unknown1007(25000)
                Unknown3001(210)
            if (SLOT_2 == 5):
                Unknown1007(-20000)
                Unknown3001(255)
            if (SLOT_2 == 6):
                Unknown1007(10000)
            Unknown2038(1)
    sprite('vr_99p4u_face00', 45)
    SFX_0('spsys_over_power')
    SFX_0('spsys_over_power')
    if SLOT_168:
        GFX_0('P4U_ka_NotJPN', 100)
    else:
        GFX_0('P4U_ka_JPN', 100)
    sprite('vr_99p4u_face00', 1)
    Unknown3001(180)
    sprite('vr_99p4u_face00', 1)
    Unknown3001(120)
    sprite('vr_99p4u_face00', 1)
    Unknown3001(60)

@State
def P4U_ka_JPN():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown23015(5)
        Unknown2007()
        Unknown4009(2)
        Unknown6001(1)
        Unknown1000(-950000)
        teleportRelativeY(-232000)
        Unknown1096(1350)
    sprite('vr_p4_ka_mozi', 1)
    Unknown1096(100)
    Unknown3001(40)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(500)
    Unknown1096(300)
    Unknown3001(70)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(550)
    Unknown1096(700)
    Unknown3001(100)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(600)
    Unknown1096(900)
    Unknown3001(130)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(6500)
    Unknown1096(800)
    Unknown3001(160)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(700)
    Unknown1096(900)
    Unknown3001(190)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(800)
    Unknown1096(1150)
    Unknown3001(220)
    sprite('vr_p4_ka_mozi', 38)
    teleportRelativeX(-2000)
    Unknown1007(800)
    Unknown1096(1100)
    Unknown3001(255)
    physicsXImpulse(-50)
    physicsYImpulse(50)
    sprite('vr_p4_ka_mozi', 1)
    Unknown3001(180)
    Unknown1096(700)
    sprite('vr_p4_ka_mozi', 1)
    Unknown3001(120)
    Unknown1096(300)
    sprite('vr_p4_ka_mozi', 1)
    Unknown3001(60)
    Unknown1096(100)

@State
def P4U_ka_NotJPN():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown23015(5)
        Unknown2007()
        Unknown4009(2)
        Unknown6001(1)
        Unknown1000(-950000)
        teleportRelativeY(-232000)
        Unknown1096(1350)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    Unknown1096(100)
    Unknown3001(40)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(500)
    Unknown1096(300)
    Unknown3001(70)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(550)
    Unknown1096(700)
    Unknown3001(100)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(600)
    Unknown1096(900)
    Unknown3001(130)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(6500)
    Unknown1096(800)
    Unknown3001(160)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(700)
    Unknown1096(900)
    Unknown3001(190)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(800)
    Unknown1096(1150)
    Unknown3001(220)
    sprite('vr_p4_ka_mozi_notjpn', 38)
    teleportRelativeX(-2000)
    Unknown1007(800)
    Unknown1096(1100)
    Unknown3001(255)
    physicsXImpulse(-50)
    physicsYImpulse(50)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    Unknown3001(180)
    Unknown1096(700)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    Unknown3001(120)
    Unknown1096(300)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    Unknown3001(60)
    Unknown1096(100)

@State
def Func_DS_Yarare1():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(0)
        Damage(100)
        PushbackX(1000000)
        AirHitstunAnimation(9)
        Unknown11064(1)
        Unknown23151(22, 104)
        Unknown11080(0)
        Unknown53(2)
        Unknown53(1)
        Unknown9016(1)
        Unknown1096(900)
        Unknown3029(1)
        AfterimageCount(3)
        Unknown3070(1)
        AfterimageColor_1(120, 100, 100, 100)
        AfterimageColor_2(60, 100, 100, 100)
        AfterimageMask_1(0, 255, 0, 255)
        AfterimageMask_2(0, 255, 0, 255)
        EnableAfterimagePulse(1)
        Unknown3069(2)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 2)
    sprite('null', 1)
    teleportRelativeX(-450000)
    Unknown1007(270000)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_0('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    sprite('null', 40)
    GFX_0('sensunage_sakura', 100)
    loopRest()
    GFX_0('Func_DS_Yarare1_2', -1)

@State
def Func_DS_Yarare1_2():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(1)
        Damage(100)
        PushbackX(1000000)
        AirHitstunAnimation(11)
        Unknown11064(1)
        Unknown23151(22, 104)
        Unknown11080(0)
        Unknown53(2)
        Unknown53(1)
        Unknown9016(1)
        Unknown1096(900)
        Unknown3029(1)
        AfterimageCount(3)
        Unknown3070(1)
        AfterimageColor_1(120, 100, 100, 100)
        AfterimageColor_2(60, 100, 100, 100)
        AfterimageMask_1(0, 255, 0, 255)
        AfterimageMask_2(0, 255, 0, 255)
        EnableAfterimagePulse(1)
        Unknown3069(2)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 2)
    sprite('null', 1)
    teleportRelativeX(-450000)
    Unknown1007(270000)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_0('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    sprite('null', 40)
    GFX_0('sensunage_sakura', 100)
    loopRest()
    GFX_0('Func_DS_Yarare1_3', -1)

@State
def Func_DS_Yarare1_3():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(2)
        Damage(100)
        PushbackX(1000000)
        AirHitstunAnimation(10)
        Unknown11064(1)
        Unknown23151(22, 104)
        Unknown53(2)
        Unknown53(1)
        Unknown9016(1)
        Unknown11080(0)
        Unknown1096(900)
        Unknown3029(1)
        AfterimageCount(3)
        Unknown3070(1)
        AfterimageColor_1(120, 100, 100, 100)
        AfterimageColor_2(60, 100, 100, 100)
        AfterimageMask_1(0, 255, 0, 255)
        AfterimageMask_2(0, 255, 0, 255)
        EnableAfterimagePulse(1)
        Unknown3069(2)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 2)
    sprite('null', 1)
    teleportRelativeX(-450000)
    Unknown1007(270000)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_0('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    sprite('null', 40)
    GFX_0('sensunage_sakura', 100)
    loopRest()
    GFX_0('Func_DS_Yarare1_4', -1)

@State
def Func_DS_Yarare1_4():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(3)
        Damage(100)
        PushbackX(1000000)
        AirPushbackX(50000)
        AirPushbackY(15000)
        AirHitstunAnimation(12)
        Unknown11064(1)
        Unknown23151(22, 104)
        Unknown53(2)
        Unknown53(1)
        Unknown9016(1)
        Unknown11080(0)
        Unknown1096(900)
        Unknown3029(1)
        AfterimageCount(3)
        Unknown3070(1)
        AfterimageColor_1(120, 100, 100, 100)
        AfterimageColor_2(60, 100, 100, 100)
        AfterimageMask_1(0, 255, 0, 255)
        AfterimageMask_2(0, 255, 0, 255)
        EnableAfterimagePulse(1)
        Unknown3069(2)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 2)
    sprite('null', 1)
    teleportRelativeX(-450000)
    Unknown1007(270000)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_0('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    sprite('null', 40)
    GFX_0('sensunage_sakura', 100)
    loopRest()
    GFX_0('Func_DS_Yarare1_5', -1)

@State
def Func_DS_Yarare1_5():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(5)
        Damage(100)
        PushbackX(1000000)
        Unknown11064(1)
        Unknown23151(22, 104)
        Unknown53(2)
        Unknown53(1)
        Unknown9016(1)
        Unknown11080(0)
        Unknown1096(900)
        Unknown3029(1)
        AfterimageCount(3)
        Unknown3070(1)
        AfterimageColor_1(120, 100, 100, 100)
        AfterimageColor_2(60, 100, 100, 100)
        AfterimageMask_1(0, 255, 0, 255)
        AfterimageMask_2(0, 255, 0, 255)
        EnableAfterimagePulse(1)
        Unknown3069(2)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 2)
    sprite('null', 1)
    teleportRelativeX(-450000)
    Unknown1007(270000)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_0('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    GFX_0('sensunage_sakura', 100)

@State
def Func_DS_Yarare2():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(0)
        Damage(100)
        PushbackX(1000000)
        GroundedHitstunAnimation(5)
        AirHitstunAnimation(9)
        Unknown11064(1)
        Unknown23151(22, 104)

        def upon_3():
            if SLOT_125:
                FreezeCount(1)
                FreezeDuration(45)
        Unknown53(2)
        Unknown53(1)
        Unknown9016(1)
        Unknown11080(0)
        Unknown1096(900)
        Unknown3029(1)
        AfterimageCount(3)
        Unknown3070(1)
        AfterimageColor_1(120, 100, 100, 100)
        AfterimageColor_2(60, 100, 100, 100)
        AfterimageMask_1(0, 255, 0, 255)
        AfterimageMask_2(0, 255, 0, 255)
        EnableAfterimagePulse(1)
        Unknown3069(2)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 2)
    sprite('null', 1)
    teleportRelativeX(-450000)
    Unknown1007(270000)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_0('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    sprite('null', 40)
    GFX_0('sensunage_sakura', 100)
    loopRest()
    GFX_0('Func_DS_Yarare2_2', -1)

@State
def Func_DS_Yarare2_2():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(1)
        Damage(100)
        PushbackX(1000000)
        AirPushbackX(50000)
        AirPushbackY(-50000)
        GroundedHitstunAnimation(5)
        AirUntechableTime(60)
        Unknown9202(60)
        Unknown11064(1)
        Unknown23151(22, 104)
        Unknown11080(0)
        Unknown53(2)
        Unknown53(1)
        Unknown9016(1)
        Unknown1096(900)
        Unknown3029(1)
        AfterimageCount(3)
        Unknown3070(1)
        AfterimageColor_1(120, 100, 100, 100)
        AfterimageColor_2(60, 100, 100, 100)
        AfterimageMask_1(0, 255, 0, 255)
        AfterimageMask_2(0, 255, 0, 255)
        EnableAfterimagePulse(1)
        Unknown3069(2)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 2)
    sprite('null', 1)
    teleportRelativeX(-450000)
    Unknown1007(270000)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_0('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    sprite('null', 40)
    GFX_0('sensunage_sakura', 100)
    loopRest()
    GFX_0('Func_DS_Yarare2_3', -1)

@State
def Func_DS_Yarare2_3():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(2)
        Damage(100)
        PushbackX(1000000)
        GroundedHitstunAnimation(5)
        AirHitstunAnimation(13)
        Unknown11064(1)
        Unknown23151(22, 104)
        Unknown11080(0)
        Unknown53(2)
        Unknown53(1)
        Unknown9016(1)
        Unknown1096(900)
        Unknown3029(1)
        AfterimageCount(3)
        Unknown3070(1)
        AfterimageColor_1(120, 100, 100, 100)
        AfterimageColor_2(60, 100, 100, 100)
        AfterimageMask_1(0, 255, 0, 255)
        AfterimageMask_2(0, 255, 0, 255)
        EnableAfterimagePulse(1)
        Unknown3069(2)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 2)
    sprite('null', 1)
    teleportRelativeX(-450000)
    Unknown1007(270000)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_0('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    sprite('null', 40)
    GFX_0('sensunage_sakura', 100)
    loopRest()
    GFX_0('Func_DS_Yarare2_4', -1)

@State
def Func_DS_Yarare2_4():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(3)
        Damage(100)
        PushbackX(1000000)
        GroundedHitstunAnimation(5)
        Unknown11064(1)
        Unknown23151(22, 104)
        Unknown53(2)
        Unknown53(1)
        Unknown9016(1)
        Unknown11080(0)
        Unknown1096(900)
        Unknown3029(1)
        AfterimageCount(3)
        Unknown3070(1)
        AfterimageColor_1(120, 100, 100, 100)
        AfterimageColor_2(60, 100, 100, 100)
        AfterimageMask_1(0, 255, 0, 255)
        AfterimageMask_2(0, 255, 0, 255)
        EnableAfterimagePulse(1)
        Unknown3069(2)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 2)
    sprite('null', 1)
    teleportRelativeX(-450000)
    Unknown1007(270000)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_0('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    sprite('null', 40)
    GFX_0('sensunage_sakura', 100)
    loopRest()
    GFX_0('Func_DS_Yarare2_5', -1)

@State
def Func_DS_Yarare2_5():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(5)
        Damage(100)
        PushbackX(1000000)
        GroundedHitstunAnimation(5)
        Unknown11064(1)
        Unknown23151(22, 104)
        Unknown53(2)
        Unknown53(1)
        Unknown9016(1)
        Unknown11080(0)
        Unknown1096(900)
        Unknown3029(1)
        AfterimageCount(3)
        Unknown3070(1)
        AfterimageColor_1(120, 100, 100, 100)
        AfterimageColor_2(60, 100, 100, 100)
        AfterimageMask_1(0, 255, 0, 255)
        AfterimageMask_2(0, 255, 0, 255)
        EnableAfterimagePulse(1)
        Unknown3069(2)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 2)
    sprite('null', 1)
    teleportRelativeX(-450000)
    Unknown1007(270000)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_0('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    GFX_0('sensunage_sakura', 100)

@State
def Func_DS_Yarare3():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(0)
        Damage(100)
        PushbackX(1000000)
        GroundedHitstunAnimation(3)
        AirHitstunAnimation(9)
        Unknown11064(1)
        Unknown23151(22, 104)
        Unknown53(2)
        Unknown53(1)
        Unknown9016(1)
        Unknown11080(0)
        Unknown1096(900)
        Unknown3029(1)
        AfterimageCount(3)
        Unknown3070(1)
        AfterimageColor_1(120, 100, 100, 100)
        AfterimageColor_2(60, 100, 100, 100)
        AfterimageMask_1(0, 255, 0, 255)
        AfterimageMask_2(0, 255, 0, 255)
        EnableAfterimagePulse(1)
        Unknown3069(2)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 2)
    sprite('null', 1)
    teleportRelativeX(-450000)
    Unknown1007(270000)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_0('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    sprite('null', 40)
    GFX_0('sensunage_sakura', 100)
    loopRest()
    GFX_0('Func_DS_Yarare3_2', -1)

@State
def Func_DS_Yarare3_2():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(1)
        Damage(100)
        PushbackX(1000000)
        GroundedHitstunAnimation(3)
        Unknown11064(1)
        Unknown23151(22, 104)
        Unknown53(2)
        Unknown53(1)
        Unknown9016(1)
        Unknown11080(0)
        Unknown1096(900)
        Unknown3029(1)
        AfterimageCount(3)
        Unknown3070(1)
        AfterimageColor_1(120, 100, 100, 100)
        AfterimageColor_2(60, 100, 100, 100)
        AfterimageMask_1(0, 255, 0, 255)
        AfterimageMask_2(0, 255, 0, 255)
        EnableAfterimagePulse(1)
        Unknown3069(2)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 2)
    sprite('null', 1)
    teleportRelativeX(-450000)
    Unknown1007(270000)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_0('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    sprite('null', 40)
    GFX_0('sensunage_sakura', 100)
    loopRest()
    GFX_0('Func_DS_Yarare3_3', -1)

@State
def Func_DS_Yarare3_3():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(2)
        Damage(100)
        PushbackX(1000000)
        GroundedHitstunAnimation(3)
        Unknown11064(1)
        Unknown23151(22, 104)
        Unknown11080(0)
        Unknown53(2)
        Unknown53(1)
        Unknown9016(1)
        Unknown1096(900)
        Unknown3029(1)
        AfterimageCount(3)
        Unknown3070(1)
        AfterimageColor_1(120, 100, 100, 100)
        AfterimageColor_2(60, 100, 100, 100)
        AfterimageMask_1(0, 255, 0, 255)
        AfterimageMask_2(0, 255, 0, 255)
        EnableAfterimagePulse(1)
        Unknown3069(2)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 2)
    sprite('null', 1)
    teleportRelativeX(-450000)
    Unknown1007(270000)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_0('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    sprite('null', 40)
    GFX_0('sensunage_sakura', 100)
    loopRest()
    GFX_0('Func_DS_Yarare3_4', -1)

@State
def Func_DS_Yarare3_4():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(3)
        Damage(100)
        PushbackX(1000000)
        GroundedHitstunAnimation(3)
        Unknown11064(1)
        Unknown23151(22, 104)
        Unknown53(2)
        Unknown53(1)
        Unknown9016(1)
        Unknown11080(0)
        Unknown1096(900)
        Unknown3029(1)
        AfterimageCount(3)
        Unknown3070(1)
        AfterimageColor_1(120, 100, 100, 100)
        AfterimageColor_2(60, 100, 100, 100)
        AfterimageMask_1(0, 255, 0, 255)
        AfterimageMask_2(0, 255, 0, 255)
        EnableAfterimagePulse(1)
        Unknown3069(2)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 2)
    sprite('null', 1)
    teleportRelativeX(-450000)
    Unknown1007(270000)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_0('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    sprite('null', 40)
    GFX_0('sensunage_sakura', 100)
    loopRest()
    GFX_0('Func_DS_Yarare3_5', -1)

@State
def Func_DS_Yarare3_5():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(5)
        Damage(100)
        PushbackX(1000000)
        GroundedHitstunAnimation(3)
        Unknown11064(1)
        Unknown23151(22, 104)
        Unknown53(2)
        Unknown53(1)
        Unknown9016(1)
        Unknown11080(0)
        Unknown1096(900)
        Unknown3029(1)
        AfterimageCount(3)
        Unknown3070(1)
        AfterimageColor_1(120, 100, 100, 100)
        AfterimageColor_2(60, 100, 100, 100)
        AfterimageMask_1(0, 255, 0, 255)
        AfterimageMask_2(0, 255, 0, 255)
        EnableAfterimagePulse(1)
        Unknown3069(2)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 2)
    sprite('null', 1)
    teleportRelativeX(-450000)
    Unknown1007(270000)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_0('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    sprite('null', 40)
    GFX_0('sensunage_sakura', 100)
    loopRest()
    GFX_0('Func_DS_Yarare3_6', -1)

@State
def Func_DS_Yarare3_6():

    def upon_IMMEDIATE():
        Unknown2010()
        AttackLevel_(5)
        Damage(100)
        PushbackX(1000000)
        GroundedHitstunAnimation(2)
        Unknown9142(60)
        Unknown9130(60)
        Unknown11064(1)
        Unknown23151(22, 104)
        Unknown53(2)
        Unknown53(1)
        Unknown9016(1)
        Unknown11080(0)
        Unknown1096(900)
        Unknown3029(1)
        AfterimageCount(3)
        Unknown3070(1)
        AfterimageColor_1(120, 100, 100, 100)
        AfterimageColor_2(60, 100, 100, 100)
        AfterimageMask_1(0, 255, 0, 255)
        AfterimageMask_2(0, 255, 0, 255)
        EnableAfterimagePulse(1)
        Unknown3069(2)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 1)
        sendToLabelUpon(54, 2)
    sprite('null', 1)
    teleportRelativeX(-450000)
    Unknown1007(270000)
    label(10)
    sprite('vr_yu204_sensu02', 3)
    sprite('vr_yu204_sensu03', 3)
    sprite('vr_yu204_sensu04', 3)
    sprite('vr_yu204_sensu05', 3)
    sprite('vr_yu204_sensu01', 3)
    SFX_0('slash_knife_slow')
    loopRest()
    gotoLabel(10)
    label(2)
    GFX_0('sensunage_sakura', 100)