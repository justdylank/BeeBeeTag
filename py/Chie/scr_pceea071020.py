@Subroutine
def ChargeDamageUp():
    Unknown48(25, 2, 67, 3, 2, 59)
    if (SLOT_67 == 1):
        Unknown10000(200)
    if (SLOT_67 == 2):
        Unknown10000(300)
    if (SLOT_67 == 3):
        Unknown10000(400)

@Subroutine
def Charge_Count():
    if SLOT_31:
        pass

@State
def PCE_PersonaCreate():

    def upon_IMMEDIATE():
        Unknown23022(1)
        Unknown13043(-75000)
        teleportRelativeY(0)
        Unknown23013(1)
    sprite('null', 32767)
    Unknown3038(1)
    Unknown24('5043455f506572736f6e6157616974000000000000000000000000000000000064000000')

@State
def PCE_PersonaWait():

    def upon_IMMEDIATE():
        SLOT_11 = 0
        SLOT_59 = (-1)
        callSubroutine('PCE_ReceptionSignal')
    sprite('null', 32767)
    Unknown3038(1)
    Unknown3001(0)
    Unknown3004(0)
    loopRest()

@Subroutine
def PCE_ReceptionSignal():

    def upon_43():
        if (SLOT_48 == 600):
            Unknown23185('PersonaMatchWin', 200)
        if (SLOT_48 == 101):
            Unknown23185('PCE_Persona5C', 50)
        if (SLOT_48 == 201):
            Unknown23185('PCE_Persona2C', 50)
        if (SLOT_48 == 102):
            Unknown23185('PCE_Persona5D', 50)
        if (SLOT_48 == 103):
            Unknown23185('PCE_Persona5B3rd', 50)
        if (SLOT_48 == 104):
            Unknown23185('PCE_Persona5C_CA', 50)
        if (SLOT_48 == 105):
            Unknown23185('PCE_Persona5D_CA', 50)
        if (SLOT_48 == 106):
            Unknown23185('PCE_PersonaD_Hasei_CA', 50)
        if (SLOT_48 == 107):
            Unknown23185('PCE_PersonaEXAttack', 50)
        if (SLOT_48 == 301):
            Unknown23185('PCE_PersonaAir5C', 50)
        if (SLOT_48 == 302):
            Unknown23185('PCE_PersonaAir4D', 50)
        if (SLOT_48 == 401):
            Unknown23185('PCE_PersonaDragonKickA', 50)
        if (SLOT_48 == 402):
            Unknown23185('PCE_PersonaDragonKickB', 50)
        if (SLOT_48 == 403):
            Unknown23185('PCE_PersonaDragonKickEx', 50)
        if (SLOT_48 == 404):
            Unknown23185('PCE_PersonaAirDragonKickA', 50)
        if (SLOT_48 == 405):
            Unknown23185('PCE_PersonaAirDragonKickB', 50)
        if (SLOT_48 == 406):
            Unknown23185('PCE_PersonaAirDragonKickEx', 50)
        if (SLOT_48 == 407):
            Unknown23185('PCE_PersonaKokutengekiA', 50)
        if (SLOT_48 == 408):
            Unknown23185('PCE_PersonaKokutengekiB', 50)
        if (SLOT_48 == 409):
            Unknown23185('PCE_PersonaKokutengekiEx', 50)
        if (SLOT_48 == 410):
            Unknown23185('PCE_PersonaKokutengeki_PS', 50)
        if (SLOT_48 == 501):
            Unknown23185('PCE_PersonaGodHand', 100)
        if (SLOT_48 == 502):
            Unknown23185('PCE_PersonaGodHandOD', 100)
        if (SLOT_48 == 503):
            Unknown23185('PCE_PersonaAguneyasutora_Far', 100)
        if (SLOT_48 == 504):
            Unknown23185('PCE_PersonaAguneyasutora_Near', 100)
        if (SLOT_48 == 505):
            Unknown23185('PCE_PersonaAguneyasutoraOD_Far', 100)
        if (SLOT_48 == 506):
            Unknown23185('PCE_PersonaAguneyasutoraOD_Near', 100)
        if (SLOT_48 == 513):
            Unknown23185('PCE_PersonaGodHand_DUO', 100)
        if (SLOT_48 == 514):
            Unknown23185('PCE_PersonaGodHand_DUOOD', 100)
        if (SLOT_48 == 9999):
            Unknown23185('PersonaDeleteAndIdling', 300)
        if (SLOT_48 == 700):
            Unknown23185('PCE_PersonaIchigeki', 100)

@Subroutine
def PersonaReset():
    Unknown4061(1)
    Unknown23022(0)
    Unknown2053(0)
    Unknown23015(11)
    Unknown2019(-1)
    Unknown1084(1)
    Unknown4009(0)
    Unknown4008(0)
    Unknown4007(0)
    EnableCollision(0)
    Unknown30041('')

    def upon_48():
        Unknown23030('pce_tomoe_swordeff', 0, 48, 0, 4, 0, 0, 0, 0)

@Subroutine
def PCE_EffectInit():
    Unknown2019(5)
    Unknown23015(11)

@Subroutine
def PCE_AttackInit():
    Unknown23028(1, 1)
    Unknown30037('')
    Unknown30035(1)
    callSubroutine('PersonaReset')
    Unknown23015(11)
    Unknown2019(10)
    if (SLOT_11 == 0):
        SLOT_58 = 1
    if (SLOT_11 == 10):
        SLOT_58 = 1
    SLOT_11 = 100
    callSubroutine('PCE_ReceptionSignal')
    Unknown11034(1)
    ProjectileDurabilityLvl(0)
    Unknown23023()
    EnableCollision(1)
    Unknown30040(1)
    Unknown30040(2)

@Subroutine
def PCE_SPAttackInit():
    Unknown23028(2, 1)
    Unknown30037('')
    Unknown30035(1)
    callSubroutine('PersonaReset')
    Unknown23015(11)
    Unknown2019(10)
    if (SLOT_11 == 0):
        SLOT_58 = 1
    if (SLOT_11 == 10):
        SLOT_58 = 1
    SLOT_11 = 110
    callSubroutine('PCE_ReceptionSignal')
    Unknown11034(1)
    ProjectileDurabilityLvl(0)
    Unknown23023()
    EnableCollision(1)
    Unknown30040(1)
    Unknown30040(2)

@Subroutine
def PCE_DDAttackInit():
    Unknown23028(3, 1)
    Unknown30037('')
    callSubroutine('PersonaReset')
    Unknown23015(11)
    Unknown2019(10)
    if (SLOT_11 == 0):
        SLOT_58 = 1
    if (SLOT_11 == 10):
        SLOT_58 = 1
    Unknown11034(1)
    ProjectileDurabilityLvl(0)
    SLOT_11 = 120
    callSubroutine('PCE_ReceptionSignal')
    Unknown23023()

@Subroutine
def PCE_CheckWarp():
    if SLOT_58:
        Unknown3001(0)
        Unknown3004(85)
        loopRelated(17, 4)

        def upon_17():
            Unknown3001(255)
            Unknown3004(0)

@Subroutine
def PCE_ForceWarp():
    SLOT_58 = 1
    Unknown3001(0)
    Unknown3004(85)
    loopRelated(17, 4)

    def upon_17():
        Unknown3001(255)
        Unknown3004(0)

@State
def PersonaDeleteAndIdling():

    def upon_IMMEDIATE():
        callSubroutine('PersonaReset')
    sprite('keep', 32767)
    if SLOT_143:
        GFX_0('PersonaDeleteEffect', 100)
        SFX_0('persona_disappear')
    SLOT_11 = 10
    SLOT_59 = (-1)
    enterState('PCE_PersonaWait')

@State
def PersonaSummonEffect():
    sprite('null', 30)
    Unknown23015(13)
    Unknown4054(11)
    Unknown4045('persona_summon', 100)

@State
def PersonaSummonEffect_PLAYER():
    sprite('null', 30)
    Unknown1007(288000)
    teleportRelativeX(112000)
    GFX_1('persona_summon_ply', 100)

@State
def PersonaDeleteEffect():

    def upon_IMMEDIATE():
        callSubroutine('PersonaReset')
        callSubroutine('PCE_ReceptionSignal')
    sprite('null', 30)
    Unknown2019(1)
    Unknown23053('190000000b000000')
    Unknown4061(1)
    Unknown3032()
    Unknown1059(-100)
    Unknown1067(150)
    Unknown3004(-20)
    Unknown23022(1)
    Unknown4054(11)
    Unknown4045('persona_delete', 100)
    Unknown2054(1)
    EnableCollision(0)
    Unknown1084(1)

@Subroutine
def Init_CantCancelPersonaAction():
    SLOT_10 = 1

    def upon_STATE_END():
        SLOT_10 = 0

@State
def PCE_Persona5C():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -50000, 50000, -50000, 300000, 0, 50000)
        callSubroutine('PCE_AttackInit')
        AttackLevel_(4)
        AttackP1(90)
        Unknown9016(1)
        AirUntechableTime(20)
        Unknown9156(27)
        AirPushbackX(11000)
        AirPushbackY(15000)
        PushbackX(25000)
        AttackAttributes(0, 1, 0, 0, 0)
        callSubroutine('ChargeDamageUp')
        Unknown23078(1)
        callSubroutine('PCE_CheckWarp')
        Unknown4007(3)
        Unknown2053(1)
        Unknown23059(1)
    sprite('to204_00', 2)
    Unknown2006()
    sprite('to204_01', 1)
    GFX_0('Zanzoh5C', 100)
    physicsXImpulse(30000)
    sprite('to204_01', 1)
    Unknown1019(60)
    sprite('to204_02', 2)
    Unknown1019(60)
    SFX_3('slash_pole_middle')
    sprite('to204_04', 1)
    Unknown23054('to204_03', 3)
    Unknown1019(0)
    RefreshMultihit()
    sprite('to204_04', 2)
    Unknown23022(0)
    sprite('to204_05', 3)
    Unknown4007(0)
    DisableAttackRestOfMove()
    sprite('to204_06', 3)
    SFX_3('cloth_l')
    sprite('to204_04', 4)
    sprite('to204_05', 4)
    sprite('to204_06', 4)
    SFX_3('cloth_l')
    sprite('to204_04', 5)
    sprite('to204_05', 5)
    sprite('to204_06', 5)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_Persona5C_CA():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -50000, 50000, -50000, 1400000, 0, 50000)
        callSubroutine('PCE_AttackInit')
        Unknown23078(1)
        callSubroutine('PCE_CheckWarp')
        Unknown4007(3)
        Unknown2053(0)
        EnableCollision(0)
        Unknown23022(1)
        Unknown2003(1)
    sprite('to204_00', 3)
    Unknown2006()
    sprite('to204_01', 1)
    GFX_0('Zanzoh5C', 100)
    physicsXImpulse(30000)
    sprite('to204_01', 1)
    Unknown1019(60)
    sprite('to204_02', 2)
    Unknown1019(60)
    SFX_3('slash_pole_middle')
    sprite('to204_04', 1)
    Unknown23054('to204_03', 3)
    Unknown1019(0)
    RefreshMultihit()
    sprite('to204_04', 2)
    sprite('to204_05', 3)
    Unknown4007(0)
    DisableAttackRestOfMove()
    sprite('to204_06', 3)
    SFX_3('cloth_l')
    sprite('to204_04', 4)
    sprite('to204_05', 4)
    sprite('to204_06', 4)
    SFX_3('cloth_l')
    sprite('to204_04', 5)
    sprite('to204_05', 5)
    sprite('to204_06', 5)
    SFX_3('cloth_l')
    sprite('to204_04', 4)
    sprite('to204_05', 4)
    sprite('to204_06', 4)
    SFX_3('cloth_l')
    sprite('to204_04', 4)
    sprite('to204_05', 4)
    sprite('to204_06', 4)
    SFX_3('cloth_l')
    sprite('to204_04', 4)
    sprite('to204_05', 4)
    sprite('to204_06', 4)
    SFX_3('cloth_l')
    sprite('to204_04', 4)
    sprite('to204_05', 4)
    sprite('to204_06', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_Persona2C():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -50000, 50000, -100000, 1400000, -50000, 50000)
        callSubroutine('PCE_AttackInit')
        AttackLevel_(4)
        Damage(1100)
        Unknown9016(1)
        Unknown9266(9)
        AirUntechableTime(30)
        AttackP1(90)
        Unknown11092(1)
        Hitstop(1)
        Unknown11001(8, 8, 8)
        AirPushbackX(14000)
        AirPushbackY(30000)
        YImpluseBeforeWallbounce(2200)
        PushbackX(5000)
        callSubroutine('ChargeDamageUp')
        Unknown23078(1)
        Unknown2053(1)
        callSubroutine('PCE_CheckWarp')

        def upon_ON_HIT_OR_BLOCK():
            Unknown2037(1)
        Unknown23059(1)
    sprite('to234_00', 3)
    Unknown2006()
    Unknown23015(0)
    Unknown2018(1, 100)
    sprite('to234_01', 2)
    SFX_3('cloth_l')
    physicsXImpulse(30000)
    sprite('to234_02', 2)
    Unknown1019(80)
    sprite('to234_00', 2)
    Unknown1019(80)
    sprite('to234_03', 2)
    Unknown1019(80)
    sprite('to234_04', 2)
    Unknown1019(80)
    sprite('to234_05', 2)
    SFX_3('slash_pole_middle')
    GFX_0('2C_ice', 100)
    Unknown1019(80)
    sprite('to234_07', 1)
    Unknown23054('to234_06', 3)
    GFX_1('toef_C2', 0)
    Unknown1084(1)
    RefreshMultihit()
    sprite('to234_07', 2)
    Unknown23022(0)
    sprite('to234_07', 6)
    DisableAttackRestOfMove()
    sprite('to234_08', 1)
    GFX_0('Zanzoh2C', 100)
    sprite('to234_08', 2)
    Unknown1019(60)
    RefreshMultihit()
    Hitstop(8)
    Unknown11001(0, 0, 0)
    AirUntechableTime(30)
    AirPushbackX(3000)
    AirPushbackY(22000)
    PushbackX(5000)
    GroundedHitstunAnimation(11)
    AirHitstunAnimation(11)
    if SLOT_2:
        Unknown21015('NmlAtk5B2nd', 1101, 0)
    clearUponHandler(10)

    def upon_ON_HIT_OR_BLOCK():
        Unknown21015('NmlAtk5B2nd', 1101, 0)
    GFX_1('toef_C2', 0)
    GFX_1('toef_C2', 0)
    GFX_1('toef_C2', 1)
    GFX_1('toef_C2', 2)
    GFX_1('toef_C2', 3)
    GFX_1('toef_C2', 4)
    sprite('to234_09', 3)
    EnableCollision(0)
    sprite('to234_10', 3)
    SFX_3('cloth_l')
    sprite('to234_11', 3)
    sprite('to234_09', 3)
    sprite('to234_10', 3)
    SFX_3('cloth_l')
    sprite('to234_11', 3)
    sprite('to234_09', 3)
    sprite('to234_10', 3)
    SFX_3('cloth_l')
    sprite('to234_11', 3)
    sprite('to234_09', 3)
    sprite('to234_10', 3)
    SFX_3('cloth_l')
    sprite('to234_11', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_Persona5D():

    def upon_IMMEDIATE():
        Unknown23184(3, 100, -50000, 50000, 0, 1400000, -50000, 50000)
        callSubroutine('PCE_AttackInit')
        Unknown2009()
        AttackLevel_(4)
        Damage(50)
        PushbackX(20000)
        Unknown30055(200000, 70, 70)
        AirPushbackY(15000)
        Hitstop(1)
        Unknown9016(1)
        Unknown11092(1)
        Unknown11042(1)
        Unknown4007(3)
        Unknown2053(1)
        callSubroutine('PCE_CheckWarp')
    sprite('to205_00', 8)
    ScreenShake(20000, 0)
    teleportRelativeX(150000)
    sprite('to205_01', 3)
    Unknown4007(0)
    Unknown30032(1)
    physicsXImpulse(34200)
    GFX_0('Zanzoh5D', 100)
    sprite('to205_02', 3)
    Unknown1019(80)
    sprite('to205_03', 3)
    Unknown1019(80)
    sprite('to205_04', 2)
    Unknown1019(80)
    sprite('to205_05', 2)
    Unknown1019(80)
    SFX_3('slash_pole_fast')
    sprite('to205_06', 2)
    Unknown1019(80)
    RefreshMultihit()
    sprite('to205_07', 3)
    Unknown1019(80)
    sprite('to205_08', 2)
    Unknown1019(80)
    RefreshMultihit()
    sprite('to205_09', 2)
    Unknown1019(80)
    sprite('to205_04', 2)
    Unknown1019(80)
    sprite('to205_05', 2)
    Unknown1019(80)
    SFX_3('slash_pole_fast')
    sprite('to205_06', 2)
    Unknown1019(80)
    RefreshMultihit()
    sprite('to205_07', 3)
    Unknown1019(80)
    sprite('to205_10', 3)
    Unknown30033(1)
    SFX_3('slash_pole_slow')
    label(819)
    sprite('to206_00', 3)
    DisableAttackRestOfMove()
    Unknown1084(1)
    sprite('to206_01', 2)
    Damage(50)
    Hitstop(0)
    AirPushbackY(3000)
    PushbackX(5000)
    Unknown9016(1)
    RefreshMultihit()
    GFX_0('Zanzoh5DD', 0)
    sprite('to206_02', 2)
    RefreshMultihit()
    SFX_3('slash_pole_fast')

    def upon_ON_HIT_OR_BLOCK():
        Unknown2038(1)
        if op(4, 2, 2, 0, 3):
            Unknown11050(5, '')
        else:
            Unknown11050(0, '')
    sprite('to206_03', 2)
    RefreshMultihit()
    sprite('to206_01', 2)
    RefreshMultihit()
    GFX_0('Zanzoh5DD', 0)
    SFX_3('slash_pole_fast')
    sprite('to206_02', 2)
    RefreshMultihit()
    sprite('to206_03', 2)
    RefreshMultihit()
    SFX_3('slash_pole_fast')
    sprite('to206_01', 2)
    RefreshMultihit()
    GFX_0('Zanzoh5DD', 0)
    sprite('to206_02', 2)
    RefreshMultihit()
    SFX_3('slash_pole_fast')
    sprite('to206_03', 2)
    RefreshMultihit()
    sprite('to206_01', 2)
    RefreshMultihit()
    GFX_0('Zanzoh5DD', 0)
    SFX_3('slash_pole_fast')
    sprite('to206_02', 2)
    RefreshMultihit()
    sprite('to206_03', 2)
    RefreshMultihit()
    SFX_3('slash_pole_fast')
    sprite('to206_01', 2)
    RefreshMultihit()
    GFX_0('Zanzoh5DD', 0)
    sprite('to206_02', 2)
    RefreshMultihit()
    SFX_3('slash_pole_fast')
    sprite('to206_03', 2)
    RefreshMultihit()
    sprite('to206_04', 3)
    GFX_0('Zanzoh5DD_2', 100)
    clearUponHandler(10)
    sprite('to206_05', 2)
    sprite('to206_06', 2)
    sprite('to206_07', 2)
    GFX_0('Zanzoh5DD_3', 100)
    SFX_3('slash_pole_middle')
    sprite('to206_09', 3)
    Unknown23054('to206_08', 3)
    EnableCollision(0)
    RefreshMultihit()
    AttackLevel_(4)
    Damage(200)
    Hitstop(8)
    AirPushbackX(44000)
    AirPushbackY(30000)
    AirHitstunAnimation(9)
    GroundedHitstunAnimation(9)
    Unknown9178(1)
    Unknown9346(1)
    AirHitstunAfterWallbounce(15)
    WallbounceReboundTime(0)
    EnableCollision(0)
    Unknown11050(0, '')
    Unknown30055(0, 0, 0)
    sprite('to206_10', 3)
    DisableAttackRestOfMove()
    sprite('to206_11', 3)
    SFX_3('cloth_l')
    sprite('to206_09', 3)
    sprite('to206_10', 3)
    sprite('to206_11', 3)
    sprite('to206_09', 3)
    SFX_3('cloth_l')
    sprite('to206_10', 3)
    sprite('to206_11', 3)
    sprite('to206_09', 3)
    sprite('to206_10', 3)
    SFX_3('cloth_l')
    sprite('to206_11', 3)
    sprite('to206_09', 3)
    sprite('to206_10', 3)
    sprite('to206_11', 3)
    SFX_3('cloth_l')
    sprite('to206_09', 3)
    sprite('to206_10', 3)
    sprite('to206_11', 3)
    sprite('to206_09', 3)
    SFX_3('cloth_l')
    sprite('to206_10', 3)
    sprite('to206_11', 3)
    SFX_3('cloth_l')
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_Persona5B3rd():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -50000, 50000, 0, 1400000, -50000, 50000)
        callSubroutine('PCE_SPAttackInit')
        AttackLevel_(4)
        Damage(1000)
        AttackP1(70)
        PushbackX(20000)
        AirPushbackX(0)
        AirPushbackY(15000)
        Unknown30055(110000, 50, 0)
        Hitstop(1)
        Unknown9016(1)
        Unknown11092(1)
        Unknown11042(1)
        AttackAttributes(0, 1, 0, 0, 0)
        callSubroutine('ChargeDamageUp')
        Unknown4007(3)
        Unknown2053(1)
        EnableCollision(1)
        Unknown2006()
        callSubroutine('PCE_CheckWarp')
        Unknown23059(1)
    sprite('to205_00', 1)
    ScreenShake(20000, 0)
    sprite('to205_01', 1)
    Unknown4007(0)
    Unknown30032(1)
    physicsXImpulse(50000)
    GFX_0('Zanzoh5D_Short', 100)
    sprite('to205_02', 1)
    sprite('to205_02', 1)
    Unknown1019(60)
    sprite('to205_03', 1)
    Unknown1019(60)
    sprite('to205_04', 2)
    Unknown1019(60)
    sprite('to205_05', 2)
    Unknown1019(60)
    SFX_3('slash_pole_fast')
    sprite('to205_06', 2)
    Unknown1019(60)
    RefreshMultihit()
    sprite('to205_07', 3)
    Unknown1019(60)
    sprite('to205_08', 2)
    Unknown1019(60)
    RefreshMultihit()
    sprite('to205_09', 2)
    Unknown1019(60)
    sprite('to205_10', 3)
    Unknown30033(1)
    SFX_3('slash_pole_slow')
    if SLOT_53:
        sendToLabel(819)
    sprite('to205_11', 1)
    Unknown30033(0)
    physicsXImpulse(20000)
    sprite('to205_11', 1)
    Unknown1019(85)
    sprite('to205_11', 1)
    Unknown1019(85)
    sprite('to205_12', 1)
    Unknown1019(85)
    sprite('to205_12', 1)
    Unknown1019(85)
    sprite('to205_14', 3)
    Unknown23054('to205_13', 3)
    GFX_0('Zanzoh5D_2', 100)
    RefreshMultihit()
    AttackLevel_(4)
    Unknown9004()
    Hitstop(8)
    AirUntechableTime(60)
    AirPushbackX(5000)
    AirPushbackY(35000)
    AirHitstunAnimation(10)
    GroundedHitstunAnimation(10)
    callSubroutine('ChargeDamageUp')
    EnableCollision(0)
    Unknown1019(0)
    sprite('to205_15', 3)
    DisableAttackRestOfMove()
    sprite('to205_16', 3)
    SFX_3('cloth_l')
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    SFX_3('cloth_l')
    sprite('to205_15', 3)
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    SFX_3('cloth_l')
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    SFX_3('cloth_l')
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    SFX_3('cloth_l')
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    SFX_3('cloth_l')
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    SFX_3('cloth_l')
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    SFX_3('cloth_l')
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    SFX_3('cloth_l')
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')
    label(819)
    sprite('to206_00', 3)
    DisableAttackRestOfMove()
    Unknown1084(1)
    sprite('to206_01', 2)
    Damage(100)
    Hitstop(0)
    AirPushbackY(3000)
    PushbackX(5000)
    Unknown9016(1)
    RefreshMultihit()
    GFX_0('Zanzoh5DD', 0)
    sprite('to206_02', 2)
    RefreshMultihit()
    SFX_3('slash_pole_fast')
    Unknown11055(1)
    sprite('to206_03', 2)
    RefreshMultihit()
    sprite('to206_01', 2)
    RefreshMultihit()
    GFX_0('Zanzoh5DD', 0)
    SFX_3('slash_pole_fast')
    sprite('to206_02', 2)
    RefreshMultihit()
    sprite('to206_03', 2)
    RefreshMultihit()
    SFX_3('slash_pole_fast')
    sprite('to206_04', 3)
    Unknown23015(0)
    Unknown2019(100)
    GFX_0('Zanzoh5DD_2', 100)
    sprite('to206_05', 2)
    sprite('to206_06', 2)
    sprite('to206_07', 2)
    GFX_0('Zanzoh5DD_3', 100)
    SFX_3('slash_pole_middle')
    sprite('to206_09', 3)
    Unknown23054('to206_08', 3)
    RefreshMultihit()
    AttackLevel_(4)
    Damage(200)
    Hitstop(8)
    AirPushbackX(44000)
    AirPushbackY(30000)
    AirHitstunAnimation(9)
    GroundedHitstunAnimation(9)
    Unknown9178(1)
    Unknown9346(1)
    AirHitstunAfterWallbounce(15)
    WallbounceReboundTime(0)
    EnableCollision(0)
    Unknown11055(0)
    sprite('to206_10', 3)
    DisableAttackRestOfMove()
    sprite('to206_11', 3)
    SFX_3('cloth_l')
    sprite('to206_09', 3)
    sprite('to206_10', 3)
    sprite('to206_11', 3)
    sprite('to206_09', 3)
    SFX_3('cloth_l')
    sprite('to206_10', 3)
    sprite('to206_11', 3)
    sprite('to206_09', 3)
    sprite('to206_10', 3)
    SFX_3('cloth_l')
    sprite('to206_11', 3)
    sprite('to206_09', 3)
    sprite('to206_10', 3)
    sprite('to206_11', 3)
    SFX_3('cloth_l')
    sprite('to206_09', 3)
    sprite('to206_10', 3)
    sprite('to206_11', 3)
    sprite('to206_09', 3)
    SFX_3('cloth_l')
    sprite('to206_10', 3)
    sprite('to206_11', 3)
    SFX_3('cloth_l')
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')
    
@State
def PCE_PersonaEXAttack():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -50000, 50000, 0, 1400000, -50000, 50000)
        callSubroutine('PCE_SPAttackInit')
        AttackLevel_(4)
        Damage(500)
        AttackP1(70)
        PushbackX(20000)
        AirPushbackX(0)
        AirPushbackY(15000)
        Unknown30055(110000, 50, 0)
        Hitstop(3)
        Unknown30065(0)
        MinimumDamagePct(10)
        Unknown9016(1)
        Unknown11092(1)
        Unknown11042(1)
        AttackAttributes(0, 1, 0, 0, 0)
        callSubroutine('ChargeDamageUp')
        Unknown4007(3)
        Unknown2053(1)
        EnableCollision(1)
        Unknown2006()
        callSubroutine('PCE_CheckWarp')
        Unknown23059(1)
        SLOT_10 = 1

        def upon_STATE_END():
            SLOT_10 = 0
    sprite('to205_00', 4)
    ScreenShake(20000, 0)
    sprite('to205_01', 4)
    Unknown4007(0)
    Unknown30032(1)
    physicsXImpulse(50000)
    GFX_0('Zanzoh5D_Short', 100)
    sprite('to205_02', 2)
    sprite('to205_02', 2)
    Unknown1019(60)
    sprite('to205_03', 2)
    Unknown1019(60)
    sprite('to205_04', 3)
    Unknown1019(60)
    sprite('to205_05', 3)
    Unknown1019(60)
    SFX_3('slash_pole_fast')
    sprite('to205_06', 4)
    Unknown1019(60)
    RefreshMultihit()
    sprite('to205_07', 4)
    Unknown1019(60)
    sprite('to205_08', 4)
    Unknown1019(60)
    RefreshMultihit()
    
    def upon_45():
        if CheckInput(0x17):
            SLOT_51 = 1
    sprite('to205_09', 2)
    Unknown1019(60)
    sprite('to205_10', 3)
    Unknown30033(1)
    SFX_3('slash_pole_slow')
    if SLOT_51:
        sendToLabel(0)
    sprite('to205_11', 1)
    Unknown30033(0)
    physicsXImpulse(20000)
    sprite('to205_11', 1)
    Unknown1019(85)
    sprite('to205_11', 1)
    Unknown1019(85)
    sprite('to205_12', 1)
    Unknown1019(85)
    sprite('to205_12', 1)
    Unknown1019(85)
    sprite('to205_14', 3)
    Unknown23054('to205_13', 3)
    GFX_0('Zanzoh5D_2', 100)
    RefreshMultihit()
    AttackLevel_(4)
    Unknown9004()
    Hitstop(8)
    AirUntechableTime(60)
    AirPushbackX(5000)
    AirPushbackY(35000)
    AirHitstunAnimation(10)
    GroundedHitstunAnimation(10)
    callSubroutine('ChargeDamageUp')
    EnableCollision(0)
    Unknown1019(0)
    sprite('to205_15', 3)
    DisableAttackRestOfMove()
    sprite('to205_16', 3)
    SFX_3('cloth_l')
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    SFX_3('cloth_l')
    sprite('to205_15', 3)
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    SFX_3('cloth_l')
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    SFX_3('cloth_l')
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')
    label(0)
    sprite('to206_00', 3)
    DisableAttackRestOfMove()
    Unknown1084(1)
    sprite('to206_01', 2)
    Damage(300)
    Hitstop(3)
    AirPushbackY(3000)
    PushbackX(5000)
    Unknown9016(1)
    RefreshMultihit()
    GFX_0('Zanzoh5DD', 0)
    sprite('to206_02', 2)
    RefreshMultihit()
    SFX_3('slash_pole_fast')
    Unknown11055(1)
    sprite('to206_03', 2)
    RefreshMultihit()
    sprite('to206_01', 2)
    RefreshMultihit()
    GFX_0('Zanzoh5DD', 0)
    SFX_3('slash_pole_fast')
    sprite('to206_02', 2)
    RefreshMultihit()
    sprite('to206_03', 2)
    RefreshMultihit()
    SFX_3('slash_pole_fast')
    sprite('to206_04', 3)
    Unknown23015(0)
    Unknown2019(100)
    GFX_0('Zanzoh5DD_2', 100)
    sprite('to206_05', 2)
    sprite('to206_06', 2)
    sprite('to206_07', 2)
    GFX_0('Zanzoh5DD_3', 100)
    SFX_3('slash_pole_middle')
    sprite('to206_09', 3)
    Unknown23054('to206_08', 3)
    RefreshMultihit()
    AttackLevel_(4)
    Damage(800)
    Hitstop(8)
    AirPushbackX(30000)
    AirPushbackY(25000)
    AirHitstunAnimation(9)
    GroundedHitstunAnimation(9)
    AirUntechableTime(45)
    Unknown11055(0)
    sprite('to206_10', 3)
    DisableAttackRestOfMove()
    SLOT_10 = 0
    sprite('to206_11', 3)
    SFX_3('cloth_l')
    sprite('to206_09', 3)
    sprite('to206_10', 3)
    sprite('to206_11', 3)
    sprite('to206_09', 3)
    SFX_3('cloth_l')
    sprite('to206_10', 3)
    sprite('to206_11', 3)
    sprite('to206_09', 3)
    sprite('to206_10', 3)
    SFX_3('cloth_l')
    sprite('to206_11', 3)
    sprite('to206_09', 3)
    sprite('to206_10', 3)
    sprite('to206_11', 3)
    SFX_3('cloth_l')
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_Persona5D_CA():

    def upon_IMMEDIATE():
        Unknown23184(3, 100, -50000, 50000, 0, 1400000, -50000, 50000)
        callSubroutine('PCE_AttackInit')
        Unknown2009()
        Unknown9016(1)
        Unknown4007(3)
        callSubroutine('PCE_CheckWarp')
        Unknown23022(1)
        Unknown2003(1)
    sprite('to205_00', 2)
    sprite('to205_10', 2)
    SFX_3('slash_pole_slow')
    sprite('to205_11', 1)
    physicsXImpulse(30000)
    sprite('to205_11', 1)
    Unknown1019(85)
    sprite('to205_11', 1)
    Unknown1019(85)
    sprite('to205_12', 1)
    Unknown1019(85)
    sprite('to205_12', 1)
    Unknown1019(85)
    sprite('to205_14', 3)
    Unknown23054('to205_13', 3)
    GFX_0('Zanzoh5D_2', 100)
    RefreshMultihit()
    Unknown1019(0)
    sprite('to205_15', 3)
    DisableAttackRestOfMove()
    sprite('to205_16', 3)
    SFX_3('cloth_l')
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    SFX_3('cloth_l')
    sprite('to205_15', 3)
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    SFX_3('cloth_l')
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    SFX_3('cloth_l')
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    SFX_3('cloth_l')
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    SFX_3('cloth_l')
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    SFX_3('cloth_l')
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    SFX_3('cloth_l')
    sprite('to205_16', 3)
    sprite('to205_14', 3)
    sprite('to205_15', 3)
    SFX_3('cloth_l')
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaD_Hasei_CA():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -50000, 50000, -50000, 1400000, 0, 50000)
        callSubroutine('PCE_AttackInit')
        Unknown2009()
        Unknown9016(1)
        Unknown23078(1)
        Unknown4007(3)
        Unknown2053(0)
        EnableCollision(0)
        callSubroutine('PCE_CheckWarp')
        Unknown23022(1)
        Unknown2003(1)
    sprite('to206_00', 3)
    sprite('to206_04', 3)
    GFX_0('Zanzoh5DD_2', 100)
    Unknown23015(0)
    Unknown2019(500)
    sprite('to206_05', 2)
    sprite('to206_06', 2)
    sprite('to206_07', 2)
    GFX_0('Zanzoh5DD_3', 100)
    SFX_3('slash_pole_middle')
    sprite('to206_09', 3)
    RefreshMultihit()
    sprite('to206_10', 3)
    DisableAttackRestOfMove()
    sprite('to206_11', 3)
    SFX_3('cloth_l')
    sprite('to206_09', 3)
    sprite('to206_10', 3)
    sprite('to206_11', 3)
    sprite('to206_09', 3)
    SFX_3('cloth_l')
    sprite('to206_10', 3)
    sprite('to206_11', 3)
    sprite('to206_09', 3)
    sprite('to206_10', 3)
    SFX_3('cloth_l')
    sprite('to206_11', 3)
    sprite('to206_09', 3)
    sprite('to206_10', 3)
    SFX_3('cloth_l')
    sprite('to206_11', 3)
    sprite('to206_09', 3)
    sprite('to206_10', 3)
    SFX_3('cloth_l')
    sprite('to206_11', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaAir5C():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -50000, 50000, 0, 300000, 0, 0)
        callSubroutine('PCE_AttackInit')
        AttackLevel_(3)
        AirPushbackY(24000)
        AirUntechableTime(19)
        AttackP1(80)
        HitOverhead(2)
        Unknown9016(1)
        AttackAttributes(1, 0, 0, 0, 0)
        callSubroutine('ChargeDamageUp')
        Unknown23078(1)
        Unknown11056(3)
        callSubroutine('PCE_CheckWarp')
        Unknown4007(3)

        def upon_43():
            if (SLOT_48 == 3001):
                sendToLabel(580)
            if (SLOT_48 == 3002):
                sendToLabel(580)
    sprite('to254_00', 1)
    physicsXImpulse(25000)
    physicsYImpulse(0)
    sprite('to254_00', 1)
    Unknown1019(80)
    YAccel(80)
    sprite('to254_00', 1)
    Unknown1019(80)
    YAccel(80)
    sprite('to254_00', 1)
    Unknown1019(80)
    YAccel(80)
    sprite('to254_01', 1)
    Unknown1019(80)
    YAccel(80)
    GFX_0('ZanzohAir5C', 100)
    sprite('to254_01', 1)
    Unknown1019(80)
    YAccel(80)
    sprite('to254_01', 1)
    Unknown1019(80)
    YAccel(80)
    SFX_3('slash_pole_middle')
    sprite('to254_03', 1)
    Unknown23054('to254_02', 3)
    RefreshMultihit()
    Unknown1019(0)
    YAccel(0)
    sprite('to254_03', 5)
    Unknown23022(0)
    sprite('to254_04', 3)
    DisableAttackRestOfMove()
    Unknown4007(0)
    sprite('to254_05', 3)
    sprite('to254_04', 3)
    SFX_3('cloth_l')
    sprite('to254_05', 3)
    sprite('to254_04', 3)
    sprite('to254_05', 3)
    label(580)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaAir4D():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -50000, 20000, 0, 300000, 0, 0)
        callSubroutine('PCE_AttackInit')
        AttackLevel_(3)
        Damage(300)
        AirUntechableTime(25)
        AttackP1(80)
        AttackP2(70)
        AirPushbackY(4000)
        PushbackX(22000)
        Hitstop(0)
        Unknown9016(1)
        AttackAttributes(1, 0, 0, 0, 0)
        Unknown11092(1)
        Unknown30055(150000, 100, 100)
        Unknown30056(50000, 100, 100)
        callSubroutine('ChargeDamageUp')
        Unknown4007(3)
        Unknown2053(1)
        callSubroutine('Init_CantCancelPersonaAction')
        callSubroutine('PCE_CheckWarp')
        Unknown23059(1)
    sprite('to255_00', 4)
    sprite('to255_01', 4)
    Unknown4007(0)
    physicsXImpulse(15000)
    physicsYImpulse(-14000)
    sprite('to255_02', 3)
    Unknown1019(80)
    YAccel(80)
    sprite('to255_03', 3)
    Unknown1019(80)
    YAccel(80)
    sprite('to255_04', 3)
    Unknown1019(80)
    YAccel(80)
    sprite('to255_05', 2)
    RefreshMultihit()
    Unknown1019(80)
    YAccel(80)
    SFX_3('slash_pole_fast')
    sprite('to255_06', 2)
    RefreshMultihit()
    Unknown1019(80)
    YAccel(80)
    GFX_0('ZanzohAir5D', 100)
    sprite('to255_07', 2)
    RefreshMultihit()
    Unknown1019(80)
    YAccel(80)
    SFX_3('slash_pole_fast')
    sprite('to255_05', 2)
    RefreshMultihit()
    Unknown1019(80)
    YAccel(80)
    sprite('to255_06', 2)
    RefreshMultihit()
    Unknown1019(80)
    YAccel(80)
    SFX_3('slash_pole_fast')
    sprite('to255_07', 2)
    RefreshMultihit()
    Unknown1019(80)
    YAccel(80)
    sprite('to255_08', 3)
    Unknown1019(0)
    YAccel(0)
    GFX_0('ZanzohAir5D_2', 100)
    sprite('to255_09', 2)
    sprite('to255_10', 2)
    sprite('to255_11', 2)
    SFX_3('slash_pole_middle')
    sprite('to255_12', 1)
    sprite('to255_13', 1)
    GFX_0('ZanzohAir5D_3', 100)
    sprite('to255_14', 1)
    sprite('to255_15', 1)
    sprite('to255_17', 3)
    Unknown23054('to255_16', 2)
    Damage(1500)
    Unknown11092(0)
    AirPushbackY(-38000)
    Hitstop(12)
    callSubroutine('ChargeDamageUp')
    Unknown30055(0, 0, 0)
    Unknown30056(0, 0, 0)
    RefreshMultihit()
    SLOT_10 = 0
    sprite('to255_18', 3)
    DisableAttackRestOfMove()
    sprite('to255_19', 3)
    Unknown2001()
    sprite('to255_17', 3)
    sprite('to255_18', 3)
    SFX_3('cloth_l')
    sprite('to255_19', 3)
    sprite('to255_17', 3)
    sprite('to255_18', 3)
    SFX_3('cloth_l')
    sprite('to255_19', 3)
    sprite('to255_17', 3)
    sprite('to255_18', 3)
    SFX_3('cloth_l')
    sprite('to255_19', 3)
    sprite('to255_17', 3)
    sprite('to255_18', 3)
    SFX_3('cloth_l')
    sprite('to255_19', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaDragonKickA():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -50000, 0, 0, 1000, 0, 0)
        callSubroutine('PCE_SPAttackInit')
        callSubroutine('PCE_CheckWarp')
    sprite('to404_00', 1)
    Unknown23022(1)
    sprite('to404_01', 2)
    sprite('to404_02', 2)
    sprite('to404_03', 2)
    sprite('to404_04', 2)
    sprite('to404_05', 2)
    sprite('to404_06', 2)
    GFX_0('Zanzoh_DragonKick', 100)
    sprite('to404_07', 2)
    GFX_0('Zanzoh_DragonKick_2', 100)
    sprite('to404_08', 2)
    sprite('to404_09', 3)
    Unknown23022(0)
    sprite('to404_10', 3)
    sprite('to404_11', 3)
    sprite('to404_09', 3)
    sprite('to404_10', 3)
    sprite('to404_11', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaDragonKickB():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -50000, 0, 0, 1000, 0, 0)
        callSubroutine('PCE_SPAttackInit')
        callSubroutine('PCE_CheckWarp')
    sprite('to404_00', 1)
    Unknown23022(1)
    sprite('to404_01', 1)
    sprite('to404_02', 1)
    sprite('to404_03', 2)
    sprite('to404_04', 2)
    sprite('to404_05', 2)
    sprite('to404_06', 3)
    GFX_0('Zanzoh_DragonKick', 100)
    sprite('to404_07', 2)
    GFX_0('Zanzoh_DragonKick_2', 100)
    sprite('to404_08', 2)
    sprite('to404_09', 3)
    Unknown23022(0)
    sprite('to404_10', 3)
    sprite('to404_11', 3)
    sprite('to404_09', 3)
    sprite('to404_10', 3)
    sprite('to404_11', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaDragonKickEx():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -50000, 0, 0, 1000, 0, 0)
        callSubroutine('PCE_SPAttackInit')
        callSubroutine('PCE_CheckWarp')
    sprite('to404_00', 1)
    Unknown23022(1)
    sprite('to404_01', 1)
    sprite('to404_02', 1)
    sprite('to404_03', 2)
    sprite('to404_04', 2)
    sprite('to404_05', 2)
    sprite('to404_06', 3)
    GFX_0('Zanzoh_DragonKick', 100)
    sprite('to404_07', 2)
    GFX_0('Zanzoh_DragonKick_2', 100)
    sprite('to404_08', 2)
    sprite('to404_09', 3)
    Unknown23022(0)
    sprite('to404_10', 3)
    sprite('to404_11', 3)
    sprite('to404_09', 3)
    sprite('to404_10', 3)
    sprite('to404_11', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaAirDragonKickA():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 100000, 0, 1000, 0, 0)
        callSubroutine('PCE_SPAttackInit')
        callSubroutine('PCE_CheckWarp')
    sprite('to404_04', 5)
    Unknown23022(1)
    sprite('to404_05', 2)
    sprite('to404_06', 2)
    GFX_0('Zanzoh_DragonKick', 100)
    sprite('to404_07', 2)
    GFX_0('Zanzoh_DragonKick_2', 100)
    sprite('to404_08', 2)
    sprite('to404_09', 3)
    sprite('to404_10', 3)
    sprite('to404_11', 3)
    sprite('to404_09', 3)
    sprite('to404_10', 3)
    sprite('to404_11', 3)
    sprite('to404_09', 3)
    sprite('to404_10', 3)
    sprite('to404_11', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaAirDragonKickB():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 100000, 0, 1000, 0, 0)
        callSubroutine('PCE_SPAttackInit')
        callSubroutine('PCE_CheckWarp')
    sprite('to404_04', 3)
    Unknown23022(1)
    sprite('to404_05', 2)
    sprite('to404_06', 2)
    GFX_0('Zanzoh_DragonKick', 100)
    sprite('to404_07', 2)
    GFX_0('Zanzoh_DragonKick_2', 100)
    sprite('to404_08', 2)
    sprite('to404_09', 3)
    sprite('to404_10', 3)
    sprite('to404_11', 3)
    sprite('to404_09', 3)
    sprite('to404_10', 3)
    sprite('to404_11', 3)
    sprite('to404_09', 3)
    sprite('to404_10', 3)
    sprite('to404_11', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaAirDragonKickEx():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 100000, 0, 1000, 0, 0)
        callSubroutine('PCE_SPAttackInit')
        callSubroutine('PCE_CheckWarp')
    sprite('to404_04', 1)
    Unknown23022(1)
    sprite('to404_05', 2)
    sprite('to404_06', 3)
    GFX_0('Zanzoh_DragonKick', 100)
    sprite('to404_07', 2)
    GFX_0('Zanzoh_DragonKick_2', 100)
    sprite('to404_08', 2)
    sprite('to404_09', 3)
    sprite('to404_10', 3)
    sprite('to404_11', 3)
    sprite('to404_09', 3)
    sprite('to404_10', 3)
    sprite('to404_11', 3)
    sprite('to404_09', 3)
    sprite('to404_10', 3)
    sprite('to404_11', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaKokutengekiA():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 220000, 60000, 0, 2000000, 0, 0)
        callSubroutine('PCE_SPAttackInit')
        Unknown2009()
        AttackLevel_(4)
        Damage(2040)
        AttackP1(100)
        AirUntechableTime(35)
        PushbackX(20000)
        AirPushbackX(24000)
        AirPushbackY(24000)
        YImpluseBeforeWallbounce(2700)
        AirHitstunAnimation(9)
        GroundedHitstunAnimation(9)
        Unknown11044(1)
        Unknown30088(1)
        callSubroutine('ChargeDamageUp')

        def upon_ON_HIT_OR_BLOCK():
            clearUponHandler(10)
            callSubroutine('Charge_Count')
        Unknown23066(1)
        Unknown2053(1)
        EnableCollision(0)
        callSubroutine('PCE_CheckWarp')
        Unknown23059(1)
    sprite('to405_00', 2)
    Unknown23022(1)
    GFX_0('KokutengekiTameC', 0)
    sprite('to405_01', 2)
    sprite('to405_02', 4)
    sprite('to405_03', 4)
    RefreshMultihit()
    ProjectileDurabilityLvl(1)
    Unknown11034(0)
    sprite('to405_04', 4)
    sprite('to405_05', 4)
    SFX_3('cloth_l')
    sprite('to405_03', 4)
    Unknown23022(0)
    DisableAttackRestOfMove()
    sprite('to405_04', 4)
    sprite('to405_05', 4)
    SFX_3('cloth_l')
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaKokutengekiB():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 220000, 60000, 0, 2000000, 0, 0)
        callSubroutine('PCE_SPAttackInit')
        Unknown2010()
        AttackLevel_(5)
        Damage(1200)
        PushbackX(35000)
        AirPushbackX(30000)
        AirPushbackY(35000)
        AirHitstunAnimation(9)
        GroundedHitstunAnimation(9)
        FatalCounter(1)
        Unknown9072(70000)
        callSubroutine('ChargeDamageUp')

        def upon_ON_HIT_OR_BLOCK():
            clearUponHandler(10)
            callSubroutine('Charge_Count')
        Unknown23066(1)
        Unknown2053(1)
        Unknown2015(200)
        callSubroutine('PCE_CheckWarp')

        def upon_43():
            if (SLOT_48 == 4001):
                Unknown9348(0)
                blockstun(31)
            if (SLOT_48 == 4003):
                sendToLabel(0)
    sprite('to405_00', 9)
    Unknown23022(1)
    GFX_0('KokutengekiTameD', 0)
    label(0)
    sprite('to405_00', 4)
    gotoLabel(0)
    label(1)
    sprite('to405_01', 2)
    sprite('to405_02', 2)
    sprite('to405_03', 4)
    Unknown21015('KokutengekiTameD', 4002, 0)
    RefreshMultihit()
    ProjectileDurabilityLvl(1)
    Unknown11034(0)
    sprite('to405_04', 4)
    sprite('to405_05', 4)
    SFX_3('cloth_l')
    sprite('to405_03', 4)
    Unknown23022(0)
    DisableAttackRestOfMove()
    sprite('to405_04', 4)
    sprite('to405_05', 4)
    SFX_3('cloth_l')
    sprite('to405_03', 4)
    Unknown2001()
    sprite('to405_04', 4)
    sprite('to405_05', 4)
    SFX_3('cloth_l')
    sprite('to405_03', 4)
    sprite('to405_04', 4)
    sprite('to405_05', 4)
    SFX_3('cloth_l')
    sprite('to405_03', 4)
    sprite('to405_04', 4)
    sprite('to405_05', 4)
    SFX_3('cloth_l')
    sprite('to405_03', 4)
    sprite('to405_04', 4)
    sprite('to405_05', 4)
    SFX_3('cloth_l')
    sprite('to405_03', 4)
    sprite('to405_04', 4)
    sprite('to405_05', 4)
    SFX_3('cloth_l')
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaKokutengekiEx():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 220000, 60000, 0, 2000000, 0, 0)
        callSubroutine('PCE_SPAttackInit')
        Unknown2009()
        AttackLevel_(5)
        Damage(2500)
        AttackP1(70)
        blockstun(14)
        AirUntechableTime(45)
        AirUntechableTime(35)
        PushbackX(20000)
        AirPushbackX(24000)
        AirPushbackY(20000)
        GroundedHitstunAnimation(9)
        AirHitstunAnimation(9)
        CounterHitAirPushbackY(30000)
        Unknown9072(60000)
        Unknown9180(1)
        Unknown30065(0)
        MinimumDamagePct(10)
        callSubroutine('ChargeDamageUp')

        def upon_ON_HIT_OR_BLOCK():
            clearUponHandler(10)
            callSubroutine('Charge_Count')
        Unknown23066(1)
        Unknown2053(1)
        EnableCollision(0)
        callSubroutine('PCE_CheckWarp')
        Unknown23059(1)
    sprite('to405_00', 6)
    Unknown23022(1)
    GFX_0('KokutengekiTameC', 0)
    sprite('to405_01', 5)
    sprite('to405_02', 5)
    sprite('to405_03', 5)
    RefreshMultihit()
    ProjectileDurabilityLvl(1)
    Unknown11034(0)
    sprite('to405_04', 4)
    sprite('to405_05', 4)
    SFX_3('cloth_l')
    sprite('to405_03', 4)
    Unknown23022(0)
    DisableAttackRestOfMove()
    sprite('to405_04', 4)
    sprite('to405_05', 4)
    SFX_3('cloth_l')
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaKokutengeki_PS():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 220000, 60000, 0, 2000000, 0, 0)
        callSubroutine('PCE_SPAttackInit')
        Unknown2010()
        AttackLevel_(4)
        AttackP1(70)
        AttackP2(90)
        AirUntechableTime(35)
        PushbackX(20000)
        AirPushbackX(10000)
        AirPushbackY(24000)
        AirHitstunAnimation(9)
        GroundedHitstunAnimation(9)
        Unknown11042(1)
        callSubroutine('ChargeDamageUp')

        def upon_ON_HIT_OR_BLOCK():
            clearUponHandler(10)
            callSubroutine('Charge_Count')
        Unknown23066(1)
        Unknown2053(1)
        EnableCollision(0)
        callSubroutine('PCE_CheckWarp')
        Unknown23059(1)
    sprite('to405_00', 2)
    Unknown23022(1)
    teleportRelativeX(-75000)
    GFX_0('KokutengekiTameC', 0)
    Unknown36(1)
    teleportRelativeX(-20000)
    Unknown35()
    sprite('to405_01', 2)
    sprite('to405_02', 4)
    sprite('to405_03', 4)
    RefreshMultihit()
    ProjectileDurabilityLvl(1)
    Unknown11034(0)
    sprite('to405_04', 4)
    sprite('to405_05', 4)
    SFX_3('cloth_l')
    sprite('to405_03', 4)
    Unknown23022(0)
    DisableAttackRestOfMove()
    sprite('to405_04', 4)
    sprite('to405_05', 4)
    SFX_3('cloth_l')
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaGodHand():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 210000, 150000, 0, 0, 0, 0)
        callSubroutine('PCE_DDAttackInit')
        Unknown2011()
        AttackLevel_(5)
        Damage(6000)
        AttackP2(60)
        MinimumDamagePct(32)
        Hitstop(0)
        Unknown11001(13, 13, 13)
        AirPushbackY(-500000)
        AirPushbackX(1000)
        YImpluseBeforeWallbounce(0)
        AirHitstunAnimation(11)
        GroundedHitstunAnimation(11)
        Unknown9310(40)
        Unknown23022(1)
        callSubroutine('ChargeDamageUp')
        Unknown4009(3)
        Unknown23066(1)
        Unknown2053(1)

        def upon_56():
            Unknown4009(0)
            Unknown2054(0)
        callSubroutine('PCE_ForceWarp')
        Unknown23059(1)
    sprite('to430_00', 3)
    GFX_0('GodHandKobusiKumo', 0)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_03', 3)
    sprite('to430_04', 3)
    SFX_3('bomb_l')
    sprite('to430_05', 6)
    GFX_0('GodHandKobusi', 0)
    GFX_0('GodHandKobusiWind', 1)
    GFX_1('toef_godhandtukene_02', 2)
    ScreenShake(0, 30000)
    RefreshMultihit()
    if SLOT_31:
        pass
    sprite('to430_06', 3)
    DisableAttackRestOfMove()
    sprite('to430_07', 3)
    sprite('to430_05', 3)
    sprite('to430_06', 3)
    sprite('to430_07', 3)
    sprite('to430_05', 3)
    sprite('to430_06', 3)
    sprite('to430_07', 3)
    sprite('to430_05', 3)
    sprite('to430_06', 3)
    sprite('to430_07', 3)
    sprite('to430_05', 3)
    sprite('to430_06', 3)
    sprite('to430_07', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaGodHandOD():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 210000, 150000, 0, 0, 0, 0)
        callSubroutine('PCE_DDAttackInit')
        Unknown2011()
        AttackLevel_(5)
        Damage(5100)
        MinimumDamagePct(22)
        AttackP2(60)
        Unknown11092(1)
        Hitstop(0)
        Unknown11001(13, 13, 13)
        AirPushbackY(-500000)
        AirPushbackX(15000)
        YImpluseBeforeWallbounce(0)
        AirHitstunAnimation(11)
        GroundedHitstunAnimation(11)
        Unknown9310(40)
        Unknown23022(1)
        Unknown11068(1)
        callSubroutine('ChargeDamageUp')
        Unknown4009(3)
        Unknown23066(1)
        Unknown2053(1)

        def upon_56():
            Unknown4009(0)
            Unknown2054(0)
        callSubroutine('PCE_ForceWarp')
        Unknown23059(1)

        def upon_12():
            SLOT_51 = 1
    sprite('to430_00', 3)
    GFX_0('GodHandKobusiKumo', 0)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_03', 3)
    sprite('to430_04', 3)
    SFX_3('bomb_l')
    sprite('to430_05', 6)
    GFX_0('GodHandKobusi', 0)
    GFX_0('GodHandKobusiWind', 1)
    GFX_1('toef_godhandtukene_02', 2)
    ScreenShake(0, 30000)
    RefreshMultihit()
    if SLOT_31:
        pass
    sprite('to430_06', 3)
    sprite('to430_00', 3)
    Unknown23184(3, 100, 430000, 150000, 0, 0, 0, 0)
    callSubroutine('PCE_ForceWarp')
    GFX_0('GodHandKobusiKumo', 0)
    if SLOT_51:
        Damage(5000)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_03', 3)
    sprite('to430_04', 3)
    SFX_3('bomb_l')
    sprite('to430_05', 6)
    GFX_0('GodHandKobusi', 0)
    GFX_0('GodHandKobusiWind', 1)
    GFX_1('toef_godhandtukene_02', 2)
    RefreshMultihit()
    ScreenShake(0, 30000)
    sprite('to430_06', 3)
    DisableAttackRestOfMove()
    sprite('to430_07', 3)
    sprite('to430_05', 3)
    sprite('to430_06', 3)
    sprite('to430_07', 3)
    sprite('to430_05', 3)
    sprite('to430_06', 3)
    sprite('to430_07', 3)
    sprite('to430_05', 3)
    sprite('to430_06', 3)
    sprite('to430_07', 3)
    sprite('to430_05', 3)
    sprite('to430_06', 3)
    sprite('to430_07', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaAguneyasutora_Far():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 50000, 0, 0, 0, -1000000, 1000000)
        callSubroutine('PCE_DDAttackInit')
        Unknown4009(3)
        Unknown23066(1)
        Unknown23022(1)
        callSubroutine('PCE_CheckWarp')
    sprite('to432_00', 2)
    GFX_0('meteoStartAura', 0)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_03', 3)
    sprite('to432_04', 3)
    sprite('to432_05', 3)
    GFX_0('meteo_takusanD', 0)
    ScreenShake(0, 10000)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaAguneyasutoraOD_Far():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 50000, 0, 0, 0, -1000000, 1000000)
        callSubroutine('PCE_DDAttackInit')
        Unknown4009(3)
        Unknown23066(1)
        Unknown23022(1)
        callSubroutine('PCE_CheckWarp')
    sprite('to432_00', 2)
    GFX_0('meteoStartAura', 0)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_03', 3)
    sprite('to432_04', 3)
    sprite('to432_05', 3)
    GFX_0('meteo_takusanD_OD', 0)
    ScreenShake(0, 10000)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaAguneyasutora_Near():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 50000, 0, 0, 0, -1000000, 1000000)
        callSubroutine('PCE_DDAttackInit')
        Unknown4009(3)
        Unknown23066(1)
        Unknown23022(1)
        callSubroutine('PCE_CheckWarp')
    sprite('to432_00', 2)
    GFX_0('meteoStartAura', 0)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_03', 3)
    sprite('to432_04', 3)
    sprite('to432_05', 3)
    GFX_0('meteo_takusanC', 0)
    ScreenShake(0, 10000)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaAguneyasutoraOD_Near():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 50000, 0, 0, 0, -1000000, 1000000)
        callSubroutine('PCE_DDAttackInit')
        Unknown4009(3)
        Unknown23066(1)
        Unknown23022(1)
        callSubroutine('PCE_CheckWarp')
    sprite('to432_00', 2)
    GFX_0('meteoStartAura', 0)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_03', 3)
    sprite('to432_04', 3)
    sprite('to432_05', 3)
    GFX_0('meteo_takusanC_OD', 0)
    ScreenShake(0, 10000)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaAguneyasutoraOD():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 50000, 0, 0, 2000000, -1000000, 1000000)
        callSubroutine('PCE_DDAttackInit')
        Unknown4009(3)
        Unknown23066(1)
        Unknown23022(1)
        callSubroutine('PCE_CheckWarp')
    sprite('to432_00', 2)
    GFX_0('meteoStartAura', 0)
    Unknown2006()
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_02', 2)
    sprite('to432_00', 2)
    sprite('to432_01', 2)
    sprite('to432_03', 3)
    sprite('to432_04', 3)
    sprite('to432_05', 3)
    GFX_0('meteo_takusanCD', 0)
    ScreenShake(0, 10000)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('to432_06', 2)
    sprite('to432_07', 2)
    sprite('to432_08', 2)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaGodHand_DUO():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 210000, 150000, 0, 0, 0, 0)
        callSubroutine('PCE_DDAttackInit')
        Unknown2011()
        AttackLevel_(5)
        Damage(2000)
        AttackP1(100)
        AttackP2(100)
        MinimumDamagePct(100)
        Hitstop(0)
        Unknown11001(13, 13, 13)
        AirPushbackY(-500000)
        AirPushbackX(1000)
        YImpluseBeforeWallbounce(0)
        AirHitstunAnimation(11)
        GroundedHitstunAnimation(11)
        Unknown9310(40)
        Unknown23022(1)
        Unknown4009(3)
        Unknown23066(1)
        Unknown2053(1)

        def upon_56():
            Unknown4009(0)
            Unknown2054(0)
        callSubroutine('PCE_ForceWarp')
        Unknown23059(1)
    sprite('to430_00', 3)
    GFX_0('GodHandKobusiKumo', 0)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_03', 3)
    sprite('to430_04', 3)
    SFX_3('bomb_l')
    sprite('to430_05', 6)
    GFX_0('GodHandKobusi', 0)
    GFX_0('GodHandKobusiWind', 1)
    GFX_1('toef_godhandtukene_02', 2)
    ScreenShake(0, 30000)
    RefreshMultihit()
    sprite('to430_06', 3)
    DisableAttackRestOfMove()
    sprite('to430_07', 3)
    sprite('to430_05', 3)
    sprite('to430_06', 3)
    sprite('to430_07', 3)
    sprite('to430_05', 3)
    sprite('to430_06', 3)
    sprite('to430_07', 3)
    sprite('to430_05', 3)
    sprite('to430_06', 3)
    sprite('to430_07', 3)
    sprite('to430_05', 3)
    sprite('to430_06', 3)
    sprite('to430_07', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PCE_PersonaGodHand_DUOOD():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 210000, 150000, 0, 0, 0, 0)
        callSubroutine('PCE_DDAttackInit')
        Unknown2011()
        AttackLevel_(5)
        Damage(1000)
        MinimumDamagePct(100)
        AttackP1(100)
        AttackP2(100)
        Hitstop(0)
        Unknown11001(13, 13, 13)
        AirPushbackY(-500000)
        AirPushbackX(15000)
        YImpluseBeforeWallbounce(0)
        AirHitstunAnimation(11)
        GroundedHitstunAnimation(11)
        Unknown9310(40)
        Unknown23022(1)
        Unknown11068(1)
        Unknown4009(3)
        Unknown23066(1)
        Unknown2053(1)

        def upon_56():
            Unknown4009(0)
            Unknown2054(0)
        callSubroutine('PCE_ForceWarp')
        Unknown23059(1)
    sprite('to430_00', 3)
    GFX_0('GodHandKobusiKumo', 0)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_03', 3)
    sprite('to430_04', 3)
    SFX_3('bomb_l')
    sprite('to430_05', 6)
    GFX_0('GodHandKobusi', 0)
    GFX_0('GodHandKobusiWind', 1)
    GFX_1('toef_godhandtukene_02', 2)
    ScreenShake(0, 30000)
    RefreshMultihit()
    sprite('to430_06', 3)
    sprite('to430_00', 3)
    Unknown23184(3, 100, 430000, 150000, 0, 0, 0, 0)
    callSubroutine('PCE_ForceWarp')
    GFX_0('GodHandKobusiKumo', 0)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_00', 3)
    sprite('to430_01', 3)
    sprite('to430_02', 3)
    sprite('to430_03', 3)
    sprite('to430_04', 3)
    SFX_3('bomb_l')
    sprite('to430_05', 6)
    GFX_0('GodHandKobusi', 0)
    GFX_0('GodHandKobusiWind', 1)
    GFX_1('toef_godhandtukene_02', 2)
    RefreshMultihit()
    Damage(1500)
    ScreenShake(0, 30000)
    sprite('to430_06', 3)
    DisableAttackRestOfMove()
    sprite('to430_07', 3)
    sprite('to430_05', 3)
    sprite('to430_06', 3)
    sprite('to430_07', 3)
    sprite('to430_05', 3)
    sprite('to430_06', 3)
    sprite('to430_07', 3)
    sprite('to430_05', 3)
    sprite('to430_06', 3)
    sprite('to430_07', 3)
    sprite('to430_05', 3)
    sprite('to430_06', 3)
    sprite('to430_07', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def Zanzoh5C():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4061(1)
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
        Unknown3004(-25)
    sprite('vr_to204_00', 2)
    sprite('vr_to204_01', 2)
    sprite('vr_to204_02', 12)

@State
def Zanzoh2C():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4061(1)
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
        Unknown3004(-23)
    sprite('vr_to234_05', 12)

@State
def __2C_ice():

    def upon_IMMEDIATE():
        Unknown4010(28)
        Unknown4009(2)
        Unknown4007(2)
        Unknown4003('toef_234_ice00.DIG', '')
        Unknown4015()
        Unknown3033()
        Unknown1007(-80000)
    sprite('null', 2)
    SFX_3('ice_fast')
    GFX_1('toef_C2_ice', 100)
    sprite('null', 1)
    SFX_3('ice_fast')
    GFX_0('2C_ice_b', 100)
    sprite('null', 11)
    SFX_3('ice_fast')
    GFX_0('2C_ice_c', 100)
    sprite('null', 10)
    Unknown3004(-25)

@State
def __2C_ice_b():

    def upon_IMMEDIATE():
        Unknown4010(28)
        Unknown4009(2)
        Unknown4007(2)
        Unknown4003('toef_234_ice01.DIG', '')
        Unknown4015()
        Unknown3033()
        Unknown23015(14)
        Unknown1007(8000)
        teleportRelativeX(135000)
    sprite('null', 12)
    GFX_1('toef_C2_ice', 100)
    sprite('null', 10)
    Unknown3004(-25)

@State
def __2C_ice_c():

    def upon_IMMEDIATE():
        Unknown4010(28)
        Unknown4009(2)
        Unknown4007(2)
        Unknown4003('toef_234_ice02.DIG', '')
        Unknown4015()
        Unknown3033()
        Unknown23015(14)
        Unknown1007(24000)
        teleportRelativeX(260000)
    sprite('null', 11)
    GFX_1('toef_C2_ice', 100)
    sprite('null', 10)
    Unknown3004(-25)

@State
def Zanzoh5D():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4061(3)
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
    sprite('vr_to205_00', 3)
    sprite('vr_to205_01', 3)
    sprite('vr_to205_02', 3)
    sprite('vr_to205_03', 2)
    sprite('vr_to205_04', 2)
    sprite('vr_to205_05', 2)
    sprite('vr_to205_06', 3)
    sprite('vr_to205_07', 2)
    sprite('vr_to205_08', 2)
    sprite('vr_to205_03', 2)
    sprite('vr_to205_04', 2)
    sprite('vr_to205_05', 2)
    sprite('vr_to205_06', 3)
    sprite('vr_to205_09', 3)
    sprite('vr_to205_10', 3)
    sprite('vr_to205_11', 2)

@State
def Zanzoh5D_Short():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4061(3)
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
    sprite('vr_to205_00', 2)
    sprite('vr_to205_01', 2)
    sprite('vr_to205_02', 2)
    sprite('vr_to205_03', 2)
    sprite('vr_to205_04', 2)
    sprite('vr_to205_05', 2)
    sprite('vr_to205_06', 3)
    sprite('vr_to205_07', 2)
    sprite('vr_to205_08', 2)
    sprite('vr_to205_09', 3)
    sprite('vr_to205_10', 3)
    sprite('vr_to205_11', 2)

@State
def Zanzoh5D_2():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4061(1)
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
        Unknown3004(-25)
    sprite('vr_to205_12', 12)

@State
def Zanzoh5DD():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4061(3)
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
    sprite('vr_to206_00', 2)
    Unknown1072(50000)
    sprite('vr_to206_00', 2)
    Unknown1072(0)
    sprite('vr_to206_00', 2)
    Unknown1072(-60000)

@State
def Zanzoh5DD_2():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4061(3)
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
    sprite('vr_to206_10', 3)
    sprite('vr_to206_11', 2)
    sprite('vr_to206_12', 2)

@State
def Zanzoh5DD_3():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4061(1)
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
        Unknown3004(-25)
    sprite('vr_to206_13', 12)

@State
def ZanzohAir5C():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4061(1)
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
        Unknown3004(-25)
    sprite('vr_to254_00', 3)
    sprite('vr_to254_01', 12)

@State
def ZanzohAir5D():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4061(3)
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
    sprite('vr_to255_01', 2)
    sprite('vr_to255_02', 2)
    sprite('vr_to255_00', 2)
    sprite('vr_to255_01', 2)
    sprite('vr_to255_02', 2)

@State
def ZanzohAir5D_2():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4061(3)
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
    sprite('vr_to255_10', 3)
    sprite('vr_to255_11', 2)
    sprite('vr_to255_12', 2)
    sprite('vr_to255_13', 2)
    sprite('vr_to255_14', 1)

@State
def ZanzohAir5D_3():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4061(1)
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
        Unknown3004(-25)
    sprite('vr_to255_15', 12)

@State
def Hyper_Counter_Barrier():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown4010(2)
        Unknown4003('ceef_400_barrier.DIG', '')
        Unknown4015()
        Unknown3033()
        Unknown1096(1250)
        Unknown3001(159)
    sprite('null', 16)
    Unknown1099(2)
    Unknown3004(-3)
    sprite('null', 22)
    Unknown1099(3)

@State
def Hyper_Counter_Barrier_r():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown4010(2)
        Unknown4003('ceef_400_barrier_r.DIG', '')
        Unknown4015()
        Unknown3033()
        Unknown1096(1250)
        Unknown3001(159)
    sprite('null', 16)
    Unknown1099(2)
    Unknown3004(-3)
    sprite('null', 22)
    Unknown1099(3)

@State
def Hyper_Counter_kick():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
        Unknown4061(2)
        Unknown3033()
        Unknown3004(-35)
    sprite('vr_ce401_00', 16)
    GFX_1('ceef_tornadokick', 0)
    GFX_1('ceef_tornadokick', 1)

@State
def Hyper_Counter_kick_oku():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
        Unknown4061(2)
        Unknown3033()
        teleportRelativeX(16000)
        Unknown1007(48000)
        Unknown3004(-35)
    sprite('vr_ce401_00', 16)
    GFX_1('ceef_tornadokick', 0)
    GFX_1('ceef_tornadokick', 1)

@State
def ceef_403_kick():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown4009(2)
        Unknown4010(2)
        Unknown4003('ceef_403_kick.DIG', '')
        Unknown4015()
        Unknown23015(1)
        Unknown4061(2)
        Unknown23122(240)
        Unknown1096(675)
        teleportRelativeX(-20000)
        Unknown1007(10000)
    sprite('null', 10)
    sprite('null', 4)
    Unknown3004(-64)

@State
def Nouten_stump():

    def upon_IMMEDIATE():
        Unknown4061(2)
        Unknown3033()
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
    sprite('vr_ce403_20', 3)
    sprite('vr_ce403_21', 3)

@State
def Nouten_stump_hit():

    def upon_IMMEDIATE():
        Unknown4061(2)
        Unknown3033()
        Unknown4007(0)
        Unknown4010(2)
        teleportRelativeX(-64000)
        Unknown3004(-20)
    sprite('vr_ce403_22', 16)

@State
def __100InchPanch_hand():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown4010(2)
        GFX_2('ceef_100inch_hand')
    sprite('null', 5)
    sprite('null', 5)
    sprite('null', 5)
    sprite('null', 5)
    sprite('null', 5)
    sprite('null', 5)
    sprite('null', 5)
    sprite('null', 5)

@State
def __100InchPanch_add():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown4010(2)
        teleportRelativeX(64000)
    sprite('null', 1)
    GFX_1('ceef_100inch_add', 100)

@State
def __100InchPanch_dash():

    def upon_IMMEDIATE():
        teleportRelativeX(100000)
        Unknown1007(220000)
    sprite('null', 2)
    GFX_1('ceef_100inch_dash', 100)

@State
def DragonKick_Launcher():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
        Unknown1007(-12000)
    sprite('null', 1)
    GFX_1('ceef_dragonkick_launcher_01', 100)

@State
def DragonKick():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4061(2)
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
        Unknown1072(-30000)
        Unknown1056(1500)
    sprite('vr_ce404_00', 2)
    GFX_1('ceef_dragonkick_00', 0)
    sprite('vr_ce404_01', 2)
    GFX_1('ceef_dragonkick_00', 0)
    sprite('vr_ce404_02', 2)
    GFX_1('ceef_dragonkick_00', 0)
    sprite('vr_ce404_03', 2)
    Unknown1099(-100)
    GFX_1('ceef_dragonkick_00', 0)
    sprite('vr_ce404_00', 2)
    GFX_1('ceef_dragonkick_00', 0)
    sprite('vr_ce404_01', 2)
    GFX_1('ceef_dragonkick_00', 0)
    sprite('vr_ce404_02', 2)
    sprite('vr_ce404_03', 2)

@State
def Zanzoh_DragonKick():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4061(3)
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
    sprite('vr_to404_00', 2)

@State
def Zanzoh_DragonKick_2():

    def upon_IMMEDIATE():
        Unknown3033()
        Unknown4061(1)
        Unknown4007(2)
        Unknown4010(2)
        Unknown4009(2)
        Unknown3004(-25)
    sprite('vr_to404_01', 12)
    GFX_1('toef_dragonkick_07', 0)

@State
def KokutengekiCEtameC():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4010(2)
        GFX_2('ceef_kokutengeki_02')
        Unknown4007(2)
    sprite('null', 5)
    sprite('null', 5)
    Unknown3004(-51)

@State
def KokutengekiCEtameEX():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4010(2)
        GFX_2('ceef_kokutengeki_02')
        Unknown4007(2)

        def upon_43():
            if (SLOT_48 == 4002):
                sendToLabel(0)
    sprite('null', 32767)
    label(0)
    sprite('null', 5)
    Unknown3004(-51)

@State
def KokutengekiTameC():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4010(2)
        Unknown3001(0)
    sprite('null', 8)
    Unknown3004(104)
    Unknown1099(-100)
    GFX_0('KokutengekiTameSphereC', 100)
    sprite('null', 3)
    Unknown3001(0)
    GFX_0('KokutengekiBomb', 100)

@State
def KokutengekiTameSphereC():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4010(2)
        Unknown4003('ceef_405_tame_sphere.DIG', '')
        Unknown3001(0)
    sprite('null', 8)
    Unknown3004(104)
    Unknown1099(-30)
    sprite('null', 3)
    Unknown3001(0)

@State
def KokutengekiTameLineC():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4010(2)
        Unknown4003('ceef_405_sen.DIG', '')
        Unknown3001(0)
    sprite('null', 5)
    Unknown3004(104)
    sprite('null', 8)
    Unknown3004(-68)

@State
def KokutengekiTameD():

    def upon_IMMEDIATE():
        GFX_2('toef_kokutenstart_03')
        Unknown4010(2)

        def upon_43():
            if (SLOT_48 == 4002):
                sendToLabel(1)
    sprite('null', 1)
    label(0)
    sprite('null', 17)
    GFX_0('KokutengekiTameSphereD', 100)
    GFX_0('KokutengekiTameLineD', 100)
    gotoLabel(0)
    label(1)
    sprite('null', 5)
    GFX_0('KokutengekiBomb', 100)

@State
def KokutengekiTameSphereD():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4010(2)
        Unknown4003('ceef_405_tame_sphere.DIG', '')
        Unknown3001(0)
    sprite('null', 17)
    Unknown3004(52)
    Unknown1099(-10)
    sprite('null', 5)
    Unknown3001(0)

@State
def KokutengekiTameLineD():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4010(2)
        Unknown4003('ceef_405_sen.DIG', '')
        Unknown3001(0)
    sprite('null', 7)
    Unknown3004(52)
    sprite('null', 15)
    Unknown3004(-34)

@State
def KokutengekiTameCD():

    def upon_IMMEDIATE():
        GFX_2('toef_kokutenstart_03')
        Unknown4010(2)
        Unknown4009(2)
    sprite('null', 12)
    GFX_0('KokutengekiTameSphereCD', 100)
    GFX_0('KokutengekiTameLineCD', 100)
    sprite('null', 3)
    Unknown3001(0)
    GFX_0('KokutengekiBomb', 100)

@State
def KokutengekiTameSphereCD():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4010(2)
        Unknown4003('ceef_405_tame_sphere.DIG', '')
        Unknown4009(2)
        Unknown3001(0)
    sprite('null', 12)
    Unknown3004(78)
    Unknown1099(-30)
    sprite('null', 3)
    Unknown3001(0)

@State
def KokutengekiTameLineCD():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4010(2)
        Unknown4003('ceef_405_sen.DIG', '')
        Unknown4009(2)
        Unknown3001(0)
        Unknown1096(1400)
    sprite('null', 9)
    Unknown1099(-60)
    Unknown3004(78)
    sprite('null', 5)
    Unknown3004(-51)

@State
def KokutengekiBomb():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('ceef_405_hajike00.DIG', '')
        Unknown23015(1)
    sprite('null', 1)
    Unknown1099(50)
    GFX_0('KokutengekiBombGroundshock', 100)
    GFX_1('toef_kokutenend_07', 100)
    SFX_3('bomb_l')
    sprite('null', 4)
    SFX_3('bomb_l')
    sprite('null', 5)
    Unknown3004(-52)
    GFX_0('KokutengekiBombCircle', 100)

@State
def KokutengekiBombCircle():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('ceef_405_hajike01.DIG', '')
        Unknown23015(1)
    sprite('null', 5)
    Unknown1099(10)
    sprite('null', 5)
    Unknown3004(-52)

@State
def KokutengekiBombGroundshock():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('ceef_405_groundshock.DIG', '')
        Unknown23015(4)
        Unknown1007(-350000)
    sprite('null', 10)
    GFX_1('toef_kokutengroundsmoke_00', 100)
    Unknown1059(80)
    Unknown1067(-10)
    Unknown3004(-26)

@State
def Chargestart():

    def upon_IMMEDIATE():
        GFX_2('ceef_startcharge_06')
        Unknown2054(1)
    sprite('null', 80)

@State
def ChargeAModel():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('ceef_431_charge00.DIG', '')
        Unknown2054(1)
        Unknown3001(0)
        Unknown1056(1400)
    sprite('null', 10)
    Unknown3004(26)
    sprite('null', 20)
    Unknown3004(-17)

@State
def ChargeBModel():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('ceef_431_charge00.DIG', '')
        GFX_2('ceef_jizokucharge_02')
        Unknown2054(1)
        Unknown3001(0)
    sprite('null', 10)
    Unknown3004(26)
    Unknown1059(30)
    sprite('null', 15)
    GFX_1('ceef_endcharge_01', 100)
    Unknown1059(0)
    sprite('null', 10)
    Unknown3004(-26)
    Unknown1059(30)

@State
def ChargeABModel():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('ceef_431_charge00.DIG', '')
        GFX_2('ceef_jizokucharge_02')
        Unknown2054(1)
        Unknown3001(0)
    sprite('null', 10)
    Unknown3004(26)
    Unknown1059(30)
    sprite('null', 23)
    GFX_1('ceef_endcharge_01', 100)
    Unknown1059(0)
    sprite('null', 10)
    Unknown3004(-26)
    Unknown1059(30)

@State
def GodHandKobusiKumo():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('ceef_430_godhand_kumo.DIG', 'ceef_430_godhand_kumo_0000.mmot')
        Unknown2054(1)
        Unknown3001(0)
    sprite('null', 5)
    physicsYImpulse(-2000)
    sprite('null', 15)
    Unknown1059(-7)
    Unknown3004(6)
    sprite('null', 45)
    sprite('null', 10)
    Unknown1059(30)
    Unknown3004(-26)
    GFX_0('GodHandKobusiKumoBrake', 100)

@State
def GodHandKobusiKumoBrake():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('ceef_430_godhand_kumobrake.DIG', '')
        Unknown2054(1)
    sprite('null', 15)
    Unknown1059(30)
    Unknown3004(-17)

@State
def GodHandKobusiKumoShock():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('ceef_430_godhand_kumoend.DIG', '')
        Unknown2054(1)
    sprite('null', 15)
    Unknown1059(30)
    Unknown3004(-17)

@State
def GodHandKobusi():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4061(4)
        Unknown2007()
        Unknown1007(100000)
    sprite('vr_to430_godhand', 4)
    physicsYImpulse(-70000)
    sprite('vr_to430_godhand', 1)
    physicsYImpulse(0)
    SFX_3('quake_l')
    sprite('vr_to430_godhand', 4)
    SFX_3('quake_l')
    sprite('vr_to430_godhand', 15)
    Unknown3004(-17)

@State
def GodHandKobusiCD():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4061(4)
        Unknown2007()
        Unknown1096(1200)
        Unknown1007(100000)
    sprite('vr_to430_godhand', 4)
    physicsYImpulse(-70000)
    sprite('vr_to430_godhand', 1)
    physicsYImpulse(0)
    SFX_3('quake_l')
    sprite('vr_to430_godhand', 4)
    SFX_3('quake_l')
    sprite('vr_to430_godhand', 15)
    Unknown3004(-17)

@State
def GodHandKobusiWind():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('ceef_430_godhand_wind.DIG', '')
    sprite('null', 30)
    Unknown3004(-9)
    Unknown1059(20)
    GFX_0('GodHandKobusiKumoShock', 100)

@State
def GodHandKobusiWindCD():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('ceef_430_godhand_wind.DIG', '')
        Unknown1096(1200)
    sprite('null', 30)
    Unknown3004(-9)
    Unknown1059(20)
    GFX_0('GodHandKobusiKumoShock', 100)

@State
def meteo_takusanC():

    def upon_IMMEDIATE():
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 0)
        sendToLabelUpon(54, 0)
        Unknown48(25, 2, 51, 3, 2, 59)
        if SLOT_51:
            pass
    label(1)
    sprite('null', 10)
    teleportRelativeX(-730000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    SFX_3('ce004')
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-130000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 10)
    teleportRelativeX(370000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-700000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    SFX_3('ce004')
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 10)
    teleportRelativeX(600000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-500000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 10)
    teleportRelativeX(400000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-300000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 10)
    teleportRelativeX(600000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-400000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 90)
    label(0)
    sprite('null', 1)

@State
def meteo_takusanC_OD():

    def upon_IMMEDIATE():
        Unknown23056()
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 0)
        sendToLabelUpon(54, 0)
        Unknown48(25, 2, 51, 3, 2, 59)
        if SLOT_51:
            pass
    label(1)
    sprite('null', 10)
    teleportRelativeX(-730000)
    Unknown23033(-55)
    GFX_0('meteo_col_OD', 100)
    SFX_3('ce004')
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-130000)
    Unknown23033(-55)
    GFX_0('meteo_col_OD', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 10)
    teleportRelativeX(370000)
    Unknown23033(-55)
    GFX_0('meteo_col_OD', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-700000)
    Unknown23033(-55)
    GFX_0('meteo_col_OD', 100)
    SFX_3('ce004')
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 10)
    teleportRelativeX(600000)
    Unknown23033(-55)
    GFX_0('meteo_col_OD', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-500000)
    Unknown23033(-55)
    GFX_0('meteo_col_OD', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 10)
    teleportRelativeX(400000)
    Unknown23033(-55)
    GFX_0('meteo_col_OD', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-300000)
    Unknown23033(-55)
    GFX_0('meteo_col_OD', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 10)
    teleportRelativeX(600000)
    Unknown23033(-55)
    GFX_0('meteo_col_OD', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-400000)
    Unknown23033(-55)
    GFX_0('meteo_col_OD', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 90)
    label(0)
    sprite('null', 1)

@State
def meteo_takusanD():

    def upon_IMMEDIATE():
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 0)
        sendToLabelUpon(54, 0)
        Unknown48(25, 2, 51, 3, 2, 59)
        if SLOT_51:
            pass
    label(1)
    sprite('null', 10)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    SFX_3('ce004')
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-100000)
    GFX_0('meteo_col', 100)
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 10)
    teleportRelativeX(400000)
    GFX_0('meteo_col', 100)
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-700000)
    GFX_0('meteo_col', 100)
    SFX_3('ce004')
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 10)
    teleportRelativeX(600000)
    GFX_0('meteo_col', 100)
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-600000)
    GFX_0('meteo_col', 100)
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 10)
    teleportRelativeX(350000)
    GFX_0('meteo_col', 100)
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-270000)
    GFX_0('meteo_col', 100)
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 10)
    teleportRelativeX(600000)
    GFX_0('meteo_col', 100)
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-400000)
    GFX_0('meteo_col', 100)
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 90)
    label(0)
    sprite('null', 1)

@State
def meteo_takusanD_OD():

    def upon_IMMEDIATE():
        Unknown23056()
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 0)
        sendToLabelUpon(54, 0)
        Unknown48(25, 2, 51, 3, 2, 59)
        if SLOT_51:
            pass
    label(1)
    sprite('null', 10)
    Unknown23033(-55)
    GFX_0('meteo_col_OD', 100)
    SFX_3('ce004')
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-100000)
    GFX_0('meteo_col_OD', 100)
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 10)
    teleportRelativeX(400000)
    GFX_0('meteo_col_OD', 100)
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-700000)
    GFX_0('meteo_col_OD', 100)
    SFX_3('ce004')
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 10)
    teleportRelativeX(600000)
    GFX_0('meteo_col_OD', 100)
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-600000)
    GFX_0('meteo_col_OD', 100)
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 10)
    teleportRelativeX(350000)
    GFX_0('meteo_col_OD', 100)
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-270000)
    GFX_0('meteo_col_OD', 100)
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 10)
    teleportRelativeX(600000)
    GFX_0('meteo_col_OD', 100)
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-400000)
    GFX_0('meteo_col_OD', 100)
    Unknown23033(-55)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 90)
    label(0)
    sprite('null', 1)

@State
def meteo_takusanCD():

    def upon_IMMEDIATE():
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 0)
        Unknown2005()
        sendToLabelUpon(54, 0)
        Unknown48(25, 2, 51, 3, 2, 59)
        if SLOT_51:
            SLOT_31 = 0
        Unknown1086(22)
        teleportRelativeX(450000)
    label(1)
    sprite('null', 5)
    sprite('null', 3)
    teleportRelativeX(-900000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    SFX_3('ce004')
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 3)
    teleportRelativeX(-370000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(800000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 3)
    teleportRelativeX(-800000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(200000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 3)
    teleportRelativeX(600000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-500000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 3)
    teleportRelativeX(400000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 3)
    teleportRelativeX(-900000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(800000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 0)
    teleportRelativeX(-500000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(-400000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 3)
    teleportRelativeX(900000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    SFX_3('ce004')
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 3)
    teleportRelativeX(-370000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 5)
    teleportRelativeX(800000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 3)
    teleportRelativeX(-800000)
    Unknown23033(-55)
    GFX_0('meteo_col', 100)
    callSubroutine('ChargeDamageUp_meteo')
    sprite('null', 90)
    label(0)
    sprite('null', 1)

@Subroutine
def ChargeDamageUp_meteo():
    if (SLOT_51 == 1):
        Unknown23029(1, 5001, 0)
    if (SLOT_51 == 2):
        Unknown23029(1, 5002, 0)
    if (SLOT_51 == 3):
        Unknown23029(1, 5003, 0)

@State
def meteo_col():

    def upon_IMMEDIATE():
        Unknown3001(0)
        Unknown2011()
        AttackLevel_(5)
        Damage(800)
        AttackP2(100)
        MinimumDamagePct(15)
        Unknown9017(1)
        Unknown9021(1)
        Hitstop(0)
        AirUntechableTime(42)
        hitstun(30)
        AirPushbackY(10000)
        PushbackX(15000)
        Unknown53(0)
        physicsYImpulse(-50000)
        physicsXImpulse(50000)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 0)
        sendToLabelUpon(54, 0)
        Unknown23015(4)

        def upon_LANDING():
            Unknown23090(25)
            SFX_3('bomb_m')

        def upon_43():
            if (SLOT_48 == 5001):
                Unknown10000(200)
            if (SLOT_48 == 5002):
                Unknown10000(300)
            if (SLOT_48 == 5003):
                Unknown10000(400)
    sprite('vr_to432_col', 1)
    RefreshMultihit()
    GFX_0('meteo', 0)
    label(1)
    sprite('vr_to432_col', 2)
    GFX_1('toef_meteosmoke_01', 0)
    gotoLabel(1)
    label(0)
    sprite('vr_to432_col', 1)
    GFX_1('toef_meteobrake_09', 0)
    ScreenShake(0, 5000)

@State
def meteo_col_OD():

    def upon_IMMEDIATE():
        Unknown3001(0)
        Unknown2011()
        AttackLevel_(5)
        Damage(1000)
        AttackP2(100)
        MinimumDamagePct(15)
        Unknown9017(1)
        Unknown9021(1)
        Hitstop(0)
        AirUntechableTime(42)
        hitstun(30)
        AirPushbackY(10000)
        PushbackX(15000)
        Unknown53(0)
        physicsYImpulse(-50000)
        physicsXImpulse(50000)
        Unknown23089(1, 1, 1, 1, 1, 0, 1, 0)
        sendToLabelUpon(54, 0)
        Unknown23015(4)

        def upon_LANDING():
            Unknown23090(25)
            SFX_3('bomb_m')

        def upon_43():
            if (SLOT_48 == 5001):
                Unknown10000(200)
            if (SLOT_48 == 5002):
                Unknown10000(300)
            if (SLOT_48 == 5003):
                Unknown10000(400)
    sprite('vr_to432_col', 1)
    RefreshMultihit()
    GFX_0('meteo', 0)
    label(1)
    sprite('vr_to432_col', 2)
    GFX_1('toef_meteosmoke_01', 0)
    gotoLabel(1)
    label(0)
    sprite('vr_to432_col', 1)
    GFX_1('toef_meteobrake_09', 0)
    ScreenShake(0, 5000)

@State
def meteo():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown2054(1)
        Unknown4003('ceef_432_meteo.DIG', '')
        GFX_2('toef_meteomoe_01')
        Unknown4010(2)
        Unknown4007(2)
        Unknown1072(45000)
        Unknown23015(4)
    sprite('null', 32767)

@State
def meteoStartAura():

    def upon_IMMEDIATE():
        GFX_2('toef_godaura_11')
        Unknown4010(2)
        Unknown2054(1)
    sprite('null', 44)

@State
def PCE_PersonaIchigeki():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(22, 104, -90000, 50000, 0, 0, 0, 0)
        callSubroutine('PCE_DDAttackInit')
        Unknown2012()
        Unknown4009(3)
        Unknown23066(1)
        Unknown2053(1)
        callSubroutine('PCE_CheckWarp')
    sprite('to450_02', 3)
    Unknown23022(1)
    teleportRelativeY(0)
    sprite('to450_00', 2)
    sprite('to450_00', 2)
    sprite('to450_01', 3)
    sprite('to450_02', 3)
    sprite('to450_00', 3)
    GFX_0('IchigekiDragonAtk', 0)
    sprite('to450_01', 3)
    sprite('to450_02', 3)
    sprite('to450_03', 3)
    sprite('to450_04', 3)
    sprite('to450_05', 3)
    sprite('to450_06', 3)
    sprite('to450_07', 3)
    sprite('to450_08', 3)
    sprite('to450_09', 1)
    DisableAttackRestOfMove()
    physicsYImpulse(60000)
    setGravity(0)
    sprite('to450_09', 1)
    YAccel(30)
    sprite('to450_10', 2)
    YAccel(30)
    sprite('to450_11', 2)
    YAccel(30)
    sprite('to450_09', 2)
    YAccel(30)
    sprite('to450_10', 2)
    YAccel(20)
    sprite('to450_11', 2)
    YAccel(10)
    sprite('to450_09', 2)
    YAccel(0)
    sprite('to450_10', 2)
    sprite('to450_11', 2)
    sprite('to450_09', 2)
    sprite('to450_10', 2)
    sprite('to450_11', 2)
    sprite('to450_09', 4)
    sprite('to450_10', 4)
    sprite('to450_11', 4)
    sprite('to450_09', 6)
    sprite('to450_10', 6)
    sprite('to450_11', 6)
    sprite('to450_09', 6)
    sprite('to450_10', 6)
    sprite('to450_11', 6)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def IchigekiDragonAtk():

    def upon_IMMEDIATE():
        Unknown2054(1)
        Unknown4010(2)
        Unknown2012()
        AttackLevel_(5)
        Hitstop(7)
        Damage(0)
        Unknown11064(1)
        AirPushbackX(0)
        AirPushbackY(60000)
        YImpluseBeforeWallbounce(0)
        AirHitstunAnimation(13)
        GroundedHitstunAnimation(13)
        AirUntechableTime(10000)
        Unknown9310(60)
        ChipDamagePct(0)

        def upon_12():
            Unknown21007(3, 32)
        ProjectileDurabilityLvl(3)
    sprite('null', 5)
    GFX_0('IchigekiMizubasira', 100)
    GFX_0('IchigekiMizubasira01', 100)
    SFX_3('blaze_normal')
    sprite('null', 5)
    SFX_3('blaze_normal')
    sprite('null', 5)
    SFX_3('blaze_normal')
    sprite('null', 5)
    SFX_3('blaze_normal')
    sprite('null', 5)
    SFX_3('blaze_normal')
    sprite('null', 5)
    SFX_3('blaze_normal')
    sprite('dmy_atk00', 3)
    GFX_0('IchigekiMizubasira05', 100)
    GFX_0('IchigekiDoragon3D', 100)
    StartMultihit()
    sprite('dmy_atk00', 15)
    RefreshMultihit()
    sprite('dmy_atk00', 30)
    Unknown2001()
    sprite('dmy_atk00', 20)

@State
def IchigekiDoragon3D():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown2054(1)
        Unknown4003('ceef_450_waterdragon.DIG', 'ceef_450_waterdragon_0000.mmot')
        Unknown4010(2)
        Unknown4009(2)
        teleportRelativeX(100000)
        Unknown1007(150000)
    sprite('null', 20)
    GFX_0('IchigekiDoragonHead3D', 100)
    ScreenShake(0, 50000)
    SFX_3('ce000')
    sprite('null', 20)
    Unknown3004(-13)

@State
def IchigekiDoragonHead3D():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown2054(1)
        Unknown4003('ceef_450_waterdragon_head.DIG', '')
        Unknown4010(2)
    sprite('null', 10)
    sprite('null', 30)
    physicsYImpulse(30000)

@State
def IchigekiMizubasira():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown2054(1)
        Unknown4003('ceef_450_watercolumn00.DIG', 'ceef_450_watercolumn00_0000.mmo')
        Unknown3001(0)
        Unknown4010(2)
        Unknown4009(2)
        Unknown4015()
    sprite('null', 45)
    GFX_1('toef_itigeki_tame_06', 100)
    sprite('null', 10)
    teleportRelativeY(5000)
    Unknown1096(1150)
    Unknown3004(51)
    sprite('null', 30)
    sprite('null', 10)
    Unknown3004(-51)

@State
def IchigekiMizubasira01():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown2054(1)
        Unknown4003('ceef_450_watercolumn01.DIG', '')
        Unknown4010(2)
        Unknown3001(0)
        Unknown4009(2)
        Unknown4015()
        Unknown1007(100000)
    sprite('null', 30)
    Unknown3004(9)
    sprite('null', 50)
    Unknown1099(0)
    sprite('null', 10)

@State
def IchigekiMizubasira05():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown2054(1)
        Unknown4010(2)
        GFX_2('toef_itigeki_mizubasira_05')
    sprite('null', 60)
    Unknown1007(200000)
    sprite('null', 50)
    Unknown3004(-10)

@State
def ChargeIchigekiModel():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('ceef_431_charge00.DIG', '')
        GFX_2('ceef_jizokucharge_02')
        Unknown2054(1)
        Unknown3001(0)
    sprite('null', 10)
    Unknown3004(26)
    Unknown1059(30)
    sprite('null', 120)
    GFX_1('ceef_endcharge_01', 100)
    Unknown1059(0)
    sprite('null', 10)
    Unknown3004(-26)
    Unknown1059(30)

@State
def IchigekiJumpSmoke():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown2054(1)
        Unknown4003('ceef_450_yukkuri_smoke.DIG', '')
    sprite('null', 20)
    sprite('null', 10)
    Unknown3004(-26)

@State
def IchigekiPicture():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown23015(1)
        Unknown6001(1)
        Unknown1001(640000)
        Unknown4061(0)
        teleportRelativeY(-384000)
        Unknown1007(500000)
        Unknown1096(2000)
        teleportRelativeX(310000)
        Unknown2054(1)
    sprite('ichigeki0', 5)
    Unknown1099(-200)
    sprite('ichigeki0', 30)
    Unknown1099(0)
    sprite('keep', 30)
    sprite('keep', 60)
    Unknown3004(-5)

@State
def IchiHaikei():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown2019(-100)
        Unknown4061(7)
        Unknown1096(3000)
        Unknown2054(1)
    sprite('vr_ce450bg', 30)
    GFX_0('IchiScrew', 0)
    sprite('vr_ce450bg', 30)
    sprite('vr_ce450bg', 60)
    Unknown3004(-5)

@State
def IchiScrew():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown2005()
        Unknown23032(50)
        Unknown23033(50)
        Unknown2019(-100)
        Unknown2054(1)
        Unknown21010(1)
        GFX_2('ceef_shutyusen_01')
        Unknown4003('ceef_450_cutinuzu00.DIG', '')
        Unknown4015()
        Unknown3001(0)
    sprite('null', 100)
    GFX_1('ceef_deathkira_la', 100)
    Unknown3004(51)
    sprite('null', 10)
    Unknown3004(-26)

@State
def IchigekiWinRainbow():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown2054(1)
        Unknown4003('ceef_450_Rainbow.DIG', '')
    sprite('null', 90)
    sprite('null', 30)
    Unknown3004(-9)

@State
def IchigekiCamera():

    def upon_IMMEDIATE():
        sendToLabelUpon(32, 0)
        sendToLabelUpon(33, 1)
        sendToLabelUpon(34, 2)
        sendToLabelUpon(35, 3)
        sendToLabelUpon(36, 4)
        sendToLabelUpon(37, 5)
        Unknown3029(1)
        Unknown3032()
        Unknown3001(80)
        Unknown3038(1)
    sprite('dmy_camera', 32767)
    label(0)
    sprite('keep', 32767)
    if SLOT_38:
        op(2, 2, 52, 0, -1)
        SLOT_22 = SLOT_0
        op(2, 2, 53, 0, -1)
        SLOT_23 = SLOT_0
    else:
        SLOT_22 = SLOT_52
        SLOT_23 = SLOT_53
    Unknown20000(1)
    Unknown20002(1)
    Unknown20003(1)
    label(1)
    sprite('keep', 12)
    SystemGFX('SCEF_FadeBlack12In', 100)
    sprite('keep', 15)
    Unknown26('SCEF_FadeBlack12In')
    SystemGFX('SCEF_FadeBlack12Out', 100)
    Unknown20001(1)
    Unknown1086(22)
    Unknown1007(900000)
    Unknown4007(22)
    Unknown20000(1)
    Unknown20002(1)
    Unknown20003(1)
    physicsYImpulse(-1428)
    setGravity(314)
    sprite('keep', 10)
    sprite('keep', 10)
    setGravity(2200)
    SystemGFX('SCEF_AstRyuhaiUpper', 100)
    Unknown36(1)
    Unknown1007(2000000)
    Unknown35()
    sprite('keep', 10)
    sprite('keep', 32767)
    label(2)
    sprite('keep', 18)
    SystemGFX('SCEF_FadeBlack12In', 100)
    sprite('keep', 10)
    Unknown26('SCEF_ShashaIn')
    SystemGFX('SCEF_ShashaOut', 100)
    Unknown26('SCEF_AstRyuhaiUpper')
    Unknown4007(0)
    Unknown1086(3)
    teleportRelativeX(390000)
    Unknown1007(300000)
    Unknown1084(1)
    Unknown20000(1)
    Unknown20001(1)
    Unknown20002(1)
    Unknown20003(1)
    Unknown20001(1)
    sprite('keep', 32767)
    Unknown26('SCEF_FadeBlack12In')
    label(3)
    sprite('keep', 50)

    def upon_3():
        SLOT_105 = (SLOT_105 + (-3))
    sprite('keep', 32767)
    clearUponHandler(3)
    label(4)
    sprite('keep', 32767)
    SLOT_51 = 0
    Unknown20009(1000)
    Unknown1084(1)
    Unknown1086(3)
    teleportRelativeY(0)

@State
def IchigekiCopyZanzoh():
    sprite('null', 60)
    Unknown2019(100)
    Unknown23053('1900000003000000')
    Unknown3032()
    Unknown3004(-1)
    sprite('keep', 300)
    Unknown3004(-5)

@State
def P4U_Cutin_Parent():

    def upon_IMMEDIATE():
        Unknown2054(1)
    sprite('null', 1)
    GFX_0('P4U_Cutin_Face', 100)

@State
def P4U_Cutin_Face():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown23015(5)
        Unknown2007()
        Unknown4009(2)
        Unknown2054(1)
        Unknown13044(0)
        Unknown6001(1)
        Unknown1000(-650000)
        teleportRelativeY(-292000)
        Unknown1096(1350)
        Unknown2037(0)
        Unknown3001(0)

        def upon_3():
            if (SLOT_2 == 2):
                Unknown1007(15000)
                Unknown3001(150)
            if (SLOT_2 == 3):
                Unknown1007(-30000)
                Unknown3001(180)
            if (SLOT_2 == 4):
                Unknown1007(25000)
                Unknown3001(210)
            if (SLOT_2 == 5):
                Unknown1007(-20000)
                Unknown3001(255)
            if (SLOT_2 == 6):
                Unknown1007(10000)
            Unknown2038(1)
    sprite('vr_99p4u_face00', 45)
    SFX_0('spsys_over_power')
    SFX_0('spsys_over_power')
    if SLOT_168:
        GFX_0('P4U_ka_NotJPN', 100)
    else:
        GFX_0('P4U_ka_JPN', 100)
    sprite('vr_99p4u_face00', 1)
    Unknown3001(180)
    sprite('vr_99p4u_face00', 1)
    Unknown3001(120)
    sprite('vr_99p4u_face00', 1)
    Unknown3001(60)

@State
def P4U_ka_JPN():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown23015(5)
        Unknown2007()
        Unknown4009(2)
        Unknown6001(1)
        Unknown1000(-950000)
        teleportRelativeY(-232000)
        Unknown1096(1350)
    sprite('vr_p4_ka_mozi', 1)
    Unknown1096(100)
    Unknown3001(40)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(500)
    Unknown1096(300)
    Unknown3001(70)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(550)
    Unknown1096(700)
    Unknown3001(100)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(600)
    Unknown1096(900)
    Unknown3001(130)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(6500)
    Unknown1096(800)
    Unknown3001(160)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(700)
    Unknown1096(900)
    Unknown3001(190)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(800)
    Unknown1096(1150)
    Unknown3001(220)
    sprite('vr_p4_ka_mozi', 38)
    teleportRelativeX(-2000)
    Unknown1007(800)
    Unknown1096(1100)
    Unknown3001(255)
    physicsXImpulse(-50)
    physicsYImpulse(50)
    sprite('vr_p4_ka_mozi', 1)
    Unknown3001(180)
    Unknown1096(700)
    sprite('vr_p4_ka_mozi', 1)
    Unknown3001(120)
    Unknown1096(300)
    sprite('vr_p4_ka_mozi', 1)
    Unknown3001(60)
    Unknown1096(100)

@State
def P4U_ka_NotJPN():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown23015(5)
        Unknown2007()
        Unknown4009(2)
        Unknown6001(1)
        Unknown1000(-950000)
        teleportRelativeY(-232000)
        Unknown1096(1350)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    Unknown1096(100)
    Unknown3001(40)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(500)
    Unknown1096(300)
    Unknown3001(70)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(550)
    Unknown1096(700)
    Unknown3001(100)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(600)
    Unknown1096(900)
    Unknown3001(130)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(6500)
    Unknown1096(800)
    Unknown3001(160)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(700)
    Unknown1096(900)
    Unknown3001(190)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(800)
    Unknown1096(1150)
    Unknown3001(220)
    sprite('vr_p4_ka_mozi_notjpn', 38)
    teleportRelativeX(-2000)
    Unknown1007(800)
    Unknown1096(1100)
    Unknown3001(255)
    physicsXImpulse(-50)
    physicsYImpulse(50)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    Unknown3001(180)
    Unknown1096(700)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    Unknown3001(120)
    Unknown1096(300)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    Unknown3001(60)
    Unknown1096(100)

@State
def PersonaMatchWin():

    def upon_IMMEDIATE():
        callSubroutine('PersonaReset')
        Unknown23023()
        Unknown23184(3, 100, 70000, 95000, 0, 0, 0, 0)
        callSubroutine('PCE_AttackInit')
        callSubroutine('PCE_CheckWarp')
        Unknown2053(1)
        Unknown2019(-100)
        Unknown23022(1)
        Unknown23015(0)
    sprite('to611_00', 4)
    sprite('to611_01', 4)
    sprite('to611_02', 4)
    sprite('to611_03', 4)
    sprite('to611_04', 4)
    sprite('to611_05', 4)
    label(0)
    sprite('to611_03', 4)
    sprite('to611_04', 4)
    sprite('to611_05', 4)
    loopRest()
    gotoLabel(0)

@State
def MatchWin2_bikkuri():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown4010(2)
    sprite('null', 7)
    sprite('null', 5)
    Unknown1096(0)
    Unknown1099(200)
    Unknown2007()
    GFX_2('ceef_matchwin2')
    sprite('null', 32767)
    Unknown1096(1000)
    Unknown1099(0)