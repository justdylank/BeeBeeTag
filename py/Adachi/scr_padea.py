@Subroutine
def ExSkillInit():
    MinimumDamagePct(10)
    Unknown30065(0)

@Subroutine
def InvSkillInit():
    Unknown30065(100)

@Subroutine
def PartnerSkillInit():
    AttackP1(70)
    Unknown11042(1)

@Subroutine
def CompositeImageSetting():
    Unknown3060(0, 'vr_magatsuaura00')
    Unknown3061(0, 4)
    Unknown3063(0, 180000)
    Unknown3064(0, -1000)
    Unknown3068('0000000060000000ff000000ff000000ff000000')

@Subroutine
def PalEffSetting():

    def upon_45():
        Unknown23030('pad_persona_limitflash', 0, 240, 0, 241, 0, 254, 0, 4)

@State
def PAD_PersonaCreate():

    def upon_IMMEDIATE():
        Unknown23022(1)
        Unknown13043(-75000)
        teleportRelativeY(0)
        Unknown23013(1)
    sprite('null', 32767)
    Unknown3038(1)
    Unknown24('5041445f506572736f6e6157616974000000000000000000000000000000000064000000')

@State
def PAD_PersonaWait():

    def upon_IMMEDIATE():
        SLOT_11 = 0
        SLOT_59 = (-1)
        callSubroutine('PAD_ReceptionSignal')
    sprite('null', 32767)
    Unknown3038(1)
    Unknown3001(0)
    Unknown3004(0)
    loopRest()

@Subroutine
def PAD_ReceptionSignal():

    def upon_43():
        if (SLOT_48 == 2001):
            Unknown23185('PAD_PersonaThrowShot', 50)
        if (SLOT_48 == 2002):
            Unknown23185('PAD_Persona5C', 50)
        if (SLOT_48 == 2003):
            Unknown23185('PAD_Persona5CC', 50)
        if (SLOT_48 == 2004):
            Unknown23185('PAD_Persona5CCC', 50)
        if (SLOT_48 == 2005):
            Unknown23185('PAD_PersonaJC', 50)
        if (SLOT_48 == 2006):
            Unknown23185('PAD_PersonaJ2C', 50)
        if (SLOT_48 == 2007):
            Unknown23185('PAD_PersonaJ2CEx', 50)
        if (SLOT_48 == 2008):
            Unknown23185('PAD_PersonaCA', 50)
        if (SLOT_48 == 2009):
            Unknown23185('PAD_Persona5AAA', 50)
        if (SLOT_48 == 2010):
            Unknown23185('PAD_Persona5AAAA', 50)
        if (SLOT_48 == 2501):
            Unknown23185('PAD_PersonaAntiAirPSkill', 50)
        if (SLOT_48 == 2502):
            Unknown23185('PAD_PersonaSearchShotPSkill', 50)
        if (SLOT_48 == 3001):
            Unknown23185('PAD_PersonaSpecialFastShot', 50)
        if (SLOT_48 == 3002):
            Unknown23185('PAD_PersonaSpecialSlowShot', 50)
        if (SLOT_48 == 3501):
            Unknown23185('PAD_PersonaSpecialSearchShot', 50)
        if (SLOT_48 == 3003):
            Unknown23185('PAD_PersonaSpecialAntiAirFast', 50)
        if (SLOT_48 == 3004):
            Unknown23185('PAD_PersonaSpecialAntiAirSlow', 50)
        if (SLOT_48 == 3005):
            Unknown23185('PAD_PersonaSpecialThrow', 50)
        if (SLOT_48 == 4001):
            SLOT_60 = 0
            Unknown23185('PAD_PersonaUltimateAssault', 50)
        if (SLOT_48 == 4021):
            SLOT_60 = 1
            Unknown23185('PAD_PersonaUltimateAssault', 50)
        if (SLOT_48 == 4002):
            SLOT_60 = 0
            Unknown23185('PAD_PersonaUltimateBlade', 50)
        if (SLOT_48 == 4003):
            SLOT_60 = 0
            Unknown23185('PAD_PersonaUltimateBladeSP', 50)
        if (SLOT_48 == 4004):
            SLOT_60 = 0
            Unknown23185('PAD_PersonaUltimateThrow', 50)
        if (SLOT_48 == 4501):
            SLOT_60 = 0
            Unknown23185('PAD_PersonaAstralHeat', 50)
        if (SLOT_48 == 4005):
            Unknown23185('PAD_PersonaUltimateDUO', 50)
        if (SLOT_48 == 4006):
            Unknown23185('PAD_PersonaUltimateDUOSP', 50)
        if (SLOT_48 == 9999):
            Unknown23185('PersonaDeleteAndIdling', 300)
        if (SLOT_48 == 10001):
            Unknown23185('PAD_PersonaMatchWin', 50)
        if (SLOT_48 == 10002):
            Unknown23185('PAD_PersonaMatchWinMist', 50)

@Subroutine
def PersonaReset():
    Unknown4061(1)
    Unknown23022(0)
    Unknown2053(0)
    Unknown3001(255)
    Unknown3032()
    callSubroutine('CompositeImageSetting')
    callSubroutine('PalEffSetting')
    Unknown23015(11)
    Unknown2019(-1)
    Unknown1084(1)
    Unknown4009(0)
    Unknown4008(0)
    Unknown4007(0)
    EnableCollision(0)
    Unknown30041('')
    Unknown2053(0)
    Unknown2034(0)

@Subroutine
def PAD_EffectInit():
    Unknown2019(5)
    Unknown23015(11)

@Subroutine
def PAD_AttackInit():
    Unknown23028(1, 1)
    Unknown30037('')
    Unknown30035(1)
    callSubroutine('PersonaReset')
    Unknown23015(11)
    Unknown2019(10)
    if (SLOT_11 == 0):
        SLOT_58 = 1
    if (SLOT_11 == 10):
        SLOT_58 = 1
    SLOT_11 = 100
    callSubroutine('PAD_ReceptionSignal')
    Unknown11034(1)
    ProjectileDurabilityLvl(0)
    Unknown23023()
    EnableCollision(1)
    Unknown30040(1)
    Unknown30040(2)

@Subroutine
def PAD_SPAttackInit():
    Unknown23028(2, 1)
    Unknown30037('')
    Unknown30035(1)
    callSubroutine('PersonaReset')
    Unknown23015(11)
    Unknown2019(10)
    if (SLOT_11 == 0):
        SLOT_58 = 1
    if (SLOT_11 == 10):
        SLOT_58 = 1
    SLOT_11 = 110
    callSubroutine('PAD_ReceptionSignal')
    Unknown11034(1)
    ProjectileDurabilityLvl(0)
    Unknown23023()
    EnableCollision(1)
    Unknown30040(1)
    Unknown30040(2)

@Subroutine
def PAD_DDAttackInit():
    Unknown23028(3, 1)
    Unknown30037('')
    callSubroutine('PersonaReset')
    Unknown23015(11)
    Unknown2019(10)
    if (SLOT_11 == 0):
        SLOT_58 = 1
    if (SLOT_11 == 10):
        SLOT_58 = 1
    Unknown11034(1)
    ProjectileDurabilityLvl(0)
    SLOT_11 = 120
    callSubroutine('PAD_ReceptionSignal')
    Unknown23023()

@Subroutine
def PAD_CheckWarp():
    if SLOT_58:
        SLOT_58 = 1

@Subroutine
def PAD_ForceWarp():
    SLOT_58 = 1

@State
def PersonaDeleteAndIdling():

    def upon_IMMEDIATE():
        callSubroutine('PersonaReset')
    sprite('keep', 32767)
    if SLOT_143:
        GFX_0('PersonaDeleteEffect', 100)
        SFX_3('persona_disappear')
    SLOT_11 = 10
    SLOT_59 = (-1)
    enterState('PAD_PersonaWait')

@State
def PersonaSummonEffect():
    sprite('null', 30)
    Unknown23015(13)
    Unknown4054(11)
    Unknown4045('persona_summon', 100)

@State
def PersonaSummonEffect_PLAYER():
    sprite('null', 30)
    Unknown1007(288000)
    teleportRelativeX(112000)
    GFX_1('persona_summon_ply', 100)

@State
def PersonaDeleteEffect():

    def upon_IMMEDIATE():
        callSubroutine('CompositeImageSetting')
        callSubroutine('PalEffSetting')
        callSubroutine('PersonaReset')
        callSubroutine('PAD_ReceptionSignal')
    sprite('null', 30)
    Unknown2019(1)
    Unknown23053('190000000b000000')
    Unknown4061(1)
    Unknown3032()
    Unknown1059(-100)
    Unknown1067(150)
    Unknown3004(-20)
    Unknown23022(1)
    Unknown4054(11)
    Unknown4045('persona_delete', 100)
    Unknown2054(1)
    EnableCollision(0)
    Unknown1084(1)

@Subroutine
def KowarePreset():
    Unknown23089(1, 0, 0, 0, 0, 0, 1, 0)
    sendToLabelUpon(54, 580)
    sendToLabelUpon(32, 580)

@State
def BintaHit():
    sprite('null', 5)
    GFX_1('ef_hit_blowm', 100)
    SFX_3('slap_fast')

@State
def adef_400_explosion():

    def upon_IMMEDIATE():
        teleportRelativeX(100000)
    sprite('null', 30)
    Unknown1096(2000)
    Unknown23067('adef_405shot')

@State
def ad400_Gun():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown1007(100000)
    sprite('ad400_10z', 3)
    SFX_3('cloth_m')
    physicsXImpulse(14000)
    physicsYImpulse(30000)
    setGravity(2000)
    GFX_0('ad400_Gun_sub', 0)
    sprite('ad400_11z', 3)
    sprite('ad400_10z', 3)
    SFX_3('cloth_m')
    sprite('ad400_11z', 3)
    sprite('ad400_10z', 3)
    SFX_3('cloth_m')
    sprite('ad400_11z', 3)
    sprite('ad400_10z', 3)
    SFX_3('cloth_m')
    Unknown3004(-12)
    sprite('ad400_11z', 3)
    sprite('ad400_10z', 3)
    SFX_3('cloth_m')

@State
def ad400_Gun_sub():

    def upon_IMMEDIATE():
        Unknown4007(2)
        Unknown4010(2)
        Unknown1007(20000)
    sprite('null', 2)
    label(0)
    sprite('null', 6)
    GFX_1('adef_400gun', 100)
    gotoLabel(0)

@State
def adef_405_muzzle_a():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown1072(40000)
    sprite('null', 5)
    Unknown4048(0)
    Unknown23067('adef_405muzzle')
    
@State
def adef_405_muzzle_b():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown1072(25000)
    sprite('null', 5)
    Unknown4048(0)
    Unknown23067('adef_405muzzle')

@State
def dmy_LowShotA_Atk():

    def upon_IMMEDIATE():
        Unknown2009()
        AttackLevel_(4)
        hitstun(25)
        blockstun(22)
        AirUntechableTime(40)
        AirPushbackX(12000)
        AirPushbackY(-26000)
        Unknown9190(1)
        Unknown9118(45)
        HitLow(2)
        AttackAttributes(0, 0, 0, 1, 0)
        Unknown23078(1)
        Unknown2053(1)
    sprite('null', 1)
    GFX_0('adef_405_shot', 100)
    sprite('dmy_LowShot_Atk', 3)
    RefreshMultihit()

@State
def dmy_LowShotB_Atk():

    def upon_IMMEDIATE():
        Unknown2009()
        AttackLevel_(4)
        hitstun(25)
        blockstun(22)
        AirUntechableTime(40)
        AirPushbackX(12000)
        AirPushbackY(-26000)
        Unknown9190(1)
        Unknown9118(45)
        HitLow(2)
        AttackAttributes(0, 0, 0, 1, 0)
        Unknown23078(1)
    sprite('null', 1)
    GFX_0('adef_405_shot', 100)
    sprite('dmy_LowShot_Atk', 3)
    RefreshMultihit()

@State
def adef_405_shot():

    def upon_IMMEDIATE():
        Unknown4007(2)
    sprite('null', 30)
    SFX_3('ad007')
    Unknown1056(960)
    Unknown1064(1200)
    Unknown23067('adef_405shot')

@State
def adef_401_dash():

    def upon_IMMEDIATE():
        teleportRelativeX(30000)
    sprite('null', 1)
    GFX_1('adef_401dash', 100)

@State
def PowerUpAura():

    def upon_IMMEDIATE():
        Unknown4011(3)
        Unknown4007(3)
    label(0)
    sprite('null', 10)
    GFX_0('PowerUpAuraSub', 103)
    gotoLabel(0)

@State
def PowerUpAuraSub():

    def upon_IMMEDIATE():
        Unknown4011(3)
        Unknown4010(2)
        Unknown1007(150000)
    sprite('null', 30)
    GFX_2('adef_401aura')

@State
def UltimateAirAssaultAsiba():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4010(2)
        GFX_2('adef_giobceaura_02')
        Unknown4007(3)
        Unknown2054(1)
        Unknown1007(40000)
        Unknown3001(180)
        callSubroutine('KowarePreset')
    sprite('null', 200)
    sprite('null', 10)
    Unknown3004(-25)
    ExitState()
    label(580)
    sprite('null', 10)
    Unknown4007(0)
    Unknown3004(-25)

@State
def mgef_450_yuka():

    def upon_IMMEDIATE():
        Unknown2054(1)
        Unknown4003('mgef_450yuka.DIG', '')
        Unknown4015()
        Unknown3032()
        Unknown23015(2)
        Unknown1086(22)
        teleportRelativeY(0)
        Unknown23089(1, 0, 0, 0, 0, 0, 1, 0)
        sendToLabelUpon(54, 580)

        def upon_32():
            Unknown21012('mgef_450_yukaSE', 32)
            sendToLabel(580)
        GFX_0('mgef_450_yukaSE', -1)
    sprite('null', 14)
    Unknown3001(0)
    GFX_1('mgef_450yuka_splash', 100)
    sprite('null', 1)
    GFX_0('Ichigeki_Atk', 100)
    sprite('null', 32767)
    Unknown1096(6000)
    Unknown3001(0)
    Unknown3004(12)
    label(580)
    sprite('null', 20)
    Unknown3004(-12)

@State
def mgef_450_yukaSE():

    def upon_IMMEDIATE():
        Unknown4010(2)
        sendToLabelUpon(32, 0)
        Unknown2037(40)
    label(1)
    sprite('null', 40)
    SFX_3('ad011')
    Unknown2038(-1)
    if SLOT_2:
        _gotolabel(1)
    label(0)
    sprite('null', 1)

@State
def Ichigeki_Atk():

    def upon_IMMEDIATE():
        Unknown2012()
        Unknown4010(3)
        Unknown2054(1)
        AttackLevel_(5)
        Damage(0)
        Unknown11034(0)
        ProjectileDurabilityLvl(3)
        Hitstop(0)
        AirHitstunAnimation(2)
        GroundedHitstunAnimation(2)
        Unknown9130(56)
        Unknown9142(9999)
        AirPushbackX(0)
        AirPushbackY(0)
        PushbackX(0)
        YImpluseBeforeWallbounce(0)
        Unknown11064(1)
        Unknown11042(1)
        Unknown11084(1)
        Unknown9021(1)
        Unknown23151(22, 104)

        def upon_12():
            Unknown23029(3, 4511, 0)
    sprite('null', 5)
    sprite('dmy_Ichigeki_Atk', 5)
    RefreshMultihit()

@State
def PAD_PersonaThrowShot():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(22, 100, 150000, 64000, 0, 0, 0, 0)
        callSubroutine('PAD_AttackInit')
        Unknown2006()
        AttackLevel_(5)
        Damage(2501)
        AttackP2(70)
        Unknown11056(2)
        Hitstop(0)
        AirUntechableTime(60)
        Unknown9266(8)
        AirHitstunAnimation(1)
        GroundedHitstunAnimation(1)
        AirPushbackY(-35000)
        AirPushbackX(2000)
        Unknown9190(1)
        Unknown11084(1)
        Unknown9021(1)
        Unknown11034(0)
        ProjectileDurabilityLvl(2)
        AttackAttributes(0, 0, 0, 1, 0)
        DisableAttackRestOfMove()
        Unknown23078(1)
        callSubroutine('PAD_CheckWarp')
        EnableCollision(0)
        Unknown2053(1)
        Unknown23022(1)
        Unknown3032()
        sendToLabelUpon(33, 1)
    sprite('mg406_00', 4)
    Unknown3001(0)
    Unknown3004(9)
    GFX_0('mgef_406_charge', 0)
    GFX_0('charge_se', 0)
    physicsYImpulse(8000)
    sprite('mg406_01', 4)
    sprite('mg406_02', 4)
    physicsYImpulse(3000)
    sprite('mg406_00', 4)
    sprite('mg406_01', 4)
    sprite('mg406_02', 4)
    physicsYImpulse(1000)
    sprite('mg406_00', 4)
    sprite('mg406_01', 4)
    sprite('mg406_02', 4)
    physicsYImpulse(200)
    sprite('mg406_00', 4)
    sprite('mg406_01', 4)
    sprite('mg406_02', 4)
    physicsYImpulse(0)
    label(1)
    sprite('mg406_03', 6)
    Unknown21012('mgef_406_charge', 32)
    sprite('mg406_04', 6)
    physicsYImpulse(1000)
    sprite('mg406_05', 8)
    physicsYImpulse(0)
    sprite('mg406_06', 4)
    sprite('mg406_07', 3)
    SFX_3('hit_m_middle')
    Unknown26('mgef_406_charge')
    GFX_0('mgef_406_throw', 0)
    sprite('mg406_08', 3)
    RefreshMultihit()
    GFX_0('mgef_406_blast', 0)
    Unknown2063()
    Unknown11044(1)
    sprite('mg406_08', 3)
    sprite('mg406_09', 3)
    sprite('mg406_10', 3)
    Unknown2001()
    Unknown23029(3, 2011, 0)
    sprite('mg406_08', 3)
    sprite('mg406_09', 3)
    sprite('mg406_10', 3)
    sprite('mg406_08', 3)
    sprite('mg406_09', 3)
    sprite('mg406_10', 3)
    sprite('mg406_08', 3)
    sprite('mg406_09', 3)
    sprite('mg406_10', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def charge_se():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown4007(2)
    label(0)
    sprite('null', 6)
    SFX_3('electric_m')
    gotoLabel(0)

@State
def mgef_406_charge():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown4007(2)
        Unknown4054(14)
        Unknown23067('mgef_406charge_bk')
        Unknown1007(40000)
        sendToLabelUpon(32, 10)
    sprite('null', 4)
    Unknown4045('mgef_406charge', 100)
    label(0)
    sprite('null', 4)
    Unknown4045('mgef_406charge04', 100)
    gotoLabel(0)
    label(10)
    Unknown1007(20000)
    teleportRelativeX(-66000)
    label(11)
    sprite('null', 4)
    Unknown4045('mgef_406charge03', 100)
    gotoLabel(11)

@State
def mgef_406_throw():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown1007(-100000)
    sprite('null', 4)
    Unknown4054(12)
    Unknown4045('mgef_406throw', 100)

@State
def mgef_406_blast():

    def upon_IMMEDIATE():
        Unknown4010(2)
        teleportRelativeY(0)
        Unknown1096(1500)
    sprite('null', 60)
    SFX_3('thunder_s')
    SFX_3('damage_magic_mh')
    SFX_3('bomb_m')
    Unknown23067('mgef_406blast')
    Unknown26('charge_se')

@State
def PAD_Persona5C():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 100000, 0, 0, 800000, 0, 0)
        callSubroutine('PAD_AttackInit')
        AttackLevel_(4)
        AttackP1(90)
        AirHitstunAnimation(10)
        AirPushbackX(2500)
        PushbackX(19800)
        AirUntechableTime(24)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown9016(1)
        DisableAttackRestOfMove()
        Unknown23078(1)
        callSubroutine('PAD_CheckWarp')
        Unknown23059(1)
    sprite('mg204_50', 4)
    if (SLOT_25 < 295000):
        Unknown1000(1865000)
        teleportRelativeX(-295000)
    Unknown2006()
    sprite('mg204_51', 3)
    sprite('mg204_52', 1)
    sprite('mg204_53', 1)
    sprite('mg204_55', 1)
    SFX_3('slash_blade_fast')
    GFX_0('mgef_204_zanzou', 100)
    RefreshMultihit()
    sprite('mg204_55', 1)
    Unknown23022(0)
    sprite('mg204_56', 4)
    DisableAttackRestOfMove()
    Unknown23029(3, 2012, 0)
    sprite('mg204_55', 4)
    sprite('mg204_56', 4)
    SFX_3('cloth_l')
    sprite('mg204_55', 4)
    sprite('mg204_56', 4)
    sprite('mg204_55', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_Persona5CC():

    def upon_IMMEDIATE():
        Unknown23023()
        callSubroutine('PAD_AttackInit')
        Unknown2006()
        AttackLevel_(4)
        AttackP1(90)
        Hitstop(10)
        GroundedHitstunAnimation(11)
        AirHitstunAnimation(11)
        AirPushbackX(0)
        AirPushbackY(-200000)
        PushbackX(-8000)
        AirUntechableTime(40)
        Unknown9190(1)
        Unknown9118(11)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown9016(1)
        DisableAttackRestOfMove()
        Unknown23078(1)
        Unknown23059(1)

        def upon_11():
            SFX_3('damage_hit_h')
    sprite('mg204_00', 2)
    SFX_3('hit_h_fast')
    sprite('mg204_01', 2)
    sprite('mg204_02', 6)
    sprite('mg204_03', 2)
    Unknown1045(10000)
    SFX_3('slash_blade_middle')
    sprite('mg204_04', 2)
    sprite('mg204_05ex01', 3)
    RefreshMultihit()
    sprite('mg204_06', 6)
    Unknown23029(3, 2013, 0)
    sprite('mg204_06ex1', 6)
    sprite('mg204_06ex2', 6)
    sprite('mg204_06', 6)
    sprite('mg204_06ex1', 6)
    sprite('mg204_06ex2', 6)
    sprite('keep', 32767)
    Unknown2004(0, 0)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_Persona5CCC():

    def upon_IMMEDIATE():
        Unknown23023()
        callSubroutine('PAD_AttackInit')
        Unknown2006()
        AttackLevel_(4)
        AttackP1(90)
        GroundedHitstunAnimation(11)
        AirHitstunAnimation(11)
        AirPushbackX(6000)
        AirPushbackY(-40000)
        PushbackX(19800)
        AirUntechableTime(40)
        Unknown9310(10)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown9016(1)
        DisableAttackRestOfMove()
        Unknown23078(1)
        Unknown23059(1)
        Unknown2004(1, 0)

        def upon_11():
            SFX_3('damage_hit_mh')
    sprite('mg204_06', 3)
    sprite('mg204_07', 3)
    sprite('mg204_08', 3)
    sprite('mg204_09', 9)
    SFX_3('hit_l_middle')
    sprite('mg204_10', 3)
    RefreshMultihit()
    ScreenShake(10000, 30000)
    sprite('mg204_11', 6)
    Unknown23029(3, 2014, 0)
    sprite('mg204_12', 6)
    sprite('mg204_13', 6)
    sprite('mg204_11', 6)
    sprite('keep', 32767)
    Unknown2004(0, 0)
    enterState('PersonaDeleteAndIdling')

@State
def mgef_204_zanzou():

    def upon_IMMEDIATE():
        Unknown4011(2)
        Unknown3033()
        Unknown4061(3)
        Unknown23015(13)
    sprite('vr_mg204_00', 30)
    Unknown3001(255)
    Unknown3004(-25)
    GFX_1('mgef_204', 0)

@State
def PAD_PersonaJC():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 100000, -100000, 0, 0, 0, 0)
        callSubroutine('PAD_AttackInit')
        AttackLevel_(4)
        AirPushbackY(-32000)
        AirPushbackX(26000)
        AirUntechableTime(29)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        AttackAttributes(1, 0, 0, 0, 0)
        Unknown9016(1)
        DisableAttackRestOfMove()
        Unknown23078(1)
        callSubroutine('PAD_CheckWarp')
        EnableCollision(0)
        Unknown2053(1)
        Unknown4007(3)
        Unknown23059(1)

        def upon_43():
            if (SLOT_48 == 9001):
                sendToLabel(580)
            if (SLOT_48 == 9002):
                sendToLabel(580)
    sprite('mg253_00', 2)
    sprite('mg253_01', 2)
    sprite('mg253_02', 2)
    sprite('mg253_03', 3)
    SFX_3('slash_rapier_middle')
    SFX_3('slash_rapier_middle')
    GFX_0('mgef_253', 100)
    sprite('mg253_04', 1)
    RefreshMultihit()
    sprite('mg253_04', 2)
    Unknown23022(0)
    sprite('mg253_05', 6)
    sprite('mg253_06', 6)
    sprite('mg253_07', 6)
    if SLOT_2:
        _gotolabel(0)
    sprite('mg253_05', 6)
    sprite('mg253_06', 6)
    sprite('mg253_07', 6)
    label(580)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def mgef_253():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown4007(2)
        Unknown3033()
        Unknown4061(3)
    sprite('vr_mg253_00', 3)
    sprite('vr_mg253_01', 15)
    Unknown3001(255)
    Unknown3004(-25)

@State
def PAD_PersonaJ2C():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 100000, -50000, 0, 0, 0, 0)
        callSubroutine('PAD_AttackInit')
        AttackLevel_(4)
        Damage(1000)
        AirUntechableTime(25)
        GroundedHitstunAnimation(10)
        AirHitstunAnimation(10)
        AirPushbackX(13000)
        AirPushbackY(48000)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        AttackAttributes(1, 0, 0, 0, 0)
        Unknown9016(1)
        Unknown11044(1)
        DisableAttackRestOfMove()
        callSubroutine('PAD_CheckWarp')
        Unknown23059(1)
        EnableCollision(0)
        Unknown2053(1)
        Unknown30048(1)

        def upon_78():
            clearUponHandler(78)
            Unknown23022(1)
            Unknown1017()
            Unknown1022()
            sendToLabel(1)
        SLOT_10 = 1

        def upon_STATE_END():
            SLOT_10 = 0
    sprite('mg232_00', 1)
    if (SLOT_25 < 205000):
        Unknown1000(1865000)
        teleportRelativeX(-205000)
    sprite('mg232_01', 1)
    sprite('mg232_02', 1)
    sprite('mg232_03', 3)
    GFX_0('mgef_232', 0)
    sprite('mg232_04', 1)
    SFX_3('slash_pole_middle')
    SFX_3('airdash_m')
    GFX_0('mgef232_ground', 100)
    Unknown3029(1)
    physicsXImpulse(60000)
    physicsYImpulse(48000)
    RefreshMultihit()
    sprite('mg232_04', 2)
    Unknown23022(0)
    sprite('mg232_05', 3)
    sprite('mg232_06', 3)
    SFX_3('airdash_m')
    sprite('mg232_04', 3)
    SFX_3('airdash_m')
    Unknown1019(50)
    YAccel(50)
    EnableCollision(0)
    Unknown3029(0)
    DisableAttackRestOfMove()
    sprite('mg232_05', 3)
    SFX_3('cloth_m')
    Unknown1019(50)
    YAccel(50)
    sprite('mg232_06', 3)
    Unknown1019(50)
    YAccel(50)
    sprite('mg232_04', 6)
    SFX_3('cloth_m')
    Unknown1019(0)
    YAccel(0)
    sprite('mg232_05', 6)
    sprite('mg232_06', 6)
    SFX_3('cloth_m')
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')
    label(1)
    sprite('keep', 32767)
    enterState('PAD_PersonaJ2CGrip')

@State
def PAD_PersonaJ2CGrip():

    def upon_IMMEDIATE():
        callSubroutine('PAD_AttackInit')
        Unknown17011('PAD_PersonaJ2CExe', 1, 1, 0)
        Unknown2006()
        Unknown11032(-1, -1, -1, -1)
        Unknown11054(-1)
        AttackP1(100)
        AttackP2(100)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        Unknown30061(1)
        Unknown11002(-1)
        AttackAttributes(0, 0, 0, 0, 1)
        Unknown30048(1)
        DisableAttackRestOfMove()
        Unknown30031(0)

        def upon_78():
            clearUponHandler(78)
            Unknown23029(3, 2017, 0)
        Unknown23078(1)
        Unknown23059(1)
        Unknown2053(1)
        Unknown23022(1)
        SLOT_10 = 1

        def upon_STATE_END():
            SLOT_10 = 0
    sprite('keep', 3)
    RefreshMultihit()
    sprite('mg232_04', 3)
    SFX_3('airdash_m')
    Unknown1019(50)
    YAccel(50)
    EnableCollision(0)
    Unknown3029(0)
    DisableAttackRestOfMove()
    sprite('mg232_05', 3)
    SFX_3('cloth_m')
    Unknown1019(50)
    YAccel(50)
    sprite('mg232_06', 3)
    Unknown1019(50)
    YAccel(50)
    sprite('mg232_04', 6)
    SFX_3('cloth_m')
    Unknown1019(0)
    YAccel(0)
    sprite('mg232_05', 6)
    sprite('mg232_06', 6)
    SFX_3('cloth_m')
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_PersonaJ2CExe():

    def upon_IMMEDIATE():
        callSubroutine('PAD_AttackInit')
        Unknown17012(1, 1, 0)
        Unknown2006()
        Unknown30031(0)
        AttackLevel_(5)
        AttackP1(100)
        AttackP2(100)
        MinimumDamagePct(0)
        Unknown9310(1)
        Unknown11072(1, 200000, 1000)
        AirHitstunAnimation(11)
        GroundedHitstunAnimation(11)
        AirPushbackY(-30000)
        Hitstop(6)
        DisableAttackRestOfMove()
        Unknown2034(1)
        Unknown23022(1)
        SLOT_10 = 1

        def upon_STATE_END():
            SLOT_10 = 0
        sendToLabelUpon(2, 0)
    sprite('mg232_04', 2)
    SFX_3('grip_hugs')
    SFX_3('bound_marble_m')
    Unknown5000(0, 0)
    Unknown5001(0, 0, 0, 0, 0)
    Unknown1018()
    Unknown1023()
    sprite('mg232_05', 2)
    Unknown5000(0, 0)
    Unknown5001(0, 0, 0, 0, 0)
    YAccel(75)
    Unknown1019(75)
    setGravity(2400)
    sprite('mg232_06', 2)
    SFX_3('cloth_m')
    Unknown1019(80)
    Unknown5000(0, 0)
    Unknown5001(0, 0, 0, 0, 0)
    Unknown23015(0)
    Unknown2018(1, 80)
    Unknown2019(100)
    sprite('mg232_04', 2)
    Unknown1019(80)
    Unknown5000(0, 0)
    Unknown5001(0, 0, 0, 0, 0)
    sprite('mg232_07', 3)
    SFX_3('cloth_m')
    Unknown1019(80)
    Unknown5000(0, 0)
    Unknown5001(0, 0, 0, 0, 0)
    sprite('mg232_08', 3)
    Unknown1019(80)
    Unknown5000(0, 0)
    Unknown5001(0, 0, 0, 0, 0)
    label(1)
    sprite('mg232_09', 3)
    SFX_3('cloth_m')

    def upon_4():
        clearUponHandler(4)
        sendToLabel(2)
    Unknown5000(0, 0)
    Unknown5001(0, 0, 0, 0, 0)
    sprite('mg232_10', 3)
    Unknown5000(0, 0)
    Unknown5001(0, 0, 0, 0, 0)
    gotoLabel(1)
    label(2)
    sprite('keep', 3)
    Unknown5000(0, 0)
    Unknown5001(0, 0, 0, 0, 0)
    sprite('mg232_11', 5)
    SFX_3('cloth_m')
    Unknown5000(3, 0)
    Unknown5001(0, 0, 0, 0, 0)
    sprite('mg232_12', 5)
    Unknown5000(4, 0)
    Unknown5001(0, 0, 0, 0, 0)
    sprite('mg232_13', 32767)
    Unknown5000(27, 0)
    Unknown5001(0, 1, 1, 0, 0)
    Unknown2018(1, 80)
    Unknown2019(1000)
    label(0)
    sprite('mg232_14', 5)
    SFX_3('damage_hit_mh')
    Unknown5000(27, 0)
    Unknown5001(0, 1, 1, 0, 0)
    ScreenShake(10000, 30000)
    Unknown1084(1)
    Unknown2034(0)
    RefreshMultihit()
    sprite('mg232_15', 3)
    Unknown23029(3, 2018, 0)
    SLOT_10 = 0
    sprite('mg232_16', 3)
    sprite('mg232_17', 3)
    sprite('mg232_18', 3)
    sprite('mg232_16', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def mgef_232():

    def upon_IMMEDIATE():
        Unknown1007(128000)
    sprite('null', 30)
    GFX_0('mgef_232_ripper', 100)
    GFX_0('mgef_232_ripper', 100)
    Unknown36(1)
    teleportRelativeX(0)
    Unknown1007(50000)
    Unknown35()
    GFX_0('mgef_232_ripper', 100)
    Unknown36(1)
    teleportRelativeX(0)
    Unknown1007(-50000)
    Unknown35()
    Unknown4048(-40000)
    Unknown4054(14)
    Unknown4045('mgef_232ripper_dust', 100)

@State
def mgef_232_ripper():

    def upon_IMMEDIATE():
        Unknown4011(2)
    sprite('null', 30)
    physicsXImpulse(60000)
    physicsYImpulse(40000)
    Unknown1108(0)
    Unknown4048(-40000)
    Unknown4054(14)
    Unknown23067('mgef_232ripper')

@State
def mgef232_ground():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown3004(-12)
    sprite('null', 30)
    GFX_2('ef_bousou_ground01')

@State
def PAD_PersonaJ2CEx():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 100000, -50000, 0, 0, 0, 0)
        callSubroutine('PAD_AttackInit')
        AttackLevel_(4)
        Damage(1000)
        AttackP2(75)
        Hitstop(6)
        AirUntechableTime(40)
        GroundedHitstunAnimation(9)
        AirHitstunAnimation(9)
        AirPushbackX(15000)
        AirPushbackY(18000)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        AttackAttributes(1, 0, 0, 0, 0)
        Unknown9016(1)
        DisableAttackRestOfMove()
        callSubroutine('PAD_CheckWarp')
        Unknown23059(1)
        EnableCollision(0)
        Unknown2053(1)

        def upon_78():
            clearUponHandler(78)
            Unknown1017()
            Unknown1022()
            sendToLabel(1)
    sprite('mg232_00', 1)
    if (SLOT_25 < 205000):
        Unknown1000(1865000)
        teleportRelativeX(-205000)
    sprite('mg232_01', 1)
    sprite('mg232_02', 1)
    sprite('mg232_03', 3)
    GFX_0('mgef_232', 0)
    sprite('mg232_04', 1)
    SFX_3('slash_pole_middle')
    SFX_3('airdash_m')
    GFX_0('mgef232_ground', 100)
    Unknown3029(1)
    physicsXImpulse(60000)
    physicsYImpulse(48000)
    RefreshMultihit()
    sprite('mg232_04', 2)
    Unknown23022(0)
    sprite('mg232_05', 3)
    sprite('mg232_06', 3)
    SFX_3('airdash_m')
    sprite('mg232_04', 3)
    SFX_3('airdash_m')
    Unknown1019(50)
    YAccel(50)
    EnableCollision(0)
    Unknown3029(0)
    DisableAttackRestOfMove()
    sprite('mg232_05', 3)
    SFX_3('cloth_m')
    Unknown1019(50)
    YAccel(50)
    sprite('mg232_06', 3)
    Unknown1019(50)
    YAccel(50)
    sprite('mg232_04', 6)
    SFX_3('cloth_m')
    Unknown1019(0)
    YAccel(0)
    sprite('mg232_05', 6)
    sprite('mg232_06', 6)
    SFX_3('cloth_m')
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')
    label(1)
    sprite('keep', 32767)
    enterState('PAD_PersonaJ2CExExe')

@State
def PAD_PersonaJ2CExExe():

    def upon_IMMEDIATE():
        callSubroutine('PAD_AttackInit')
        Unknown2006()
        Unknown11032(200000, -200000, 200000, -200000)
        AttackLevel_(4)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        AttackP1(60)
        AttackP2(100)
        AttackAttributes(1, 0, 0, 0, 0)
        AirHitstunAnimation(11)
        GroundedHitstunAnimation(11)
        AirPushbackX(20000)
        AirPushbackY(-40000)
        AirUntechableTime(40)
        Unknown9016(1)
        DisableAttackRestOfMove()
        EnableCollision(0)
        Unknown2034(1)

        def upon_11():
            clearUponHandler(11)
            clearUponHandler(2)
            SFX_3('damage_hit_mh')
            Unknown1019(-5)
            YAccel(-5)
            setGravity(0)
            sendToLabel(0)

        def upon_LANDING():
            clearUponHandler(11)
            Unknown1084(1)
            ScreenShake(10000, 30000)
            sendToLabel(0)
    sprite('mg232_05', 3)
    DisableAttackRestOfMove()
    Unknown1018()
    Unknown1023()
    Unknown1019(30)
    YAccel(50)
    sprite('mg232_06', 3)
    sprite('mg232_07', 3)
    SFX_3('cloth_m')
    Unknown1019(30)
    YAccel(75)
    setGravity(1500)
    sprite('mg232_08', 3)
    sprite('mg232_11', 3)
    SFX_3('cloth_m')
    sprite('mg232_12', 3)
    sprite('mg232_13', 3)
    sprite('mg232_14', 5)
    RefreshMultihit()
    physicsXImpulse(40000)
    physicsYImpulse(-40000)
    sprite('mg232_15', 60)
    label(0)
    sprite('mg232_16', 6)
    sprite('mg232_17', 6)
    Unknown1084(1)
    sprite('mg232_18', 6)
    sprite('mg232_16', 6)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_PersonaCA():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, -100000, 0, 0, 0, 0, 0)
        callSubroutine('PAD_AttackInit')
        Unknown2006()
        DisableAttackRestOfMove()
        callSubroutine('PAD_ForceWarp')
        Unknown23059(1)
        Unknown23022(1)
    sprite('mg204_50', 2)
    sprite('mg204_51', 2)
    sprite('mg204_52', 2)
    sprite('mg204_53', 1)
    sprite('mg204_55', 3)
    SFX_3('slash_blade_fast')
    GFX_0('mgef_204_zanzou', 100)
    sprite('mg204_56', 4)
    DisableAttackRestOfMove()
    Unknown23029(3, 2012, 0)
    sprite('mg204_55', 4)
    sprite('mg204_56', 4)
    SFX_3('cloth_l')
    sprite('mg204_55', 4)
    sprite('mg204_56', 4)
    sprite('mg204_55', 4)
    sprite('mg204_56', 4)
    sprite('mg204_55', 4)
    sprite('mg204_56', 4)
    sprite('mg204_55', 4)
    sprite('mg204_56', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')
    
@State
def PAD_Persona5AAA():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 250000, 0, 0, 0, 0, 0)
        callSubroutine('PAD_AttackInit')
        Unknown11034(0)
        AttackLevel_(4)
        GroundedHitstunAnimation(11)
        AirHitstunAnimation(11)
        AirUntechableTime(60)
        PushbackX(-4000)
        AirPushbackX(-8000)
        AirPushbackY(20000)
        Unknown9310(5)
        Unknown9016(1)
        Unknown11032(-1, -1, 400000, 0)
        Unknown9266(3)
        callSubroutine('PAD_ForceWarp')
        Unknown23078(1)
        Unknown2054(1)
        Unknown2053(1)
        Unknown23022(1)
        EnableCollision(0)
            
        def upon_78():
            clearUponHandler(78)
            Unknown23029(3, 4012, 0)
            SFX_3('counter_slash_l23')
            sendToLabel(1)

        def upon_LANDING():
            clearUponHandler(2)
            sendToLabel(10)
    sprite('mg430_00', 2)
    sprite('mg430_01', 2)
    sprite('mg430_02', 2)
    sprite('mg430_03', 2)
    sprite('mg430_04', 2)
    sprite('mg430_05', 2)
    sprite('mg430_06', 2)
    sprite('mg430_07ex01', 4)
    SFX_3('slash_sword_slow')
    GFX_0('mgef_430_UBzanzou', 100)
    sprite('mg430_08', 4)
    Unknown23029(3, 2020, 0)
    Unknown23022(0)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')
    label(1)
    sprite('mg430_09', 5)
    sprite('mg430_10', 5)
    sprite('mg430_08', 5)
    sprite('mg430_09', 5)
    sprite('mg430_10', 5)
    sprite('mg430_08', 5)
    Unknown3001(255)
    Unknown3004(-12)
    sprite('mg430_09', 5)
    sprite('mg430_10', 5)
    label(580)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')
    
@State
def PAD_Persona5AAAA():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(22, 100, -100000, 1000000, 0, 0, 0, 0)
        callSubroutine('PAD_SPAttackInit')
        callSubroutine('ExSkillInit')
        Unknown2006()
        AttackLevel_(4)
        Damage(2000)
        AirUntechableTime(60)
        Hitstop(0)
        Unknown11001(12, 12, 12)
        GroundedHitstunAnimation(10)
        AirHitstunAnimation(10)
        AirPushbackX(0)
        AirPushbackY(-200000)
        PushbackX(6000)
        Unknown9190(1)
        Unknown9118(15)
        Unknown9016(1)
        Unknown9266(3)
        Unknown11034(1)
        Unknown30088(1)
        ProjectileDurabilityLvl(0)
        AttackAttributes(1, 0, 0, 0, 0)
        DisableAttackRestOfMove()
        callSubroutine('PAD_ForceWarp')
        EnableCollision(0)
        Unknown2053(1)
        Unknown23022(1)

        def upon_14():
            Unknown21012('jumonjiBigslashTate', 32)
            Unknown21012('jumonjiBigslashSock00', 32)
            Unknown21012('jumonjiBigslashSock01', 32)

        def upon_LANDING():
            sendToLabel(0)
    sprite('mg233_00', 1)
    Unknown3032()
    Unknown3001(0)
    Unknown3029(1)
    sprite('mg233_01', 2)
    Unknown3004(65)
    SFX_3('persona_destroy')
    sprite('mg233_02', 2)
    sprite('mg233_03', 2)
    sprite('mg233_04', 2)
    sprite('mg233_05', 2)
    physicsYImpulse(-5000)
    sprite('mg233_06', 2)
    sprite('mg233_07', 32767)
    physicsYImpulse(-200000)
    GFX_0('jumonjiBigslashTate', 104)
    label(0)
    sprite('mg233_08', 2)
    Unknown1084(1)
    Unknown3029(0)
    ScreenShake(20000, 0)
    SFX_3('ad002')
    sprite('mg233_09', 3)
    GFX_0('jumonjiBigslashSock00', 1)
    RefreshMultihit()
    Unknown23022(0)
    sprite('mg233_10', 5)
    GFX_0('jumonjiBigslashSock01', 100)
    SFX_3('thunder_l')
    sprite('mg233_11', 5)
    Unknown23029(3, 2021, 0)
    sprite('mg233_12', 5)
    sprite('mg233_13', 5)
    sprite('mg233_14', 5)
    sprite('mg233_15', 10)
    sprite('mg233_15', 20)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_PersonaSpecialFastShot():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 100000, 0, 0, 2000000, 0, 2000000)
        callSubroutine('PAD_SPAttackInit')
        Unknown2006()
        DisableAttackRestOfMove()
        callSubroutine('PAD_ForceWarp')
        EnableCollision(0)
        Unknown2053(1)
        Unknown4007(3)

        def upon_14():
            Unknown21012('UltimateLaserJizoku', 32)
            Unknown21012('CreateUltimateLaserMatome', 32)
            Unknown21012('CreateUltimateLaser00', 32)
            Unknown21012('CreateUltimateLaser01', 32)
            Unknown21012('rotationSphere', 32)
            Unknown21012('rotationSphere1', 32)
            Unknown21012('rotationSphere2', 32)
            Unknown21012('rotationSphere3', 32)
            Unknown21012('rotationSphere4', 32)
            Unknown21012('mgef_jiodinetame', 32)
            Unknown21012('mgef_giodineshock', 32)
    sprite('mg205_00', 2)
    GFX_0('mgef_jiodinetame', 0)
    Unknown4007(0)
    sprite('mg205_01', 2)
    sprite('mg205_02', 2)
    sprite('mg205_03', 3)
    SFX_3('ad000')
    sprite('mg205_06', 2)
    sprite('mg205_07', 2)
    sprite('mg205_08', 3)
    Unknown26('mgef_jiodinetame')
    Unknown26('rotationSphere1')
    Unknown26('rotationSphere2')
    Unknown26('rotationSphere3')
    Unknown26('rotationSphere4')
    Unknown26('CreateUltimateLaser02')
    Unknown26('CreateUltimateLaser03')
    Unknown26('CreateUltimateLaser01')
    GFX_0('UltimateLaserJizoku', 0)
    GFX_0('mgef_giodineshock', 0)
    GFX_0('Persona5D_Atk', 100)
    Unknown23029(1, 3021, 0)
    sprite('mg205_09', 3)
    SFX_3('ad001')
    sprite('mg205_10', 3)
    sprite('mg205_08', 4)
    Unknown21012('UltimateLaserJizoku', 32)
    Unknown21012('CreateUltimateLaserMatome', 32)
    Unknown21012('CreateUltimateLaser00', 32)
    Unknown21012('rotationSphere', 32)
    Unknown21012('mgef_giodineshock', 32)
    sprite('mg205_09', 4)
    sprite('mg205_10', 4)
    sprite('mg205_08', 4)
    sprite('mg205_09', 4)
    sprite('mg205_10', 4)
    sprite('mg205_08', 5)
    sprite('mg205_09', 5)
    sprite('mg205_10', 5)
    sprite('mg205_08', 5)
    sprite('mg205_09', 5)
    sprite('mg205_10', 5)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_PersonaSpecialSlowShot():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 100000, 0, 0, 2000000, 0, 2000000)
        callSubroutine('PAD_SPAttackInit')
        callSubroutine('ExSkillInit')
        Unknown2006()
        DisableAttackRestOfMove()
        callSubroutine('PAD_ForceWarp')
        EnableCollision(0)
        Unknown2053(1)
        Unknown4007(3)
        SLOT_10 = 1

        def upon_STATE_END():
            SLOT_10 = 0

        def upon_14():
            Unknown21012('UltimateLaserJizoku', 32)
            Unknown21012('CreateUltimateLaserMatome', 32)
            Unknown21012('CreateUltimateLaser00', 32)
            Unknown21012('CreateUltimateLaser01', 32)
            Unknown21012('rotationSphere', 32)
            Unknown21012('rotationSphere1', 32)
            Unknown21012('rotationSphere2', 32)
            Unknown21012('rotationSphere3', 32)
            Unknown21012('rotationSphere4', 32)
            Unknown21012('mgef_jiodinetame', 32)
            Unknown21012('mgef_giodineshock', 32)
    sprite('mg205_00', 4)
    GFX_0('CreateUltimateLaserMatome', 0)
    Unknown4007(0)
    sprite('mg205_01', 4)
    GFX_0('mgef_jiodinetame', 0)
    sprite('mg205_02', 4)
    sprite('mg205_03', 3)
    SFX_3('ad000')
    sprite('mg205_04', 3)
    sprite('mg205_05', 3)
    sprite('mg205_03', 3)
    sprite('mg205_04', 3)
    sprite('mg205_05', 3)
    sprite('mg205_03', 3)
    SFX_3('ad000')
    sprite('mg205_04', 3)
    sprite('mg205_05', 3)
    sprite('mg205_03', 3)
    sprite('mg205_04', 3)
    sprite('mg205_06', 3)
    sprite('mg205_07', 3)
    sprite('mg205_08', 3)
    Unknown26('mgef_jiodinetame')
    Unknown26('rotationSphere1')
    Unknown26('rotationSphere2')
    Unknown26('rotationSphere3')
    Unknown26('rotationSphere4')
    Unknown26('CreateUltimateLaser02')
    Unknown26('CreateUltimateLaser03')
    Unknown26('CreateUltimateLaser01')
    GFX_0('UltimateLaserJizoku', 0)
    GFX_0('mgef_giodineshock', 0)
    GFX_0('Persona5D_Atk', 100)
    Unknown23029(1, 3022, 0)
    sprite('mg205_09', 3)
    sprite('mg205_10', 3)
    sprite('mg205_08', 3)
    SFX_3('ad001')
    sprite('mg205_09', 3)
    sprite('mg205_10', 3)
    sprite('mg205_08', 3)
    sprite('mg205_09', 3)
    sprite('mg205_10', 3)
    sprite('mg205_08', 3)
    SFX_3('ad001')
    sprite('mg205_09', 3)
    sprite('mg205_10', 3)
    sprite('mg205_08', 3)
    sprite('mg205_09', 3)
    sprite('mg205_10', 3)
    sprite('mg205_08', 3)
    SFX_3('ad001')
    sprite('mg205_09', 3)
    sprite('mg205_10', 3)
    sprite('mg205_08', 3)
    sprite('mg205_09', 3)
    sprite('mg205_10', 3)
    sprite('mg205_08', 3)
    SFX_3('ad001')
    sprite('mg205_08', 4)
    sprite('mg205_09', 4)
    sprite('mg205_10', 4)
    sprite('mg205_08', 4)
    sprite('mg205_09', 4)
    sprite('mg205_10', 4)
    sprite('mg205_08', 5)
    sprite('mg205_09', 5)
    sprite('mg205_10', 5)
    sprite('mg205_08', 5)
    sprite('mg205_09', 5)
    sprite('mg205_10', 5)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def UltimateLaserJizoku():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown3001(32)
        Unknown4003('mgef_jiodine_jizoku.DIG', '')
        callSubroutine('KowarePreset')
        Unknown4010(2)
    sprite('null', 10)
    Unknown3004(32)
    sprite('null', 60)
    sprite('null', 10)
    Unknown3004(-25)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3004(-16)

@State
def CreateUltimateLaserMatome():

    def upon_IMMEDIATE():
        callSubroutine('KowarePreset')
    sprite('null', 35)
    GFX_0('rotationSphere', 0)
    sprite('null', 5)
    GFX_0('CreateUltimateLaser00', 100)
    sprite('null', 2)
    GFX_0('CreateUltimateLaser01', 100)
    sprite('null', 2)
    GFX_0('CreateUltimateLaser02', 100)
    sprite('null', 2)
    GFX_0('CreateUltimateLaser03', 100)
    sprite('null', 2)
    GFX_0('CreateUltimateLaser02', 100)
    sprite('null', 70)
    GFX_0('CreateUltimateLaser03', 100)
    sprite('null', 1)
    Unknown21012('CreateUltimateLaser02', 33)
    Unknown21012('CreateUltimateLaser03', 33)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3032()
    Unknown3004(-16)
    Unknown21012('CreateUltimateLaser02', 33)
    Unknown21012('CreateUltimateLaser03', 33)

@State
def rotationSphere():

    def upon_IMMEDIATE():
        callSubroutine('KowarePreset')
    sprite('null', 10)
    GFX_0('rotationSphere1', 0)
    sprite('null', 10)
    GFX_0('rotationSphere2', 0)
    sprite('null', 10)
    GFX_0('rotationSphere3', 0)
    sprite('null', 10)
    GFX_0('rotationSphere4', 0)
    sprite('null', 40)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3032()
    Unknown3004(-16)

@State
def rotationSphere1():

    def upon_IMMEDIATE():
        Unknown4003('mgef_tamaaround.DIG', '')
        callSubroutine('KowarePreset')
    sprite('null', 40)
    sprite('null', 10)
    Unknown3004(-25)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3032()
    Unknown3004(-16)

@State
def rotationSphere2():

    def upon_IMMEDIATE():
        Unknown4003('mgef_tamaaround.DIG', '')
        callSubroutine('KowarePreset')
    sprite('null', 30)
    sprite('null', 10)
    Unknown3004(-25)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3032()
    Unknown3004(-16)

@State
def rotationSphere3():

    def upon_IMMEDIATE():
        Unknown4003('mgef_tamaaround.DIG', '')
        callSubroutine('KowarePreset')
    sprite('null', 20)
    sprite('null', 10)
    Unknown3004(-25)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3032()
    Unknown3004(-16)

@State
def rotationSphere4():

    def upon_IMMEDIATE():
        Unknown4003('mgef_tamaaround.DIG', '')
        callSubroutine('KowarePreset')
    sprite('null', 10)
    sprite('null', 10)
    Unknown3004(-25)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3032()
    Unknown3004(-16)

@State
def CreateUltimateLaser00():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown3001(0)
        Unknown4003('mgef_jiodine_start00.DIG', '')
        callSubroutine('KowarePreset')
    sprite('null', 10)
    Unknown3004(26)
    sprite('null', 60)
    sprite('null', 20)
    Unknown3004(-12)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3004(-16)

@State
def CreateUltimateLaser01():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4003('mgef_jiodine_start01.DIG', '')
        callSubroutine('KowarePreset')
    sprite('null', 42)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3004(-16)

@State
def CreateUltimateLaser02():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown1011(0, 100000)
        Unknown1070(400, 1000)
        Unknown1062(800, 1000)
        Unknown4003('mgef_jiodine_start02.DIG', '')
        callSubroutine('KowarePreset')
        sendToLabelUpon(33, 1)
    label(0)
    sprite('null', 5)
    Unknown3001(255)
    sprite('null', 5)
    Unknown3001(0)
    gotoLabel(0)
    label(1)
    sprite('null', 1)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3004(-16)

@State
def CreateUltimateLaser03():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown1011(-100000, 0)
        Unknown1070(400, 1000)
        Unknown1062(800, 1000)
        Unknown4003('mgef_jiodine_start02.DIG', '')
        callSubroutine('KowarePreset')
        sendToLabelUpon(33, 1)
    label(0)
    sprite('null', 5)
    Unknown3001(255)
    sprite('null', 5)
    Unknown3001(0)
    gotoLabel(0)
    label(1)
    sprite('null', 1)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3004(-16)

@State
def mgef_jiodinetame():

    def upon_IMMEDIATE():
        callSubroutine('KowarePreset')
    sprite('null', 60)
    GFX_2('mgef_jiodinetame')
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3032()
    Unknown3004(-16)

@State
def mgef_giodineshock():

    def upon_IMMEDIATE():
        callSubroutine('KowarePreset')
    sprite('null', 60)
    GFX_2('mgef_giodineshock')
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3032()
    Unknown3004(-16)

@State
def Persona5D_Atk():

    def upon_IMMEDIATE():
        Unknown2010()
        Unknown4010(2)
        Unknown4007(2)
        Damage(250)
        Unknown11092(1)
        AirUntechableTime(47)
        Unknown9178(1)
        WallbounceReboundTime(0)
        Hitstop(0)
        AirHitstunAnimation(12)
        GroundedHitstunAnimation(12)
        AirPushbackX(100000)
        AirPushbackY(1000)
        Unknown30056(130000, 60, 60)
        YImpluseBeforeWallbounce(0)
        PushbackX(8000)
        Unknown9021(1)
        Unknown9266(3)
        Unknown11057(800)
        Unknown2037(24)

        def upon_ON_HIT_OR_BLOCK():
            if SLOT_51:
                SLOT_51 = 0
            else:
                SLOT_51 = 1

        def upon_43():
            if (SLOT_48 == 3021):
                AttackLevel_(3)
                Damage(700)
                AttackP2(80)
                Unknown2037(6)
            if (SLOT_48 == 3022):
                AttackLevel_(4)
                Damage(250)
                AttackP2(75)
                Unknown2037(24)
                Unknown9362(1)
                Unknown9364(40)
                PushbackX(19800)
    sprite('null', 1)
    label(1)
    sprite('dmy_Persona5D_Atk', 3)
    RefreshMultihit()
    Unknown2038(-1)
    if (not (SLOT_2 <= 0)):
        gotoLabel(1)

@State
def PAD_PersonaSpecialSearchShot():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(22, 100, -100000, 1000000, 0, 0, 0, 0)
        callSubroutine('PAD_SPAttackInit')
        callSubroutine('ExSkillInit')
        Unknown2006()
        AttackLevel_(4)
        Damage(2500)
        AirUntechableTime(60)
        Hitstop(0)
        Unknown11001(12, 12, 12)
        GroundedHitstunAnimation(10)
        AirHitstunAnimation(10)
        AirPushbackX(0)
        AirPushbackY(-200000)
        PushbackX(6000)
        Unknown9190(1)
        Unknown9118(5)
        Unknown9120(15)
        Unknown9016(1)
        Unknown9266(3)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        AttackAttributes(1, 0, 0, 0, 0)
        DisableAttackRestOfMove()
        callSubroutine('PAD_ForceWarp')
        EnableCollision(0)
        Unknown2053(1)
        Unknown23022(1)

        def upon_14():
            Unknown21012('jumonjiBigslashTate', 32)
            Unknown21012('jumonjiBigslashSock00', 32)
            Unknown21012('jumonjiBigslashSock01', 32)

        def upon_LANDING():
            sendToLabel(0)
    sprite('mg233_00', 1)
    Unknown3032()
    Unknown3001(0)
    Unknown3029(1)
    sprite('mg233_01', 2)
    Unknown3004(65)
    SFX_3('persona_destroy')
    sprite('mg233_02', 2)
    sprite('mg233_03', 2)
    sprite('mg233_04', 2)
    sprite('mg233_05', 2)
    physicsYImpulse(-5000)
    sprite('mg233_06', 2)
    sprite('mg233_07', 32767)
    physicsYImpulse(-200000)
    GFX_0('jumonjiBigslashTate', 104)
    label(0)
    sprite('mg233_08', 2)
    Unknown1084(1)
    Unknown3029(0)
    ScreenShake(20000, 0)
    SFX_3('ad002')
    sprite('mg233_09', 3)
    GFX_0('jumonjiBigslashSock00', 1)
    RefreshMultihit()
    Unknown23022(0)
    sprite('mg233_10', 5)
    GFX_0('jumonjiBigslashSock01', 100)
    SFX_3('thunder_l')
    sprite('mg233_11', 5)
    Unknown23029(3, 3511, 0)
    sprite('mg233_12', 5)
    sprite('mg233_13', 5)
    sprite('mg233_14', 5)
    sprite('mg233_15', 10)
    sprite('mg233_15', 20)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def jumonjiBigslashTate():

    def upon_IMMEDIATE():
        Unknown4003('mgef_jumonji_slash00.DIG', '')
        Unknown23015(2)
        Unknown3032()
        Unknown1072(90000)
        teleportRelativeX(65000)
        Unknown1064(0)
        Unknown3001(0)
        callSubroutine('KowarePreset')
    sprite('null', 20)
    Unknown3004(12)
    Unknown1067(65)
    sprite('null', 15)
    Unknown1067(0)
    sprite('null', 10)
    Unknown1067(-80)
    Unknown3004(-26)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3004(-16)

@State
def jumonjiBigslashSock00():

    def upon_IMMEDIATE():
        Unknown4003('mgef_jumonji_slash01.DIG', '')
        Unknown23015(2)
        Unknown3032()
        Unknown1064(925)
        Unknown1056(700)
        Unknown3001(150)
        callSubroutine('KowarePreset')
    sprite('null', 10)
    GFX_0('jumonji_ground', 100)
    sprite('null', 10)
    Unknown3004(-26)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3004(-16)

@State
def jumonji_ground():

    def upon_IMMEDIATE():
        teleportRelativeX(65000)
        GFX_2('mgef_jumonji')
    sprite('null', 8)
    GFX_1('mgef_jumonji_sub', 100)
    sprite('null', 8)
    GFX_1('mgef_jumonji_sub', 100)
    sprite('null', 8)
    GFX_1('mgef_jumonji_sub', 100)
    sprite('null', 8)
    GFX_1('mgef_jumonji_sub', 100)
    Unknown3004(-16)
    sprite('null', 8)
    GFX_1('mgef_jumonji_sub', 100)

@State
def jumonjiBigslashSock01():

    def upon_IMMEDIATE():
        Unknown4003('mgef_jumonji_slash02.DIG', '')
        Unknown23015(2)
        Unknown3032()
        Unknown1064(925)
        Unknown1056(700)
        Unknown3001(255)
        callSubroutine('KowarePreset')
    sprite('null', 15)
    sprite('null', 20)
    Unknown1059(20)
    Unknown3004(-12)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3004(-16)

@State
def PAD_PersonaSpecialAntiAirFast():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 100000, 0, 0, 2000000, 0, 0)
        callSubroutine('PAD_SPAttackInit')
        Unknown2006()
        AttackLevel_(4)
        Damage(1200)
        Hitstop(10)
        AttackP2(80)
        Unknown11092(1)
        Unknown11056(0)
        AirPushbackY(-40000)
        AirHitstunAnimation(11)
        GroundedHitstunAnimation(11)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        AirUntechableTime(30)
        Unknown9310(1)
        Unknown9016(1)
        callSubroutine('PAD_ForceWarp')
        Unknown23078(1)
        EnableCollision(0)
        Unknown2053(1)
        Unknown23059(1)
        if Unknown23145('PAD_Persona5C'):
            teleportRelativeX(50000)
        if Unknown23145('PAD_Persona5CC'):
            teleportRelativeX(100000)
    sprite('mg432_00', 3)
    sprite('mg432_01', 3)
    sprite('mg432_02', 3)
    sprite('mg432_03', 3)
    teleportRelativeX(100000)
    SFX_3('hit_m_slow')
    sprite('mg432_04ex01', 3)
    Unknown8000(0, 1, 1)
    ScreenShake(10000, 0)
    sprite('mg432_05', 3)
    sprite('mg432_06', 6)
    sprite('mg432_07', 2)
    sprite('mg432_08', 2)
    GFX_0('mgef_432', 0)
    sprite('mg432_09', 2)
    SFX_3('slash_blade_slow')
    SFX_3('blaze_long')
    sprite('mg432_10', 3)
    RefreshMultihit()
    AttackLevel_(5)
    Damage(1800)
    Hitstop(0)
    Unknown11001(13, 13, 13)
    AirUntechableTime(55)
    GroundedHitstunAnimation(10)
    AirHitstunAnimation(10)
    AirPushbackX(15000)
    AirPushbackY(50000)
    AttackAttributes(0, 0, 0, 1, 0)
    Unknown11034(0)
    ProjectileDurabilityLvl(1)
    Unknown9311()
    Unknown11084(1)
    Unknown9021(1)
    Unknown9266(1)

    def upon_12():
        SLOT_31 = 1
        Unknown23029(3, 3017, 0)
    sprite('mg432_11', 4)
    GFX_0('vr_mg432_zanzou2', 100)
    Unknown23029(3, 3011, 0)
    sprite('mg432_12', 4)
    sprite('mg432_13', 4)
    sprite('mg432_14', 4)
    sprite('mg432_12', 4)
    sprite('mg432_13', 4)
    sprite('mg432_14', 4)
    sprite('mg432_12', 4)
    sprite('mg432_13', 4)
    sprite('mg432_14', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def mgef_432():

    def upon_IMMEDIATE():
        teleportRelativeX(32000)
    sprite('null', 2)
    sprite('null', 60)
    Unknown4049(1350)
    Unknown4045('mgef_432aura', 100)

@State
def vr_mg432_zanzou2():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown3032()
        Unknown4061(1)
        Unknown2019(-100)
        Unknown2054(1)
        Unknown21010(1)
    sprite('vr_mg432_11', 4)
    Unknown3001(255)
    Unknown3004(-63)

@Subroutine
def AntiAirSlowCreateAura():
    GFX_0('mgef_434_aura', 100)
    GFX_0('mgef_434_aura_bk', 103)
    GFX_0('mgef_434_aura_sub', 0)
    GFX_0('mgef_434_aura_sub', 1)
    GFX_0('mgef_434_aura_sub', 2)
    GFX_0('mgef_434_aura_sub', 3)
    GFX_0('mgef_434_aura_sub', 4)
    GFX_0('mgef_434_aura_sub', 5)
    GFX_0('mgef_434_aura_sub', 6)
    GFX_0('mgef_434_aura_sub', 7)
    GFX_0('mgef_434_aura_sub', 8)
    GFX_0('mgef_434_aura_sub', 9)
    GFX_0('mgef_434_aura_sub', 10)

@Subroutine
def AntiAirSlowDeleteAura():
    Unknown26('mgef_434_aura')
    Unknown26('mgef_434_aura_bk')
    Unknown26('mgef_434_aura_sub')

@Subroutine
def AntiAirSlowCreateTornado():
    GFX_0('mgef_434SE', 100)
    GFX_0('mgef_434_tornado', 100)

@Subroutine
def AntiAirSlowDeleteTornado():
    Unknown21012('mgef_434SE', 32)
    Unknown21012('mgef_434_tornado', 32)
    Unknown21012('mgef_434_tornado2', 32)
    Unknown21012('mgef_434_tornado3', 32)

@State
def PAD_PersonaSpecialAntiAirSlow():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 0, 0, 0, 2000000, 0, 0)
        callSubroutine('PAD_SPAttackInit')
        Unknown2006()
        AttackLevel_(5)
        Damage(350)
        AttackP2(80)
        Unknown11092(1)
        AirPushbackX(500)
        Unknown30055(50000, 80, 0)
        AirPushbackY(120000)
        YImpluseBeforeWallbounce(3000)
        AirUntechableTime(1000)
        AirHitstunAnimation(10)
        GroundedHitstunAnimation(10)
        Unknown9310(1)
        Unknown11057(600)
        Unknown11055(1)
        Unknown9021(1)
        Unknown11084(1)
        Unknown11034(0)
        ProjectileDurabilityLvl(1)
        AttackAttributes(0, 0, 0, 1, 0)
        callSubroutine('PAD_ForceWarp')
        Unknown4009(3)
        Unknown2053(1)

        def upon_12():
            clearUponHandler(12)
            Unknown23029(3, 3014, 0)
            sendToLabel(1)

        def upon_STATE_END():
            callSubroutine('AntiAirSlowDeleteAura')
            callSubroutine('AntiAirSlowDeleteTornado')
            Unknown21012('mgef_434_tornado4', 32)
    sprite('mg434_00', 3)
    SFX_3('ad017')
    callSubroutine('AntiAirSlowCreateAura')
    sprite('mg434_01', 3)
    sprite('mg434_02', 3)
    sprite('mg434_00', 3)
    sprite('mg434_01', 3)
    sprite('mg434_02', 3)
    sprite('mg434_00', 3)
    sprite('mg434_01', 3)
    sprite('mg434_02', 3)
    sprite('mg434_00', 3)
    sprite('mg434_01', 3)
    sprite('mg434_03', 3)
    SFX_3('ad016')
    callSubroutine('AntiAirSlowDeleteAura')
    callSubroutine('AntiAirSlowCreateTornado')
    sprite('mg434_04', 3)
    sprite('mg434_05', 3)
    RefreshMultihit()
    sprite('mg434_06', 3)
    RefreshMultihit()
    sprite('mg434_07', 3)
    RefreshMultihit()
    sprite('mg434_08', 3)
    RefreshMultihit()
    sprite('mg434_06', 3)
    RefreshMultihit()
    sprite('mg434_07', 3)
    RefreshMultihit()
    sprite('mg434_08', 3)
    RefreshMultihit()
    sprite('mg434_06', 4)
    DisableAttackRestOfMove()
    Unknown23029(3, 3012, 0)
    sprite('mg434_07', 6)
    callSubroutine('AntiAirSlowDeleteTornado')
    sprite('mg434_08', 6)
    sprite('mg434_06', 6)
    Unknown21012('mgef_434_tornado4', 32)
    sprite('mg434_07', 6)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')
    label(1)
    sprite('mg434_05', 3)

    def upon_12():
        ScreenShake(10000, 30000)
    RefreshMultihit()
    sprite('mg434_06', 3)
    RefreshMultihit()
    sprite('mg434_07', 3)
    RefreshMultihit()
    sprite('mg434_08', 3)
    RefreshMultihit()
    sprite('mg434_06', 3)
    RefreshMultihit()
    sprite('mg434_07', 3)
    RefreshMultihit()
    sprite('mg434_08', 3)
    RefreshMultihit()
    sprite('mg434_06', 3)
    RefreshMultihit()
    sprite('mg434_07', 3)
    RefreshMultihit()
    sprite('mg434_08', 3)
    RefreshMultihit()
    sprite('mg434_06', 3)
    RefreshMultihit()
    sprite('mg434_07', 6)
    DisableAttackRestOfMove()
    Unknown23029(3, 3012, 0)
    sprite('mg434_08', 6)
    sprite('mg434_06', 6)
    sprite('mg434_07', 6)
    sprite('mg434_08', 6)
    sprite('mg434_06', 6)
    sprite('mg434_07', 6)
    sprite('mg434_08', 6)
    sprite('mg434_06', 6)
    callSubroutine('AntiAirSlowDeleteTornado')
    Unknown4009(0)
    sprite('mg434_07', 6)
    sprite('mg434_08', 6)
    sprite('mg434_06', 6)
    sprite('mg434_07', 6)
    sprite('mg434_08', 6)
    sprite('mg434_06', 6)
    Unknown21012('mgef_434_tornado4', 32)
    sprite('mg434_07', 6)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def mgef_434_aura():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown2054(1)
        Unknown21010(1)
    label(0)
    sprite('null', 4)
    Unknown4054(14)
    Unknown4045('mgef_434aura', 100)
    gotoLabel(0)

@State
def mgef_434_aura_bk():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown2054(1)
        Unknown21010(1)
        Unknown1007(50000)
        Unknown4054(15)
        Unknown23067('mgef_434aura_bk')
    sprite('null', 32767)

@State
def mgef_434_aura_sub():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown2054(1)
        Unknown21010(1)
        Unknown4054(14)
        Unknown23067('mgef_434aura_sub00')
    label(0)
    sprite('null', 4)
    Unknown4054(14)
    Unknown4045('mgef_434aura_sub', 100)
    gotoLabel(0)

@Subroutine
def SpecialAntiAirInit():
    Unknown2054(1)
    Unknown21010(1)
    Unknown3001(0)
    Unknown3004(16)

@State
def mgef_434_tornado():

    def upon_IMMEDIATE():
        Unknown4003('mgef_434tornado.DIG', '')
        Unknown4015()
        Unknown3032()
        Unknown23015(13)
        GFX_0('mgef_434_tornado2', 100)
        GFX_0('mgef_434_tornado3', 100)
        GFX_0('mgef_434_tornado4', 100)
        GFX_2('mgef_434red')
        callSubroutine('SpecialAntiAirInit')
        callSubroutine('KowarePreset')
    label(0)
    sprite('null', 1)
    GFX_1('mgef_434tornado', 100)
    gotoLabel(0)
    label(580)
    sprite('null', 18)
    Unknown3004(-14)

@State
def mgef_434_tornado2():

    def upon_IMMEDIATE():
        Unknown4003('mgef_434tornado2.DIG', '')
        Unknown4015()
        Unknown3032()
        Unknown23015(13)
        callSubroutine('SpecialAntiAirInit')
        callSubroutine('KowarePreset')
    sprite('null', 32767)
    label(580)
    sprite('null', 18)
    Unknown3004(-14)

@State
def mgef_434_tornado3():

    def upon_IMMEDIATE():
        Unknown4003('mgef_434tornado3.DIG', '')
        Unknown4015()
        Unknown3032()
        Unknown23015(14)
        callSubroutine('SpecialAntiAirInit')
        callSubroutine('KowarePreset')
    sprite('null', 32767)
    label(580)
    sprite('null', 18)
    Unknown3004(-14)

@State
def mgef_434_tornado4():

    def upon_IMMEDIATE():
        callSubroutine('SpecialAntiAirInit')
        callSubroutine('KowarePreset')
    label(0)
    sprite('null', 4)
    GFX_1('mgef_434aura2', 100)
    gotoLabel(0)
    label(580)
    loopRest()

@State
def mgef_434SE():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown4009(2)
        sendToLabelUpon(32, 0)
        Unknown2037(100)
    label(1)
    sprite('null', 16)
    SFX_3('ad019')
    Unknown2038(-1)
    loopRest()
    if SLOT_2:
        _gotolabel(1)
    label(0)
    sprite('null', 1)

@State
def PAD_PersonaSpecialThrow():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 100000, 0, 0, 2000000, 0, 0)
        callSubroutine('PAD_DDAttackInit')
        Unknown17011('PAD_PersonaSpecialThrowExe', 2, 0, 0)
        AttackLevel_(4)
        MinimumDamagePct(20)
        AttackP1(100)
        Unknown11092(1)
        Unknown30048(1)
        Unknown11032(-1, -1, 200000, 0)
        Unknown11054(200000)
        Unknown11002(-1)
        Unknown30061(1)
        Unknown11082(1)
        callSubroutine('PAD_CheckWarp')
        Unknown2054(1)
        Unknown23022(1)
        Unknown2034(1)
        Unknown30031(0)

        def upon_12():
            Unknown23029(3, 3015, 0)
        SLOT_10 = 1

        def upon_STATE_END():
            SLOT_10 = 0
    sprite('mg433_00', 3)
    if (SLOT_25 < 205000):
        Unknown1000(1865000)
        teleportRelativeX(-205000)
    Unknown3032()
    Unknown3001(0)
    Unknown3004(28)
    sprite('mg433_01', 3)
    sprite('mg433_02', 3)
    Unknown3001(255)
    Unknown3004(0)
    sprite('mg433_03', 3)
    SFX_3('cut_l')
    sprite('mg433_04', 2)
    GFX_0('mgef_433_slash', 100)
    sprite('mg433_05', 3)
    RefreshMultihit()
    sprite('mg433_06', 5)
    StartMultihit()
    sprite('mg433_07', 5)
    sprite('mg433_08', 5)
    sprite('mg433_09', 5)
    sprite('mg433_07', 5)
    sprite('mg433_08', 5)
    sprite('mg433_09', 5)
    sprite('mg433_07', 5)
    sprite('mg433_08', 5)
    sprite('mg433_09', 5)
    sprite('mg433_08', 5)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def mgef_433_slash():

    def upon_IMMEDIATE():
        Unknown4011(2)
        Unknown4003('mgef_433slash.DIG', '')
        Unknown4015()
        Unknown3032()
        teleportRelativeX(75000)
        Unknown1007(-75000)
        Unknown2054(1)
    sprite('null', 14)
    GFX_0('mgef_433_slash_sub', 100)
    GFX_0('mgef_433_slash_bg', 100)

@State
def mgef_433_slash_sub():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown2054(1)
    sprite('null', 3)
    teleportRelativeX(192000)
    Unknown1007(576000)
    Unknown4054(12)
    Unknown4045('mgef_433slash', 100)
    sprite('null', 3)
    teleportRelativeX(-64000)
    Unknown1007(-350000)
    Unknown4054(12)
    Unknown4045('mgef_433slash', 100)

@State
def mgef_433_slash_bg():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown2054(1)
        teleportRelativeX(225000)
        Unknown1007(640000)
    sprite('null', 5)
    sprite('null', 1)
    Unknown4048(17000)
    Unknown4054(14)
    Unknown4045('mgef_433slash_bg', 100)

@State
def PAD_PersonaSpecialThrowGrip():

    def upon_IMMEDIATE():
        callSubroutine('PAD_SPAttackInit')
        Unknown17011('PAD_PersonaSpecialThrowExe', 2, 0, 0)
        Unknown2006()
        AttackLevel_(4)
        AttackP1(100)
        Unknown30048(1)
        Unknown11032(-1, -1, -1, -1)
        Unknown11054(-1)
        Unknown11002(-1)
        Unknown30061(1)
        Unknown11082(1)
        callSubroutine('PAD_CheckWarp')
        Unknown2034(1)
        Unknown30031(0)
    sprite('mg433_05', 3)
    RefreshMultihit()
    sprite('mg433_06', 5)
    StartMultihit()
    Unknown23029(3, 3013, 0)
    sprite('mg433_07', 5)
    sprite('mg433_08', 5)
    sprite('mg433_09', 5)
    sprite('mg433_07', 5)
    sprite('mg433_08', 5)
    sprite('mg433_09', 5)
    sprite('mg433_07', 5)
    sprite('mg433_08', 5)
    sprite('mg433_09', 5)
    sprite('mg433_08', 5)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_PersonaSpecialThrowExe():

    def upon_IMMEDIATE():
        callSubroutine('PAD_SPAttackInit')
        Unknown17012(2, 1, 0)
        Unknown2006()
        AttackLevel_(4)
        Damage(1000)
        MinimumDamagePct(20)
        AttackP1(100)
        AttackP2(100)
        Unknown11092(1)
        Hitstop(6)
        AirUntechableTime(50)
        Unknown9310(60)
        GroundedHitstunAnimation(11)
        AirHitstunAnimation(11)
        AirPushbackX(0)
        AirPushbackY(-30000)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        Unknown11064(1)
        Unknown11068(1)
        Unknown11069('PAD_PersonaSpecialThrowExe')
        Unknown30031(0)
        Unknown23022(1)
    sprite('mg433_05', 2)
    StartMultihit()
    Unknown5000(17, 0)
    Unknown5001(0, 4, 4, 0, 0)
    Unknown2018(1, 80)
    sprite('mg433_06', 10)
    RefreshMultihit()
    Unknown5000(29, 0)
    Unknown5001(0, 4, 4, 0, 0)
    Unknown8003(0, 1, 1)
    ScreenShake(20000, 0)
    sprite('mg433_07', 4)
    sprite('mg433_08', 4)
    sprite('mg433_09', 4)
    sprite('mg433_10', 5)
    sprite('mg433_11', 5)
    sprite('mg433_12', 5)
    sprite('mg433_13', 5)
    SFX_3('slash_blade_slow')
    sprite('mg433_14', 5)
    SFX_3('damage_slash_m')
    SFX_3('counter_slash_l23')
    ScreenShake(20000, 0)
    AttackLevel_(4)
    Damage(1500)
    Hitstop(12)
    Unknown9016(1)
    RefreshMultihit()
    GFX_0('mgef_433_stab', -1)
    sprite('mg433_15', 5)
    sprite('mg433_12', 5)
    sprite('mg433_13', 5)
    SFX_3('slash_blade_slow')
    sprite('mg433_14', 5)
    SFX_3('damage_slash_m')
    SFX_3('counter_slash_l23')
    ScreenShake(20000, 0)
    RefreshMultihit()
    GFX_0('mgef_433_stab', -1)
    sprite('mg433_15', 6)
    sprite('mg433_12', 6)
    sprite('mg433_16', 6)
    sprite('mg433_17', 6)
    sprite('mg433_18', 6)
    SFX_3('slash_blade_slow')
    sprite('mg433_19', 2)
    sprite('mg433_19', 3)
    sprite('mg433_20', 6)
    SFX_3('damage_slash_mh')
    SFX_3('counter_slash_l45')
    SFX_3('counter_slash_l45')
    SFX_3('counter_slash_l45')
    ScreenShake(60000, 60000)
    AttackLevel_(5)
    Damage(3000)
    MinimumDamagePct(25)
    Hitstop(17)
    Unknown9310(0)
    GroundedHitstunAnimation(1)
    AirHitstunAnimation(1)
    AirPushbackX(10000)
    AirPushbackY(50000)
    AirUntechableTime(60)
    Unknown11064(0)
    RefreshMultihit()
    Unknown11069('')
    GFX_0('mgef_433_stab', -1)
    Unknown23029(3, 3016, 0)
    sprite('mg433_21', 6)
    sprite('mg433_22', 6)
    sprite('mg433_23', 6)
    sprite('mg433_21', 6)
    sprite('mg433_22', 6)
    sprite('mg433_23', 6)
    sprite('mg433_21', 6)
    sprite('mg433_22', 6)
    sprite('mg433_23', 6)
    sprite('mg433_21', 6)
    sprite('mg433_22', 8)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def mgef_433_stab():

    def upon_IMMEDIATE():
        Unknown1086(22)
        teleportRelativeY(0)
    sprite('null', 1)
    GFX_1('mgef_433stab', 100)

@State
def PAD_PersonaAntiAirPSkill():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 120000, 0, 0, 2000000, 0, 0)
        callSubroutine('PAD_SPAttackInit')
        Unknown2006()
        AttackLevel_(4)
        Damage(1800)
        AirUntechableTime(60)
        GroundedHitstunAnimation(10)
        AirHitstunAnimation(10)
        AirPushbackX(2500)
        AirPushbackY(45000)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        Unknown9016(1)
        callSubroutine('PAD_ForceWarp')
        Unknown23078(1)
        EnableCollision(0)
        Unknown2034(1)
        Unknown2053(1)
        Unknown23059(1)
        callSubroutine('PartnerSkillInit')
    sprite('mg432_05', 3)
    sprite('mg432_06', 3)
    sprite('mg432_07', 2)
    sprite('mg432_08', 2)
    sprite('mg432_09', 2)
    SFX_3('slash_blade_slow')
    sprite('mg432_10', 3)
    RefreshMultihit()
    sprite('mg432_11', 2)
    GFX_0('vr_mg432_zanzou2', 100)
    Unknown23029(3, 2511, 0)
    sprite('mg432_12', 4)
    sprite('mg432_13', 4)
    sprite('mg432_14', 4)
    sprite('mg432_12', 4)
    sprite('mg432_13', 4)
    sprite('mg432_14', 4)
    sprite('mg432_12', 4)
    sprite('mg432_13', 4)
    sprite('mg432_14', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_PersonaSearchShotPSkill():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(22, 100, -100000, 1000000, 0, 0, 0, 0)
        callSubroutine('PAD_SPAttackInit')
        callSubroutine('PartnerSkillInit')
        Unknown2006()
        AttackLevel_(4)
        Damage(2300)
        AirUntechableTime(30)
        Unknown9310(1)
        Hitstop(0)
        Unknown11001(12, 12, 12)
        GroundedHitstunAnimation(11)
        AirHitstunAnimation(11)
        AirPushbackY(-50000)
        AirPushbackX(0)
        PushbackX(6000)
        Unknown9016(1)
        Unknown9266(3)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        AttackAttributes(1, 0, 0, 0, 0)
        DisableAttackRestOfMove()
        callSubroutine('PAD_ForceWarp')
        EnableCollision(0)
        Unknown2053(1)

        def upon_14():
            Unknown21012('jumonjiBigslashTate', 32)
            Unknown21012('jumonjiBigslashSock00', 32)
            Unknown21012('jumonjiBigslashSock01', 32)

        def upon_LANDING():
            sendToLabel(0)
        callSubroutine('PartnerSkillInit')
    sprite('mg233_00', 12)
    Unknown3032()
    Unknown3001(0)
    Unknown3029(1)
    sprite('mg233_01', 2)
    Unknown1086(22)
    teleportRelativeY(1000000)
    Unknown3004(65)
    SFX_3('persona_destroy')
    sprite('mg233_02', 2)
    sprite('mg233_03', 2)
    sprite('mg233_04', 2)
    sprite('mg233_02', 2)
    sprite('mg233_05', 4)
    physicsYImpulse(-5000)
    sprite('mg233_06', 4)
    sprite('mg233_07', 32767)
    physicsYImpulse(-200000)
    GFX_0('jumonjiBigslashTate', 104)
    label(0)
    sprite('mg233_08', 2)
    Unknown1084(1)
    Unknown3029(0)
    ScreenShake(20000, 0)
    SFX_3('ad002')
    sprite('mg233_09', 3)
    GFX_0('jumonjiBigslashSock00', 1)
    RefreshMultihit()
    sprite('mg233_10', 5)
    GFX_0('jumonjiBigslashSock01', 100)
    SFX_3('thunder_l')
    sprite('mg233_11', 5)
    Unknown23029(3, 2512, 0)
    sprite('mg233_12', 5)
    sprite('mg233_13', 5)
    sprite('mg233_14', 5)
    sprite('mg233_15', 10)
    sprite('mg233_15', 20)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_PersonaUltimateAssault():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 0, 0, 0, 0, 0, 0)
        callSubroutine('PAD_DDAttackInit')
        Unknown23056()
        callSubroutine('PAD_ForceWarp')
        Unknown23066(1)
        Unknown2054(1)
        Unknown2053(1)
        Unknown23022(1)
        Unknown3032()
        Unknown3001(0)
        Unknown3004(10)
        Unknown23032(50)
        Unknown23033(80)
    sprite('mg430_00', 4)
    sprite('mg430_01', 4)
    sprite('mg430_02', 4)
    sprite('mg430_00', 4)
    sprite('mg430_01', 4)
    sprite('mg430_02', 4)
    sprite('mg430_03', 4)
    sprite('mg430_04', 21)
    sprite('mg430_05', 2)
    sprite('mg430_06', 2)
    sprite('mg430_07', 4)
    SFX_3('slash_sword_slow')
    GFX_0('mgef_430_zanzou', 100)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    GFX_0('mgef_430', 100)
    GFX_0('mgef_430SE', 100)
    GFX_0('UltimateAssault_atk', 100)
    if (SLOT_60 == 1):
        Unknown2037(1)
        Unknown23029(1, 4021, 0)
    if (SLOT_60 == 2):
        Unknown23029(1, 4022, 0)
    if (SLOT_60 == 3):
        Unknown2037(1)
        Unknown23029(1, 4023, 0)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    if SLOT_2:
        GFX_0('mgef_430', 100)
        GFX_0('mgef_430SE', 100)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    if SLOT_2:
        GFX_0('mgef_430', 100)
        GFX_0('mgef_430SE', 100)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 6)
    if SLOT_2:
        GFX_0('mgef_430', 100)
        GFX_0('mgef_430SE', 100)
    sprite('mg430_08', 6)
    sprite('mg430_09', 6)
    sprite('mg430_10', 6)
    sprite('mg430_08', 6)
    sprite('mg430_09', 6)
    sprite('mg430_10', 6)
    sprite('mg430_08', 6)
    sprite('mg430_09', 6)
    sprite('mg430_10', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def UltimateAssault_atk():

    def upon_IMMEDIATE():
        Unknown2011()
        Unknown23056()
        Unknown4010(2)
        Unknown4009(2)
        AttackLevel_(3)
        Damage(930)
        MinimumDamagePct(12)
        AttackP2(60)
        Unknown11092(1)
        GroundedHitstunAnimation(9)
        AirHitstunAnimation(9)
        AirPushbackX(2000)
        AirPushbackY(20000)
        PushbackX(0)
        AirUntechableTime(60)
        Unknown11034(2)
        ProjectileDurabilityLvl(0)
        Hitstop(0)
        Unknown11001(12, 12, 12)
        Unknown11084(1)
        Unknown9016(1)
        Unknown11057(750)
        Unknown1096(25000)
        Unknown2037(8)

        def upon_11():
            clearUponHandler(11)
            sendToLabel(1)

        def upon_43():
            if (SLOT_48 == 4021):
                Unknown30070('UltimateAssaultSP_atk')
                Damage(500)
                Unknown11110(80)
                MinimumDamagePct(10)
                Unknown2037(23)

                def upon_11():
                    clearUponHandler(11)
                    sendToLabel(2)
            if (SLOT_48 == 4022):
                Unknown30070('UltimateAssaultDUO_atk')
                Damage(200)
                MinimumDamagePct(100)
                AttackP1(100)
                AttackP2(100)
                Unknown2037(8)

                def upon_11():
                    clearUponHandler(11)
                    sendToLabel(1)
            if (SLOT_48 == 4023):
                Unknown30070('UltimateAssaultDUOSP_atk')
                Damage(100)
                MinimumDamagePct(100)
                AttackP1(100)
                AttackP2(100)
                Unknown2037(23)

                def upon_11():
                    clearUponHandler(11)
                    sendToLabel(2)
    sprite('dmy_UltimateAirAssault_Atk', 3)
    Unknown23151(22, 103)
    RefreshMultihit()
    sprite('null', 1)
    Unknown23029(3, 4011, 0)
    ExitState()
    label(1)
    sprite('dmy_UltimateAirAssault_Atk', 4)
    Unknown23151(22, 103)
    RefreshMultihit()
    Unknown2038(-1)
    if (not (SLOT_2 <= 0)):
        gotoLabel(1)
    else:
        gotoLabel(100)
    label(2)
    sprite('dmy_UltimateAirAssault_Atk', 3)
    Unknown23151(22, 103)
    RefreshMultihit()
    Unknown2038(-1)
    if (not (SLOT_2 <= 10)):
        gotoLabel(2)
    else:
        gotoLabel(3)
    label(3)
    sprite('dmy_UltimateAirAssault_Atk', 2)
    Unknown23151(22, 103)
    RefreshMultihit()
    Unknown2038(-1)
    if (not (SLOT_2 <= 0)):
        gotoLabel(3)
    else:
        gotoLabel(100)
    label(100)
    sprite('dmy_UltimateAirAssault_Atk', 4)
    Unknown23151(22, 103)
    RefreshMultihit()
    Unknown11001(12, 25, 25)
    sprite('null', 1)
    Unknown23029(3, 4011, 0)
    ExitState()

@State
def mgef_430_zanzou():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown3032()
        Unknown4061(1)
        Unknown2054(1)
        Unknown21010(1)
        callSubroutine('PAD_EffectInit')
        GFX_0('mgef_430_slash', 100)
    sprite('vr_mg430_07', 4)

@State
def mgef_430_UBzanzou():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown4009(2)
        Unknown3032()
        Unknown4061(1)
        Unknown2054(1)
        Unknown21010(1)
        callSubroutine('PAD_EffectInit')
        GFX_0('mgef_430_slash', 100)
    sprite('vr_mg430_07', 4)

@State
def mgef_430_slash():

    def upon_IMMEDIATE():
        Unknown2054(1)
        Unknown4061(3)
        Unknown3033()
        callSubroutine('PAD_EffectInit')
    sprite('vr_mg430_00', 30)
    Unknown3004(-19)

@State
def mgef_430():

    def upon_IMMEDIATE():
        Unknown4009(2)
        Unknown6001(1)
        Unknown23032(50)
        Unknown23033(50)
        callSubroutine('KowarePreset')
    sprite('null', 60)
    Unknown23067('mgef_430red')
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3032()
    Unknown3004(-16)

@State
def mgef_430SE():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown2037(7)
    label(1)
    sprite('null', 4)
    SFX_3('cut_m')
    Unknown2038(-1)
    if SLOT_2:
        _gotolabel(1)

@State
def PAD_PersonaUltimateThrow():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 100000, 0, 0, 2000000, 0, 0)
        callSubroutine('PAD_DDAttackInit')
        Unknown17011('PAD_PersonaUltimateThrowExe', 3, 0, 0)
        AttackLevel_(4)
        AttackP1(80)
        Unknown30048(1)
        Unknown11032(-1, -1, 200000, 0)
        Unknown11054(200000)
        Unknown11002(-1)
        Unknown30061(1)
        Unknown11082(1)
        callSubroutine('PAD_CheckWarp')
        Unknown2054(1)
        Unknown23022(1)
        Unknown2034(1)
        Unknown30031(0)

        def upon_12():
            Unknown23029(3, 4018, 0)
        SLOT_10 = 1

        def upon_STATE_END():
            SLOT_10 = 0
    sprite('mg433_00', 3)
    if (SLOT_25 < 205000):
        Unknown1000(1865000)
        teleportRelativeX(-205000)
    Unknown3032()
    Unknown3001(0)
    Unknown3004(28)
    sprite('mg433_01', 3)
    sprite('mg433_02', 3)
    Unknown3001(255)
    Unknown3004(0)
    sprite('mg433_03', 3)
    SFX_3('cut_l')
    sprite('mg433_04', 2)
    GFX_0('mgef_433_slash', 100)
    sprite('mg433_05', 3)
    RefreshMultihit()
    sprite('mg433_06', 5)
    StartMultihit()
    sprite('mg433_07', 5)
    sprite('mg433_08', 5)
    sprite('mg433_09', 5)
    sprite('mg433_07', 5)
    sprite('mg433_08', 5)
    sprite('mg433_09', 5)
    sprite('mg433_07', 5)
    sprite('mg433_08', 5)
    sprite('mg433_09', 5)
    sprite('mg433_08', 5)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_PersonaUltimateThrowExe():

    def upon_IMMEDIATE():
        callSubroutine('PAD_DDAttackInit')
        Unknown17012(2, 1, 0)
        AttackLevel_(4)
        Damage(500)
        MinimumDamagePct(20)
        AttackP1(100)
        AttackP2(100)
        Unknown11092(1)
        Hitstop(6)
        AirUntechableTime(50)
        Unknown9310(60)
        GroundedHitstunAnimation(11)
        AirHitstunAnimation(11)
        AirPushbackX(0)
        AirPushbackY(-30000)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        Unknown11064(1)
        Unknown11069('PAD_PersonaUltimateThrowExe')
        Unknown2054(1)
        Unknown23022(1)
        Unknown30031(0)
        SLOT_10 = 1

        def upon_STATE_END():
            SLOT_10 = 0
    sprite('mg433_05', 2)
    StartMultihit()
    Unknown5000(17, 0)
    Unknown5001(0, 4, 4, 0, 0)
    Unknown2018(1, 80)
    sprite('mg433_06', 10)
    RefreshMultihit()
    Unknown5000(29, 0)
    Unknown5001(0, 4, 4, 0, 0)
    Unknown8003(0, 1, 1)
    ScreenShake(20000, 0)
    sprite('mg433_07', 4)
    sprite('mg433_08', 4)
    sprite('mg433_09', 4)
    sprite('mg433_10', 3)
    sprite('mg433_11', 3)
    sprite('mg433_12', 3)
    sprite('mg433_13', 3)
    SFX_3('slash_blade_slow')
    sprite('mg433_14', 3)
    SFX_3('damage_slash_m')
    SFX_3('counter_slash_l23')
    ScreenShake(20000, 0)
    AttackLevel_(4)
    Damage(1000)
    Hitstop(12)
    Unknown9016(1)
    RefreshMultihit()
    GFX_0('mgef_433_stab', -1)
    sprite('mg433_12', 3)
    sprite('mg433_13', 3)
    SFX_3('slash_blade_slow')
    sprite('mg433_14', 3)
    SFX_3('damage_slash_m')
    SFX_3('counter_slash_l23')
    ScreenShake(20000, 0)
    RefreshMultihit()
    Damage(2000)
    GFX_0('mgef_433_stab', -1)
    sprite('mg433_11', 3)
    sprite('mg432_00', 6)
    sprite('mg432_03', 3)
    SFX_3('hit_m_slow')
    sprite('mg432_04ex01', 3)
    RefreshMultihit()
    SFX_3('damage_slash_m')
    SFX_3('counter_slash_l23')
    GFX_0('mgef_433_stab', -1)
    Unknown8000(0, 1, 1)
    ScreenShake(10000, 0)
    Hitstop(6)
    Unknown11072(3, 0, 100000)

    def upon_12():
        Unknown11069('PAD_PersonaUltimateReGrip')
        enterState('PAD_PersonaUltimateReGrip')
    sprite('mg432_05', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_PersonaUltimateReGrip():

    def upon_IMMEDIATE():
        callSubroutine('PAD_DDAttackInit')
        Unknown17011('PAD_PersonaUltimateFinish', 3, 1, 0)
        Unknown2006()
        Unknown11032(-1, -1, -1, -1)
        Unknown11054(-1)
        AttackP1(100)
        AttackP2(100)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        Unknown30061(1)
        Unknown11002(-1)
        AttackAttributes(0, 0, 0, 0, 1)
        Unknown30048(1)
        DisableAttackRestOfMove()
        Unknown30031(0)
        Unknown23078(1)
        Unknown23059(1)
        Unknown2053(1)
        Unknown23022(1)
        SLOT_10 = 1

        def upon_STATE_END():
            SLOT_10 = 0
    sprite('mg432_04ex01', 3)
    RefreshMultihit()
    sprite('mg432_05', 3)
    sprite('mg432_06', 6)
    sprite('mg432_07', 3)
    sprite('mg432_08', 3)
    sprite('mg432_09', 3)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_PersonaUltimateFinish():

    def upon_IMMEDIATE():
        callSubroutine('PAD_DDAttackInit')
        Unknown17012(2, 1, 0)
        AttackLevel_(4)
        Damage(1200)
        MinimumDamagePct(20)
        AttackP1(100)
        AttackP2(60)
        Unknown11092(1)
        Hitstop(6)
        AirUntechableTime(50)
        Unknown9310(60)
        GroundedHitstunAnimation(11)
        AirHitstunAnimation(11)
        AirPushbackX(0)
        AirPushbackY(-30000)
        AttackAttributes(0, 1, 0, 0, 0)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        Unknown11064(1)
        Unknown11069('PAD_PersonaUltimateFinish')
        Unknown2054(1)
        Unknown23022(1)
        Unknown30031(0)
    sprite('mg432_04ex01', 3)
    StartMultihit()
    Unknown5000(29, 0)
    Unknown5001(0, 1, 1, 0, 0)
    sprite('mg432_05', 3)
    Unknown5000(29, 0)
    Unknown5001(0, 1, 1, 0, 0)
    sprite('mg432_06', 6)
    Unknown5000(29, 0)
    Unknown5001(0, 1, 1, 0, 0)
    sprite('mg432_07', 3)
    Unknown5000(29, 0)
    Unknown5001(0, 1, 1, 0, 0)
    sprite('mg432_08', 3)
    Unknown5000(29, 0)
    Unknown5001(0, 1, 1, 0, 0)
    sprite('mg432_09', 3)
    Unknown5000(29, 0)
    Unknown5001(0, 1, 1, 0, 0)
    sprite('mg204_55', 3)
    StartMultihit()
    Unknown5000(0, 0)
    Unknown5001(2, 1, 1, 0, 0)
    sprite('mg204_00', 3)
    SFX_3('hit_h_fast')
    Unknown5000(25, 0)
    Unknown5001(0, 1, 1, 0, 0)
    sprite('mg204_01', 3)
    Unknown5000(26, 0)
    Unknown5001(0, 1, 1, 0, 0)
    sprite('mg204_02', 6)
    Unknown5000(26, 0)
    Unknown5001(0, 1, 1, 0, 0)
    sprite('mg204_03', 3)
    SFX_3('slash_blade_middle')
    Unknown5000(26, 0)
    Unknown5001(0, 1, 1, 0, 0)
    sprite('mg204_04', 2)
    Unknown5000(26, 0)
    Unknown5001(0, 1, 1, 0, 0)
    sprite('mg204_05', 3)
    SFX_0('damage_hit_h')
    Unknown5000(28, 0)
    Unknown5001(0, 1, 1, 0, 0)
    sprite('mg204_06', 6)
    sprite('mg433_11', 6)
    teleportRelativeX(100000)
    sprite('mg433_12', 6)
    sprite('mg433_16', 9)
    sprite('mg433_17', 9)
    sprite('mg433_18', 9)
    SFX_3('slash_blade_slow')
    sprite('mg433_19', 3)
    sprite('mg433_20', 6)
    SFX_3('damage_slash_mh')
    SFX_3('counter_slash_l45')
    SFX_3('counter_slash_l45')
    SFX_3('counter_slash_l45')
    ScreenShake(60000, 60000)
    AttackLevel_(5)
    Damage(4500)
    MinimumDamagePct(25)
    Hitstop(17)
    Unknown9310(0)
    GroundedHitstunAnimation(1)
    AirHitstunAnimation(1)
    AirPushbackX(10000)
    AirPushbackY(50000)
    AirUntechableTime(60)
    Unknown11064(0)
    RefreshMultihit()
    Unknown11069('')
    GFX_0('mgef_433_stab', -1)
    Unknown23029(3, 4019, 0)
    sprite('mg433_21', 6)
    sprite('mg433_22', 6)
    sprite('mg433_23', 6)
    sprite('mg433_21', 6)
    sprite('mg433_22', 6)
    sprite('mg433_23', 6)
    sprite('mg433_21', 6)
    sprite('mg433_22', 6)
    sprite('mg433_23', 6)
    sprite('mg433_21', 6)
    sprite('mg433_22', 8)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_PersonaUltimateBlade():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 250000, 0, 0, 0, 0, 0)
        callSubroutine('PAD_DDAttackInit')
        Unknown23056()
        AttackLevel_(5)
        Damage(2500)
        AirHitstunAnimation(5)
        MinimumDamagePct(25)
        AttackP2(100)
        hitstun(130)
        Unknown9310(1)
        Unknown11056(3)
        Unknown9016(1)
        Unknown11032(-1, -1, 400000, 0)
        Unknown11064(1)
        Unknown9266(3)
        Unknown11069('PAD_PersonaUltimateBlade')
        Unknown11001(0, 0, 0)
        callSubroutine('PAD_ForceWarp')
        Unknown23078(1)
        Unknown2054(1)
        Unknown2053(1)
        Unknown23022(1)

        def upon_78():
            clearUponHandler(78)
            Unknown23029(3, 4012, 0)
            GFX_0('UltimateBladeShockYoko', 0)
            GFX_0('mgef_430_mist', 100)
            sendToLabel(1)

        def upon_LANDING():
            clearUponHandler(2)
            sendToLabel(10)
    sprite('mg430_00', 3)
    sprite('mg430_01', 3)
    sprite('mg430_02', 3)
    sprite('mg430_03', 3)
    sprite('mg430_04', 6)
    sprite('mg430_05', 2)
    sprite('mg430_06', 2)
    sprite('mg430_07ex01', 4)
    SFX_3('slash_sword_slow')
    GFX_0('mgef_430_UBzanzou', 100)
    sprite('mg430_08', 4)
    Unknown23029(3, 4014, 0)
    Unknown23022(0)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')
    label(1)
    sprite('mg430_09', 5)
    Unknown26('mgef_430_UBzanzou')
    sprite('mg430_10', 5)
    sprite('mg430_08', 5)
    sprite('mg430_09', 5)
    sprite('mg430_10', 5)
    sprite('mg430_08', 5)
    Unknown3001(255)
    Unknown3004(-12)
    sprite('mg430_09', 5)
    sprite('mg430_10', 5)
    sprite('mg233_00', 10)
    Unknown1086(22)
    Unknown1007(1200000)
    Unknown3001(0)
    Unknown3004(0)
    Unknown3029(1)
    Unknown20000(1)
    Unknown20009(1300)
    sprite('mg233_00', 20)
    GFX_0('Cardrotate00', 0)
    sprite('mg233_01', 2)
    Unknown3004(65)
    SFX_3('persona_destroy')
    sprite('mg233_02', 2)
    sprite('mg233_03', 2)
    sprite('mg233_04', 2)
    sprite('mg233_02', 2)
    sprite('mg233_03', 2)
    sprite('mg233_04', 2)
    sprite('mg233_02', 2)
    sprite('mg233_03', 2)
    sprite('mg233_04', 2)
    sprite('mg233_05', 4)
    Unknown20001(1)
    physicsYImpulse(-5000)
    sprite('mg233_06', 4)
    sprite('mg233_07', 32767)
    physicsYImpulse(-200000)
    GFX_0('UltimateBladeShockTate', 104)
    label(10)
    sprite('mg233_08', 2)
    Unknown1084(1)
    Unknown3029(0)
    Unknown20009(1000)
    ScreenShake(20000, 0)
    SFX_3('ad002')
    sprite('mg233_09', 3)
    GFX_0('UltimateBladeShock00', 1)
    RefreshMultihit()
    AttackLevel_(5)
    Damage(4800)
    MinimumDamagePct(30)
    Unknown11064(0)
    AttackP1(80)
    GroundedHitstunAnimation(2)
    AirHitstunAnimation(2)
    Unknown9155()
    Unknown9016(1)
    Unknown9266(3)
    PushbackX(6000)
    AttackP2(60)
    Unknown9156(21)
    Unknown9130(20)
    Unknown9142(300)
    Unknown30048(1)
    Unknown11069('')
    Unknown23029(3, 4013, 0)
    sprite('mg233_10', 5)
    GFX_0('UltimateBladeShock01', 100)
    Unknown20001(0)
    SFX_3('thunder_l')
    sprite('mg233_11', 5)
    sprite('mg233_12', 5)
    Unknown20000(0)
    sprite('mg233_13', 5)
    sprite('mg233_14', 5)
    sprite('mg233_15', 50)
    label(580)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def Cardrotate00():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4010(2)
        Unknown4003('adef_jumonji_cardbrake00.DIG', 'bcef_jumonji_cardbrake00_000.mm')
        Unknown23015(1)
        Unknown1096(2000)
    sprite('null', 15)
    sprite('null', 10)
    Unknown3001(0)
    GFX_0('Cardrotate01', 100)

@State
def Cardrotate01():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4010(2)
        Unknown4003('adef_jumonji_cardbrake01.DIG', 'bcef_jumonji_cardbrake01_000.mm')
        Unknown23015(1)
        Unknown1096(2000)
    sprite('null', 8)
    Unknown23117(-16731137, 8)
    sprite('null', 1)
    GFX_1('mgef_cardcrash_03', 100)

@State
def mgef_430_mist():

    def upon_IMMEDIATE():
        pass
    sprite('null', 15)
    GFX_0('mgef_430_mist_a', 100)
    GFX_0('mgef_430_mist_b', 100)
    sprite('null', 120)

@State
def mgef_430_mist_a():

    def upon_IMMEDIATE():
        Unknown6001(1)

        def upon_3():
            Unknown23057(50)
            Unknown23058(50)
        Unknown2055(32)
    label(0)
    sprite('null', 8)
    GFX_1('mgef_610mist_a', 100)
    gotoLabel(0)

@State
def mgef_430_mist_b():

    def upon_IMMEDIATE():
        teleportRelativeX(31000)
        Unknown1007(5000)
    sprite('null', 1)
    Unknown4054(9)
    Unknown4045('mgef_610mist_b', 100)

@State
def mgef_430_mist_3d():

    def upon_IMMEDIATE():
        Unknown4003('mgef_610mist', '')
        Unknown4015()
        Unknown23015(1)
        Unknown3032()
        Unknown3013(0)
        Unknown3019(0)
        Unknown1056(1100)
        Unknown1064(880)
        Unknown6001(1)

        def upon_3():
            Unknown23032(50)
            Unknown23033(110)
    sprite('null', 16)
    Unknown3001(0)
    Unknown3004(8)
    sprite('null', 40)
    Unknown3001(128)
    Unknown3004(0)
    sprite('null', 128)
    Unknown3004(-2)

@State
def mgef_430_mist_3d_2():

    def upon_IMMEDIATE():
        Unknown4003('mgef_610mist2', '')
        Unknown4015()
        Unknown23015(15)
        Unknown3032()
        Unknown3013(0)
        Unknown3019(0)
        Unknown1056(-600)
        Unknown1064(400)
        Unknown6001(1)

        def upon_3():
            Unknown23032(50)
            Unknown23033(80)
    sprite('null', 16)
    Unknown3001(0)
    Unknown3004(8)
    sprite('null', 92)
    Unknown3001(128)
    Unknown3004(0)
    sprite('null', 64)
    Unknown3004(-4)

@State
def UltimateBladeShockYoko():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown4010(2)
        Unknown4003('mgef_jumonji_slash00.DIG', '')
        Unknown2054(1)
        Unknown1064(0)
        Unknown3001(250)
        Unknown1072(5000)
        Unknown23015(2)
        Unknown1007(110000)
        Unknown23032(160)
        sendToLabelUpon(32, 0)
    sprite('null', 10)
    Unknown1067(120)
    sprite('null', 138)
    Unknown1067(0)
    sprite('null', 10)
    Unknown1067(-100)
    Unknown3004(-26)
    ExitState()
    label(0)
    sprite('null', 9)
    Unknown1067(120)
    sprite('null', 146)
    Unknown1067(0)
    sprite('null', 10)
    Unknown1067(-100)
    Unknown3004(-26)
    ExitState()

@State
def UltimateBladeShockTate():

    def upon_IMMEDIATE():
        Unknown4003('mgef_jumonji_slash00.DIG', '')
        Unknown23015(2)
        Unknown3032()
        Unknown1072(90000)
        teleportRelativeX(65000)
        Unknown1064(0)
        Unknown3001(0)
        Unknown2054(1)
        callSubroutine('KowarePreset')
    sprite('null', 20)
    Unknown3004(12)
    Unknown1067(65)
    sprite('null', 15)
    Unknown1067(0)
    sprite('null', 10)
    Unknown1067(-80)
    Unknown3004(-26)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3004(-16)

@State
def UltimateBladeShock00():

    def upon_IMMEDIATE():
        Unknown4003('mgef_jumonji_slash01.DIG', '')
        Unknown23015(2)
        Unknown3032()
        Unknown1064(925)
        Unknown1056(700)
        Unknown3001(150)
        Unknown2054(1)
        callSubroutine('KowarePreset')
    sprite('null', 10)
    GFX_0('jumonji_ground', 100)
    sprite('null', 10)
    Unknown3004(-26)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3004(-16)

@State
def UltimateBladeShock01():

    def upon_IMMEDIATE():
        Unknown4003('mgef_jumonji_slash02.DIG', '')
        Unknown23015(2)
        Unknown3032()
        Unknown1064(925)
        Unknown1056(700)
        Unknown3001(255)
        Unknown2054(1)
        callSubroutine('KowarePreset')
    sprite('null', 15)
    sprite('null', 20)
    Unknown1059(20)
    Unknown3004(-12)
    ExitState()
    label(580)
    sprite('null', 16)
    Unknown3004(-16)

@State
def PAD_PersonaUltimateBladeSP():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 250000, 0, 0, 0, 0, 0)
        callSubroutine('PAD_DDAttackInit')
        Unknown23056()
        AttackLevel_(5)
        Damage(2500)
        AirHitstunAnimation(5)
        MinimumDamagePct(25)
        AttackP2(100)
        hitstun(100)
        Unknown9310(1)
        Unknown11056(3)
        Unknown9016(1)
        Unknown11032(-1, -1, 400000, 0)
        Unknown11064(1)
        Unknown9266(3)
        Unknown11069('PAD_PersonaUltimateBladeSP')
        Unknown11001(0, 0, 0)
        callSubroutine('PAD_ForceWarp')
        Unknown23078(1)
        Unknown2054(1)
        Unknown2053(1)
        Unknown23022(1)

        def upon_78():
            clearUponHandler(78)
            Unknown23029(3, 4015, 0)
            GFX_0('UltimateBladeShockYoko', 0)
            Unknown21007(1, 32)
            GFX_0('mgef_430_mist', 100)
            sendToLabel(1)

        def upon_LANDING():
            clearUponHandler(2)
            sendToLabel(10)
    sprite('mg430_00', 3)
    sprite('mg430_01', 3)
    sprite('mg430_02', 3)
    sprite('mg430_03', 3)
    sprite('mg430_04', 6)
    sprite('mg430_05', 2)
    sprite('mg430_06', 2)
    sprite('mg430_07ex01', 4)
    SFX_3('slash_sword_slow')
    GFX_0('mgef_430_UBzanzou', 100)
    sprite('mg430_08', 4)
    Unknown23029(3, 4017, 0)
    Unknown23022(0)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')
    label(1)
    sprite('mg430_09', 5)
    Unknown26('mgef_430_UBzanzou')
    sprite('mg430_10', 5)
    sprite('mg430_08', 5)
    Unknown3001(255)
    Unknown3004(-12)
    sprite('mg430_09', 5)
    sprite('mg430_10', 5)
    sprite('null', 27)
    sprite('mg450_06', 4)
    Unknown20000(1)
    Unknown1086(22)
    teleportRelativeX(-150000)
    teleportRelativeY(0)
    Unknown3004(32)
    Unknown2005()
    sprite('mg450_07', 4)
    sprite('mg450_08', 4)
    sprite('mg450_06', 4)
    sprite('mg450_07', 4)
    sprite('mg450_08', 4)
    sprite('mg450_06', 4)
    sprite('mg450_07', 4)
    sprite('mg450_08', 4)
    sprite('mg450_06', 4)
    sprite('mg450_07', 4)
    sprite('mg450_08', 4)
    sprite('mg450_06', 4)
    sprite('mg450_07', 2)
    sprite('mg450_09', 3)
    GFX_0('UltimateBladeShockTate', 104)
    Unknown36(1)
    teleportRelativeX(-150000)
    Unknown35()
    sprite('mg450_10', 3)
    GFX_0('UltimateBladeShock00', 1)
    Unknown36(1)
    teleportRelativeX(-150000)
    Unknown35()
    sprite('mg450_11ex01', 1)
    RefreshMultihit()
    Unknown4009(3)
    Unknown23078(0)
    Damage(230)
    Hitstop(2)
    GroundedHitstunAnimation(1)
    AirPushbackX(3000)
    AirPushbackY(-100000)
    YImpluseBeforeWallbounce(2500)
    PushbackX(6000)
    AirUntechableTime(180)
    Unknown9155()
    Unknown9310(1)
    Unknown9190(1)
    Unknown9118(60)
    Unknown11056(1)
    Unknown30048(1)
    sprite('mg450_11ex01', 1)
    RefreshMultihit()
    sprite('mg450_11ex01', 1)
    RefreshMultihit()
    sprite('mg450_11ex01', 1)
    RefreshMultihit()
    sprite('mg450_12ex01', 1)
    RefreshMultihit()
    sprite('mg450_12ex01', 1)
    RefreshMultihit()
    sprite('mg450_12ex01', 1)
    RefreshMultihit()
    sprite('mg450_12ex01', 1)
    RefreshMultihit()
    Damage(4700)
    Unknown11064(0)
    AttackP2(60)
    Unknown11069('')
    GFX_0('UltimateBladeShock01', 100)
    Unknown36(1)
    teleportRelativeX(-150000)
    Unknown35()
    SFX_3('thunder_l')
    Unknown23029(3, 4016, 0)
    sprite('mg450_12', 3)
    GFX_0('mgef_433_stab', -1)
    sprite('mg450_13', 65)
    StartMultihit()
    sprite('keep', 3)
    StartMultihit()
    Unknown20000(0)
    label(580)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_PersonaUltimateDUO():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 100000, 0, 0, 0, 0, 0)
        callSubroutine('PAD_DDAttackInit')
        Unknown23056()
        AttackLevel_(5)
        Damage(1000)
        AirHitstunAnimation(5)
        MinimumDamagePct(100)
        AttackP1(100)
        AttackP2(100)
        hitstun(130)
        Unknown9310(1)
        Unknown11056(3)
        Unknown9016(1)
        Unknown11032(-1, -1, 400000, 0)
        Unknown11064(1)
        Unknown9266(3)
        Unknown11069('PAD_PersonaUltimateDUO')
        Unknown11001(0, 0, 0)
        callSubroutine('PAD_ForceWarp')
        Unknown23078(1)
        Unknown2054(1)
        Unknown2053(1)
        Unknown23022(1)

        def upon_78():
            clearUponHandler(78)
            Unknown23029(3, 4211, 0)
            GFX_0('UltimateBladeShockYoko', 0)
            GFX_0('mgef_430_mist', 100)
            sendToLabel(1)

        def upon_LANDING():
            clearUponHandler(2)
            sendToLabel(10)
    sprite('mg430_00', 3)
    sprite('mg430_01', 3)
    sprite('mg430_02', 3)
    sprite('mg430_03', 3)
    sprite('mg430_04', 6)
    sprite('mg430_05', 2)
    sprite('mg430_06', 2)
    sprite('mg430_07ex01', 4)
    SFX_3('slash_sword_slow')
    GFX_0('mgef_430_UBzanzou', 100)
    sprite('mg430_08', 4)
    Unknown23029(3, 4213, 0)
    Unknown23022(0)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')
    label(1)
    sprite('mg430_09', 5)
    Unknown26('mgef_430_UBzanzou')
    sprite('mg430_10', 5)
    sprite('mg430_08', 5)
    sprite('mg430_09', 5)
    sprite('mg430_10', 5)
    sprite('mg430_08', 5)
    Unknown3001(255)
    Unknown3004(-12)
    sprite('mg430_09', 5)
    sprite('mg430_10', 5)
    sprite('mg233_00', 10)
    Unknown1086(22)
    Unknown1007(1200000)
    Unknown3001(0)
    Unknown3004(0)
    Unknown3029(1)
    Unknown20000(1)
    Unknown20009(1300)
    sprite('mg233_00', 20)
    GFX_0('Cardrotate00', 0)
    sprite('mg233_01', 2)
    Unknown3004(65)
    SFX_3('persona_destroy')
    sprite('mg233_02', 2)
    sprite('mg233_03', 2)
    sprite('mg233_04', 2)
    sprite('mg233_02', 2)
    sprite('mg233_03', 2)
    sprite('mg233_04', 2)
    sprite('mg233_02', 2)
    sprite('mg233_03', 2)
    sprite('mg233_04', 2)
    sprite('mg233_05', 4)
    Unknown20001(1)
    physicsYImpulse(-5000)
    sprite('mg233_06', 4)
    sprite('mg233_07', 32767)
    physicsYImpulse(-200000)
    GFX_0('UltimateBladeShockTate', 104)
    label(10)
    sprite('mg233_08', 2)
    Unknown1084(1)
    Unknown3029(0)
    Unknown20009(1000)
    ScreenShake(20000, 0)
    SFX_3('ad002')
    sprite('mg233_09', 3)
    GFX_0('UltimateBladeShock00', 1)
    RefreshMultihit()
    AttackLevel_(5)
    Damage(1500)
    Unknown11064(0)
    GroundedHitstunAnimation(2)
    AirHitstunAnimation(2)
    Unknown9155()
    Unknown9016(1)
    Unknown9266(3)
    PushbackX(6000)
    Unknown9156(21)
    Unknown9130(20)
    Unknown9142(300)
    Unknown30048(1)
    Unknown11069('')
    Unknown23029(3, 4212, 0)
    sprite('mg233_10', 5)
    GFX_0('UltimateBladeShock01', 100)
    Unknown20001(0)
    SFX_3('thunder_l')
    sprite('mg233_11', 5)
    sprite('mg233_12', 5)
    Unknown20000(0)
    sprite('mg233_13', 5)
    sprite('mg233_14', 5)
    sprite('mg233_15', 50)
    label(580)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_PersonaUltimateDUOSP():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown23184(3, 100, 100000, 0, 0, 0, 0, 0)
        callSubroutine('PAD_DDAttackInit')
        Unknown23056()
        AttackLevel_(5)
        Damage(1000)
        AirHitstunAnimation(5)
        MinimumDamagePct(100)
        AttackP1(100)
        AttackP2(100)
        hitstun(100)
        Unknown9310(1)
        Unknown11056(3)
        Unknown9016(1)
        Unknown11032(-1, -1, 400000, 0)
        Unknown11064(1)
        Unknown9266(3)
        Unknown11069('PAD_PersonaUltimateDUOSP')
        Unknown11001(0, 0, 0)
        callSubroutine('PAD_ForceWarp')
        Unknown23078(1)
        Unknown2054(1)
        Unknown2053(1)
        Unknown23022(1)

        def upon_78():
            clearUponHandler(78)
            Unknown23029(3, 4214, 0)
            GFX_0('UltimateBladeShockYoko', 0)
            Unknown21007(1, 32)
            GFX_0('mgef_430_mist', 100)
            sendToLabel(1)

        def upon_LANDING():
            clearUponHandler(2)
            sendToLabel(10)
    sprite('mg430_00', 3)
    sprite('mg430_01', 3)
    sprite('mg430_02', 3)
    sprite('mg430_03', 3)
    sprite('mg430_04', 6)
    sprite('mg430_05', 2)
    sprite('mg430_06', 2)
    sprite('mg430_07ex01', 4)
    SFX_3('slash_sword_slow')
    GFX_0('mgef_430_UBzanzou', 100)
    sprite('mg430_08', 4)
    Unknown23029(3, 4216, 0)
    Unknown23022(0)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('mg430_08', 4)
    sprite('mg430_09', 4)
    sprite('mg430_10', 4)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')
    label(1)
    sprite('mg430_09', 5)
    Unknown26('mgef_430_UBzanzou')
    sprite('mg430_10', 5)
    sprite('mg430_08', 5)
    Unknown3001(255)
    Unknown3004(-12)
    sprite('mg430_09', 5)
    sprite('mg430_10', 5)
    sprite('null', 27)
    sprite('mg450_06', 4)
    Unknown20000(1)
    Unknown1086(22)
    teleportRelativeX(-150000)
    teleportRelativeY(0)
    Unknown3004(32)
    Unknown2005()
    sprite('mg450_07', 4)
    sprite('mg450_08', 4)
    sprite('mg450_06', 4)
    sprite('mg450_07', 4)
    sprite('mg450_08', 4)
    sprite('mg450_06', 4)
    sprite('mg450_07', 4)
    sprite('mg450_08', 4)
    sprite('mg450_06', 4)
    sprite('mg450_07', 4)
    sprite('mg450_08', 4)
    sprite('mg450_06', 4)
    sprite('mg450_07', 2)
    sprite('mg450_09', 3)
    GFX_0('UltimateBladeShockTate', 104)
    Unknown36(1)
    teleportRelativeX(-150000)
    Unknown35()
    sprite('mg450_10', 3)
    GFX_0('UltimateBladeShock00', 1)
    Unknown36(1)
    teleportRelativeX(-150000)
    Unknown35()
    sprite('mg450_11ex01', 1)
    RefreshMultihit()
    Unknown4009(3)
    Unknown23078(0)
    Damage(100)
    Unknown11064(0)
    Hitstop(2)
    GroundedHitstunAnimation(1)
    AirPushbackX(3000)
    AirPushbackY(-100000)
    YImpluseBeforeWallbounce(2500)
    PushbackX(6000)
    AirUntechableTime(180)
    Unknown9155()
    Unknown9310(1)
    Unknown9190(1)
    Unknown9118(60)
    Unknown11056(1)
    Unknown30048(1)
    sprite('mg450_11ex01', 1)
    RefreshMultihit()
    sprite('mg450_11ex01', 1)
    RefreshMultihit()
    sprite('mg450_11ex01', 1)
    RefreshMultihit()
    sprite('mg450_12ex01', 1)
    RefreshMultihit()
    sprite('mg450_12ex01', 1)
    RefreshMultihit()
    sprite('mg450_12ex01', 1)
    RefreshMultihit()
    sprite('mg450_12ex01', 1)
    RefreshMultihit()
    Damage(1300)
    Unknown11069('')
    GFX_0('UltimateBladeShock01', 100)
    Unknown36(1)
    teleportRelativeX(-150000)
    Unknown35()
    SFX_3('thunder_l')
    Unknown23029(3, 4215, 0)
    sprite('mg450_12', 3)
    GFX_0('mgef_433_stab', -1)
    sprite('mg450_13', 65)
    StartMultihit()
    sprite('keep', 3)
    StartMultihit()
    Unknown20000(0)
    label(580)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_PersonaAstralHeat():

    def upon_IMMEDIATE():
        Unknown23023()
        Unknown1086(22)
        teleportRelativeX(-350000)
        callSubroutine('PAD_DDAttackInit')
        Unknown2006()
        Unknown17011('PAD_PersonaAstralHeatExe', 6, 1, 0)
        AttackLevel_(4)
        Damage(0)
        Unknown11032(-1, -1, -1, -1)
        Unknown11054(-1)
        Unknown11034(1)
        ProjectileDurabilityLvl(0)
        Unknown30061(1)
        Unknown11002(-1)
        AttackAttributes(0, 0, 0, 0, 1)
        Unknown30048(1)
        DisableAttackRestOfMove()
        Unknown30031(0)
        Unknown23022(1)
        sendToLabelUpon(32, 0)
    sprite('null', 60)
    GFX_0('mgef_450_mg_start', 100)
    GFX_0('Ichigeki_Camera', 100)
    Unknown38(4, 1)
    SFX_3('ad011')
    sprite('mg232_00ex', 4)
    physicsXImpulse(15000)
    SFX_3('blood_l')
    SFX_3('blood_l')
    SFX_3('ad011')
    sprite('mg232_03ex', 4)
    sprite('mg232_04ex', 4)
    RefreshMultihit()
    SFX_3('ad011')
    sprite('mg232_05ex', 3)
    sprite('mg232_06ex', 3)
    physicsXImpulse(0)
    sprite('mg232_07ex', 3)
    ExitState()
    label(0)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def mgef_450_mg_start():

    def upon_IMMEDIATE():
        sendToLabelUpon(32, 10)
        Unknown4007(2)
    sprite('null', 20)
    GFX_2('mgef_450start')
    Unknown3001(0)
    Unknown3004(12)
    sprite('null', 10)
    Unknown3001(255)
    Unknown3004(0)
    GFX_1('mgef_450start_sub', 100)
    GFX_0('mgef_450_mg_start2', 100)
    label(0)
    sprite('null', 10)
    GFX_1('mgef_450start_sub', 100)
    gotoLabel(0)
    label(10)
    sprite('null', 20)
    Unknown3004(-12)

@State
def mgef_450_mg_start2():

    def upon_IMMEDIATE():
        sendToLabelUpon(32, 10)
        Unknown4007(2)
        GFX_1('mgef_450start_sub3', 100)
    label(0)
    sprite('null', 16)
    GFX_1('mgef_450start_sub2', 100)
    gotoLabel(0)
    label(10)
    sprite('null', 20)
    Unknown3004(-12)

@State
def Ichigeki_Camera():

    def upon_IMMEDIATE():

        def upon_43():
            if (SLOT_48 == 4522):
                sendToLabel(1)
            if (SLOT_48 == 4523):
                sendToLabel(2)
    sprite('null', 32767)
    Unknown1086(22)
    Unknown20000(1)
    Unknown20002(1)
    Unknown20003(1)
    label(1)
    sprite('null', 30)
    sprite('null', 170)
    GFX_0('IchigekiPicture', 100)
    sprite('null', 6)
    SystemGFX('SCEF_ShashaIn', 100)
    Unknown36(1)
    Unknown1072(180000)
    physicsYImpulse(140000)
    Unknown35()
    sprite('null', 12)
    Unknown21012('mgef_450_yukaSE', 32)
    Unknown26('mgef_450_yuka')
    SystemGFX('SCEF_FadeBlack12In', 100)
    sprite('null', 20)
    Unknown1000(0)
    sprite('null', 10)
    Unknown23024(3)
    Unknown23029(2, 4502, 0)
    sprite('null', 12)
    Unknown36(22)
    Unknown1000(10000)
    Unknown35()
    Unknown26('SCEF_ShashaIn')
    Unknown26('SCEF_FadeBlack12In')
    SystemGFX('SCEF_FadeBlack12Out', 100)
    sprite('null', 110)
    GFX_0('IchigekiSE', 100)
    sprite('null', 90)
    sprite('null', 32767)
    Unknown23029(2, 4503, 0)
    label(2)
    sprite('null', 10)
    Unknown23024(0)
    sprite('null', 70)
    SFX_3('whitenoize')
    SFX_3('whitenoize')
    SFX_3('whitenoize')
    SFX_3('whitenoize')
    SFX_3('whitenoize')
    SFX_3('whitenoize')
    sprite('null', 120)
    Unknown23029(2, 4504, 0)
    sprite('null', 45)
    SFX_3('ad003')
    Unknown23029(3, 4521, 0)
    sprite('null', 30)
    Unknown23029(2, 4506, 0)
    Unknown23029(3, 4505, 0)
    sprite('null', 32767)
    Unknown20006(1)

@State
def IchigekiPicture():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown23015(3)
        Unknown3001(0)
        Unknown3004(2)
        Unknown6001(1)
        Unknown1001(640000)
        Unknown1007(1430000)
    sprite('ichigeki0', 25)
    physicsYImpulse(1000)
    GFX_0('adef_450_light', 100)
    sprite('ichigeki0', 5)
    tag_voice(0, 'pad292_0', 'pad292_1', '', '')
    YAccel(80)
    sprite('ichigeki0', 5)
    YAccel(80)
    sprite('ichigeki0', 30)
    YAccel(80)
    sprite('ichigeki0', 50)
    sprite('ichigeki0', 30)
    sprite('ichigeki0', 30)
    tag_voice(0, 'pad293_0', 'pad293_1', '', '')
    sprite('ichigeki0', 20)
    Unknown3004(-16)

@State
def adef_450_light():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown6001(1)
        Unknown23032(50)
        Unknown23033(100)
        Unknown4054(1)
        Unknown23067('adef_450light')
        Unknown1064(750)
    sprite('null', 16)
    Unknown3001(0)
    Unknown3004(8)
    sprite('null', 32767)
    Unknown3001(128)
    Unknown3004(0)

@State
def IchigekiSE():
    sprite('null', 40)
    SFX_3('ad009')
    sprite('null', 40)
    SFX_3('ad009')
    sprite('null', 40)
    SFX_3('ad009')
    sprite('null', 40)
    SFX_3('ad009')
    sprite('null', 40)
    SFX_3('ad009')
    sprite('null', 40)
    SFX_3('ad009')

@State
def PAD_PersonaAstralHeatExe():

    def upon_IMMEDIATE():
        Unknown17012(6, 1, 0)
        Damage(0)
        Unknown11064(1)
        Unknown11084(1)
        DisableAttackRestOfMove()
        Unknown11050(5, '')
        Unknown11023(1)
        Unknown23022(1)
        Unknown23015(0)
        Unknown2018(1, 80)
        Unknown2019(100)

        def upon_43():
            if (SLOT_48 == 4502):
                sendToLabel(1)
            if (SLOT_48 == 4503):
                sendToLabel(2)
            if (SLOT_48 == 4504):
                sendToLabel(3)
            if (SLOT_48 == 4506):
                sendToLabel(4)
            if (SLOT_48 == 4508):
                sendToLabel(10)
    sprite('mg232_04ex', 5)
    physicsXImpulse(2500)
    SFX_3('grip_hugs')
    SFX_3('bound_marble_m')
    Unknown5000(0, 0)
    Unknown5001(0, 0, 0, 0, 0)
    sprite('mg232_05ex', 5)
    Unknown5000(0, 0)
    Unknown5001(0, 0, 0, 0, 0)
    sprite('mg232_07ex', 6)
    Unknown5000(0, 0)
    Unknown5001(0, 0, 0, 0, 0)
    sprite('null', 32767)
    physicsXImpulse(0)
    SFX_3('blood_h')
    SFX_3('blood_h')
    GFX_0('mgef_450_mg_grab', 100)
    Unknown21012('mgef_450_mg_start', 32)
    Unknown21012('mgef_450_mg_start2', 32)
    Unknown23029(4, 4522, 0)
    Unknown3029(0)
    Unknown36(22)
    Unknown3001(0)
    Unknown35()
    label(1)
    sprite('mg232_04ex', 3)
    RefreshMultihit()
    AttackLevel_(0)
    hitstun(900)
    AirUntechableTime(900)
    Unknown9310(900)
    GroundedHitstunAnimation(11)
    AirHitstunAnimation(11)
    AirPushbackY(-50000)
    AirPushbackX(0)
    Unknown3001(0)
    Unknown36(22)
    teleportRelativeY(1500000)
    Unknown3001(255)
    Unknown2035(1)
    Unknown35()
    sprite('null', 32767)
    label(2)
    sprite('mg450_00', 4)
    Unknown2018(1, 80)
    Unknown2019(1000)
    Unknown3032()
    Unknown3001(0)
    Unknown3004(2)
    Unknown1086(22)
    Unknown1007(20000)
    Unknown3029(1)
    Unknown3069(2)
    Unknown3070(1)
    AfterimageCount(2)
    AfterimageColor_1(224, 255, 0, 0)
    AfterimageColor_2(0, 255, 0, 0)
    AfterimageSize_1(1050)
    sprite('mg450_01', 4)
    sprite('mg450_02', 4)
    sprite('mg450_00', 4)
    sprite('mg450_01', 4)
    sprite('mg450_02', 4)
    sprite('mg450_00', 4)
    sprite('mg450_01', 4)
    sprite('mg450_02', 4)
    sprite('mg450_00', 4)
    sprite('mg450_01', 4)
    sprite('mg450_02', 4)
    sprite('mg450_00', 4)
    sprite('mg450_01', 4)
    sprite('mg450_02', 4)
    sprite('mg450_00', 4)
    sprite('mg450_01', 4)
    sprite('mg450_02', 4)
    sprite('mg450_00', 4)
    sprite('mg450_01', 4)
    sprite('mg450_02', 4)
    sprite('mg450_00', 4)
    SFX_3('quake_s')
    ScreenShake(2000, 2000)
    sprite('mg450_01', 4)
    ScreenShake(2000, 2000)
    sprite('mg450_02', 4)
    ScreenShake(2000, 2000)
    sprite('mg450_00', 4)
    ScreenShake(2000, 2000)
    sprite('mg450_01', 4)
    SFX_3('quake_s')
    ScreenShake(2000, 2000)
    sprite('mg450_02', 4)
    ScreenShake(2000, 2000)
    sprite('mg450_00', 4)
    ScreenShake(2000, 2000)
    sprite('mg450_01', 4)
    ScreenShake(2000, 2000)
    sprite('mg450_02', 4)
    SFX_3('quake_s')
    ScreenShake(5000, 5000)
    sprite('mg450_00', 4)
    ScreenShake(5000, 5000)
    sprite('mg450_01', 4)
    ScreenShake(5000, 5000)
    sprite('mg450_02', 4)
    ScreenShake(5000, 5000)
    sprite('mg450_00', 4)
    SFX_3('quake_s')
    ScreenShake(5000, 5000)
    sprite('mg450_01', 4)
    ScreenShake(5000, 5000)
    sprite('mg450_02', 4)
    ScreenShake(5000, 5000)
    sprite('mg450_00', 4)
    ScreenShake(5000, 5000)
    sprite('mg450_01', 4)
    SFX_3('quake_s')
    ScreenShake(5000, 5000)
    sprite('mg450_02', 4)
    ScreenShake(5000, 5000)
    sprite('mg450_00', 4)
    ScreenShake(5000, 5000)
    sprite('mg450_01', 4)
    ScreenShake(5000, 5000)
    sprite('mg450_02', 4)
    SFX_3('quake_s')
    ScreenShake(5000, 5000)
    sprite('mg450_03', 4)
    ScreenShake(10000, 10000)
    GFX_0('mgef_450_aura', 100)
    sprite('mg450_04', 4)
    SFX_3('airdash_m')
    ScreenShake(10000, 10000)
    sprite('mg450_05', 4)
    ScreenShake(10000, 10000)
    sprite('mg450_06', 4)
    SFX_3('ad017')
    SFX_3('airdash_m')
    ScreenShake(10000, 10000)
    Unknown3001(255)
    Unknown3004(0)
    GFX_0('mgef_450_aura2', 0)
    GFX_0('mgef_450_aura2', 1)
    GFX_0('mgef_450_aura2', 2)
    GFX_0('mgef_450_aura2', 3)
    GFX_0('mgef_450_aura2', 4)
    GFX_0('mgef_450_aura2', 5)
    GFX_0('mgef_450_aura2', 6)
    GFX_0('mgef_450_aura2', 7)
    GFX_0('mgef_450_aura2', 8)
    GFX_0('mgef_450_aura2', 9)
    GFX_0('mgef_450_aura2', 10)
    GFX_0('mgef_450_aura2', 11)
    GFX_0('mgef_450_aura2', 12)
    GFX_0('mgef_450_aura2', 13)
    GFX_0('mgef_450_aura2', 14)
    sprite('mg450_07', 4)
    SFX_3('airdash_m')
    ScreenShake(10000, 10000)
    sprite('mg450_08', 4)
    ScreenShake(10000, 10000)
    sprite('mg450_06', 4)
    SFX_3('ad017')
    SFX_3('airdash_m')
    ScreenShake(10000, 10000)
    sprite('mg450_07', 4)
    ScreenShake(10000, 10000)
    sprite('mg450_08', 4)
    SFX_3('airdash_m')
    ScreenShake(15000, 15000)
    sprite('mg450_06', 4)
    ScreenShake(15000, 15000)
    sprite('mg450_07', 4)
    SFX_3('ad017')
    SFX_3('airdash_m')
    ScreenShake(15000, 15000)
    sprite('mg450_08', 4)
    ScreenShake(15000, 15000)
    sprite('mg450_06', 4)
    SFX_3('airdash_m')
    ScreenShake(15000, 15000)
    sprite('mg450_07', 4)
    ScreenShake(15000, 15000)
    sprite('mg450_08', 4)
    SFX_3('ad017')
    SFX_3('airdash_m')
    ScreenShake(15000, 15000)
    sprite('mg450_09', 3)
    ScreenShake(20000, 20000)
    sprite('mg450_10', 3)
    SFX_3('slash_blade_slow')
    SFX_3('slash_blade_slow')
    SFX_3('slash_blade_slow')
    ScreenShake(50000, 50000)
    sprite('mg450_11', 3)
    ScreenShake(50000, 50000)
    sprite('mg450_12', 3)
    ScreenShake(50000, 50000)
    SFX_3('cut_m')
    SFX_3('cut_m')
    SFX_3('cut_m')
    SFX_3('damage_ichigeki_slash')
    SFX_3('damage_ichigeki_slash')
    SFX_3('damage_ichigeki_slash')
    SFX_3('blaze_long')
    Unknown23029(4, 4523, 0)
    GFX_0('Win_TVnoise_off', 100)
    Unknown26('mgef_450_aura')
    Unknown26('mgef_450_aura2')
    Unknown3029(0)
    Unknown3038(1)
    Unknown36(22)
    Unknown3001(0)
    Unknown2035(1)
    Unknown35()
    sprite('mg450_13', 32767)
    label(3)
    sprite('mg450_13', 3)
    GFX_0('TV_OffBlack', 100)
    RefreshMultihit()
    Damage(43104)
    MinimumDamagePct(100)
    Unknown23011(9)
    AirPushbackX(0)
    AirPushbackY(0)
    Unknown9095()
    Unknown11064(3)
    sprite('mg450_13', 90)
    sprite('mg450_13', 32767)
    Unknown26('Win_TVnoise_off')
    label(4)
    sprite('mg610_00', 6)
    sendToLabelUpon(32, 10)
    Unknown1086(3)
    teleportRelativeX(-90000)
    Unknown1007(20000)
    Unknown3038(0)
    sprite('mg610_01', 6)
    sprite('mg610_02', 6)
    sprite('mg610_00', 6)
    sprite('mg610_01', 6)
    sprite('mg610_02', 6)
    sprite('mg610_03', 6)
    sprite('mg610_04', 6)
    sprite('mg610_05', 6)
    ScreenShake(15000, 25000)
    Unknown8004(0, 1, 1)
    GFX_1('mgef_groundtsuki', 0)
    sprite('mg610_06', 6)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    sprite('mg610_06', 6)
    GFX_0('mgef_450_mist', 100)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    sprite('mg610_06', 6)
    Unknown3032()
    Unknown3004(-7)
    Unknown23130(16711680, 60, 1)
    Unknown2035(1)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    sprite('mg610_06', 6)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    sprite('mg610_06', 6)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    Unknown3038(1)
    label(5)
    sprite('mg610_06', 6)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    gotoLabel(5)
    label(10)
    sprite('null', 32767)
    Unknown3032()
    Unknown3038(1)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def mgef_450_mg_grab():

    def upon_IMMEDIATE():
        teleportRelativeX(128000)
    sprite('null', 16)
    GFX_1('mgef_450grab', 100)

@State
def mgef_450_aura():

    def upon_IMMEDIATE():
        Unknown1007(64000)
        GFX_2('mgef_450aura_bk')
        Unknown3001(0)
        Unknown3004(25)
    label(0)
    sprite('null', 2)
    GFX_1('mgef_450aura', 100)
    gotoLabel(0)

@State
def mgef_450_aura2():

    def upon_IMMEDIATE():
        pass
    label(0)
    sprite('null', 4)
    GFX_1('mgef_450aura_sub', 100)
    gotoLabel(0)

@State
def Win_TVnoise_off():

    def upon_IMMEDIATE():
        Unknown4003('mgef_tvnoise_off.DIG', '')
        Unknown4015()
        Unknown23015(1)
        Unknown3032()
        Unknown3001(0)
        Unknown6001(1)
        Unknown23032(45)
        Unknown23033(80)
        Unknown21010(1)
        Unknown2054(1)
    sprite('null', 80)
    Unknown3004(40)
    sprite('null', 5)
    Unknown23130(0, 5, 1)
    SFX_3('tv_off')
    SFX_3('tv_off')
    SFX_3('tv_off')
    SFX_3('tv_off')
    SFX_3('tv_off')
    SFX_3('tv_off')
    sprite('null', 32767)

@State
def TV_OffBlack():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown4061(3)
        Unknown3032()
        Unknown13044(1)
        Unknown23015(4)
        Unknown2007()
        Unknown1096(1500)
        Unknown6001(1)
        Unknown23032(75)
        Unknown23033(50)
        sendToLabelUpon(32, 0)
    sprite('vr_mg450_black', 120)
    Unknown3038(1)
    sprite('vr_mg450_black', 45)
    Unknown3038(0)
    sprite('vr_mg450_black', 10)
    GFX_0('TVnoise', 100)
    Unknown3004(-25)

@State
def TVnoise():

    def upon_IMMEDIATE():
        Unknown23015(4)
        Unknown3032()
        Unknown23032(45)
        Unknown23033(80)
        Unknown4003('mgef_tvnoise.DIG', '')
    sprite('null', 10)
    SFX_3('whitenoize_short')
    SFX_3('whitenoize_short')
    Unknown3001(0)
    Unknown3004(25)
    sprite('null', 5)
    sprite('null', 30)
    Unknown3004(-8)

@State
def mgef_450_mist():

    def upon_IMMEDIATE():
        pass
    sprite('null', 15)
    SFX_3('ad020')
    GFX_0('mgef_450_mist_a', 100)
    GFX_0('mgef_450_mist_3d', 100)
    GFX_0('mgef_450_mist_3d_2', 100)
    GFX_0('mgef_450_mist_b', 100)
    sprite('null', 60)
    sprite('null', 64)
    Unknown23029(3, 4507, 0)
    Unknown23029(2, 4508, 0)
    Unknown26('mgef_610_mist_a')

@State
def mgef_450_mist_a():

    def upon_IMMEDIATE():
        Unknown6001(1)
        Unknown23032(50)
        Unknown23033(50)
    label(0)
    sprite('null', 8)
    GFX_1('mgef_610mist_a', 100)
    gotoLabel(0)

@State
def mgef_450_mist_b():

    def upon_IMMEDIATE():
        teleportRelativeX(31000)
        Unknown1007(5000)
    sprite('null', 1)
    Unknown4054(9)
    Unknown4045('mgef_610mist_b', 100)

@State
def mgef_450_mist_3d():

    def upon_IMMEDIATE():
        Unknown4003('mgef_610mist', '')
        Unknown4015()
        Unknown6001(1)
        Unknown23032(50)
        Unknown23033(110)
        Unknown23015(1)
        Unknown3032()
        Unknown3013(0)
        Unknown3019(0)
        Unknown1056(1100)
        Unknown1064(880)
    sprite('null', 64)
    Unknown3001(0)
    Unknown3004(2)
    sprite('null', 40)
    Unknown3001(128)
    Unknown3004(0)
    sprite('null', 128)
    Unknown3004(-2)

@State
def mgef_450_mist_3d_2():

    def upon_IMMEDIATE():
        Unknown4003('mgef_610mist2', '')
        Unknown4015()
        Unknown6001(1)
        Unknown23032(50)
        Unknown23033(80)
        Unknown23015(15)
        Unknown3032()
        Unknown3013(0)
        Unknown3019(0)
        Unknown1056(-600)
        Unknown1064(400)
    sprite('null', 32)
    Unknown3001(0)
    Unknown3004(4)
    sprite('null', 92)
    Unknown3001(128)
    Unknown3004(0)
    sprite('null', 64)
    Unknown3004(-4)

@State
def P4U_Cutin_Parent():

    def upon_IMMEDIATE():
        Unknown2054(1)
    sprite('null', 1)
    GFX_0('P4U_Cutin_Face', 100)

@State
def P4U_Cutin_Face():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown23015(5)
        Unknown2007()
        Unknown4009(2)
        Unknown2054(1)
        Unknown13044(1)
        Unknown6001(1)
        Unknown1000(-650000)
        teleportRelativeY(-292000)
        Unknown1096(1350)
        Unknown3001(0)
        Unknown2037(0)

        def upon_3():
            if (SLOT_2 == 2):
                Unknown1007(15000)
                Unknown3001(150)
            if (SLOT_2 == 3):
                Unknown1007(-30000)
                Unknown3001(180)
            if (SLOT_2 == 4):
                Unknown1007(25000)
                Unknown3001(210)
            if (SLOT_2 == 5):
                Unknown1007(-20000)
                Unknown3001(255)
            if (SLOT_2 == 6):
                Unknown1007(10000)
            Unknown2038(1)
    sprite('vr_99p4u_face00', 45)
    SFX_0('spsys_over_power')
    SFX_0('spsys_over_power')
    if SLOT_168:
        GFX_0('P4U_ka_NotJPN', 100)
    else:
        GFX_0('P4U_ka_JPN', 100)
    sprite('vr_99p4u_face00', 1)
    Unknown3001(180)
    sprite('vr_99p4u_face00', 1)
    Unknown3001(120)
    sprite('vr_99p4u_face00', 1)
    Unknown3001(60)

@State
def P4U_ka_JPN():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown23015(5)
        Unknown2007()
        Unknown4009(2)
        Unknown6001(1)
        Unknown1000(-950000)
        teleportRelativeY(-232000)
        Unknown1096(1350)
    sprite('vr_p4_ka_mozi', 1)
    Unknown1096(100)
    Unknown3001(40)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(500)
    Unknown1096(300)
    Unknown3001(70)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(550)
    Unknown1096(700)
    Unknown3001(100)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(600)
    Unknown1096(900)
    Unknown3001(130)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(6500)
    Unknown1096(800)
    Unknown3001(160)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(700)
    Unknown1096(900)
    Unknown3001(190)
    sprite('vr_p4_ka_mozi', 1)
    teleportRelativeX(-2000)
    Unknown1007(800)
    Unknown1096(1150)
    Unknown3001(220)
    sprite('vr_p4_ka_mozi', 38)
    teleportRelativeX(-2000)
    Unknown1007(800)
    Unknown1096(1100)
    Unknown3001(255)
    physicsXImpulse(-50)
    physicsYImpulse(50)
    sprite('vr_p4_ka_mozi', 1)
    Unknown3001(180)
    Unknown1096(700)
    sprite('vr_p4_ka_mozi', 1)
    Unknown3001(120)
    Unknown1096(300)
    sprite('vr_p4_ka_mozi', 1)
    Unknown3001(60)
    Unknown1096(100)

@State
def P4U_ka_NotJPN():

    def upon_IMMEDIATE():
        Unknown3032()
        Unknown23015(5)
        Unknown2007()
        Unknown4009(2)
        Unknown6001(1)
        Unknown1000(-950000)
        teleportRelativeY(-232000)
        Unknown1096(1350)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    Unknown1096(100)
    Unknown3001(40)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(500)
    Unknown1096(300)
    Unknown3001(70)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(550)
    Unknown1096(700)
    Unknown3001(100)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(600)
    Unknown1096(900)
    Unknown3001(130)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(6500)
    Unknown1096(800)
    Unknown3001(160)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(700)
    Unknown1096(900)
    Unknown3001(190)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    teleportRelativeX(-2000)
    Unknown1007(800)
    Unknown1096(1150)
    Unknown3001(220)
    sprite('vr_p4_ka_mozi_notjpn', 38)
    teleportRelativeX(-2000)
    Unknown1007(800)
    Unknown1096(1100)
    Unknown3001(255)
    physicsXImpulse(-50)
    physicsYImpulse(50)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    Unknown3001(180)
    Unknown1096(700)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    Unknown3001(120)
    Unknown1096(300)
    sprite('vr_p4_ka_mozi_notjpn', 1)
    Unknown3001(60)
    Unknown1096(100)

@State
def ad600_Cort():

    def upon_IMMEDIATE():
        Unknown1007(300000)
        Unknown3032()
    sprite('ad600_05z', 8)
    sprite('ad600_06z', 8)
    sprite('ad600_07z', 16)
    Unknown3004(-16)
    physicsXImpulse(-8000)
    setGravity(600)

@State
def adef_401aura():

    def upon_IMMEDIATE():
        Unknown4010(2)
        Unknown4007(2)
    label(0)
    sprite('null', 5)
    GFX_0('adef_401aura_sub', 100)
    gotoLabel(0)

@State
def adef_401aura_sub():

    def upon_IMMEDIATE():
        Unknown4010(2)
    sprite('null', 30)
    GFX_2('adef_401aura')

@State
def adef_601aura():

    def upon_IMMEDIATE():
        Unknown4007(2)
    label(0)
    sprite('null', 5)
    GFX_1('adef_401aura', 100)
    gotoLabel(0)

@State
def PAD_PersonaMatchWin():

    def upon_IMMEDIATE():
        callSubroutine('PersonaReset')
        Unknown23023()
        Unknown23184(3, 100, -90000, 20000, 0, 0, 0, 0)
        callSubroutine('PAD_AttackInit')
        callSubroutine('PAD_CheckWarp')
        Unknown2053(1)
        Unknown2019(100)
        Unknown23022(1)
        Unknown23015(0)
        if random_(2, 0, 50):
            Unknown23029(3, 10004, 0)
            Unknown2037(1)

        def upon_43():
            if (SLOT_48 == 10003):
                sendToLabel(10)
    sprite('mg610_00', 6)
    sprite('mg610_01', 6)
    sprite('mg610_02', 6)
    sprite('mg610_00', 6)
    sprite('mg610_01', 6)
    sprite('mg610_02', 6)
    sprite('mg610_03', 6)
    sprite('mg610_04', 6)
    sprite('mg610_05', 6)
    ScreenShake(15000, 25000)
    Unknown8004(0, 1, 1)
    GFX_1('mgef_groundtsuki', 0)
    sprite('mg610_06', 6)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    sprite('mg610_06', 6)
    if SLOT_2:
        GFX_0('adef_DangerCaution', 104)
    else:
        GFX_0('mgef_610_mist', 100)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    sprite('mg610_06', 6)
    if (not SLOT_2):
        Unknown3032()
        Unknown3004(-7)
        Unknown23130(16711680, 60, 1)
        Unknown2035(1)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    sprite('mg610_06', 6)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    sprite('mg610_06', 6)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    if (not SLOT_2):
        Unknown3038(1)
    label(1)
    sprite('mg610_06', 6)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    gotoLabel(1)
    label(10)
    sprite('null', 32767)
    Unknown3038(1)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def PAD_PersonaMatchWinMist():

    def upon_IMMEDIATE():
        callSubroutine('PersonaReset')
        Unknown23023()
        Unknown23184(3, 100, -90000, 20000, 0, 0, 0, 0)
        callSubroutine('PAD_AttackInit')
        callSubroutine('PAD_CheckWarp')
        Unknown2053(1)
        Unknown4019(3)
        Unknown2020(1)
        Unknown23022(1)
        Unknown23015(0)

        def upon_43():
            clearUponHandler(43)
            sendToLabel(10)
    sprite('mg610_00', 6)
    sprite('mg610_01', 6)
    sprite('mg610_02', 6)
    sprite('mg610_00', 6)
    sprite('mg610_01', 6)
    sprite('mg610_02', 6)
    sprite('mg610_03', 6)
    sprite('mg610_04', 6)
    sprite('mg610_05', 6)
    ScreenShake(15000, 25000)
    Unknown8004(0, 1, 1)
    GFX_1('mgef_groundtsuki', 0)
    sprite('mg610_06', 6)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    sprite('mg610_06', 6)
    GFX_0('mgef_610_mist', 100)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    sprite('mg610_06', 6)
    Unknown3032()
    Unknown3004(-7)
    Unknown23130(16711680, 60, 1)
    Unknown2035(1)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    sprite('mg610_06', 6)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    sprite('mg610_06', 6)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    Unknown3038(1)
    label(1)
    sprite('mg610_06', 6)
    sprite('mg610_07', 6)
    sprite('mg610_08', 6)
    gotoLabel(1)
    label(10)
    sprite('null', 32767)
    Unknown3038(1)
    sprite('keep', 32767)
    enterState('PersonaDeleteAndIdling')

@State
def mgef_610_mist():

    def upon_IMMEDIATE():
        pass
    sprite('null', 15)
    SFX_3('ad020')
    GFX_0('mgef_610_mist_a', 100)
    GFX_0('mgef_610_mist_3d', 100)
    GFX_0('mgef_610_mist_3d_2', 100)
    GFX_0('mgef_610_mist_b', 100)
    sprite('null', 60)
    sprite('null', 64)
    Unknown23029(3, 10003, 0)
    Unknown21015('PersonaMatch', 10003, 0)
    Unknown26('mgef_610_mist_a')

@State
def mgef_610_mist_a():

    def upon_IMMEDIATE():
        Unknown6001(1)
        Unknown23057(50)
        Unknown23058(50)
    label(0)
    sprite('null', 8)
    GFX_1('mgef_610mist_a', 100)
    gotoLabel(0)

@State
def mgef_610_mist_b():

    def upon_IMMEDIATE():
        teleportRelativeX(31000)
        Unknown1007(5000)
    sprite('null', 1)
    Unknown4054(9)
    Unknown4045('mgef_610mist_b', 100)

@State
def mgef_610_mist_3d():

    def upon_IMMEDIATE():
        Unknown4003('mgef_610mist', '')
        Unknown4015()
        Unknown6001(1)
        Unknown23057(50)
        Unknown23058(110)
        Unknown23015(1)
        Unknown3032()
        Unknown3013(0)
        Unknown3019(0)
        Unknown1056(1250)
        Unknown1064(880)
    sprite('null', 64)
    Unknown3001(0)
    Unknown3004(2)
    sprite('null', 40)
    Unknown3001(128)
    Unknown3004(0)
    sprite('null', 128)
    Unknown3004(-2)

@State
def mgef_610_mist_3d_2():

    def upon_IMMEDIATE():
        Unknown4003('mgef_610mist2', '')
        Unknown4015()
        Unknown6001(1)
        Unknown23057(50)
        Unknown23058(80)
        Unknown23015(15)
        Unknown3032()
        Unknown3013(0)
        Unknown3019(0)
        Unknown1056(-600)
        Unknown1064(400)
    sprite('null', 32)
    Unknown3001(0)
    Unknown3004(4)
    sprite('null', 92)
    Unknown3001(128)
    Unknown3004(0)
    sprite('null', 64)
    Unknown3004(-4)

@State
def adef_DangerCaution():

    def upon_IMMEDIATE():
        pass
    sprite('null', 60)
    sprite('null', 5)
    Unknown23084(1)
    SFX_3('cloth_h')
    SFX_3('cloth_h')
    SFX_3('cloth_h')
    GFX_0('adef_tape', 100)
    Unknown36(1)
    Unknown23015(1)
    Unknown23057(0)
    Unknown23058(20)
    Unknown1072(30000)
    Unknown1110(256000, 0)
    Unknown1096(1000)
    Unknown35()
    sprite('null', 5)
    SFX_3('cloth_h')
    SFX_3('cloth_h')
    SFX_3('cloth_h')
    GFX_0('adef_tape2', 100)
    Unknown36(1)
    Unknown23015(1)
    Unknown23057(90)
    Unknown23058(0)
    Unknown1072(-45000)
    Unknown1110(-256000, 0)
    Unknown1096(1000)
    Unknown35()
    sprite('null', 5)
    SFX_3('cloth_h')
    SFX_3('cloth_h')
    SFX_3('cloth_h')
    GFX_0('adef_tape', 100)
    Unknown36(1)
    Unknown23015(1)
    Unknown23057(10)
    Unknown23058(100)
    Unknown1072(-20000)
    Unknown1110(256000, 0)
    Unknown1096(800)
    Unknown35()
    sprite('null', 5)
    SFX_3('cloth_h')
    SFX_3('cloth_h')
    SFX_3('cloth_h')
    GFX_0('adef_tape2', 100)
    Unknown36(1)
    Unknown23015(3)
    Unknown23057(100)
    Unknown23058(70)
    Unknown1072(20000)
    Unknown1110(-256000, 0)
    Unknown1096(1750)
    Unknown35()
    sprite('null', 5)
    SFX_3('cloth_h')
    SFX_3('cloth_h')
    SFX_3('cloth_h')
    GFX_0('adef_tape2', 100)
    Unknown36(1)
    Unknown23015(3)
    Unknown23057(100)
    Unknown23058(35)
    Unknown1072(-22500)
    Unknown1110(-256000, 0)
    Unknown1096(2400)
    Unknown35()
    sprite('null', 5)
    SFX_3('cloth_h')
    SFX_3('cloth_h')
    SFX_3('cloth_h')
    GFX_0('adef_tape', 100)
    Unknown36(1)
    Unknown23015(3)
    Unknown23057(20)
    Unknown23058(40)
    Unknown1072(-15000)
    Unknown1110(256000, 0)
    Unknown1096(1450)
    Unknown35()
    sprite('null', 5)
    SFX_3('cloth_h')
    SFX_3('cloth_h')
    SFX_3('cloth_h')
    GFX_0('adef_tape3', 100)
    sprite('null', 5)
    SFX_3('cloth_h')
    SFX_3('cloth_h')
    SFX_3('cloth_h')
    GFX_0('adef_tape', 100)
    Unknown36(1)
    Unknown23015(1)
    Unknown23057(90)
    Unknown23058(110)
    Unknown1072(80000)
    Unknown1110(256000, 0)
    Unknown1096(1750)
    Unknown35()
    GFX_0('adef_tape', 100)
    Unknown36(1)
    Unknown23015(1)
    Unknown23057(0)
    Unknown23058(40)
    Unknown1072(-20000)
    Unknown1110(256000, 0)
    Unknown1096(700)
    Unknown35()
    sprite('null', 32767)

@State
def adef_tape():

    def upon_IMMEDIATE():
        Unknown2007()
        Unknown4003('adef_tape.DIG', '')
        Unknown4015()
        Unknown3032()
        Unknown6001(1)
        sendToLabelUpon(32, 0)
    sprite('null', 1)
    Unknown3001(0)
    sprite('null', 16)
    Unknown3001(255)
    sprite('null', 32767)
    physicsXImpulse(0)
    physicsYImpulse(0)
    Unknown4003('adef_tape_end.DIG', '')

@State
def adef_tape2():

    def upon_IMMEDIATE():
        Unknown2007()
        Unknown4003('adef_tape2.DIG', '')
        Unknown4015()
        Unknown3032()
        Unknown6001(1)
        sendToLabelUpon(32, 0)
    sprite('null', 1)
    Unknown3001(0)
    sprite('null', 16)
    Unknown3001(255)
    sprite('null', 32767)
    physicsXImpulse(0)
    physicsYImpulse(0)
    Unknown4003('adef_tape2_end.DIG', '')

@State
def adef_tape3():

    def upon_IMMEDIATE():
        Unknown2007()
        Unknown4003('adef_tape3.DIG', '')
        Unknown4015()
        Unknown3032()
        Unknown6001(1)
        Unknown23057(50)
        Unknown23058(50)
        Unknown23015(3)
        Unknown1072(20000)
        Unknown1096(1500)
        sendToLabelUpon(32, 0)
    sprite('null', 1)
    Unknown3001(0)
    sprite('null', 16)
    Unknown3001(255)
    sprite('null', 32767)
    physicsXImpulse(0)
    physicsYImpulse(0)
    Unknown4003('adef_tape3_end.DIG', '')